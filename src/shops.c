...
int get_cost(CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, bool fBuy)
{
  ...
  if(fBuy)
  { /* Volk - if fBuy is TRUE, then player is buying - apply buying * * * * * margin */
    ...
    if(cost > 0 && IS_CLANNED(ch) && str_cmp(ch->pcdata->clan->name, "Halcyon"))
    {
      if(IS_INFLUENCED(ch, ch->in_room->area))  /* If their clan is * * * * * influencing 
                                                 ...
                                                 * * cheaper */
        cost -= (int)(cost * .10);
      else  /* Ok they are clanned and not * * * halcyon so charge more */
        cost += (int)(cost * .10);
      if(cost <= 0)
        cost = 1;
    }
...
bool buy_obj(CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, int noi)
{
  ...
  {
    act(AT_TELL, "$n tells you 'I'm sorry but you can't afford to buy $p.'", keeper, obj, ch, TO_VICT);
    ch->reply = keeper;
    return FALSE;
  }
  /*
  ...
  if(noi == 1)
  {
    ...
    if(converted)
      act(AT_ACTION, "The shopkeeper quickly takes what $E needs and hands the rest back.", ch, NULL, keeper, TO_CHAR);
  }
  else if(noi > 1)
  {
    ...
    if(converted)
      snprintf(buf, MSL, "You buy %d of $p. You hand all your money to the shopkeeper.", noi);
    else
      snprintf(buf, MSL, "You buy %d of $p for %d %s coin%s.", noi, cost, string, cost > 1 ? "s" : "");
    act(AT_ACTION, buf, ch, obj, NULL, TO_CHAR);
    act(AT_ACTION, "The shopkeeper puts them into a bag and hands it to you.", ch, NULL, NULL, TO_CHAR);
    if(converted)
      act(AT_ACTION, "The shopkeeper quickly takes what $E needs and hands the rest back.", ch, NULL, keeper, TO_CHAR);
  }
  return TRUE;
...
void do_buy(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP))
  {
    ...
    if(GET_MONEY(keeper, CURR_SILVER) >= 50000)
    {
      ...
      act(AT_ACTION, "$n puts some $T into a large safe.", keeper, NULL, curr_types[CURR_SILVER], TO_ROOM);
    }
  ...
  { /* not a pet shop - volk */
    ...
    {
      act(AT_TELL, "$n tells you 'That object is too powerful for you.. Hope you know what you're doing.", keeper, NULL, ch, TO_VICT);
...
//         return;
    ...
    if(ch->carry_weight + (get_obj_weight(obj, FALSE) * noi) + (noi > 1 ? 2 : 0) > can_carry_w(ch))
    {
...
void do_list(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP))
  {
    ...
    for(pet = pRoomIndexNext->first_person; pet; pet = pet->next_in_room)
    {
      if(xIS_SET(pet->act, ACT_PET) && IS_NPC(pet))
      {
        ...
        if(pet->level > 100)
          pet->level = 100;
        ch_printf(ch, "[%2d] %4d   silver - %s\r\n", pet->level, 100 * pet->level, pet->short_descr);
      }
    }
    if(!found)
  ...
  else
  {
    ...
    if(!str_cmp(arg, "stat"))
    {
      ...
      {
        act(AT_TELL, "$N tells you 'I dont have any object like that.'", ch, NULL, keeper, TO_CHAR);
        ch->reply = keeper;
        return;
      }
      ...
      act(AT_TELL, "$N tells you 'this item here is used as......'", ch, NULL, keeper, TO_CHAR);
      snprintf(buf, MSL, "Special properties: %s", !xIS_EMPTY(obj->extra_flags) ? ext_flag_string(&obj->extra_flags, o_flags) : "Nothing");
      ...
      act(AT_LBLUE, buf, ch, obj, keeper, TO_CHAR);
      snprintf(buf, MSL, "Item's wear locations are %s", flag_string(obj->wear_flags, w_flags));
      act(AT_LBLUE, buf, ch, obj, keeper, TO_CHAR);
      ...
      switch (obj->item_type)
      {
        ...
        case ITEM_QUIVER:
          snprintf(buf, MSL, "%s appears to %s.",
                   ...
                   300 ? "have a medium capacity" : obj->value[0] < 500 ? "have a medium to large capacity" : obj->value[0] < 751 ? "have a large capacity" : "have a giant capacity");
          act(AT_LBLUE, buf, ch, obj, keeper, TO_CHAR);
          break;
        case ITEM_PILL:
        case ITEM_SCROLL:
        case ITEM_POTION:
          ...
          break;
        case ITEM_SALVE:
          snprintf(buf, MSL, "Has %d(%d) applications of level %d", obj->value[1], obj->value[2], obj->value[0]);
        ...
        case ITEM_WAND:
        case ITEM_STAFF:
          snprintf(buf, MSL, "Has %d(%d) charges of level %d", obj->value[1], obj->value[2], obj->value[0]);
          if(obj->value[3] >= 0 && obj->value[3] < top_sn)
          {
            mudstrlcat(buf, " '", MSL);
            mudstrlcat(buf, skill_table[obj->value[3]]->name, MSL);
        ...
        case ITEM_WEAPON:
          snprintf(buf, MSL, "Damage is %d to %d (average %d)%s", obj->value[1], obj->value[2], (obj->value[1] + obj->value[2]) / 2, IS_OBJ_STAT(obj, ITEM_POISONED) ? ", and is poisonous." : ".");
          act(AT_LBLUE, buf, ch, obj, keeper, TO_CHAR);
          snprintf(buf, MSL, "Skill needed: %s", weapon_skills[obj->value[4]]);
          act(AT_LBLUE, buf, ch, obj, keeper, TO_CHAR);
          snprintf(buf, MSL, "Damage type:  %s", capitalize(attack_table[obj->value[3]]));
          act(AT_LBLUE, buf, ch, obj, keeper, TO_CHAR);
          break;
        case ITEM_ARMOR:
    ...
    for(obj = keeper->first_carrying; obj; obj = obj->next_content)
    {
      ...
      if(obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && (cost = get_cost(ch, keeper, obj, TRUE)) || obj->pIndexData->vnum == 41002 || obj->pIndexData->vnum == 41004)
      {
        if(!found)
        {
          found = TRUE;
        ...
        pager_printf(ch, "&C[%2d %7d  %-11s %-40s %10s]\r\n", obj->level, cost, curr_types[obj->currtype], capitalize(obj->short_descr), obj_sizes[obj->size]);
      }
    }
...
void do_sell(CHAR_DATA *ch, char *argument)
{
  ...
  if(!will_buy(keeper, obj) || obj->pIndexData->cost <= 0)
  {
     ...
     */
    if(!xIS_SET(keeper->act, ACT_CRAFTSHOP) || (obj->pIndexData->vnum != 41002 && obj->pIndexData->vnum != 41004))
    {
  ...
  if(GET_MONEY(keeper, CURR_GOLD) < 500 || GET_MONEY(keeper, CURR_SILVER) < 5000 || GET_MONEY(keeper, CURR_BRONZE) < 50000 || GET_MONEY(keeper, CURR_COPPER) < 50000)
  {
  ...
  if(GET_VALUE(obj, type) == CURR_GOLD)
  {
    if(cost > 5000)
    {
      ...
      act_printf(AT_TELL, keeper, obj, ch, TO_VICT, "$n tells you 'I don't carry %d gold on me.'", cost);
      ch->reply = keeper;
      return;
    }
    ...
    {
      act(AT_TELL, "$n tells you 'I don't have enough gold to buy $p.'", keeper, obj, ch, TO_VICT);
      act_printf(AT_TELL, keeper, obj, ch, TO_VICT, "$n tells you 'I only have %d gold try coming back later.'", GET_MONEY(keeper, CURR_GOLD));
      ch->reply = keeper;
      return;
    }
  }
  ...
  if(GET_VALUE(obj, type) == CURR_SILVER)
  {
    ...
    {
      act(AT_TELL, "$n tells you 'I don't have enough silver to buy $p.'", keeper, obj, ch, TO_VICT);
      act_printf(AT_TELL, keeper, obj, ch, TO_VICT, "$n tells you 'The most I can give you for it is %d silver.'", GET_MONEY(keeper, CURR_SILVER));
      ch->reply = keeper;
      return;
    }
  }
  ...
  if(GET_VALUE(obj, type) == CURR_BRONZE)
  {
    ...
    {
      act(AT_TELL, "$n tells you 'I don't have enough bronze to buy $p.'", keeper, obj, ch, TO_VICT);
      act_printf(AT_TELL, keeper, obj, ch, TO_VICT, "$n tells you 'The most I can give you for it is %d bronze.'", GET_MONEY(keeper, CURR_BRONZE));
      ch->reply = keeper;
      return;
    }
  }
  ...
  if(GET_VALUE(obj, type) == CURR_COPPER)
  {
    ...
    {
      act(AT_TELL, "$n tells you 'I don't have enough copper to buy $p.'", keeper, obj, ch, TO_VICT);
      act_printf(AT_TELL, keeper, obj, ch, TO_VICT, "$n tells you 'The most I can give you for it is %d copper.'", GET_MONEY(keeper, CURR_COPPER));
      ch->reply = keeper;
      return;
    }
  }
  ...
  if(GET_VALUE(obj, type) == CURR_SILVER)
  {
    ...
    snprintf(arg, MIL, "You sell $p for %d %s coin%s.", cost, curr_types[CURR_SILVER], cost > 1 ? "s" : "");
  ...
  {
    act(AT_ACTION, "$n sells $p.", ch, obj, NULL, TO_ROOM);
    snprintf(arg, MIL, "You sell $p for %d %s coin%s.", cost, curr_types[CURR_GOLD], cost > 1 ? "s" : "");
    act(AT_ACTION, arg, ch, obj, NULL, TO_CHAR);
  ...
  {
    act(AT_ACTION, "$n sells $p.", ch, obj, NULL, TO_ROOM);
    snprintf(arg, MIL, "You sell $p for %d %s coin%s.", cost, curr_types[CURR_BRONZE], cost > 1 ? "s" : "");
    act(AT_ACTION, arg, ch, obj, NULL, TO_CHAR);
  ...
  {
    act(AT_ACTION, "$n sells $p.", ch, obj, NULL, TO_ROOM);
    snprintf(arg, MIL, "You sell $p for %d %s coin%s.", cost, curr_types[CURR_COPPER], cost > 1 ? "s" : "");
    act(AT_ACTION, arg, ch, obj, NULL, TO_CHAR);
...
void do_value(CHAR_DATA *ch, char *argument)
{
  ...
  if(GET_VALUE(obj, type) == CURR_GOLD)
  {
    snprintf(buf, MSL, "$n tells you 'I'll give you %d %s coins for $p.'", cost, curr_types[primcurr]);
 ...
 */
void repair_one_obj(CHAR_DATA *ch, CHAR_DATA *keeper, OBJ_DATA *obj, char *arg, int maxmoney, char *fixstr, char *fixstr2)
{
  char                    buf[MSL];
  int                     cost;
  short                   primcurr;
  ...
  else if((cost = str_cmp("all", arg) ? cost : 11 * cost / 10) && !can_afford(ch, cost, CURR_BRONZE))
  {
    snprintf(buf, MSL, "$N tells you, 'It will cost %d coin%s of %s to %s %s...'", cost, cost == 1 ? "" : "s", curr_types[CURR_BRONZE], fixstr, obj->name);
  ...
  else
  {
    ...
    snprintf(buf, MSL, "$N charges you %d %s coin%s to %s $p.", cost, curr_types[CURR_BRONZE], cost == 1 ? "" : "s", fixstr);
    ...
    else if(GET_MONEY(keeper, CURR_BRONZE) >= maxmoney)
    {
      ...
      act(AT_ACTION, "$n puts some $T into a large safe.", keeper, NULL, curr_types[CURR_BRONZE], TO_ROOM);
    }
  ...
  if(!str_cmp(argument, "all"))
  {
    for(obj = ch->first_carrying; obj; obj = obj->next_content)
    {
      if(obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj)
         && can_see_obj(keeper, obj) && (obj->item_type == ITEM_ARMOR || obj->item_type == ITEM_WEAPON || obj->item_type == ITEM_WAND || obj->item_type == ITEM_STAFF))
...
void appraise_all(CHAR_DATA *ch, CHAR_DATA *keeper, char *fixstr)
{
  ...
  {
    if(obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && (obj->item_type == ITEM_ARMOR || obj->item_type == ITEM_WEAPON || obj->item_type == ITEM_WAND || obj->item_type == ITEM_STAFF))
    {
      if(!can_drop_obj(ch, obj))
        ch_printf(ch, "You can't let go of %s.\r\n", obj->name);
      else if((cost = get_repaircost(ch, keeper, obj)) < 0)
      ...
      {
        snprintf(buf, MSL, "$N tells you, 'It will cost %d piece%s of %s to %s %s'", cost, cost == 1 ? "" : "s", curr_types[primcurr], fixstr, obj->name);
        act(AT_TELL, buf, ch, NULL, keeper, TO_CHAR);
        total += cost;
      }
    }
  ...
  {
    send_to_char("\r\n", ch);
    snprintf(buf, MSL, "$N tells you, 'It will cost %d piece%s of %s in total.'", total, cost == 1 ? "" : "s", curr_types[primcurr]);
    act(AT_TELL, buf, ch, NULL, keeper, TO_CHAR);
...
void do_appraise(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, MSL, "$N tells you, 'It will cost %d piece%s of bronze to %s that...'", cost, cost == 1 ? "" : "s", fixstr);
  act(AT_TELL, buf, ch, NULL, keeper, TO_CHAR);
...
void do_shops(CHAR_DATA *ch, char *argument)
{
  ...
  for(shop = first_shop; shop; shop = shop->next)
    pager_printf(ch,
                 ...
                 shop->keeper, shop->profit_buy, shop->profit_sell, shop->open_hour, shop->close_hour, shop->buy_type[0], shop->buy_type[1], shop->buy_type[2], shop->buy_type[3], shop->buy_type[4]);
  return;
}
