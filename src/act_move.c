...
void do_build_walk(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_IMMORTAL(ch) && IS_SET(ch->pcdata->flags, PCFLAG_BUILDWALK))
  {
    ...
    if(!xit)
    {
      ...
      {
        send_to_char("&GYou cannot buildwalk anymore as there are no empty higher number rooms to be found.\r\n", ch);
        return;
      }
    ...
    else
    {
      ...
      ch_printf(ch, "&GCannot buildwalk back into a room that you already created an exit.\r\r\n\n");
    }
...
bool will_fall(CHAR_DATA *ch, int fall)
{
  ...
  if(IS_SET(ch->in_room->room_flags, ROOM_NOFLOOR) && CAN_GO(ch, DIR_DOWN) && (!IS_AFFECTED(ch, AFF_FLYING) || (ch->mount && !IS_AFFECTED(ch->mount, AFF_FLYING))))
  {
    if(fall > 80)
    {
...
ch_ret move_char(CHAR_DATA *ch, EXIT_DATA *pexit, int fall, bool running)
{
  ...
  if(IS_AFFECTED(ch, AFF_OTTOS_DANCE))
  {
    ...
    if(chance < 4)
    {
      act(AT_ACTION, "You attempt to move, but are too caught up in the dance.", ch, NULL, NULL, TO_CHAR);
      act(AT_ACTION, "$n nearly falls attempting to move away while dancing.", ch, NULL, NULL, TO_ROOM);
      return FALSE;
    }
    if(chance == 4)
      act(AT_ACTION, "You somehow manage to dance in the direction you want to go.", ch, NULL, NULL, TO_CHAR);
    act(AT_ACTION, "$n dances away from the music.", ch, NULL, NULL, TO_ROOM);
  }

  if(IS_AFFECTED(ch, AFF_IMPRISONED))
  {
    ch_printf(ch, "You cannot leave the magical prison that surrounds you!\r\n");
    return FALSE;
  }
...
// Make players config obj id when in tradeskills rooms
  ...
  if(!IS_NPC(ch))
  {
    // HIDE AND SNEAK CHECKS based on luck and dexterity - Vladaar
    short                   chance = get_curr_dex(ch) * 2 + get_curr_lck(ch) / 2; // more 
                                                                                  // 
    // based 
    // on
    // dex
    // than 
    // lck
    if(ch->pcdata->learned[gsn_sneak] > 80)
      chance += 3;
    if(ch->pcdata->learned[gsn_hide] > 80)
      chance += 4;

    if(chance - number_range(1, 30) < 35)
    {
      if((IS_AFFECTED(ch, AFF_HIDE) || IS_AFFECTED(ch, AFF_SNEAK)) && ch->pcdata->learned[gsn_hide] > 0)
      {
        send_to_char("Your footfalls give notice to your location.\r\n", ch);
 ...
 if                      (IS_AFFECTED(ch, AFF_BURROW))
  {
    ...
    if(xIS_SET(ch->act, PLR_MUSIC) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_MUSIC)))
    {
      ...
      if(ch->in_room && (ch->in_room->vnum == 2758))
        send_to_char("!!SOUND(sound/1.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 10001))
        send_to_char("!!SOUND(sound/2.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 7501))
        send_to_char("!!SOUND(sound/3.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 18600))
        send_to_char("!!SOUND(sound/4.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 27023))
        send_to_char("!!SOUND(sound/5.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 29001))
        send_to_char("!!SOUND(sound/6.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 4008))
        send_to_char("!!SOUND(sound/7.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 28251))
        send_to_char("!!SOUND(sound/8.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 18300))
        send_to_char("!!SOUND(sound/9.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 13027))
        send_to_char("!!SOUND(sound/10.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 6279))
        send_to_char("!!SOUND(sound/11.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 4900))
        send_to_char("!!SOUND(sound/12.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 5300))
        send_to_char("!!SOUND(sound/13.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 34002))
        send_to_char("!!SOUND(sound/14.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 29100))
        send_to_char("!!SOUND(sound/15.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 10800))
        send_to_char("!!SOUND(sound/16.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 100))
        send_to_char("!!SOUND(sound/17.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 26023))
        send_to_char("!!SOUND(sound/18.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 1850))
        send_to_char("!!SOUND(sound/19.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 31000))
        send_to_char("!!SOUND(sound/20.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 2200))
        send_to_char("!!SOUND(sound/21.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 8310))
        send_to_char("!!SOUND(sound/22.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 28001))
        send_to_char("!!SOUND(sound/23.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 986))
        send_to_char("!!SOUND(sound/24.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 1951))
        send_to_char("!!SOUND(sound/25.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 2500))
        send_to_char("!!SOUND(sound/26.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 2904))
        send_to_char("!!SOUND(sound/26.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 10101))
        send_to_char("!!SOUND(sound/26.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 10400))
        send_to_char("!!SOUND(sound/swim.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 28350))
        send_to_char("!!SOUND(sound/27.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 19500))
        send_to_char("!!SOUND(sound/28.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 31600))
        send_to_char("!!SOUND(sound/29.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 31300))
        send_to_char("!!SOUND(sound/30.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 2300))
        send_to_char("!!SOUND(sound/31.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 18012))
        send_to_char("!!SOUND(sound/32.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 4103))
        send_to_char("!!SOUND(sound/33.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 11601))
        send_to_char("!!SOUND(sound/34.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 10201))
        send_to_char("!!SOUND(sound/35.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 25000))
        send_to_char("!!SOUND(sound/36.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 32000))
        send_to_char("!!SOUND(sound/37.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 602))
        send_to_char("!!SOUND(sound/38.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 16700))
        send_to_char("!!SOUND(sound/39.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 35805))
        send_to_char("!!SOUND(sound/40.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 2600))
        send_to_char("!!SOUND(sound/41.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 2551))
        send_to_char("!!SOUND(sound/42.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 35601))
        send_to_char("!!SOUND(sound/43.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 11510))
        send_to_char("!!SOUND(sound/44.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 1400))
        send_to_char("!!SOUND(sound/45.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 35745))
        send_to_char("!!SOUND(sound/46.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 13101))
        send_to_char("!!SOUND(sound/47.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 13151))
        send_to_char("!!SOUND(sound/48.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 702))
        send_to_char("!!SOUND(sound/49.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 1750))
        send_to_char("!!SOUND(sound/50.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 3910))
        send_to_char("!!SOUND(sound/51.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 8100))
        send_to_char("!!SOUND(sound/52.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 30021))
        send_to_char("!!SOUND(sound/53.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 47113))
        send_to_char("!!SOUND(sound/54.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 5529))
        send_to_char("!!SOUND(sound/55.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 6350))
        send_to_char("!!SOUND(sound/56.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 6684))
        send_to_char("!!SOUND(sound/57.wav)\r\n", ch);
      if(ch->in_room && (ch->in_room->vnum == 25201))
        send_to_char("!!SOUND(sound/58.wav)\r\n", ch);
    ...
    if(!IS_NPC(ch) && IS_SET(ch->pcdata->tag_flags, TAG_FROZEN) && IS_SET(ch->pcdata->tag_flags, TAG_PLAYING))
    {
    ...
    if(ch->in_room->sector_type == SECT_OCEAN && ch->move == 0 && IS_AFFECTED(ch, AFF_FLYING))
    {
  ...
  if(!pexit || (to_room = pexit->to_room) == NULL)
  {
    if(drunk && ch->position != POS_MOUNTED && ch->in_room->sector_type != SECT_WATER_SWIM
       && ch->in_room->sector_type != SECT_WATER_NOSWIM && ch->in_room->sector_type != SECT_UNDERWATER && ch->in_room->sector_type != SECT_OCEANFLOOR)
    {
  ...
  if(ch->morph != NULL && !str_cmp(ch->morph->morph->name, "fish")
     && (to_room->sector_type != SECT_UNDERWATER && to_room->sector_type != SECT_OCEAN
         ...
         && to_room->sector_type != SECT_OCEANFLOOR && to_room->sector_type != SECT_RIVER && to_room->sector_type != SECT_WATER_NOSWIM))
  {
    act(AT_ACTION, "You flop about on the land as there is no water!", ch, NULL, NULL, TO_CHAR);
    act(AT_ACTION, "$n flops about on the land as there is no water!", ch, NULL, NULL, TO_ROOM);
    return rSTOP;
  }
  ...
  if(xIS_SET(ch->act, PLR_BOAT)
     && (to_room->sector_type != SECT_UNDERWATER && to_room->sector_type != SECT_OCEAN
         && to_room->sector_type != SECT_LAKE && to_room->sector_type != SECT_WATER_SWIM
         && to_room->sector_type != SECT_OCEANFLOOR && to_room->sector_type != SECT_RIVER && to_room->sector_type != SECT_WATER_NOSWIM && to_room->sector_type != SECT_DOCK))
  {
    act(AT_ACTION, "You cannot sail a ship onto land!", ch, NULL, NULL, TO_CHAR);
    return rSTOP;
  }
  ...
  if((to_room->sector_type == SECT_UNDERWATER || to_room->sector_type == SECT_OCEAN
      || to_room->sector_type == SECT_LAKE || to_room->sector_type == SECT_WATER_SWIM
      || to_room->sector_type == SECT_RIVER || to_room->sector_type == SECT_WATER_NOSWIM) && IS_AFFECTED(ch, AFF_FLAMING_SHIELD))
  {
  ...
  if(IS_NPC(ch))
  {
    if(xIS_SET(ch->act, ACT_WATER)
       && (to_room->sector_type != SECT_OCEANFLOOR || to_room->sector_type != SECT_OCEAN
           || to_room->sector_type != SECT_UNDERWATER || to_room->sector_type != SECT_WATER_SWIM || to_room->sector_type != SECT_WATER_NOSWIM || to_room->sector_type != SECT_RIVER))
    {
      return FALSE;
    }
  }
...
//  fix for beastmaster
  ...
  {
    if(IS_NPC(ch) && !xIS_SET(ch->act, ACT_PET) && (IS_SET(pexit->exit_info, EX_NOMOB) || IS_SET(to_room->room_flags, ROOM_NO_MOB)))
    {
    ...
    { // Allow fall throughs in this
      ...
      case SECT_OCEAN:
        if(IS_AFFECTED(ch, AFF_FLYING)) // && ch->race == RACE_DRAGON )
        {
          ...
          send_to_char("You sail your ship within the mighty ocean.\r\n", ch);
        ...
        else
        {
          ...
          {
            send_to_char("You cannot float on water, you will have to land and swim.\r\n", ch);
            return rSTOP;
          }
        }
        break;
  ...
  if(!IS_ETHEREAL(ch) && !IS_IMMORTAL(ch))
    xREMOVE_BIT(ch->act, PLR_SHADOWFORM);

  if(IS_ETHEREAL(ch))
    xSET_BIT(ch->act, PLR_SHADOWFORM);

  if(IS_SET(pexit->exit_info, EX_CLOSED) && !IS_PASS(ch))
    ...
    if(!IS_SET(pexit->exit_info, EX_SECRET) && !IS_SET(pexit->exit_info, EX_DIG))
    {
      ...
      else
      {
        if(IS_AFFECTED(ch, AFF_FLYING)
           ...
           && pexit->to_room->sector_type != SECT_INSIDE && !IS_SET(ch->in_room->room_flags, ROOM_INDOORS) && !IS_SET(pexit->to_room->room_flags, ROOM_INDOORS))
          autofly = TRUE;
        else if(xIS_SET(ch->act, PLR_AUTODOOR) && !IS_SET(pexit->exit_info, EX_LOCKED))
          autoopen = TRUE;
        else
  ...
  if(!xIS_SET(ch->act, ACT_BEASTPET))
  {
    if(IS_NPC(ch) && (IS_SET(pexit->exit_info, EX_NOMOB) || IS_SET(to_room->room_flags, ROOM_NO_MOB)))
    {
      return rSTOP;
    }
  }
  if(!IS_IMMORTAL(ch) && !IS_NPC(ch) && ch->in_room->area != to_room->area)
  {
    if(ch->level < to_room->area->low_hard_range)
    {
      if(ch->race == RACE_DRAGON && to_room->area->low_hard_range < 100)
      {
        ...
        send_to_char("A voice in your mind says, 'You might not be ready for this path, but because you are a Dragon anything may be possible!\r\n", ch);
      }
      else if(ch->race != RACE_DRAGON || (to_room->area->low_hard_range == 100 && ch->level < 100))
      {
        set_char_color(AT_TELL, ch);
  ...
  if(!fall && !IS_NPC(ch))
  {
     ...
     */
    if(IS_SET(to_room->area->flags, AFLAG_ANTIPKILL) && !IS_SET(ch->in_room->area->flags, AFLAG_ANTIPKILL) && (IS_PKILL(ch) && !IS_IMMORTAL(ch)))
    {
    ...
    if((in_room->sector_type == SECT_WATER_SWIM
        || in_room->sector_type == SECT_WATER_NOSWIM || in_room->sector_type == SECT_OCEAN) && (to_room->sector_type == SECT_UNDERWATER || to_room->sector_type == SECT_OCEANFLOOR))
    {
      if(ch->pcdata->learned[gsn_swim] > 50)
      {
    ...
    if(ch->mount)
    {
      ...
      if(ch->race == RACE_DRAGON && ch->Class == CLASS_BLUE
         && (ch->in_room->sector_type == SECT_OCEAN
             ...
             || ch->in_room->sector_type == SECT_RIVER || ch->in_room->sector_type == SECT_OCEANFLOOR || ch->in_room->sector_type == SECT_WATER_SWIM || ch->in_room->sector_type == SECT_LAKE))
      {
        move += 1;
      }
  ...
  if(to_room->height > 0)
  {
    ...
    {
      ch_printf(ch, "You are too tall to fit in there by %d inches.\r\nYou may want to try the crawl command.\r\n", gap);
      return rSTOP;
    }
  }
  ...
  if(IS_NPC(ch) || !xIS_SET(ch->act, PLR_WIZINVIS))
  {
    ...
    if(!IS_AFFECTED(ch, AFF_SNEAK))
    {
      ...
      {
        act_printf(AT_ACTION, ch, NULL, (void *)dir_name[door], TO_ROOM, "$n %s $T.", txt);
      }
    }
    else
    {
      CHAR_DATA *             temp_char, *temp_next;
      char                    sneak_buf1[MSL], sneak_buf2[MSL];
      bool                    hasMount = ch->mount ? TRUE : FALSE;

      if(hasMount)
      {
        snprintf(sneak_buf1, MSL, "$n attempts to sneak away upon %s.%c", IS_NPC(ch->mount) ? ch->mount->short_descr : ch->mount->name, '\0');
        snprintf(sneak_buf2, MSL, "$n %s %s upon %s.%c", txt, dir_name[door], IS_NPC(ch->mount) ? ch->short_descr : ch->name, '\0');
      }
      else
      {
  ...
  if(IS_NPC(ch) || !xIS_SET(ch->act, PLR_WIZINVIS))
  {
    ...
    else
    {
      ...
      if(hasMount)
      {
        snprintf(sneak_buf1, MSL, "$n attempts to sneak closer upon %s.%c", IS_NPC(ch->mount) ? ch->mount->short_descr : ch->mount->name, '\0');
        snprintf(sneak_buf2, MSL, "$n %s from %s upon %s.%c", txt, dtxt, IS_NPC(ch->mount) ? ch->short_descr : ch->name, '\0');
      }
      else
      {
      ...
      for(temp_char = ch->in_room->first_person; temp_char; temp_char = temp_next)
      {
        ...
        if(IS_AFFECTED(temp_char, AFF_DETECT_SNEAK) || (IS_AFFECTED(temp_char, AFF_DEMONIC_SIGHT) && (IS_AFFECTED(temp_char, AFF_NOSIGHT) || !IS_AFFECTED(temp_char, AFF_BLINDNESS))))
        {
          act(AT_ACTION, sneak_buf1, ch, NULL, temp_char, TO_VICT);
          act(AT_ACTION, sneak_buf2, ch, NULL, temp_char, TO_VICT);
        }
...
/* Righto - we'll check if ch is in a group first. If not, hit them with encounter. If so
  ...
  if(!IS_GROUPED(ch))
    ...
    random = TRUE;
  else  /* ch is in a group - where is * * * leader */
...
void                    do_crouch(CHAR_DATA *ch, char *argument)
{
 ...
 if                      (IS_AFFECTED(ch, AFF_BURROW))
  ...
  if(ch->position == POS_FIGHTING || ch->position == POS_BERSERK || ch->position == POS_EVASIVE || ch->position == POS_DEFENSIVE || ch->position == POS_AGGRESSIVE)
  {
...
void                    do_north(CHAR_DATA *ch, char *argument)
{
  ...
  if                      (xIS_SET(ch->act, PLR_BOAT))
  {
    send_to_char("Your on a ship, you need to steer a course and then sail in that direction.\r\n", ch);
    return;
  }
  ...
  if                      ((arg[0] != '\0') && (!str_cmp(arg, "elude")) && (arg2[0] != '\0'))
  {
    ...
    if(chance > 7)
    {
      ...
      act(AT_PLAIN, "$n suddenly eludes you when you weren't expecting it!", ch, NULL, victim, TO_VICT);
      stop_follower(victim);
    }
    else if(chance <= 7)
    {
      act(AT_PLAIN, "$N tries to manuever suddenly to stop you from following!", ch, NULL, victim, TO_VICT);
    }
...
void                    do_south(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_NPC(ch))
  {
    if(ch->pcdata->invite > 0 && (ch->in_room->vnum == 54000 || ch->in_room->vnum == 57000 || ch->in_room->vnum == 40166))
    {
      char_from_room(ch);
    ...
    if(ch->pcdata->lair > 0)
    {
      if(ch->in_room->vnum == 54000 && !str_cmp(ch->pcdata->htown_name, "paleon city"))
      {
        char_from_room(ch);
      ...
      if(ch->in_room->vnum == 57000 && !str_cmp(ch->pcdata->htown_name, "dakar city"))
      {
        char_from_room(ch);
        char_to_room(ch, get_room_index(ch->pcdata->lair));
        do_look(ch, (char *)"auto");
      ...
      if(ch->in_room->vnum == 40166 && !str_cmp(ch->pcdata->htown_name, "forbidden city"))
      {
        char_from_room(ch);
        char_to_room(ch, get_room_index(ch->pcdata->lair));
        do_look(ch, (char *)"auto");
...
EXIT_DATA *             find_door(CHAR_DATA *ch, char *arg, bool quiet)
{
  ...
  else
  {
    ...
    {
      if((quiet || IS_SET(pexit->exit_info, EX_ISDOOR)) && pexit->keyword && nifty_is_name_prefix(arg, pexit->keyword))
        return pexit;
    }
    if                      (!quiet)
...
void                    do_open(CHAR_DATA *ch, char *argument)
{
  ...
  if                      ((pexit = find_door(ch, arg, TRUE)) != NULL)
  {
    ...
    if(!IS_SET(pexit->exit_info, EX_SECRET) || (pexit->keyword && nifty_is_name_prefix(arg, pexit->keyword)))
    {
...
void                    do_bashdoor(CHAR_DATA *ch, char *argument)
{
  ...
  if((pexit = find_door(ch, arg, FALSE)) != NULL)
  {
    ...
    if(xIS_SET(ch->act, PLR_BATTLE))
      send_to_char("!!SOUND(sound/doorbash.wav)\r\n", ch);
    ...
    {
      act(AT_GREEN, "WHAAM!  You slam into $d and realize it would take more than you can give to budge it!", ch, NULL, keyword, TO_CHAR);
      act(AT_GREEN, "$n slams into $d!", ch, NULL, keyword, TO_ROOM);
      return;
    }

    if(!IS_SET(pexit->exit_info, EX_BASHPROOF) && ch->move >= 15 && number_percent() < (chance + 4 * (get_curr_str(ch) - 16)))
    {
    ...
    {
      act(AT_GREEN, "WHAAAAM!!!  You bash against the $d, but it doesn't budge.", ch, NULL, keyword, TO_CHAR);
      act(AT_GREEN, "WHAAAAM!!!  $n bashes against the $d, but it holds strong.", ch, NULL, keyword, TO_ROOM);
      damage(ch, ch, (ch->max_hit / 20) + 10, gsn_bashdoor);
    }
  }
  else
  {
    act(AT_GREEN, "WHAAAAM!!!  You bash against the wall, but it doesn't budge.", ch, NULL, NULL, TO_CHAR);
    act(AT_GREEN, "WHAAAAM!!!  $n bashes against the wall, but it holds strong.", ch, NULL, NULL, TO_ROOM);
    damage(ch, ch, (ch->max_hit / 20) + 10, gsn_bashdoor);
  }
  return;
}
...
void                    do_stand(CHAR_DATA *ch, char *argument)
{
 ...
 if                      (IS_AFFECTED(ch, AFF_BURROW))
  ...
  if(ch->position == POS_SLEEPING && xIS_SET(ch->affected_by, AFF_MOUNTED))
  {
    send_to_char("You suddenly jerk awake from a dream, that you were going to stand up.\r\n", ch);
    interpret(ch, (char *)"wake");
    return;
  }
  ...
  {
    send_to_char("You are disabled by pyschic powers ripping through your mind currently.\r\n", ch);
    return;
  }
  ...
  {
    act(AT_BLOOD, "No longer resting, you dismiss your unholy sphere.", ch, NULL, NULL, TO_CHAR);
  ...
  switch (ch->position)
  {
    case POS_MEDITATING:
      ...
      act(AT_ACTION, "$n stops meditating and regains $s awareness.", ch, NULL, NULL, TO_ROOM);
      set_position(ch, POS_STANDING);
      break;
    ...
    case POS_CRAWL:
      ...
      act(AT_ACTION, "$n gets off $s hands and knees standing fully up.", ch, NULL, NULL, TO_ROOM);
      set_position(ch, POS_STANDING);
      break;
...
void                    do_trance(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_trance))
  {
    ...
    {
      set_char_color(AT_RED, ch);
      send_to_char("Your adrenaline is pumping too hard for the deep trance!\r\n&cYou are able to calm yourself enough for the lesser trance.\r\n", ch);
    }

    if(ch->mana <= ch->max_mana / 2 && get_timer(ch, TIMER_RECENTFIGHT) <= 0)
    {
      WAIT_STATE(ch, (16));
      act(AT_CYAN, "You enter a deep trance state within your meditation.", ch, NULL, NULL, TO_CHAR);
    ...
    else
    {
      if(get_timer(ch, TIMER_RECENTFIGHT) <= 0)
      {
        send_to_char("&cYou have too much mana already collected to enter the deep trance, but enter a lesser trance-like state.\r\n", ch);
      }
    ...
    if(xIS_SET(ch->act, PLR_BATTLE))
      send_to_char("!!SOUND(sound/meditate.wav)\r\n", ch);
  ...
  if(can_use_skill(ch, number_percent(), gsn_meditate))
  {
    ...
    switch (ch->position)
    {
      case POS_SLEEPING:
        ...
        act(AT_ACTION, "$n wakes up rolls $s eyes back, and begins chanting in meditation.", ch, NULL, NULL, TO_ROOM);
        set_position(ch, POS_MEDITATING);
        break;

      case POS_RESTING:
        send_to_char("You stop resting and begin meditating.\r\n", ch);
        act(AT_ACTION, "$n stops resting rolls $s eyes back, and begins chanting in meditation.", ch, NULL, NULL, TO_ROOM);
        set_position(ch, POS_MEDITATING);
        break;

      case POS_STANDING:
        send_to_char("You begin meditating.\r\n", ch);
        act(AT_ACTION, "$n rolls $s eyes back, and begins chanting in meditation.", ch, NULL, NULL, TO_ROOM);
        set_position(ch, POS_MEDITATING);
        break;

      case POS_SITTING:
        send_to_char("You stand up and begin meditating.\r\n", ch);
        act(AT_ACTION, "$n stands up rolls $s eyes back, and begins chanting in meditation.", ch, NULL, NULL, TO_ROOM);
        set_position(ch, POS_MEDITATING);
        learn_from_success(ch, gsn_meditate);
        return;
 ...
 if                      (IS_AFFECTED(ch, AFF_BURROW))
  ...
  switch (ch->position)
  {
    case POS_MEDITATING:
      ...
      act(AT_ACTION, "$n stops meditating and sprawls out haphazardly.", ch, NULL, NULL, TO_ROOM);
      set_position(ch, POS_RESTING);
      break;
    case POS_SLEEPING:
      if(obj == NULL)
...
void                    do_sleep(CHAR_DATA *ch, char *argument)
{
  ...
  switch (ch->position)
  {
    case POS_MEDITATING:
      ...
      act(AT_ACTION, "$n stops meditating and slumps over, dead asleep.", ch, NULL, NULL, TO_ROOM);
    ...
    case POS_MOUNTED:
    {
      act(AT_ACTION, "You begin to doze off in the saddle.", ch, NULL, NULL, TO_CHAR);
      act(AT_ACTION, "$n begins to doze off in the saddle.", ch, NULL, NULL, TO_ROOM);
      set_position(ch, POS_SLEEPING);
    }
      break;
    case POS_STANDING:
...
void                    do_wake(CHAR_DATA *ch, char *argument)
{
  ...
  if                      (arg[0] == '\0')
  {
    if(xIS_SET(ch->affected_by, AFF_MOUNTED))
    {
      ...
      act(AT_ACTION, "You wake up from a fitful sore sleep in your saddle.", ch, NULL, NULL, TO_CHAR);
      act(AT_ACTION, "$n wakes up suddenly with a sudden jerk in their saddle.", ch, NULL, NULL, TO_ROOM);
      return;
    }
  ...
  {
    act(AT_ACTION, "You wake $M nearly knocking them out of the saddle.", ch, NULL, victim, TO_CHAR);
    set_position(victim, POS_MOUNTED);
    act(AT_ACTION, "$n wakes you nearly knocking you out of your saddle.", ch, NULL, victim, TO_VICT);
    return;
  }
...
void                    do_climb(CHAR_DATA *ch, char *argument)
{
  ...
  if((pexit = find_door(ch, argument, TRUE)) != NULL && IS_SET(pexit->exit_info, EX_xCLIMB))
  {
    if(xIS_SET(ch->act, PLR_BATTLE) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_BATTLE)))
      send_to_char("!!SOUND(sound/climb.wav)\r\n", ch);
...
void                    do_enter(CHAR_DATA *ch, char *argument)
{
  ...
  if                      (argument[0] == '\0')
  {
    ...
    if                      (ch->in_room->sector_type != SECT_INSIDE && IS_OUTSIDE(ch))
      for                     (pexit = ch->in_room->first_exit; pexit; pexit = pexit->next)
        if(pexit->to_room && (pexit->to_room->sector_type == SECT_INSIDE || IS_SET(pexit->to_room->room_flags, ROOM_INDOORS)))
        {
          move_char(ch, pexit, 0, FALSE);
          return;
        }
...
short                   max_holdbreath(CHAR_DATA *ch)
{
  ...
  switch (ch->race)
  {
    default:
      ...
      case RACE_DRAGON:breath = 100;
      break;

      case RACE_GNOME:case RACE_ELF:case RACE_DROW:case RACE_HALFLING:breath = 40;
      break;

      case RACE_DWARF:case RACE_TROLL:case RACE_ORC:case RACE_OGRE:breath = 80;
      break;

      case RACE_PIXIE:breath = 30;
      break;
  }
...
void                    fbite_msg(CHAR_DATA *ch, int percentage)
{
  ...
  if(percentage > 90)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 0%\r\n" : "&WFrost bite:(          &W)&D\r\n", ch);
  else if(percentage > 80)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 10%\r\n" : "&WFrost bite: (&R+         &W)&D\r\n", ch);
  else if(percentage > 70)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 20%\r\n" : "&WFrost bite: (&R++        &W)&D\r\n", ch);
  else if(percentage > 60)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 30%\r\n" : "&WFrost bite: (&R+++       &W)&D\r\n", ch);
  else if(percentage > 50)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 40%\r\n" : "&WFrost bite: (&Y++++      &W)&D\r\n", ch);
  else if(percentage > 40)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 50%\r\n" : "&WFrost bite: (&Y+++++     &W)&D\r\n", ch);
  else if(percentage > 30)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 60%\r\n" : "&WFrost bite: (&Y++++++    &W)&D\r\n", ch);
  else if(percentage > 20)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 70%\r\n" : "&WFrost bite: (&G+++++++   &W)&D\r\n", ch);
  else if(percentage > 10)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 80%\r\n" : "&WFrost bite: (&G++++++++  &W)&D\r\n", ch);
  else if(percentage > 0)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 90%\r\n" : "&WFrost bite: (&G+++++++++ &W)&D\r\n", ch);
  else if(percentage == 0)
    send_to_char(IS_BLIND(ch) ? "Frost bite: 100%\r\n" : "&WFrost bite: (&G++++++++++&W)&D\r\n", ch);
...
void                    breath_msg(CHAR_DATA *ch, int percentage)
{
  ...
  if(percentage > 90)
    send_to_char(IS_BLIND(ch) ? "Breath left: 0%\r\n" : "&WBreath left:(          &W)&D\r\n", ch);
  else if(percentage > 80)
    send_to_char(IS_BLIND(ch) ? "Breath left: 10%\r\n" : "&WBreath left: (&R+         &W)&D\r\n", ch);
  else if(percentage > 70)
    send_to_char(IS_BLIND(ch) ? "Breath left: 20%\r\n" : "&WBreath left: (&R++        &W)&D\r\n", ch);
  else if(percentage > 60)
    send_to_char(IS_BLIND(ch) ? "Breath left: 30%\r\n" : "&WBreath left: (&R+++       &W)&D\r\n", ch);
  else if(percentage > 50)
    send_to_char(IS_BLIND(ch) ? "Breath left: 40%\r\n" : "&WBreath left: (&Y++++      &W)&D\r\n", ch);
  else if(percentage > 40)
    send_to_char(IS_BLIND(ch) ? "Breath left: 50%\r\n" : "&WBreath left: (&Y+++++     &W)&D\r\n", ch);
  else if(percentage > 30)
    send_to_char(IS_BLIND(ch) ? "Breath left: 60%\r\n" : "&WBreath left: (&Y++++++    &W)&D\r\n", ch);
  else if(percentage > 20)
    send_to_char(IS_BLIND(ch) ? "Breath left: 70%\r\n" : "&WBreath left: (&G+++++++   &W)&D\r\n", ch);
  else if(percentage > 10)
    send_to_char(IS_BLIND(ch) ? "Breath left: 80%\r\n" : "&WBreath left: (&G++++++++  &W)&D\r\n", ch);
  else if(percentage > 0)
    send_to_char(IS_BLIND(ch) ? "Breath left: 90%\r\n" : "&WBreath left: (&G+++++++++ &W)&D\r\n", ch);
  else if(percentage == 0)
    send_to_char(IS_BLIND(ch) ? "Breath left: 100%\r\n" : "&WBreath left: (&G++++++++++&W)&D\r\n", ch);
...
/*  Bug fix - if not already underwater, fly/float will stop sinking.  */
  if                      (room->sector_type != SECT_UNDERWATER && (IS_AFFECTED(ch, AFF_FLYING) || IS_AFFECTED(ch, AFF_FLOATING)))
                            return;

  for                     (pexit = ch->in_room->first_exit; pexit; pexit = pexit->next)
  {
    if(pexit->to_room && (pexit->to_room->sector_type == SECT_UNDERWATER || pexit->to_room->sector_type == SECT_OCEANFLOOR) && pexit->vdir == DIR_DOWN)
    {
      if(!can_swim(ch) && (number_bits(time) == 0))
      { /* Chance to sink */
        act(AT_CYAN, "You sink down into the briny depths!", ch, NULL, NULL, TO_CHAR);
        act(AT_CYAN, "$n can't keep up $s paddle, and slowly sinks into the depths.", ch, NULL, NULL, TO_ROOM);

        if(pexit->to_room->first_person)
          act(AT_CYAN, "$n can't keep swimming above, and slowly sinks down before you.", pexit->to_room->first_person, NULL, NULL, TO_ROOM);
        char_from_room(ch);
...
/*  Let's deal with sinking objects too  */
      ...
      for                     (obj = room->first_content; obj; obj = obj->next_content)
      {
        if(number_bits(3) == 0)
        {
          ...
          {
            act(AT_CYAN, "$p slowly sinks down into the depths.", room->first_person, obj, NULL, TO_CHAR);
            act(AT_CYAN, "$p slowly sinks down into the depths.", room->first_person, obj, NULL, TO_ROOM);
          }
          if(pexit->to_room->first_person)
          {
            act(AT_CYAN, "$p sinks down from above.", pexit->to_room->first_person, obj, NULL, TO_CHAR);
            act(AT_CYAN, "$p sinks down from above.", pexit->to_room->first_person, obj, NULL, TO_ROOM);
          }
          obj_from_room(obj);
...
/*  Mobs can't drown - yet! Nor immortals. */
  if                      (IS_NPC(ch) || ch->Class == CLASS_BLUE || ch->Class == CLASS_VAMPIRE )
                            return;
...
/*  Players are swimming or above the surface of the water. They may also have a boat.  */
  ...
  if                      (ch->in_room->sector_type == SECT_WATER_SWIM || ch->in_room->sector_type == SECT_OCEAN || ch->in_room->sector_type == SECT_LAKE || ch->in_room->sector_type == SECT_RIVER)
  {
    ...
    if(xIS_SET(ch->act, PLR_BOAT))
    {
      ...
      if(mess < 2)
        send_to_char("The ship is nearly covered by a large wave, before suddenly rising back to the top.\r\n", ch);
      return;
    }
    ...
    int                     mov;
    int                     swim = ch->pcdata->learned[gsn_swim] - number_range(1, (ch->pcdata->learned[gsn_swim] / 2));

    if(ch->move > 0)
    { /* With moves, player can swim or * struggle to swim */
      ...
      if(swim > 90)
      {
        ...
        switch (number_range(1, 3))
        {
          ...
          case 1:
            send_to_char("&CYou roll over onto your back and switch to backstroke.&D\r\n", ch);
            learn_from_success(ch, gsn_swim);
            break;

          case 2:
            send_to_char("&CWith both arms in front of you, you confidently launch into a butterfly stroke.&D\r\n", ch);
            learn_from_success(ch, gsn_swim);
            break;

          case 3:
            send_to_char("&CKicking gracefully, you breaststroke through the water.&D\r\n", ch);
            learn_from_success(ch, gsn_swim);
            break;
        }
      }
      else if(swim > 70)
      {
        ...
        if(number_bits(3) == 0)
        {
          send_to_char("&CYou paddle through the water confidently, with even strokes.&D\r\n", ch);
          if(xIS_SET(ch->act, PLR_BATTLE) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_BATTLE)))
            send_to_char("!!SOUND(sound/swim.wav)\r\n", ch);
      ...
      else if(swim > 10)
      {
        ...
        if(number_bits(2) == 0)
        {
          send_to_char("&CYou are a little clumsy, but manage to paddle evenly enough in the water.&D\r\n", ch);
          learn_from_success(ch, gsn_swim);
          if(xIS_SET(ch->act, PLR_BATTLE) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_BATTLE)))
            send_to_char("!!SOUND(sound/swim.wav)\r\n", ch);
      ...
      else if(swim > 0)
      {
        ...
        if(number_bits(2) == 0)
        {
          send_to_char("&CYou struggle but manage to keep your head above the surface, barely.&D\r\n", ch);
    ...
    { /* No moves, start damaging hp * * (only a little - more chance to * * * sink) */
      ...
      {
        if(number_bits(3) == 0)
          send_to_char("&CYou panic as you sink underwater, but manage to breathe under it easily enough.&D\r\n", ch);
        return;
      }

      if(ch->in_room->vnum == inroom)
      { /* Character didn't sink */
        if(number_bits(1) == 0)
          send_to_char("&RToo exhausted to swim, you choke on a mouthful of water!&D\r\n", ch);
        if(xIS_SET(ch->act, PLR_BATTLE))
          send_to_char("!!SOUND(sound/swim.wav)\r\n", ch);
        ...
        send_to_char("&CToo exhausted to swim, you hold your breath and &Rsink&C underwater!&D\r\n", ch);
      if(xIS_SET(ch->act, PLR_BATTLE))
        send_to_char("!!SOUND(sound/swim.wav)\r\n", ch);
  ...
  { /* Players are underwater (and * * drowning). Swim has no affect. */
    ...
    if((percentage > 50 && number_bits(1) == 0) || number_bits(2) == 0)
    {
      ...
      if(number_bits(2) == 0)
      {
        if(percentage > 90)
          ...
          send_to_char("&CYou are starting to tire, and will need to surface soon.&D\r\n", ch);
        else if(percentage == 0)
