...
#define TMARK(room)  (SET_BIT((room)->room_flags, BFS_MARK))
#define UNTMARK(room)  (REMOVE_BIT((room)->room_flags, BFS_MARK))
...
void do_track(CHAR_DATA *ch, char *argument)
{
  ...
  if(xIS_SET(ch->act, PLR_BATTLE))
    send_to_char("!!SOUND(sound/track.wav)\r\n", ch);
...
void do_landmark(CHAR_DATA *ch, char *argument)
{
  ...
  ch->landmark = STRALLOC(argument);  /* Setup their landmark so it can be auto done */
  ...
  switch (dir)
  {
    ...
    case BFS_ALREADY_THERE:
      send_to_char("\r\n&cYou're at your landmark! and can now type &WEXPLORE&c to enter.&D", ch);
      if(ch->landmark)
        STRFREE(ch->landmark);
      break;
