...
void do_companion(CHAR_DATA *ch, char *argument)
{
  ...
  if(ch->race == RACE_ANIMAL || xIS_SET(ch->act, ACT_BEASTMELD) || ch->Class == CLASS_DRAGONLORD)
  {
    send_to_char("You cannot companion from that form.\r\n", ch);
    return;
  }
  ...
  {
    send_to_char("You must be fully healed to use companion command.\r\n", ch);
    return;
  }
  ...
  if(!str_cmp(arg, "revert"))
  {
    ...
    snprintf(buf, MIL, "%s has reverted back to their former status with the companion command.", ch->name);
...
// level range - Aurin 12/9/2010
  ...
  if(buddy)
  {
    ...
    restore_char(ch);
    snprintf(buf, MIL, "%s has used the companion command to regress to a lower status.", ch->name);
    echo_to_all(AT_MAGIC, buf, ECHOTAR_ALL);
