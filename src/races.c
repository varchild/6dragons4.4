    ...
    switch (UPPER(word[0]))
    {
      ...
      case 'E':
        if(!str_cmp(word, "End"))
        {
          ...
          if(ra < 0 || ra >= MAX_RACE)
          {
            bug("Load_race_file: Race (%s) bad/not found (%d)", race->race_name ? race->race_name : race_name ? race_name : "name not found", ra);
            if(race_name)
              STRFREE(race_name);
            DISPOSE(race);
            return FALSE;
...
void do_races(CHAR_DATA *ch, char *argument)
{
  ...
  for(iRace = 0; iRace < MAX_PC_RACE; iRace++)
  {
    ...
    if(IS_IMMORTAL(ch))
      pager_printf(ch, "%-1s &gMultiplier:&W %-5d &gHit: &W%-3d &gMana: &W%-3d", "", race_table[iRace]->exp_multiplier, race_table[iRace]->hit, race_table[iRace]->mana);
    pager_printf(ch, "\r\n");
    counter++;
  }
  return;
...
{
  struct race_type       *race;
  char                    buf[MSL], arg1[MIL], arg2[MIL], arg3[MIL];
  int                     value, v2, ra;
  ...
  write_race_file(ra);
  ch_printf(ch, "Updated %s on race %s[%d].\r\n", arg2, (race_table[ra] && VLD_STR(race_table[ra]->race_name)) ? race_table[ra]->race_name : "(Unknown)", ra);
  return;
}
...
void do_showrace(CHAR_DATA *ch, char *argument)
{
  ...
  ch_printf(ch, "Base: Str: %-3d Dex: %-3d Wis: %-3d Int: %-3d\r\n", race->base_str, race->base_dex, race->base_wis, race->base_int);
  ch_printf(ch, "      Con: %-3d Cha: %-3d Lck: %-3d\r\n", race->base_con, race->base_cha, race->base_lck);
  ...
  send_to_char(!xIS_EMPTY(race->affected) ? ext_flag_string(&race->affected, a_flags) : "Nothing", ch);
  send_to_char("\r\n", ch);
