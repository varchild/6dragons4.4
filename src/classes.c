...
const char             *const npc_class[MAX_NPC_CLASS] = {
  "shaman", "druid", "mage", "bat", "warrior", "crusader",
  "thief", "dragon lord", "black", "gold", "silver", "red", "blue", "hellspawn",
  "angel", "vampire", "monk", "bard", "shadowknight", "psionic", "hero",
  "twoheaded", "balrog", "sorcerer", "rogue", "wolf", "owl", "fish", "bear",
  "gryphon", "treant", "beastmaster"
};
...
bool create_new_class(int Class, char *argument)
{
  ...
  class_table[Class]->move_min = 0;
  class_table[Class]->move_max = 0;
...
void do_setclass(CHAR_DATA *ch, char *argument)
{
  ...
  if(!VLD_STR(arg1))
  {
    ...
    send_to_char("  hpmin hpmax manamin manamax expbase\r\n", ch);
    ...
    for(i = 0; i < MAX_CLASS; i++)
    {
      if(class_table[i] && VLD_STR(class_table[i]->who_name))
      {
        if(!str_cmp(argument, class_table[i]->who_name))
        {
          ...
          send_to_char("Done!\r\n", ch);
          send_to_char("Note this still isnt complete, as far as the nanny function is concerned!\r\n", ch);
          return;
        }
      }
    }
...
void do_showclass(CHAR_DATA *ch, char *argument)
{
  ...
  pager_printf(ch, "&wCLASS: &W%s\r\n&w", VLD_STR(Class->who_name) ? Class->who_name : "(Not set)");
  pager_printf(ch, "&wFilename: &W%s\r\n&w", VLD_STR(Class->filename) ? Class->filename : "(Not set)");
  ...
  send_to_char("\r\n", ch);
  pager_printf(ch, "&wMax Skill Adept: &W%-3d             &wThac0 : &W%-5d     &wThac32: &W%d\r\n", Class->skill_adept, Class->thac0_00, Class->thac0_32);
  pager_printf(ch,
               "&wHp Min/Hp Max  : &W%-2d/%-2d     &wMana Min/Max: &W%-2d/%-2d &wMove Min/Max:  &W%-2d/%-2d &wExpBase: &W%d\r\n",
               Class->hp_min, Class->hp_max, Class->mana_min, Class->mana_max, Class->move_min, Class->move_max, Class->exp_base);
  pager_printf(ch, "&W%s &wClass\r\n", Class->starting ? "Starting" : "Non-Starting");
  pager_printf(ch, "&wAffected by:  &W%s\r\n", !xIS_EMPTY(Class->affected) ? ext_flag_string(&Class->affected, a_flags) : "Nothing");
  ...
  if(arg2 && arg2[0] != '\0')
  {
    ...
    for(x = low; x <= hi; x++)
    {
      ...
      for(y = gsn_first_spell; y < gsn_top_sn; y++)
        ...
        {
          pager_printf(ch, "  &[skill]%-7s %-19s%3d     ", skill_tname[skill_table[y]->type], skill_table[y]->name, skill_table[y]->skill_adept[cl]);
          if(++cnt % 2 == 0)
            send_to_pager("\r\n", ch);
        }
      if(cnt % 2 != 0)
...
void do_classes(CHAR_DATA *ch, char *argument)
{
  ...
  for(iClass = 0; iClass < MAX_PC_CLASS; iClass++)
  {
    ...
    if(IS_IMMORTAL(ch))
      pager_printf(ch,
                   " &cStarting:&W%3s &cExpbase:&W %-5d &cHP: &W%3d-%-3d &cMana: &W%3d-%-3d &cMV: &W%3d-%-3d",
                   class_table[iClass]->starting ? "Yes" : "No",
                   class_table[iClass]->exp_base, class_table[iClass]->hp_min,
                   class_table[iClass]->hp_max, class_table[iClass]->mana_min, class_table[iClass]->mana_max, class_table[iClass]->move_min, class_table[iClass]->move_max);
    pager_printf(ch, "&D\r\n");
    counter++;
  }
  return;
      ...
      case 'E':
        if(!str_cmp(word, "End"))
          if(!str_cmp(word, "End"))
          {
            ...
            {
              bug("Load_class_file: Class (%s) bad/not found (%d)", Class->who_name ? Class->who_name : "name not found", cl);
              if(VLD_STR(Class->who_name))
                STRFREE(Class->who_name);
              if(VLD_STR(Class->filename))
                STRFREE(Class->filename);
...
void write_class_file(int cl)
{
  ...
  fprintf(fpout, "Reclass        %d %d %d\n", Class->reclass1, Class->reclass2, Class->reclass3);
...
{
  send_to_char("&c[&C20&c]&W Blacksmith\r\n&c[&C21&c]&W Baker\r\n&c[&C22&c]&W Tanner\r\n&c[&C24&c]&W Jeweler\r\n&c[&C25&c]&W Carpenter\r\n&c[&C26]&W Dragonic", ch);
