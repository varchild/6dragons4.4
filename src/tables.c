...
const char             *const skill_tname[] = { "unknown", "Spell", "Skill", "Weapon", "Tongue", "Racial", "Unused", "Song" };
  ...
  for(sn = 0; sn <= top_sn; sn++)
  {
    if((skill = skill_table[sn]) != NULL)
    {
      for(aff = skill->affects; aff; aff = aff->next)
        if(aff->location == APPLY_WEAPONSPELL || aff->location == APPLY_WEARSPELL || aff->location == APPLY_REMOVESPELL || aff->location == APPLY_STRIPSN || aff->location == APPLY_RECURRINGSPELL)
        {
...
void save_skill_table()
{
  ...
  {
    perror(SKILL_FILE);
    bug("%s Cannot open %s for writting", __FUNCTION__, SKILL_FILE);
    return;
  }
  for(x = 0; x < top_sn; x++)
  {
...
void save_herb_table()
{
  ...
  {
    bug("%s Cannot open %s for writting", __FUNCTION__, HERB_FILE);
    perror(HERB_FILE);
    return;
  }
...
// Look through city files and see if city bank = bank->name in question to protect it from getting deleted - Vladaar

          CITY_DATA              *city;

          for(city = first_city; city; city = city->next)
          {
            if(!str_cmp(bank->name, city->bank))
          ...
          if ( str_cmp( bank->name, "dakar" ) && str_cmp( bank->name, "paleon" ) && str_cmp(bank->name, "forbidden"))
          {
          if((current_time - bank->lastused) / 31622400)
          { /* If not used for 366 days get * rid of it */
