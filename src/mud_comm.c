...
void do_mpgenmob(CHAR_DATA *ch, char *argument)
{
  ...
  // check for basher mobs
  if(!str_cmp(ch->in_room->area->filename, "psewer.are") || !str_cmp(ch->in_room->area->filename, "dsewer.are"))
    vnum = 8722;
  else
  {
...
void do_mpkill(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg)) == NULL)
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpkill [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  if(victim == ch)
  {
...
{ /* get color code from command * string */
  char                    color[MIL];
  const char             *cptr;
...
void do_mpechoaround(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpechoaround [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
...
void do_mpoutcast(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "MpOutcast [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
...
void do_mpclear(CHAR_DATA *ch, char *argument)
{
  ...
  if(ch->in_room->vnum == ROOM_AUTH_START)
    ...
    progbug(ch, "Mpclear - Bad mob, [%s]! Bad arg, [%s]! Invalid room [%d]", ch->name, argument, ch->in_room->vnum);
...
void do_mpassist(CHAR_DATA *ch, char *argument)
{
  ...
  if(!(victim = get_char_room(ch, arg)))
  {
    progbug(ch, "MpAssist [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  if(victim->fighting)
  {
    ...
    if(!IS_NPC(victim))
      ch_printf(target, "%s takes up a fighting position and defends %s by attacking you .\r\n", ch->short_descr, victim->name);
    else
...
void do_mptrade(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mptrade [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }

  if(str_cmp(arg1, "dragonic") && (victim->race == RACE_DRAGON || victim->Class == CLASS_DRAGONLORD))
  {
    send_to_char("&cThe city manager is shaking too hard in fear that a dragon is present to think of trades.\r\n", victim);
    return;
  }
  ...
  if(!str_cmp(arg1, "trade"))
  {
    ...
    if(victim->pcdata->learned[gsn_jewelry] > 0)
      victim->pcdata->learned[gsn_jewelry] = 0;
    if(victim->pcdata->learned[gsn_fell] > 0)
      victim->pcdata->learned[gsn_fell] = 0;
    if(victim->pcdata->learned[gsn_mill] > 0)
      victim->pcdata->learned[gsn_mill] = 0;
    if(victim->pcdata->learned[gsn_unearth] > 0)
      victim->pcdata->learned[gsn_unearth] = 0;
    if(victim->pcdata->learned[gsn_produce] > 0)
      victim->pcdata->learned[gsn_produce] = 0;

    victim->pcdata->craftpoints = 0;
    act(AT_CYAN, "The city manager rips up $n's apprenticeship card.", victim, NULL, ch, TO_ROOM);
  ...
  else if(!str_cmp(arg1, "blacksmith"))
  {
    if(victim->pcdata->tradeclass > 0)
      return;
    act(AT_CYAN, "The city manager signs off on $n's apprenticeship card.", victim, NULL, ch, TO_ROOM);
    act(AT_CYAN, "The city manager signs off on your apprenticeship card.", ch, NULL, NULL, TO_CHAR);
  ...
  else if(!str_cmp(arg1, "jeweler"))
  {
    if(victim->pcdata->tradeclass > 0)
      return;
    act(AT_CYAN, "The city manager signs off on $n's apprenticeship card.", victim, NULL, ch, TO_ROOM);
    act(AT_CYAN, "The city manager signs off on your apprenticeship card.", ch, NULL, NULL, TO_CHAR);
    victim->pcdata->tradeclass = 24;
    victim->pcdata->tradelevel = 1;
    victim->pcdata->learned[gsn_mine] = 10;
    victim->pcdata->learned[gsn_jewelry] = 10;
    save_char_obj(victim);
  }
  else if(!str_cmp(arg1, "carpenter"))
  {
    if(victim->pcdata->tradeclass > 0)
      return;
    act(AT_CYAN, "The city manager signs off on $n's apprenticeship card.", victim, NULL, ch, TO_ROOM);
    act(AT_CYAN, "The city manager signs off on your apprenticeship card.", ch, NULL, NULL, TO_CHAR);
    victim->pcdata->tradeclass = 25;
    victim->pcdata->tradelevel = 1;
    victim->pcdata->learned[gsn_fell] = 10;
    victim->pcdata->learned[gsn_mill] = 10;
    save_char_obj(victim);
  }
  else if(!str_cmp(arg1, "dragonic"))
  {
    if(victim->pcdata->tradeclass > 0)
      return;
    act(AT_CYAN, "Ulave signs off on $n's apprenticeship card.", victim, NULL, ch, TO_ROOM);
    act(AT_CYAN, "Ulave signs off on your apprenticeship card.", ch, NULL, NULL, TO_CHAR);
    victim->pcdata->tradeclass = 26;
    victim->pcdata->tradelevel = 1;
    victim->pcdata->learned[gsn_unearth] = 10;
    victim->pcdata->learned[gsn_produce] = 10;
    save_char_obj(victim);
  }
...
void do_mpinduct(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpinduct [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  ...
  {
    send_to_char("You cannot join that quest for a clan, your primal instincts demand you stay aloof.\r\n", victim);
    return;
  }
    ...
    // clan recently!
  {
    ...
    if(timer < (sysdata.clan_timer * 86400))
    {
      ...
      if(days > 0)
        ch_printf(victim, "Outcast, you have %d days, %d hours and %d minutes until you can join another clan!\r\n", days, hours, mins);
      else if(hours > 0)
        ch_printf(victim, "Outcast, you have %d hours and %d minutes until you can join another clan!\r\n", hours, mins);
      else if(mins > 0)
        ch_printf(victim, "Outcast, you have %d mins and %d seconds until you can join another clan!\r\n", mins, timer);
      else if(timer > 0)
        ch_printf(victim, "Outcast, you have %d seconds until you can join another clan!\r\n", timer);
      else
        progbug(ch, "MpInduct - Target [%s] has a bugged pcdata->clan_timer!", victim->name);
...
//         return;
  ...
  if(timer < (sysdata.clan_timer * 86400))
  {
    ...
    if(!str_cmp(clan->name, quest->name) && (timer > 0 || days > 0 || mins > 0 || hours > 0))
    {
...
void do_mpechoat(CHAR_DATA *ch, char *argument)
{
  ...
  if(!(victim = get_char_world(ch, arg)))
  {
    progbug(ch, "Mpechoat [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
...
void do_mpsoundaround(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpsoundaround [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
...
void do_mpsoundat(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpsoundat [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
...
void do_mpmusicaround(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpmusicaround [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  actflags = ch->act;
  xREMOVE_BIT(ch->act, ACT_SECRETIVE);
...
void do_mpmusic(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpmusic [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  actflags = ch->act;
  xREMOVE_BIT(ch->act, ACT_SECRETIVE);
...
void do_mpmusicat(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpmusicat [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  actflags = ch->act;
  xREMOVE_BIT(ch->act, ACT_SECRETIVE);
...
void do_mpadvance(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_AVATAR(victim) || IS_DUALAVATAR(victim) || IS_TRIAVATAR(victim))
  {
    ...
    act(AT_IMMORT, "$n makes some arcane gestures with $s hands, then points $s fingers at you!", ch, NULL, victim, TO_VICT);
    act(AT_IMMORT, "$n makes some arcane gestures with $s hands, then points $s fingers at $N!", ch, NULL, victim, TO_NOTVICT);
...
void do_mpqkamount(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg[0] == '\0' || arg2[0] == '\0')
  {
    progbug(ch, "In do_mpqkamount - Bad syntax [%s]", ch->name);
    return;
  }

  if(!(victim = get_char_world(ch, arg)))
  {
    progbug(ch, "In do_mpqkamount - No such victim %s [%s]", arg, ch->name);
    return;
  }
  ...
  if(!quest)
  {
    progbug(ch, "Mpquest - No quest named [%s]! [%d][%s]", arg2, ch->pIndexData->vnum, ch->name);
    return;
  }
  ...
  {
    progbug(ch, "Mpquest - No chapter [%s]! [%d][%s]", argument, ch->pIndexData->vnum, ch->name);
    return;
  }
...
void do_mpquest(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(argument, "remove"))
  {
    ...
    for(chquest = victim->pcdata->first_quest; chquest; chquest = chquest->next)
    {
      ...
      UNLINK(chquest, victim->pcdata->first_quest, victim->pcdata->last_quest, next, prev);
      DISPOSE(chquest);
      ch_printf(victim, "Quest %s has been removed from you.\r\n", quest->name);
      return;
    }
    send_to_char("That player isn't currently on that quest.\r\n", ch);
    return;
  ...
  if(victim->level < quest->level && victim->race != RACE_DRAGON)
  {
    ch_printf(victim, "&RYou must be level %d to start this quest.\r\n&D", quest->level);
    return;
  }
  if(chap && chap->level > victim->level && victim->race != RACE_DRAGON)
  {
    ch_printf(victim, "&RYou must be level %d to start this chapter.\r\n&D", chap->level);
    return;
  }
...
void do_mpnuisance(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg1)) == NULL)
  {
    ...
    if(!(victim = get_char_world(ch, arg1)))
      progbug(ch, "Mpnuisance [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg1);
    return;
  }
  if(IS_NPC(victim))
  {
...
void do_mpunnuisance(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg1)) == NULL)
  {
    send_to_char("Victim must be in room.\r\n", ch);
    if(!(victim = get_char_world(ch, arg1)))
      progbug(ch, "Mpunnuisance [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg1);
    return;
  }
...
void do_mpbodybag(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg)) == NULL)
  {
    send_to_char("Victim must be in room.\r\n", ch);
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpbodybag [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  if(IS_NPC(victim))
  {
...
void do_mpmorph(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg1)) == NULL)
  {
    send_to_char("Victim must be in room.\r\n", ch);
    if(!(victim = get_char_world(ch, arg1)))
      progbug(ch, "Mpmorph [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg1);
    return;
  }
...
void do_mpunmorph(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg)) == NULL)
  {
    send_to_char("Victim must be in room.\r\n", ch);
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpunmorph [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  if(!victim->morph)
  {
...
void do_mp_practice(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg1)) == NULL)
  {
    ...
    if(!(victim = get_char_world(ch, arg1)))
      progbug(ch, "Mppractice [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg1);
    return;
  }
  ...
  if(victim->level < skill_table[sn]->skill_level[victim->Class]
     || victim->level < skill_table[sn]->skill_level[victim->secondclass] || victim->level < skill_table[sn]->skill_level[victim->thirdclass])
  {
    act_printf(AT_TELL, ch, NULL, victim, TO_VICT, "$n attempts to tutor you in %s, but it's beyond your comprehension.", skill_name);
    return;
  }
  ...
  {
    act_printf(AT_TELL, ch, NULL, victim, TO_VICT, "$n shows some knowledge of %s, but yours is clearly superior.", skill_name);
    return;
  }
...
void do_mpscatter(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg1)) == NULL)
  {
    send_to_char("Victim must be in room.\r\n", ch);
    if(!(victim = get_char_world(ch, arg1)))
      progbug(ch, "Mpscatter [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg1);
    return;
  }
...
void do_mp_slay(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg1)) == NULL)
  {
    send_to_char("Victim must be in room.\r\n", ch);
    if(!(victim = get_char_world(ch, arg1)))
      progbug(ch, "Mpslay [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg1);
    return;
  }
...
void do_mp_log(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, MSL, "&p%-2.2d/%-2.2d | %-2.2d:%-2.2d  &P%s:  &p%s", t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, ch->short_descr, argument);
...
void do_mp_restore(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg1)) == NULL)
  {
    send_to_char("Victim must be in room.\r\n", ch);
    if(!(victim = get_char_world(ch, arg1)))
      progbug(ch, "Mprestore [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg1);
    return;
  }
...
void do_mp_open_passage(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
  {
    progbug(ch, "MpOpenPassage - Bad syntax [%s][%d]", ch->name, ch->pIndexData ? ch->pIndexData->vnum : 0);
    return;
  }
...
void do_mpapply(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, MSL, "%s@%s new %s %s %s applying...",
           victim->name, victim->desc->host, race_table[victim->race]->race_name, class_table[victim->Class]->who_name, IS_PKILL(victim) ? "(Deadly)" : "(Peaceful)");
...
void do_mp_withdraw(CHAR_DATA *ch, char *argument)
{
  ...
  money = atoi(arg);
  if(GET_MONEY(ch, DEFAULT_CURR) < 1000000000 && money < 1000000000 && ch->in_room && economy_has(ch->in_room->area, money, DEFAULT_CURR))
  {
    GET_MONEY(ch, DEFAULT_CURR) += money;
...
void do_mpdelay(CHAR_DATA *ch, char *argument)
{
  ...
  if(!(victim = get_char_room(ch, arg)))
  {
    send_to_char("They aren't here.\r\n", ch);
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpdelay [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  if(IS_IMMORTAL(victim))
  {
...
void do_mpeat(CHAR_DATA *ch, char *argument)
{
  ...
  char                    arg[MIL];
  char                    buf[MSL];
  CHAR_DATA              *victim, *target_victim = NULL, *utarget = NULL;
  int                     vnum;
  ROOM_INDEX_DATA        *location;
  ...
  act(AT_RED, "$n spots $N holding her dragon egg, and with her mighty jaws, devours $N!", ch, NULL, victim, TO_ROOM);
  ...
  obj_from_char(dragonegg); /* Take the egg from the one carrying it */
  char_from_room(victim);
  ...
  for(vnum = 4400; vnum <= 4421; vnum++)
  {
    if(!(location = get_room_index(vnum)))
      continue;
    for(target_victim = location->first_person; target_victim; target_victim = target_victim->next_in_room)
    {
...
void do_mppeace(CHAR_DATA *ch, char *argument)
{
  ...
  if((victim = get_char_room(ch, arg)) == NULL)
  {
    ...
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mppeace [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
  if(victim->fighting)
    stop_fighting(victim, TRUE);
...
void do_mpmset(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(IS_NPC(victim))
    { /* Broken by Haus... fixed by * Thoric */
...
void do_mpboat(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!(victim = get_char_world(ch, arg)))
      progbug(ch, "Mpboat [%d]- victim [%s] does not exist", ch->pIndexData ? ch->pIndexData->vnum : 0, arg);
    return;
  }
