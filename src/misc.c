...
void do_fill(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg2[0] != '\0')
  {
    if((dest_item == ITEM_CONTAINER || dest_item == ITEM_QUIVER) && (!str_cmp(arg2, "all") || !str_prefix("all.", arg2)))
    {
      all = TRUE;
      source = NULL;
    }
    ...
    else
    {
      ...
      {
        ch_printf(ch, "You cannot find anything to fill %s with.\r\n", obj->short_descr );
        return;
      }
    }
  }
    ...
    switch (source->item_type)
    {
      default: /* put something in container */
        if(!source->in_room /* disallow inventory items */
           ...
           || (IS_OBJ_STAT(source, ITEM_PROTOTYPE) && !can_take_proto(ch))
           || ch->carry_weight + get_obj_weight(source, FALSE) > can_carry_w(ch)
           || (get_real_obj_weight(source) + get_real_obj_weight(obj) / obj->count) > obj->value[0] || (obj->item_type == ITEM_QUIVER && source->item_type != ITEM_PROJECTILE))
        {
      ...
      case ITEM_CONTAINER:
        if(source->item_type == ITEM_CONTAINER && IS_SET(source->value[1], CONT_CLOSED))
        {
      ...
      case ITEM_CORPSE_NPC:
        ...
        for(; otmp; otmp = otmp_next)
        {
          ...
          if(!CAN_WEAR(otmp, ITEM_TAKE)
             ...
             || (get_real_obj_weight(source) + get_real_obj_weight(obj) / obj->count) > obj->value[0] || (obj->item_type == ITEM_QUIVER && otmp->item_type != ITEM_PROJECTILE))
            continue;
          obj_from_obj(otmp);
          obj_to_obj(otmp, obj);
          found = TRUE;
...
void do_drink(CHAR_DATA *ch, char *argument)
{
  ...
  switch (obj->item_type)
  {
    ...
    case ITEM_BLOOD:
      if(IS_BLOODCLASS(ch) && !IS_NPC(ch))
      {
        ...
        send_to_char("It is not in your nature to drink blood.\r\n", ch);
      break;

    case ITEM_FOUNTAIN:
      if(!oprog_use_trigger(ch, obj, NULL, NULL, NULL))
      {
        act(AT_ACTION, "$n drinks from $p.", ch, obj, NULL, TO_ROOM);
    ...
    case ITEM_DRINK_CON:
      ...
      /*
       * Ok adding affects based on ingriedents 
       */
      if(!IS_NPC(ch) && obj->extra > 0)
      {

        if(ch->pcdata->tmp_resist > 0)  // remove resist if already have
          // one
        {
        ...
        if(obj->extra == 5)
        {
          ch->pcdata->tmp_resist = 1;
          SET_BIT(ch->resistant, 1);
        }
        else if(obj->extra == 6)
        {
          ch->pcdata->tmp_resist = 2;
          SET_BIT(ch->resistant, 2);
        }
        else if(obj->extra == 7)
        {
          ch->pcdata->tmp_resist = 4;
          SET_BIT(ch->resistant, 4);
        }
        else if(obj->extra == 8)
        {
          ch->pcdata->tmp_resist = 8;
          SET_BIT(ch->resistant, 8);
        }
        else if(obj->extra == 9)
        {
          ch->pcdata->tmp_resist = 16;
          SET_BIT(ch->resistant, 16);
        }
        else if(obj->extra == 10)
        {
          ch->pcdata->tmp_resist = 32;
          SET_BIT(ch->resistant, 32);
        }
        else if(obj->extra == 11)
        {
          if(!IS_NPC(ch) && ch->race != RACE_DWARF)
            ch->pcdata->condition[COND_DRUNK] = 60;
          if(!IS_NPC(ch) && ch->race == RACE_DWARF)
            ch->pcdata->condition[COND_DRUNK] = 10;
        }
        else if(obj->extra == 12)
        {
          ch->pcdata->tmp_resist = 128;
          SET_BIT(ch->resistant, 128);
        }
        else if(obj->extra == 13)
        {
          ch->pcdata->tmp_resist = 256;
          SET_BIT(ch->resistant, 256);
        }
        else if(obj->extra == 14)
        {
          ch->pcdata->tmp_resist = 512;
          SET_BIT(ch->resistant, 512);
        }
        else if(obj->extra == 15)
        {
          ch->pcdata->tmp_resist = 2048;
          SET_BIT(ch->resistant, 2048);
        }
        else if(obj->extra == 16)
        {
          ch->pcdata->tmp_resist = 4096;
          SET_BIT(ch->resistant, 4096);
        }
        else if(obj->extra == 17)
        {
          ch->pcdata->tmp_resist = 2097152;
          SET_BIT(ch->resistant, 2097152);
        }
        else if(obj->extra == 18)
        {
          ch->pcdata->tmp_resist = 1048576;
          SET_BIT(ch->resistant, 1048576);

        }
        else if(obj->extra == 19)
        {
          ch->pcdata->tmp_resist = 8388608;
          SET_BIT(ch->resistant, 8388608);
        }
        else if(obj->extra == 20)
        {
          ch->pcdata->tmp_resist = 16777216;
          SET_BIT(ch->resistant, 16777216);
        }
        else if(obj->extra == 21)
        {
          af.type = gsn_noscent;
          if(obj->value[6] == 1)
          ...
          af.modifier = 0;
          af.bitvector = meb(AFF_NOSCENT);
          if(obj->value[6] == 1)
            af.level = (obj->level / 2) + 100;
          if(obj->value[6] == 2)
            af.level = (obj->level / 2) + 125;
          if(obj->value[6] == 3)
            af.level = (obj->level / 2) + 250;
          affect_join(ch, &af);
          act(AT_MAGIC, "Your odor is masked from wilderness beasts.", ch, NULL, NULL, TO_CHAR);
          act(AT_MAGIC, "$n's odor is masked from wilderness beasts.", ch, NULL, NULL, TO_ROOM);
        }
        else if(obj->extra == 22)
        {
          af.type = gsn_easy_victim;
          if(obj->value[6] == 1)
            af.duration = (obj->level / 2) + 100;
          if(obj->value[6] == 2)
            af.duration = (obj->level / 2) + 125;
          ...
          if(obj->value[6] == 3)
            af.level = (obj->level / 2) + 250;
          affect_join(ch, &af);
          act(AT_MAGIC, "Your odor is enhanced to wilderness beasts.", ch, NULL, NULL, TO_CHAR);
          act(AT_MAGIC, "$n's odor is enhanced to wilderness beasts.", ch, NULL, NULL, TO_ROOM);

        }
        else if(obj->extra == 23)
        {
          af.type = gsn_defend;
          ...
          af.modifier = -30;
          affect_join(ch, &af);
          act(AT_WHITE, "Your reflexes and dexterity are suddenly razor sharp.", ch, NULL, NULL, TO_CHAR);
        }
        else if(obj->extra == 24)
        { // pepper ingredient
          // Make this a cure wounds instead
          if(ch->hit < ch->max_hit)
            ch->hit += number_range(15, 30) * obj->value[6];
          if(ch->hit > ch->max_hit)
            ch->hit = ch->max_hit;
          act(AT_WHITE, "Your wounds begin to heal up.", ch, NULL, NULL, TO_CHAR);
        }
      ...
      if(obj->value[6] == 1)
      {
        act(AT_ACTION, "MmMmh, nothing satisfies like homemade mixed drink.", ch, NULL, NULL, TO_CHAR);
        act(AT_MAGIC, "\r\nYou feel a warmth fill your being.", ch, NULL, NULL, TO_CHAR);
        af.type = gsn_crafted_drink1;
        af.duration = (obj->level / 2) + 100;
  ...
  if(IS_NPC(ch) && xIS_SET(ch->act, ACT_PET))
  {
    ...
    if(owner)
    {
      ...
      {
        act(AT_ACTION, "$n is no longer thirsty.", ch, NULL, NULL, TO_ROOM);
      }
      else if(owner->pcdata->petthirsty >= 1000)
      {
        act(AT_ACTION, "$n is bloated.", ch, NULL, NULL, TO_ROOM);
        return;
      }
...
// Add a check here for eating pills/herbs

    if(!IS_NPC(ch) && (obj->item_type != ITEM_PILL && obj->item_type != ITEM_HERB) && ch->pcdata->condition[COND_FULL] >= STATED)
    {
      send_to_char("You are too full to eat more.\r\n", ch);
      return;
    }

    if(!IS_NPC(ch) && ch->fighting && (obj->item_type == ITEM_PILL || obj->item_type == ITEM_HERB))
    {
      ...
      if(IS_NPC(who_fighting(ch)))  // PvE
      {
        ...
        {
          ch_printf(ch, "You must wait %d seconds before eating that.\r\n", sysdata.potionspve - timer);
          return;
        }
      }
      else if(!IS_NPC(who_fighting(ch)))  // PvP
      {
  ...
  if(ch->fighting && number_percent() > (get_curr_dex(ch) * 2 + 47))
  {
    snprintf(buf, MSL, "%s",
             (ch->in_room->sector_type == SECT_UNDERWATER ||
              ...
              SECT_WATER_NOSWIM) ? "dissolves in the water" : (ch->in_room->sector_type == SECT_AIR || IS_SET(ch->in_room->room_flags, ROOM_NOFLOOR)) ? "falls far below" : "is trampled underfoot");
    ...
    switch (obj->item_type)
    {
      ...
      case ITEM_FOOD:
        ...
        if(!IS_NPC(ch))
        {
          ...
          if(obj->value[6] == 1)
          {
            act(AT_ACTION, "MmMmh, nothing satisfies like homemade food.", ch, NULL, NULL, TO_CHAR);
            act(AT_MAGIC, "\r\nYou feel a warmth fill your being.", ch, NULL, NULL, TO_CHAR);
            af.type = gsn_crafted_food1;
            af.duration = (obj->level / 2) + 100;
            af.location = APPLY_AFFECT;
            af.modifier = 0;
...
/*
strawberries = 5 watermelons = 6 lettuce = 7 pickles = 8 wheat = 9 oranges = 10 peaches = 11
carrots = 12 cinnamon = 13 fig = 14 blueberries = 15 pears = 16 onions = 17 sugar-cane = 18 ginger = 19
raspberries = 20 corn = 21 radishes = 22 garlic = 23 pepper = 24
*/
          ...
          if(!IS_NPC(ch) && obj->extra > 0)
          {
            if(ch->pcdata->tmp_resist > 0)  // remove resist if already
              // have one
            {
              REMOVE_BIT(ch->resistant, ch->pcdata->tmp_resist);
            }
            ...
            {
              ch->pcdata->tmp_resist = 2097152; // doh, it looks like I
              // need this to be a int
              // instead of short
              SET_BIT(ch->resistant, 2097152);
            }
            else if(obj->extra == 18)
            ...
            {
              ch->pcdata->tmp_resist = 64;
              SET_BIT(ch->resistant, 64);
            }
          ...
          edible = obj->value[0];
          edible *= 2;  // due to stated = 200 now
          foodcond *= 2;
          gain_condition(ch, COND_FULL, (edible * foodcond) / 8);

          if(ch->pcdata->condition[COND_FULL] >= 1 && ch->pcdata->condition[COND_FULL] < STATED)
        ...
        {
          if((obj->value[3] != 0
              || (foodcond < 4 && number_range(0, foodcond + 1) == 0) || (obj->item_type == ITEM_COOK && obj->value[2] == 0)) && ch->Class != CLASS_HELLSPAWN && ch->race != RACE_DRAGON)
          {
            /*
  ...
  if(IS_NPC(ch) && xIS_SET(ch->act, ACT_PET))
  {
    ...
    if(owner)
    {
      ...
      {
        act(AT_ACTION, "$n is no longer hungry.", ch, NULL, NULL, TO_ROOM);
      }
      else if(owner->pcdata->pethungry >= 1000)
      {
        act(AT_ACTION, "$n is full.", ch, NULL, NULL, TO_ROOM);
        return;
      }
...
// Volk: If we've reached this stage, check their potion timer first!!
  ...
  else
  {
    ...
    if(!IS_NPC(ch) && ch->pcdata->condition[COND_THIRST] <= STATED)
...
void pullorpush(CHAR_DATA *ch, OBJ_DATA *obj, bool pull)
{
  ...
  if(!oprog_use_trigger(ch, obj, NULL, NULL, NULL))
  {
    ...
    if(pSlots)
    {
      act_printf(AT_ACTION, ch, obj, NULL, TO_CHAR, "You %s $p lever.", pull ? "pull" : "push");
      act_printf(AT_ACTION, ch, obj, NULL, TO_ROOM, "$n %s $p lever.", pull ? "pulls" : "pushes");
    }
    else
    {
