...
void do_red(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_IMMORTAL(ch) && !xIS_SET(ch->act, PLR_PLAYING) && !xIS_SET(ch->act, PLR_WAITING))
  {
...
void end_tag(void)
{
  ...
  for(d = first_descriptor; d; d = d_next)
  {
    ...
    if(d->connected != CON_PLAYING || !d->character || !d->character->in_room)
      continue;
    if(d->character->in_room->vnum < FTAG_MIN_VNUM - 1 || d->character->in_room->vnum > FTAG_MAX_VNUM)
      continue;
    ...
    ch_printf(d->character, "Your glory has been increased by 10 for Playing Freeze TAG!\r\n");
  }
}
...
void check_team_frozen(CHAR_DATA *ch)
{
  ...
  for(d = first_descriptor; d; d = d_next)
  {
    ...
    if(cred && !xIS_SET(d->character->act, PLR_RED))
      continue;
    if(!xIS_SET(d->character->act, PLR_FROZEN)) /* At least one person on the team isn't
                                                 * frozen */
      return;
  }
   ...
   */
  snprintf(buf, sizeof(buf), "&R[&WFreeze Tag&R] &WThe %s &Wteam has won FREEZE TAG!!!", cblue ? "&RRED" : "&CBLUE");
  tag_channel(NULL, buf);
  end_tag();
}
...
void do_ftag(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg1, "start"))
  {
    ...
    {
      send_to_char("You can't afford the 10 silver it cost to start a game of freeze tag.\r\n", ch);
      return;
    }
    if(tag_game.status != TAG_OFF)
    {
    ...
    tag_channel(NULL, "&R[&WEvent&R] &CA Freeze Tag Game has started! Type '&WFTAG JOIN&C' to play!");
...
void do_tag(CHAR_DATA *ch, char *argument)
{
  ...
  if((xIS_SET(ch->act, PLR_RED) && xIS_SET(victim->act, PLR_RED)) || (xIS_SET(ch->act, PLR_BLUE) && xIS_SET(victim->act, PLR_BLUE)))
  {
...
void auto_tag(void)
{
  ...
  if(count < 8)
    return;
  tag_channel(NULL, "&R[&WEvent&R] &CA Freeze Tag Game has started! Type '&cftag join&C' to play!");
  tag_game.status = TAG_ISWAIT;
  tag_game.timer = 3;
  tag_game.playing = 0;
  tag_game.next = -1;
...
void tag_update(void)
{
  ...
  else if(tag_game.status == TAG_ISWAIT)
  {
    ...
    if(tag_game.timer > 0)
    {
      sprintf(buf, "&R[&WFreeze Tag&R] &C%d minute%s left to join freeze tag.", tag_game.timer, tag_game.timer == 1 ? "" : "s");
      tag_channel(NULL, buf);
    }
    else
    {
      ...
      {
        end_tag();
        sprintf(buf, "&R[&WFreeze Tag&R] &CNot enough people for freeze tag. Game aborted.");
        tag_channel(NULL, buf);
        return;
      }
      else
  ...
  else if(tag_game.status == TAG_ISPLAY)
  {
    ...
    {
      end_tag();
      sprintf(buf, "&R[&WFreeze Tag&R] &CNo one left in freeze tag, next game in %d minutes.", tag_game.next);
      tag_channel(NULL, buf);
      return;
    }
    switch (tag_game.timer)
    {
      case 0:
        end_tag();
        sprintf(buf, "&R[&WFreeze Tag&R] &CTime has run out for freeze tag, next game will start in %d minutes.", tag_game.next);
        tag_channel(NULL, buf);
        return;
      ...
      case 10:
      case 15:
        sprintf(buf, "&R[&WFreeze Tag&R]&Y %d &C minute%s remaining in freeze tag.", tag_game.timer, tag_game.timer > 1 ? "s" : "");
        tag_channel(NULL, buf);
      default:
        tag_game.timer--;
        break;
...
bool is_tagging(CHAR_DATA *ch)
{
  ...
  if(xIS_SET(ch->act, PLR_PLAYING) && xIS_SET(ch->act, PLR_WAITING) && tag_game.status != TAG_OFF)
    return TRUE;
