...
void load_banlist(void)
{
  ...
  {
    bug("load_banlist: Cannot open %s", BAN_LIST);
    perror(BAN_LIST);
    return;
  }
  for(;;)
  {
    ...
    switch (UPPER(word[0]))
    {
      ...
      case 'E':
        if(!str_cmp(word, "END"))
        { /* File should always contain END */
          FileClose(fp);
          log_string("Done.");
          return;
        }
...
void fread_ban(FILE * fp, int type)
{
  ...
  if(type == BAN_RACE || type == BAN_CLASS)
  {
    ...
    { /* The file is corupted throw out * * * * this ban structure */
  ...
  { /* Bad type throw out the ban * * * * structure */
...
void save_banlist(void)
{
  ...
  for(pban = first_ban; pban; pban = pban->next)
  {
    fprintf(fp, "SITE\n");
    fprintf(fp, "%s~\n", pban->name);
    fprintf(fp, "%d %d %d %d %d %d\n", pban->level, pban->duration, pban->unban_date, pban->prefix, pban->suffix, pban->warn);
...
void do_allow(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_NPC(ch))
  { /* No mobs allowing sites */
    send_to_char("Monsters are too dumb to do that!\r\n", ch);
    return;
  }

  if(!ch->desc)
  { /* No desc is a bad thing */
  ...
  if(arg1[0] == '\0' || arg2[0] == '\0')
    goto syntax_message;
  if(arg2[0] == '#')
  { /* Use #1 to ban the first ban in * the list specified */
    temp = arg2;
    temp++;
    if(!is_number(temp))
    {
...
int add_ban(CHAR_DATA *ch, char *arg1, char *arg2, int btime, int type)
{
  ...
  switch (ch->substate)
  {
    ...
    case SUB_NONE:
    {
      ...
      {
        level = atoi(arg2);
        if(level < 0 || level > LEVEL_AJ_GENERAL)
        {
      ...
      switch (type)
      {
        case BAN_CLASS:
          ...
          CREATE(pban, BAN_DATA, 1);
          pban->name = STRALLOC(class_table[value]->who_name);
          pban->flag = value;
          pban->level = level;
        ...
        case BAN_RACE:
          ...
          CREATE(pban, BAN_DATA, 1);
          pban->name = STRALLOC(race_table[value]->race_name);
          pban->flag = value;
          pban->level = level;
          pban->ban_by = STRALLOC(ch->name);
          LINK(pban, first_ban_race, last_ban_race, next, prev);
        ...
        case BAN_SITE:
        {
          ...
          if(!name)
          { /* Double check to make sure name * isnt null */
            /*
...
void show_bans(CHAR_DATA *ch, int type)
{
    ...
    case BAN_SITE:
      ...
      for(bnum = 1; pban; pban = pban->next, bnum++)
      {
        pager_printf(ch, "[%2d] %-4s (%2d) %-24s %-15s %4d  %c%s%c\r\n",
                     bnum, (pban->warn) ? "YES" : "no", pban->level, pban->ban_time, pban->ban_by, pban->duration, (pban->prefix) ? '*' : ' ', pban->name, (pban->suffix) ? '*' : ' ');
      }
      return;
    case BAN_RACE:
  ...
  set_pager_color(AT_PLAIN, ch);
  for(bnum = 1; pban; pban = pban->next, bnum++)
    pager_printf(ch, "[%2d] %-4s (%2d) %-24s %-15s %4d  %s\r\n", bnum, (pban->warn) ? "YES" : "no", pban->level, pban->ban_time, pban->ban_by, pban->duration, pban->name);
  return;
}
...
bool check_total_bans(DESCRIPTOR_DATA *d)
{
  ...
  for(pban = first_ban; pban; pban = pban->next)
  {
    if(pban->level != LEVEL_AJ_GENERAL)
      continue;
  ...
  for(; pban; pban = pban->next)
  {
    if(type == BAN_CLASS && pban->flag == ch->Class)
    {
      ...
      if(ch->level > pban->level)
      {
        if(pban->warn)
        {
          snprintf(buf, MSL, "%s class logging in from %s.", pban->name, ch->desc->host);
    ...
    if(type == BAN_RACE && pban->flag == ch->race)
    {
      ...
      if(ch->level > pban->level)
      {
        if(pban->warn)
        {
          snprintf(buf, MSL, "%s race logging in from %s.", pban->name, ch->desc->host);
          log_string_plus(buf, LOG_WARN, sysdata.log_level);
    ...
    if(type == BAN_SITE)
    {
      ...
      if(fMatch)
      {
        ...
        if(ch->level > pban->level)
        {
          if(pban->warn)
          {
            snprintf(buf, MSL, "%s logging in from site %s.", ch->name, ch->desc->host);
            log_string_plus(buf, LOG_WARN, sysdata.log_level);
