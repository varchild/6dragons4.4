...
void do_dnd(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_NPC(ch) && ch->pcdata)
    ...
    else
    {
      ...
      interpret(ch, (char *)"sooc Turning all channels off to get work done.  Please do not disturb with beep command unless emergency.");
      interpret(ch, (char *)"ooc Turning all channels off to get work done.  Please do not disturb with beep command unless emergency.");
...
{
  CMDTYPE                *cmd;
  int                     value = 0, col, hash, curr_lvl, count;
      ...
      for(cmd = command_hash[hash]; cmd; cmd = cmd->next)
      {
        ...
        if((cmd->level == curr_lvl) && cmd->level <= MAX_LEVEL)
        {
          if(ch->level < (MAX_LEVEL - 2) && cmd->cshow == FALSE)
            continue;
          if(VLD_STR(ch->pcdata->bestowments) && is_name(cmd->name, ch->pcdata->bestowments))
          {
          ...
          if(count == 1)
            pager_printf(ch, "\r\n[LEVEL %-2d]\r\n", curr_lvl);
          col++;
          pager_printf(ch, "%s&W%-13.13s%s", cmd->do_fun == skill_notfound ? "&R[&D" : " ", cmd->name, cmd->do_fun == skill_notfound ? "&R]&D" : " ");
          if(col == 5)
          {
            send_to_pager("\r\n", ch);
            col = 0;
...
void do_rank(CHAR_DATA *ch, char *argument)
{
  ...
  else
  {
...

    {
      ...
      while(rcount > 0)
      {
        if(rankstring[rcount] == '&' || rankstring[rcount] == '^' || rankstring[rcount] == '\0')
          rankstring[rcount] == '\0';
        else
          break;
        rcount--;
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    if((int)d->descriptor == desc)
    {
      if(get_trust(victim) >= get_trust(ch))
      {
        send_to_char("&RNever try to use a command against a higher Staff member...\r\n", ch);
        send_to_char("&RYou have been warned....\r\n", ch);
        act(AT_RED, "$n tried to FileClose you against your wishes.", ch, NULL, victim, TO_VICT);
        return;
      }
...
void announce_all(short AT_COLOR, const char *argument, short tar)
{
  DESCRIPTOR_DATA        *d;
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    if(d->connected == CON_PLAYING || d->connected == CON_EDITING)
    {
      ...
      if(tar == ECHOTAR_PC && IS_NPC(d->character))
        continue;
      else if(tar == ECHOTAR_IMM && !IS_STAFF(d->character))
        continue;
      set_char_color(AT_COLOR, d->character);
      send_to_char(argument, d->character);

      send_to_char("\r\n", d->character);
      if(xIS_SET(d->character->act, PLR_COMMUNICATION))
        send_to_char("!!SOUND(sound/announce.wav)\r\n", d->character);
...
void do_send(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg1[0] == '\0')
  {
    send_to_char("send whom (and where)?\r\n", ch);
    return;
  }
  if(!str_cmp(arg1, "all") && get_trust(ch) >= LEVEL_AJ_SGT)
  {
    for(d = first_descriptor; d; d = d->next)
    {
      if(d->connected == CON_PLAYING && d->character != ch && d->character->in_room && d->newstate != 2 && can_see(ch, d->character))
      {
        char                    buf[MSL];
     ...
     && victim->desc && (victim->desc->connected == CON_PLAYING || victim->desc->connected == CON_EDITING) && IS_SET(victim->pcdata->flags, PCFLAG_DND))
  {
    ...
    pager_printf(victim, "Your DND flag just foiled %s's send command.\r\n", ch->name);
    return;
  }
  /*
   * end of modification 
  ...
  if(get_trust(victim) >= get_trust(ch))
  {
    send_to_char("&RNever try to use a command against a higher Staff member...\r\n", ch);
    send_to_char("&RYou have been warned....\r\n", ch);
    act(AT_RED, "$n tried to send you against your wishes.", ch, NULL, victim, TO_VICT);
    return;
  }
  ...
  if(ch != victim)
    act(AT_IMMORT, "$n has sent you.", ch, NULL, victim, TO_VICT);
  do_look(victim, (char *)"auto");
  if(!IS_STAFF(victim) && !IS_NPC(victim) && !in_hard_range(victim, location->area))
    act(AT_DANGER, "Warning:  this player's level is not within the area's level range.", ch, NULL, NULL, TO_CHAR);
}

void do_resend(CHAR_DATA *ch, char *argument)
{
  char                    arg[MIL];
  CHAR_DATA              *victim;
  char                    buf[MSL];
...
void do_rstat(CHAR_DATA *ch, char *argument)
{
  ...
  AFFECT_DATA            *paf;
  int                     cnt;
  const static char      *dir_text[] = { "n", "e", "s", "w", "u", "d", "ne", "nw", "se", "sw", "?", "ex" };
  ...
  if(!str_cmp(arg, "ex") || !str_cmp(arg, "exits"))
  {
    ...
    for(cnt = 0, pexit = location->first_exit; pexit; pexit = pexit->next)
      ch_printf(ch,
                ...
                VLD_STR(pexit->description) ? pexit->description : "(none).\r\n", pexit->rexit ? pexit->rexit->vnum : 0, pexit->rvnum, pull_type_name(pexit->pulltype), pexit->pull);
    return;
  }
  ...
  ch_printf(ch, "&cVnum: &w%d   &cSector: &w%d (%s)   &cLight: &w%d", location->vnum, location->sector_type, sect, location->light);
  ...
  for(paf = location->first_affect; paf; paf = paf->next)
    ch_printf(ch, "&cAffect: &w%s &cby &w%d.\r\n", flag_string(paf->location, a_types), paf->modifier);
  ...
  for(cnt = 0, pexit = location->first_exit; pexit; pexit = pexit->next)
    ch_printf(ch, "%2d) %-2s to %-5d.  Key: %d  Keywords: %s  Flags: %s.\r\n", ++cnt,
              dir_text[pexit->vdir], pexit->to_room ? pexit->to_room->vnum : 0, pexit->key, VLD_STR(pexit->keyword) ? pexit->keyword : "(none)", flag_string(pexit->exit_info, ex_flags));
  return;
}
...
void do_ostat(CHAR_DATA *ch, char *argument)
{
  ...
  ch_printf(ch, "&cArea: &w%s &c-&W %s\r\n", obj->pIndexData->area ? obj->pIndexData->area->name : "(none)", obj->pIndexData->area ? obj->pIndexData->area->filename : "(none)");
  ch_printf(ch, "&cType: &w%s  ", item_type_name(obj));
  ch_printf(ch, "&cCount:  &w%d  ", obj->pIndexData->count);
  if(obj->whichcity != NULL)
    ch_printf(ch, "&cCity:   &w%s  ", obj->whichcity);
  ...
  if(obj->size == 77)
  {
    ch_printf(ch, "&cSize:&w %d\r\n", obj->size);
  }
  else
  {
    ch_printf(ch, "&cSize:&w %d, %s (proto %d)\r\n", obj->size, obj_sizes[obj->size], obj->pIndexData->size);
  }
  ch_printf(ch, "&cExtra:&w %d &cExtra2:&w %d &cExtra3:&w %d &cExtra4:&w %d\r\n", obj->extra, obj->extra2, obj->extra3, obj->extra4);
  if(obj->pIndexData->vnum == 16243 || obj->pIndexData->vnum == 16244
     || obj->pIndexData->vnum == 11247 || obj->pIndexData->vnum == 16245
     || obj->pIndexData->vnum == 11248 || obj->pIndexData->vnum == 11249
     || obj->pIndexData->vnum == 56107 || obj->pIndexData->vnum == 56108
     || obj->pIndexData->vnum == 56106 || obj->pIndexData->vnum == 10967 || obj->pIndexData->vnum == 16246 || obj->pIndexData->vnum == 11250 || obj->pIndexData->vnum == 35548)
  {
    ch_printf(ch, "&cCrafts: &w%d %d %d %d %d %d %d %d %d %d\r\n%d %d %d %d %d %d %d %d %d %d",
              obj->craft1, obj->craft2, obj->craft3, obj->craft4, obj->craft5, obj->craft6,
              obj->craft7, obj->craft8, obj->craft9, obj->craft10, obj->craft11, obj->craft12,
              obj->craft13, obj->craft14, obj->craft15, obj->craft16, obj->craft17, obj->craft18, obj->craft19, obj->craft20);
  }

  ch_printf(ch, "&cColor: &w%-2d   &cOwner: &w%-12s &cGlory: &w%-3d&c/&w%-3d\r\n", obj->color, obj->owner ? obj->owner : "(nobody)", obj->glory, (obj->level * 2) - 1);
  ...
  ch_printf(ch, "&cIn object: &w%s  ", obj->in_obj == NULL ? "(none)" : obj->in_obj->short_descr);
  if(obj->auctioned)
  ...
  ch_printf(ch, "&cIndex Values : &w%d %d %d %d %d %d %d.\r\n", obj->pIndexData->value[0],
            obj->pIndexData->value[1], obj->pIndexData->value[2], obj->pIndexData->value[3], obj->pIndexData->value[4], obj->pIndexData->value[5], obj->pIndexData->value[6]);
...
{
  char                    arg[MIL], hpbuf[MSL], mnbuf[MSL], mvbuf[MSL], bdbuf[MSL];
  AFFECT_DATA            *paf;
  CHAR_DATA              *victim;
  SKILLTYPE              *skill;
  int                     x;
  ...
  if(IS_NPC(victim) && get_trust(ch) < LEVEL_AJ_LT && xIS_SET(victim->act, ACT_STATSHIELD))
  {
    set_pager_color(AT_IMMORT, ch);
    send_to_pager("Their godly glow prevents you from getting a good look.\r\n", ch);
    return;
  ...
  if(!IS_NPC(victim))
    pager_printf(ch, "&cStatus : &w%-10s", CAN_PKILL(victim) ? "Deadly" : IS_PKILL(victim) ? "Pre-Deadly" : "Non-Deadly");
  if(IS_NPC(victim))
  ...
  if(get_trust(ch) >= LEVEL_AJ_CPL && !IS_NPC(victim) && victim->desc)
    if(!str_cmp(victim->name, "Vladaar"))
    {
      pager_printf(ch,
                   "&cHost: &w127.0.0.1   Descriptor: %d  &cTrust: &w%d  &cAuthBy: &w%s\r\n",
                   victim->desc->descriptor, victim->trust, VLD_STR(victim->pcdata->authed_by) ? victim->pcdata->authed_by : "(unknown)");
    }
    else
    {
      pager_printf(ch,
                   "&cHost: &w%s   Descriptor: %d  &cTrust: &w%d  &cAuthBy: &w%s\r\n",
                   victim->desc->host, victim->desc->descriptor, victim->trust, VLD_STR(victim->pcdata->authed_by) ? victim->pcdata->authed_by : "(unknown)");
    }
  if(victim->fightingip)
  {
    pager_printf(ch, "&cFighting IP &C%s\r\n", victim->fightingip);
  }
  if(!IS_NPC(victim) && victim->pcdata->release_date != 0)
    pager_printf(ch, "&cHelled until %24.24s by %s.\r\n", ctime(&victim->pcdata->release_date), VLD_STR(victim->pcdata->helled_by) ? victim->pcdata->helled_by : "(unknown)");
  pager_printf(ch,
  ...
  pager_printf(ch,
               ...
               get_curr_str(victim), get_curr_int(victim), get_curr_wis(victim), get_curr_dex(victim), get_curr_con(victim), get_curr_cha(victim), get_curr_lck(victim));
  pager_printf(ch, "&cLevel   : &P%-2d              ", victim->level);
  if(IS_NPC(victim))
  {
    pager_printf(ch, "&cClass  : &w%s             &cRace      : &w%s\r\n", npc_class[victim->Class], npc_race[victim->race]);
  }
  else
  {
  ...
  pager_printf(ch, "&cDamroll : &C%-5d           &cWimpy  : &w%-5d           &cPosition  : &w%d\r\n", GET_DAMROLL(victim), victim->wimpy, victim->position);
  pager_printf(ch, "&cFighting: &w%-13s   &cMaster : &w%-13s   &cLeader    : &w%s\r\n",
               victim->fighting ? victim->fighting->who->name : "(none)", victim->master ? victim->master->name : "(none)", victim->leader ? victim->leader->name : "(none)");
  if(IS_NPC(victim))
    pager_printf(ch,
                 "&cHating  : &w%-13s   &cHunting: &w%-13s   &cFearing   : &w%s\r\n",
                 victim->hating ? victim->hating->name : "(none)", victim->hunting ? victim->hunting->name : "(none)", victim->fearing ? victim->fearing->name : "(none)");
  if(IS_NPC(victim))
    pager_printf(ch,
                 ...
                 victim->pIndexData->damsizedice, victim->pIndexData->damplus, victim->numattacks, victim->position, victim->defposition);
  pager_printf(ch, "&cMentalState: &w%-3d   &cEmotionalState: &w%-3d   ", victim->mental_state, victim->emotional_state);
  if(!IS_NPC(victim))
    ...
    send_to_pager("\r\n", ch);
  pager_printf(ch,
               "&cSave versus: &w%d %d %d %d %d       &cItems: &w(%d/%d)  &cWeight &w(%d/%d)  &cHeight &w%d\r\n",
               victim->saving_poison_death, victim->saving_wand,
               victim->saving_para_petri, victim->saving_breath, victim->saving_spell_staff, victim->carry_number, can_carry_n(victim), victim->carry_weight, can_carry_w(victim), victim->height);
  pager_printf(ch, "&cYear: &w%-5d  &cSecs: &w%d  &cTimer: &w%d\r\n", calculate_age(victim), (int)victim->played, victim->timer);
  ...
  else
  {
    ...
    if(victim->pcdata->nuisance)
    {
      pager_printf(ch,
                   ...
                   victim->pcdata->nuisance->flags, MAX_NUISANCE_STAGE, victim->pcdata->nuisance->power, ctime(&victim->pcdata->nuisance->time));
    }
  }
  if(victim->morph)
  {
  ...
  pager_printf(ch, "&cAffected by: &C%s\r\n", !xIS_EMPTY(victim->affected_by) ? ext_flag_string(&victim->affected_by, a_flags) : "Nothing\r\n");

  pager_printf(ch, "&cSpeaks: &w%d   &cSpeaking: &w%d   &cExperience: &w%d", victim->speaks, victim->speaking, victim->exp);
  ...
  if(IS_NPC(victim))
    pager_printf(ch, "&cShortdesc  : &w%s\r\n&cLongdesc   : &w%s",
                 VLD_STR(victim->short_descr) ? victim->short_descr : "(none set)", VLD_STR(victim->long_descr) ? victim->long_descr : "(none set)\r\n");
  else
  {
  ...
  for(paf = victim->first_affect; paf; paf = paf->next)
  {
    ...
    {
      ch_printf(ch, "&c%s: &w'%s' mods %s", skill_tname[skill->type], skill->name, a_types[paf->location % REVERSE_APPLY]);
      if(paf->location == APPLY_RESISTANT || paf->location == APPLY_IMMUNE || paf->location == APPLY_SUSCEPTIBLE)
      ...
      ch_printf(ch, " at lvl %d for %d rnds with bits %s.\r\n", paf->level, paf->duration, !xIS_EMPTY(paf->bitvector) ? ext_flag_string(&paf->bitvector, a_flags) : "(none)");
    }
  }
  if(victim->pcdata && victim->pcdata->ship)
    pager_printf(ch,
                 "&cSHIP DATA: Ship occupying: &w%d&c  Location on ship: &w%d&c  Location of Water:&w %d &cShip Hull &w%d\r\n",
                 victim->pcdata->ship, victim->pcdata->boatvnum, victim->pcdata->watervnum, victim->pcdata->hull);
...
void do_gwhere(CHAR_DATA *ch, char *argument)
{
  ...
  if(!pmobs)
  {
    for(d = first_descriptor; d; d = d->next)
      if((d->connected == CON_PLAYING || d->connected == CON_EDITING)
         && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room && can_see(ch, victim) && victim->level >= low && victim->level <= high)
      {
        found = TRUE;
...
void do_gfighting(CHAR_DATA *ch, char *argument)
{
  ...
  if(!pmobs && !phating && !phunting)
  {
    for(d = first_descriptor; d; d = d->next)
      if((d->connected == CON_PLAYING || d->connected == CON_EDITING)
         && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room && can_see(ch, victim) && victim->fighting && victim->level >= low && victim->level <= high)
      {
        found = TRUE;
        pager_printf(ch,
                     ...
                     IS_NPC(victim->fighting->who) ? victim->fighting->who->pIndexData->vnum : 0, victim->in_room->area->name, victim->in_room == NULL ? 0 : victim->in_room->vnum);
        count++;
      }
  }
    ...
    for(victim = first_char; victim; victim = victim->next)
      if(IS_NPC(victim) && victim->in_room && can_see(ch, victim) && victim->hating && victim->level >= low && victim->level <= high)
      {
        ...
        pager_printf(ch,
                     ...
                     IS_NPC(victim->hating->who) ? victim->hating->who->short_descr : victim->hating->who->name,
                     IS_NPC(victim->hating->who) ? victim->hating->who->pIndexData->vnum : 0, victim->in_room->area->name, victim->in_room == NULL ? 0 : victim->in_room->vnum);
        count++;
      }
  }
  else if(phunting)
    ...
    for(victim = first_char; victim; victim = victim->next)
      if(IS_NPC(victim) && victim->in_room && can_see(ch, victim) && victim->hunting && victim->level >= low && victim->level <= high)
      {
        ...
        pager_printf(ch,
                     ...
                     IS_NPC(victim->hunting->who) ? victim->hunting->who->short_descr : victim->hunting->who->name,
                     IS_NPC(victim->hunting->who) ? victim->hunting->who->pIndexData->vnum : 0, victim->in_room->area->name, victim->in_room == NULL ? 0 : victim->in_room->vnum);
        count++;
      }
  }
    ...
    if(obj->in_room && !str_cmp(buf2, obj->short_descr) && (obj->pIndexData->vnum == 11))
    {
      ...
      pager_printf(ch,
                   ...
                   capitalize(arg1), IS_OBJ_STAT(obj,
                                                 ITEM_CLANCORPSE) ? "&RPK" :
                   "&R  ", obj->in_room->area->name, obj->in_room->vnum, obj->timer < 1 ? "&w" : obj->timer < 5 ? "&R" : obj->timer < 10 ? "&Y" : "&w", obj->timer);
      if(bag)
      {
        obj_from_room(obj);
...
void do_owhere(CHAR_DATA *ch, char *argument)
{
  ...
  for(obj = first_object; obj; obj = obj->next)
  {
    ...
    if(outer_obj->carried_by)
    {
      ...
      strunc(field, 18);
      snprintf(buf + strlen(buf), MSL - strlen(buf), "%5d %-18s ", (IS_NPC(outer_obj->carried_by) ? outer_obj->carried_by->pIndexData->vnum : 0), field);
      if(outer_obj != obj)
      {
        snprintf(field, MIL, "%-18s", obj->in_obj->name);
        strunc(field, 18);
        snprintf(buf + strlen(buf), MSL - strlen(buf), "%5d %-18s ", obj->in_obj->pIndexData->vnum, field);
      }
      mudstrlcat(buf, "\r\n", MSL);
    ...
    else if(outer_obj->in_room)
    {
      ...
      strunc(field, 18);
      snprintf(buf + strlen(buf), MSL - strlen(buf), "%5d %-18s ", outer_obj->in_room->vnum, field);
      if(outer_obj != obj)
      {
        snprintf(field, MIL, "%-18s", obj->in_obj->name);
        strunc(field, 18);
...
/*  Snoop notification for higher imms, if desired, uncomment this */
  ...
  act(AT_YELLOW, "A common house hold fly appears in your hand, then is transported to follow your victim.", ch, NULL, NULL, TO_CHAR);
...
void do_advance(CHAR_DATA *ch, char *argument)
{
  ...
  if(level <= victim->level)
  {
    ...
    if(victim->level > LEVEL_DEMIGOD || IS_STAFF(victim))
    {
    ...
    if(level < victim->level)
    {
      ...
      send_to_char("Cursed and forsaken!  The Staff have lowered your level...\r\n", victim);
    }
    else
    {
  ...
  else
  {
    ...
    {
      default:
        send_to_char("The Staff feel fit to raise your level!\r\n", victim);
        break;
      case LEVEL_IMMORTAL:
...
void do_trust(CHAR_DATA *ch, char *argument)
{
  ...
  if(get_trust(victim) >= get_trust(ch))
  {
    send_to_char("&RNever try to use a command against a higher Staff member...\r\n", ch);
    send_to_char("&RYou have been warned....\r\n", ch);
    act(AT_RED, "$n tried to change your trust level against your wishes.", ch, NULL, victim, TO_VICT);
    return;
  }
  ...
  for(;;)
  {
    ...
    if(pRoomIndex)
      if(!IS_SET(pRoomIndex->room_flags, ROOM_PRIVATE)
         && !IS_SET(pRoomIndex->room_flags, ROOM_SOLITARY) && !IS_SET(pRoomIndex->room_flags, ROOM_NO_ASTRAL) && !IS_SET(pRoomIndex->room_flags, ROOM_PROTOTYPE))
        break;
  }
  if(victim->fighting)
    stop_fighting(victim, TRUE);
...
void do_restore(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg, "all"))
  {
    ...
    for(vch = first_char; vch; vch = vch_next)
    {
      ...
      if(!IS_NPC(vch) && !in_arena(vch))
      {
        ...
        update_pos(vch);
        if(boost)
          act(AT_IMMORT, "$n has restored you, and boosted your stats above maximum!", ch, NULL, vch, TO_VICT);
        else
  ...
  else
  {
    ...
    if(get_trust(ch) < LEVEL_AJ_CPL && victim != ch && !(IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE)))
    {
      send_to_char("You can't do that.\r\n", ch);
      return;
    }
    ...
    update_pos(victim);
    if(boost)
      act(AT_IMMORT, "$n has restored you, and boosted your stats above maximum!", ch, NULL, victim, TO_VICT);
    else
...
void do_afreeze(CHAR_DATA *ch, char *argument)
{
  char                    arg[MIL];
  CHAR_DATA              *victim;
...
void do_log(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg, "all"))
  {
    if(ch->level < 107)
    {
      send_to_char("You cannot log all.  Logging all uses alot of memory.\r\nThus can now only be done by Colonel's and above.\r\n", ch);
      return;
    }
  ...
  /*
   * No level check, staff can log anyone.
   */
...
void do_users(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(IS_NPC(person) || IS_STAFF(person))
      continue;
    ...
    if(IS_PUPPET(person))
    {
      ...
      snprintf(ip, MSL, "%d.%d.%d.%d", number_range(2, 254), number_range(2, 254), number_range(2, 254), number_range(2, 254));
      puppets++;
      pager_printf(ch, " %3d| %-17s |%4d| %-12s | %s&D\r\n", number_range(1, 15), st, number_range(0, 100), person->name, ip);
    }
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    ...
    if((d->original && !str_cmp(d->original->name, "Vladaar")) || (d->character && !str_cmp(d->character->name, "Vladaar")))
      snprintf(ip, sizeof(ip), "%s", "127.0.0.1");
    else
      snprintf(ip, sizeof(ip), "%s", d->host);

    if(arg[0] == '\0')
    {
      if(get_trust(ch) >= LEVEL_AJ_CAPTAIN || (d->character && can_see(ch, d->character) && !is_ignoring(d->character, ch)))
      {
        count++;
        pager_printf(ch, " %3d| %-17s |%4d| %-12s | %s\r\n", d->descriptor, st, d->idle / 4, d->original ? d->original->name : d->character ? d->character->name : "(None!)", ip);
      }

    }
    else
    {
      if((get_trust(ch) >= LEVEL_AJ_GENERAL || (d->character && can_see(ch, d->character))) && (!str_prefix(arg, d->host) || (d->character && !str_prefix(arg, d->character->name))))
      {
        count++;
        pager_printf(ch, " %3d| %2d|%4d| %-12s | %s\r\n", d->descriptor, d->connected, d->idle / 4, d->original ? d->original->name : d->character ? d->character->name : "(None!)", d->host);
      }
    }
  }
  if(puppets > 0 && ch->level > 107)
    pager_printf(ch, "%d user%s and %d puppet%s.\r\n", count, count == 1 ? "" : "s", puppets, puppets == 1 ? "" : "s");
  else
...
bool vnums_are_free(const char *check, int low_range, int high_range)
{
  ...
  for(cArea = first_area; cArea; cArea = cArea->next)
  {
    if(IS_SET(cArea->status, AREA_DELETED))
      continue;
    if((check_rooms || check_all) && ((low_range >= cArea->low_r_vnum && low_range <= cArea->hi_r_vnum) || (high_range >= cArea->low_r_vnum && high_range <= cArea->hi_r_vnum)))
      free_vnums = FALSE;
    if((check_mobs || check_all) && ((low_range >= cArea->low_m_vnum && low_range <= cArea->hi_m_vnum) || (high_range >= cArea->low_m_vnum && high_range <= cArea->hi_m_vnum)))
      free_vnums = FALSE;
    if((check_objs || check_all) && ((low_range >= cArea->low_o_vnum && low_range <= cArea->hi_o_vnum) || (high_range >= cArea->low_o_vnum && high_range <= cArea->hi_o_vnum)))
      free_vnums = FALSE;
  }
  /*
...
void do_rassign(CHAR_DATA *ch, char *argument)
{
  ...
  if(!victim->pcdata->area)
  {
    ...
    ch_printf(ch, "Players room range is at %d - %d.\r\n", victim->pcdata->r_range_lo, victim->pcdata->r_range_hi);
    return;
  }
  send_to_char("Done.\r\n", ch);
  set_char_color(AT_IMMORT, victim);
...
void do_pfreload(CHAR_DATA *ch, char *argument)
{
  ...
  if(exists_player(victim->name))
  {
    ...
    Load = load_char_obj(d, name, FALSE, FALSE, TRUE);  /* Volk - true is quiet * mode */
    victim = d->character;
    victim->desc = d;
    victim->timer = 0;
    /*
...
void do_loadup(CHAR_DATA *ch, char *argument)
{
  ...
  if(stat(fname, &fst) != -1)
  {
    ...
    ch_printf(ch, "Player %s loaded from room %d%s.\r\n", capitalize(name), old_room_vnum, !old_room_vnum ? "&RUnknown&D" : "");
    return;
  }
...
void do_fixchar(CHAR_DATA *ch, char *argument)
{
  ...
  if(victim->pcdata->mb != 0 && victim->Class == CLASS_MONK)
  {
    victim->pcdata->mb = 0;
...
void do_destroy(CHAR_DATA *ch, char *argument)
{
  ...
  if(!remove(buf))
  {
    ...
    if(!remove(buf))
      send_to_char("Player's staff data destroyed.\r\n", ch);
    else if(errno != ENOENT)
    {
      ch_printf(ch, "Unknown error #%d - %s (staff data). Report to http://6dragons.tk\r\n", errno, strerror(errno));
  ...
  {
    set_char_color(AT_WHITE, ch);
    ch_printf(ch, "Unknown error #%d - %s. Report to http://6dragons.tk\r\n", errno, strerror(errno));
...
MORTALS (including trusted characters), Staff (characters with level higher than
...
void do_cset(CHAR_DATA *ch, char *argument)
{
  ...
  if(argument[0] == '\0')
  {
    ...
    pager_printf(ch, "  &wMorph Optimization is:        &W%-7s", (sysdata.morph_opt) ? "ON" : "off");
    pager_printf(ch, "&wSaving Pets is:                &W%s\r\n", (sysdata.save_pets) ? "ON" : "off");
    pager_printf(ch, "  &wPkill looting is:             &W%-7s", (sysdata.pk_loot) ? "ON" : "off");
    ...
    pager_printf(ch,
                 ...
                 sysdata.pulseweather, sysdata.secpertick, sysdata.pulsepersec, sysdata.pulseaffect);
    pager_printf(ch,
    ...
    pager_printf(ch, "  &wAutoboot Time:                &W%2.2d:%-4.2d", sysdata.autoboot_hour, sysdata.autoboot_minute);
    ...
    pager_printf(ch, "  &wclass1maxadept: &W%-3d &wclass2maxadept: &W%-3d &wclass3maxadept: &W%-3d\r\n", sysdata.class1maxadept, sysdata.class2maxadept, sysdata.class3maxadept);
  ...
  if(!str_cmp(arg, "happy"))
  {
    ...
    sysdata.happy = !sysdata.happy;

    if(sysdata.happy)
    {
      echo_to_all(AT_RED, "[&WEvent&R] &CIt is time for 6Dragons HAPPY HOUR! Get killing!\r\n", ECHOTAR_ALL);
      echo_to_all(AT_RED, "Enjoy an hour of Double Experience when fighting, Double Experience when Crafting or Triple Experience if Grouped with other players.\r\n", ECHOTAR_ALL);
    }
    else
      echo_to_all(AT_RED, "[&WEvent&R] &CBoo! the 6Dragons Happy Hour is now over.\r\n", ECHOTAR_ALL);
...
void do_hell(CHAR_DATA *ch, char *argument)
{
  ...
  do_look(victim, (char *)"auto");
  ch_printf(victim, "&CThe Staff are not pleased with your actions.\r\r\n\n" "&YYou shall remain in hell for %d %s%s.\r\n", time, (h_d ? "hour" : "day"), (time == 1 ? "" : "s"));
...
*/
  ...
  if(!location)
    location = ch->in_room;
  MOBtrigger = FALSE;
  act(AT_MAGIC, "$n disappears in a cloud of light.", victim, NULL, ch, TO_NOTVICT);
  char_from_room(victim);
  char_to_room(victim, location);
  send_to_char("The staff have smiled on you and released you from hell early!\r\n", victim);
  do_look(victim, (char *)"auto");
  if(victim != ch)
...
void do_vsearch(CHAR_DATA *ch, char *argument)
{
  ...
  for(obj = first_object; obj != NULL; obj = obj->next)
  {
    ...
    if(in_obj->carried_by != NULL)
      ...
      pager_printf(ch, "[%2d] [%-5d] %s in %s.\r\n", obj_counter, ((in_obj->in_room) ? in_obj->in_room->vnum : 0), obj_short(obj), (in_obj->in_room == NULL) ? "somewhere" : in_obj->in_room->name);
...
void do_sedit(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg2[0] == '\0' || !str_cmp(arg2, "show"))
  {
    ...
    ch_printf(ch, "ONoArg: %s\r\nCFound: %s\r\nOFound: %s\r\n",
              social->others_no_arg ? social->others_no_arg : "(not set)", social->char_found ? social->char_found : "(not set)", social->others_found ? social->others_found : "(not set)");
    ch_printf(ch, "VFound: %s\r\nCAuto : %s\r\nOAuto : %s\r\n",
              social->vict_found ? social->vict_found : "(not set)", social->char_auto ? social->char_auto : "(not set)", social->others_auto ? social->others_auto : "(not set)");
    return;
  }
...
void do_cedit(CHAR_DATA *ch, char *argument)
{
  ...
  if(get_trust(ch) > LEVEL_AJ_SGT && !str_cmp(arg1, "save") && !str_cmp(arg2, "cmdtable"))
  {
    save_commands();
    send_to_char("Saved.\r\n", ch);
    return;
  ...
  if(!VLD_STR(arg2) || !str_cmp(arg2, "show"))
  {
    ch_printf(ch, "Command:  %s\r\nLevel:    %d"
              "\r\nPosition: %d\r\nLog:      %d"
              "\r\nCode:     %s\r\nShown:    %s\r\nFlags:  %s\r\n",
              command->name, command->level, command->position, command->log, skill_name(command->do_fun), (command->cshow == 1 ? "Yes" : "No"), flag_string(command->flags, cmd_flags));
...
void do_qpset(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg[0] == '\0' || arg2[0] == '\0' || amount < 0)
  {
    ...
    send_to_char("Syntax: qpset <character> review\r\n", ch);
  ...
  set_char_color(AT_IMMORT, victim);
  if(!str_cmp(arg2, (char *)"review"))
  {
    victim->quest_curr += 250;
    victim->quest_accum += 250;
...
void do_pcrename(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(oldname, MSL, "%s%c/%s", PLAYER_DIR, tolower(victim->pcdata->filename[0]), capitalize(victim->pcdata->filename));
...
void do_life(CHAR_DATA *ch, char *argument)
{
  ...
  if(ch->pcdata->tmplevel < LEVEL_IMMORTAL && !IS_STAFF(ch))
  {
    error(ch);
    return;
  }
  if((arg[0] == '\0') && (IS_STAFF(ch)) && (!ch->pcdata->tmplevel > LEVEL_IMMORTAL))
  {
...
void do_lookskills(CHAR_DATA *ch, char *argument)
{
  char                    arg[MIL];
  CHAR_DATA              *victim;
  int                     sn;
  ...
  if(arg[0] == '\0')
  {
    send_to_char("&zSyntax: lookskills <player>.\r\n", ch);
    return;
  }
  ...
  if(!IS_NPC(victim))
  {
    ...
    pager_printf(ch, "&CYou are looking at the skills of %s\r\n", victim->name);
    ...
    if(IS_DRAGON(victim))
    {
      ...
      for(sn = 0; sn < top_sn && skill_table[sn] && skill_table[sn]->name; sn++)
      {
        ...
        if(victim->pcdata->dlearned[sn] == 0)
          continue;
        pager_printf(ch, "&c%20s %3d%% ", skill_table[sn]->name, victim->pcdata->dlearned[sn]);
        if(++col % 3 == 0)
          send_to_pager("\r\n", ch);
      }
  ...
  if(!victim || !IS_NPC(victim))
    pager_printf(ch, "\r\n\r\n&C%s is a %s race with the class of %s", victim->name, race_table[victim->race]->race_name, class_table[victim->Class]->who_name);
  send_to_pager(".\r\n\r\n", ch);
  if(!IS_NPC(victim))
...
void do_saveall(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, MSL * 2, "&RYou have been saved by your friendly &r6 Dragons&R Staff member: %s ...&D", ch->name);
  do_echo(ch, buf);
...
void do_promote(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg2, "level"))
  {
    ...
    if(victim->level < 100)
    {
      snprintf(buf, MIL, "\r\n\r\nPlease congratulate our newest &rSix Dragons &WStaff member %s!", victim->name);
...
void do_boost(CHAR_DATA *ch, char *argument)
{
  ...
  {
    set_char_color(AT_IMMORT, ch);
    send_to_char("Syntax: Boost < all, last >\r\n", ch);
    return;
  }
  if(!str_cmp(arg, "last"))
  {
    ...
    if(!last_heaven_all_time)
      ch_printf(ch, "There has been no boosting since reboot.\r\n");
    else
    {
      time_passed = current_time - last_heaven_all_time;
      hour = (int)(time_passed / 3600);
      minute = (int)((time_passed - (hour * 3600)) / 60);
      ch_printf(ch, "The last boosting was %d hours and %d minutes ago.\r\n", hour, minute);
    }
    if(!ch->pcdata)
      return;
    if(!ch->pcdata->heaven_time)
    {
      send_to_char("You have never done a boosting.\r\n", ch);
      return;
    }
    time_passed = current_time - ch->pcdata->heaven_time;
    hour = (int)(time_passed / 3600);
    minute = (int)((time_passed - (hour * 3600)) / 60);
    ch_printf(ch, "Your boosting was %d hours and %d minutes ago.\r\n", hour, minute);
    return;
  }
  if(!str_cmp(arg, "all"))
  {
     ...
     * Check if the player did a restore all within the last 18 hours. 
     */
    if(ch->level < (MAX_LEVEL - 1) && ((current_time - last_heaven_all_time) < RESTORE_INTERVAL || (current_time - ch->pcdata->heaven_time) < RESTORE_INTERVAL))
    {
      send_to_char("Sorry, you can't do a boosting this soon again.\r\n", ch);
      return;
    }
    if(doubleexp && ch->level < (MAX_LEVEL - 1))
    {
      send_to_char("Sorry, you can't do a boosting while double exp hour is going on.\r\n", ch);
      return;
    }
    act(AT_YELLOW, "You have boosted all players throughout the realms!", ch, NULL, NULL, TO_CHAR);
    ...
    for(vch = first_char; vch; vch = vch_next)
    {
      ...
      if(IS_STAFF(vch)) // Stop immortals from getting the 
        // 
        // 
        continue;
      if(!IS_NPC(vch))
      {
        if(!IS_AFFECTED(vch, AFF_BOOST))
        {

          if(xIS_SET(vch->act, PLR_BATTLE))
            send_to_char("!!SOUND(sound/boost.wav)\r\n", vch);

          act(AT_YELLOW, "A tremendous surge of power fills you!", vch, NULL, NULL, TO_CHAR);
...
void do_ftick(CHAR_DATA *ch, char *argument)
{
  ...
  if(!arg || arg[0] == '\0')
  {
    send_to_char("Syntax: ftick char|obj|area|mob|violence|aggr|auction|all\r\n", ch);
    return;
  }
...
void do_oclaim(CHAR_DATA *ch, char *argument)
{
  ...
  for(obj = first_object; obj; obj = obj->next)
  {
    if(can_see_obj(ch, obj) && (obj->pIndexData->vnum == vnum || nifty_is_name(arg1, obj->name)) && (!vch || vch == carried_by(obj)))
  ...
  for(obj = first_object; obj; obj = obj->next)
  {
    if(can_see_obj(ch, obj) && (obj->pIndexData->vnum == vnum || nifty_is_name_prefix(arg1, obj->name)) && (!vch || vch == carried_by(obj)))
      if((count += obj->count) = number)
      {
        found = TRUE;
        break;
...
bool puppetload(CHAR_DATA *ch, int pnum)
{
  CHAR_DATA              *puppet = NULL;
  int                     x;
  bool                    found = TRUE;

  if(puppet == NULL && pnum == 1)
  {
    do_loadup(ch, ch->pcdata->alt1);
    puppet = get_char_world(ch, ch->pcdata->alt1);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(16007));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 2)
  {
    do_loadup(ch, ch->pcdata->alt2);
    puppet = get_char_world(ch, ch->pcdata->alt2);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(35515));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 3)
  {
    do_loadup(ch, ch->pcdata->alt3);
    puppet = get_char_world(ch, ch->pcdata->alt3);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(11210));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 4)
  {
    do_loadup(ch, ch->pcdata->alt4);
    puppet = get_char_world(ch, ch->pcdata->alt4);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(41004));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 5)
  {
    do_loadup(ch, ch->pcdata->alt5);
    puppet = get_char_world(ch, ch->pcdata->alt5);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(42178));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 6)
  {
    do_loadup(ch, ch->pcdata->alt6);
    puppet = get_char_world(ch, ch->pcdata->alt6);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(10816));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 7)
  {
    do_loadup(ch, ch->pcdata->alt7);
    puppet = get_char_world(ch, ch->pcdata->alt7);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(18633));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 8)
  {
    do_loadup(ch, ch->pcdata->alt8);
    puppet = get_char_world(ch, ch->pcdata->alt8);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(41023));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 9)
  {
    do_loadup(ch, ch->pcdata->alt9);
    puppet = get_char_world(ch, ch->pcdata->alt9);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(53170));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 10)
  {
    do_loadup(ch, ch->pcdata->alt10);
    puppet = get_char_world(ch, ch->pcdata->alt10);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(16059));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 10)
  {
    send_to_char("\r\npuppet alt10 is NULL\r\n", ch);
    return FALSE;
  }
  if(puppet == NULL && pnum == 11)
  {
    do_loadup(ch, ch->pcdata->alt11);
    puppet = get_char_world(ch, ch->pcdata->alt11);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
  ...
  if(puppet == NULL && pnum == 11)
  {
    send_to_char("\r\npuppet alt11 is NULL\r\n", ch);
    return FALSE;
  }
  if(puppet == NULL && pnum == 12)
  {
    do_loadup(ch, ch->pcdata->alt12);
    puppet = get_char_world(ch, ch->pcdata->alt12);
    char_from_room(puppet);
    get_curr_players();
    puppet->pcdata->condition[COND_THIRST] = STATED;
    puppet->pcdata->condition[COND_FULL] = STATED;
    restore_char(puppet);
    if(!str_cmp(ch->name, "Vladaar"))
      char_to_room(puppet, get_room_index(41051));
    else
      char_to_room(puppet, get_room_index(1218));
  ...
  if(puppet == NULL && pnum == 12)
  {
    send_to_char("\r\npuppet alt12 is NULL\r\n", ch);
    return FALSE;
  }

  if(pnum == 1 || pnum == -1)
  {
    if(ch->pcdata->alt1 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt1 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt1)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt1);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
        ch_printf(ch, "Alt %s is already a puppet!\r\n", ch->pcdata->alt1);
      else
      { /* linkdead not puppet */

        ch_printf(ch, "Alt %s is linkdead, resetting them to puppet status.\r\n", ch->pcdata->alt1);
        SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
        // d->character->desc->puppet = 1;
        puppet->redirect = ch;
      }
    }

    else if(exists_player(ch->pcdata->alt1))
    {
  ...
  if(pnum == 2 || pnum == -1)
  {
    if(ch->pcdata->alt2 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt2 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt2)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt2);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt2))
    {
  ...
  if(pnum == 3 || pnum == -1)
  {
    if(ch->pcdata->alt3 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt3 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt3)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt3);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt3))
    {
  ...
  if(pnum == 4 || pnum == -1)
  {
    ...
    else if(puppet = (get_char_world(ch, ch->pcdata->alt4)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt4);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt4))
    {
      do_loadup(ch, ch->pcdata->alt4);
      puppet = get_char_world(ch, ch->pcdata->alt4);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
  ...
  if(pnum == 5 || pnum == -1)
  {
    if(ch->pcdata->alt5 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt5 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt5)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt5);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt5))
    {
      do_loadup(ch, ch->pcdata->alt5);
      puppet = get_char_world(ch, ch->pcdata->alt5);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
  ...
  if(pnum == 6 || pnum == -1)
  {
    if(ch->pcdata->alt6 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt6 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt6)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt6);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt6))
    {
      do_loadup(ch, ch->pcdata->alt6);
      puppet = get_char_world(ch, ch->pcdata->alt6);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
  ...
  if(pnum == 7 || pnum == -1)
  {
    if(ch->pcdata->alt7 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt7 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt7)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt7);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt7))
    {
      do_loadup(ch, ch->pcdata->alt7);
      puppet = get_char_world(ch, ch->pcdata->alt7);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
  ...
  if(pnum == 8 || pnum == -1)
  {
    if(ch->pcdata->alt8 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt8 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt8)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt8);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt8))
    {
      do_loadup(ch, ch->pcdata->alt8);
      puppet = get_char_world(ch, ch->pcdata->alt8);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
  ...
  if(pnum == 9 || pnum == -1)
  {
    if(ch->pcdata->alt9 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt9 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt9)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt9);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt9))
    {
      do_loadup(ch, ch->pcdata->alt9);
      puppet = get_char_world(ch, ch->pcdata->alt9);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
  ...
  if(pnum == 10 || pnum == -1)
  {
    if(ch->pcdata->alt10 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt10 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt10)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt10);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt10))
    {
      do_loadup(ch, ch->pcdata->alt10);
      puppet = get_char_world(ch, ch->pcdata->alt10);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
  ...
  if(pnum == 11 || pnum == -1)
  {
    if(ch->pcdata->alt11 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt11 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt11)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt11);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt11))
    {
      do_loadup(ch, ch->pcdata->alt11);
      puppet = get_char_world(ch, ch->pcdata->alt11);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
  ...
  if(pnum == 12 || pnum == -1)
  {
    if(ch->pcdata->alt12 == NULL)
    {
      found = FALSE;
      send_to_char("\r\nAlt12 is NULL\r\n", ch);
      return FALSE;
    }
    else if(puppet = (get_char_world(ch, ch->pcdata->alt12)))
    {
      if(puppet->desc)
        ch_printf(ch, "Alt %s is already playing!\r\n", ch->pcdata->alt12);
      else if(puppet->pcdata && IS_SET(puppet->pcdata->flags, PCFLAG_PUPPET))
    ...
    else if(exists_player(ch->pcdata->alt12))
    {
      do_loadup(ch, ch->pcdata->alt12);
      puppet = get_char_world(ch, ch->pcdata->alt12);
      SET_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
...
void do_puppeteer(CHAR_DATA *ch, char *argument)
{
  char                    arg[256], *buf = NULL;
  int                     nopuppets = 0;
  CHAR_DATA              *puppet = NULL;
  char                    arg2[MIL];
  ...
  if(!str_cmp(arg, "alt1"))
  {
    ...
    if(argument)
    {
      if(exists_player(argument))
        ch->pcdata->alt1 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
    return;
  }
  ...
  if(!str_cmp(arg, "alt2"))
  {
    ...
    {
      if(exists_player(argument))
        ch->pcdata->alt2 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  if(!str_cmp(arg, "alt3"))
  {
    ...
    {
      if(exists_player(argument))
        ch->pcdata->alt3 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  if(!str_cmp(arg, "alt4"))
  {
    ...
    {
      if(exists_player(argument))
        ch->pcdata->alt4 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  if(!str_cmp(arg, "alt5"))
  {
    ...
    {
      if(exists_player(argument))
        ch->pcdata->alt5 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  {
    if(ch->pcdata->alt6)
      STRFREE(ch->pcdata->alt6);
    if(argument)
    {
      if(exists_player(argument))
        ch->pcdata->alt6 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  {
    if(ch->pcdata->alt7)
      STRFREE(ch->pcdata->alt7);
    if(argument)
    {
      if(exists_player(argument))
        ch->pcdata->alt7 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  {
    if(ch->pcdata->alt8)
      STRFREE(ch->pcdata->alt8);
    if(argument)
    {
      if(exists_player(argument))
        ch->pcdata->alt8 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  {
    if(ch->pcdata->alt9)
      STRFREE(ch->pcdata->alt9);
    if(argument)
    {
      if(exists_player(argument))
        ch->pcdata->alt9 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  if(!str_cmp(arg, "alt10"))
  {
    if(ch->pcdata->alt10)
      STRFREE(ch->pcdata->alt10);
    if(argument)
    {
      if(exists_player(argument))
        ch->pcdata->alt10 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  if(!str_cmp(arg, "alt11"))
  {
    if(ch->pcdata->alt11)
      STRFREE(ch->pcdata->alt11);
    if(argument)
    {
      if(exists_player(argument))
        ch->pcdata->alt11 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  if(!str_cmp(arg, "alt12"))
  {
    if(ch->pcdata->alt12)
      STRFREE(ch->pcdata->alt12);
    if(argument)
    {
      if(exists_player(argument))
        ch->pcdata->alt12 = STRALLOC(argument);
      else
        send_to_char("That player doesn't exist! Buffer cleared.\r\n", ch);
    }
    send_to_char("Done.\r\n", ch);
  ...
  if(!str_cmp(arg, "load"))
  {
    if(!argument || argument[0] == '\0')
    {
      send_to_char("Usage: puppeteer load #\r\n       puppeteer load all\r\n", ch);
      return;
    }
    ...
    if(!str_cmp(arg2, "all"))
    {
      send_to_char("Loading all your alts!\r\n", ch);
      if(ch->pcdata->alt1 != NULL)
      {
        puppetload(ch, 1);
      }
    ...
    for(puppet = first_char; puppet; puppet = puppet->next)
    {
      if(IS_NPC(puppet))
        continue;
   ...
   if(( num_descriptors + nopuppets ) > sysdata.maxplayers )
   {
      sysdata.maxplayers = num_descriptors + nopuppets;
  ...
  if(!str_cmp(arg, "quit"))
  {
    ...
    if(!argument || argument[0] == '\0')
    {
      send_to_char("Usage: puppeteer quit (altname)\r\n       puppeteer quit all\r\n", ch);
      return;
    }

    if(!str_cmp(argument, "all"))
    {
      send_to_char("Quitting all your alts!\r\n", ch);

      if(ch->pcdata->alt1)
      {
        puppet = get_char_world(ch, ch->pcdata->alt1);
        if(puppet && IS_PUPPET(puppet))
        {
          REMOVE_BIT(puppet->pcdata->flags, PCFLAG_PUPPET);
          // d->character->desc->puppet = 0;
          do_quit(puppet, (char *)"");
        }
      }
    ...
    else if(!str_cmp(arg2, "1") && ch->pcdata->alt1)
    {
      puppet = get_char_world(ch, ch->pcdata->alt1);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "2") && ch->pcdata->alt2)
    {
      puppet = get_char_world(ch, ch->pcdata->alt2);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "3") && ch->pcdata->alt3)
    {
      puppet = get_char_world(ch, ch->pcdata->alt3);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "4") && ch->pcdata->alt4)
    {
      puppet = get_char_world(ch, ch->pcdata->alt4);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "5") && ch->pcdata->alt5)
    {
      puppet = get_char_world(ch, ch->pcdata->alt5);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "6") && ch->pcdata->alt6)
    {
      puppet = get_char_world(ch, ch->pcdata->alt6);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "7") && ch->pcdata->alt7)
    {
      puppet = get_char_world(ch, ch->pcdata->alt7);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "8") && ch->pcdata->alt8)
    {
      puppet = get_char_world(ch, ch->pcdata->alt8);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "9") && ch->pcdata->alt9)
    {
      puppet = get_char_world(ch, ch->pcdata->alt9);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "10") && ch->pcdata->alt10)
    {
      puppet = get_char_world(ch, ch->pcdata->alt10);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "11") && ch->pcdata->alt11)
    {
      puppet = get_char_world(ch, ch->pcdata->alt11);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    else if(!str_cmp(arg2, "12") && ch->pcdata->alt12)
    {
      puppet = get_char_world(ch, ch->pcdata->alt12);
      if(puppet && IS_PUPPET(puppet))
      {
    ...
    {
      send_to_char("That's not your alt's name!\r\n", ch);
      return;
    }
  ...
  send_to_char("Usage: puppeteer alt1-12 (altname)\r\n", ch);
  send_to_char("       puppeteer load/quit #\r\n", ch);
  send_to_char("       puppeteer load/quit all\r\n", ch);
  send_to_char("       switch (altname), force (altname) (command)\r\n", ch);
  ch_printf(ch, "Your current alts are: %s, %s, %s, %s, %s, %s.\r\n                       %s, %s, %s, %s, %s, %s\r\n", ch->pcdata->alt1, ch->pcdata->alt2, ch->pcdata->alt3, ch->pcdata->alt4,
            ch->pcdata->alt5, ch->pcdata->alt6, ch->pcdata->alt7, ch->pcdata->alt8, ch->pcdata->alt9, ch->pcdata->alt10, ch->pcdata->alt11, ch->pcdata->alt12);
  return;
...
void do_slay(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg[0] == '\0')
  {
    send_to_char("Slay whom method?\r\n", ch);
    send_to_char("immolate shatter pounce demon dog split\r\n", ch);
    return;
  }
