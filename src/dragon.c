...
extern void             failed_casting(SKILLTYPE * skill, CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj);
...
void do_fumes(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_fumes))
  {

    if(xIS_SET(ch->act, PLR_BATTLE))
      send_to_char("!!SOUND(sound/fumes.wav)\r\n", ch);

    act(AT_GREY, "You breathe out a thick burning smokey fume!", ch, NULL, NULL, TO_CHAR);
    act(AT_GREY, "$n breathes out a thick burning smokey fume throughout the area.", ch, NULL, NULL, TO_ROOM);
    ...
    for(vch = ch->in_room->first_person; vch; vch = vch_next)
    {
      ...
      if(IS_AFFECTED(vch, AFF_BLISTERING))
        continue;

      global_retcode = damage(ch, vch, 1, gsn_fumes);

      AFFECT_DATA             af;

      af.type = gsn_fumes;
      af.duration = get_curr_wis(ch) * 2;
    ...
    learn_from_success(ch, gsn_fumes);
  }
  else
  {
    act(AT_GREY, "You try to breathe out fumes into the area, but get distracted!", ch, NULL, NULL, TO_CHAR);
    learn_from_failure(ch, gsn_fumes);
  }
...
if(ch->race != RACE_DRAGON && ch->race != RACE_CELESTIAL && ch->race != RACE_DEMON && ch->race != RACE_VAMPIRE && ch->Class != CLASS_OWL && ch->Class != CLASS_GRYPHON && ch->Class != CLASS_BAT)
  ...
  if(!IS_NPC(ch))
  {
    ...
    {
      act(AT_CYAN, "You suddenly have appendages growing on your back and sprout forth wings!", ch, NULL, NULL, TO_CHAR);
      act(AT_CYAN, "$n suddenly has appendages growing on $s back and sprouts forth wings!", ch, NULL, NULL, TO_ROOM);
  ...
  if(can_use_skill(ch, number_percent(), gsn_wings))
  {
    ...
    af.type = gsn_wings;
    af.duration = (ch->level * 4) + get_curr_con(ch) * 5;
    af.location = APPLY_NONE;
    af.modifier = 0;
    af.level = ch->level;
    af.bitvector = meb(AFF_FLYING);
    affect_to_char(ch, &af);
    act(AT_GREY, "You carefully unfold your wings and take flight into the air.", ch, NULL, NULL, TO_CHAR);
    act(AT_GREY, "$n unfolds $s wings and takes flight into the air.", ch, NULL, NULL, TO_ROOM);
    learn_from_success(ch, gsn_wings);
  }
  else
  {
    act(AT_GREY, "You carefully unfold your wings, but stumble as you try to fly into the air.", ch, NULL, NULL, TO_CHAR);
    act(AT_GREY, "$n unfolds $s wings, but stumbles trying to fly into the air.", ch, NULL, NULL, TO_ROOM);
...
void do_dominate(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_ballistic) && chance > 75)
  {
    ...
    if(!IS_NPC(victim))
    {
      ...
      ch_printf(ch, "You gaze deeply into the eyes of %s, dominating their soul.", capitalize(victim->short_descr));
...
*/
  ...
  if(can_use_skill(ch, number_percent(), gsn_ballistic))
  {
    ...
    if(miss > 100)
    {
      act(AT_MAGIC, "You magically hurl a stone at $N, but they quickly dodge out of the way.", ch, NULL, victim, TO_CHAR);
      act(AT_MAGIC, "You quickly sidestep a stone that $n magically hurled at you.", ch, NULL, victim, TO_VICT);
      act(AT_MAGIC, "$n magically hurls a stone at $N, but misses.", ch, NULL, victim, TO_NOTVICT);
    ...
    {
      act(AT_MAGIC, "You summon the energy to magically project a stone at $N.", ch, NULL, victim, TO_CHAR);
...
// Fixed so can't target self -- Aurin 12/3/2010
  ...
  if(can_use_skill(ch, number_percent(), gsn_gut))
  {
    ...
    global_retcode = damage(ch, victim, low, gsn_gut);  /* nerfed gut some - Vladaar */
    chance = number_range(1, 100);
    if(chance < 95)
      return;
...
void do_steel_flex(CHAR_DATA *ch, char *argument)
{
  short                   nomore;
  AFFECT_DATA             af;
  ...
  if(ch->mana <= nomore)
  {
    send_to_char("You do not have enough mana form steel flex.\r\n", ch);
    return;
  }

  if(can_use_skill(ch, number_percent(), gsn_steel_flex))
  {
    if(xIS_SET(ch->act, PLR_BATTLE))
      send_to_char("!!SOUND(sound/tears.wav)\r\n", ch);

    affect_strip(ch, gsn_steel_flex);
    WAIT_STATE(ch, skill_table[gsn_steel_flex]->beats);
    ch->mana = (ch->mana - nomore);
    learn_from_success(ch, gsn_steel_flex);
      act(AT_CYAN, "You magically flex your steel-like dragon scales!", ch, NULL, NULL, TO_CHAR);
      act(AT_CYAN, "$n's dragon scales seem to ripple as they flex!", ch, NULL, NULL, TO_ROOM);
    af.type = gsn_steel_flex;
    af.duration = ch->level * 2 +100;
    ...
    act(AT_CYAN, "You try to magically flex your steel-like scales, but get distracted.", ch, NULL, NULL, TO_CHAR);
  learn_from_failure(ch, gsn_steel_flex);
  return;
}
...
void do_cone(CHAR_DATA *ch, char *argument)
{
  ...
  if(ch->race == RACE_DRAGON)
  {
    send_to_char("Your dragon form prevents the use of this skill.\r\n", ch);
    return;
  }
  ...
  if(arg[0] == '\0')
  {
    ...
    {
      send_to_char("Who are you trying to cone attack?\r\n", ch);
      return;
    }
  }
  else if((victim = get_char_room(ch, arg)) == NULL)
  ...
  if(IS_NPC(ch))
  {
    act(AT_DGREY, "You launch a cone of shadow energy from your hand!", ch, NULL, victim, TO_CHAR);
    act(AT_DGREY, "$n launches a cone of shadow energy that engulfs you!", ch, NULL, victim, TO_VICT);
    act(AT_DGREY, "$n launches a cone of shadow energy, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
  ...
  else if(ch->pcdata->tmpclass == 9 || twoheaded == 2)
  {
    ...
    act(AT_YELLOW, "You launch a cone of holy energy from your hand!", ch, NULL, victim, TO_CHAR);
    act(AT_YELLOW, "$n launches a cone of holy energy that engulfs you!", ch, NULL, victim, TO_VICT);
    act(AT_YELLOW, "$n launches a cone of holy energy, engulfing $N inits fury!", ch, NULL, victim, TO_NOTVICT);
    global_retcode = damage(ch, victim, insane, gsn_cone);
    WAIT_STATE(ch, skill_table[gsn_cone]->beats);
    ch->mana = (ch->mana - nomore);
    return;
  ...
  else if(ch->pcdata->tmpclass == 10 || twoheaded == 3)
  {
    ...
    act(AT_GREY, "You launch a cone of iron shards from your hand!", ch, NULL, victim, TO_CHAR);
    act(AT_GREY, "$n launches a cone of iron shards that tear into you!", ch, NULL, victim, TO_VICT);
    act(AT_GREY, "$n launches a cone of iron shards that tear into $N!", ch, NULL, victim, TO_NOTVICT);
    global_retcode = damage(ch, victim, insane, gsn_cone);
    WAIT_STATE(ch, skill_table[gsn_cone]->beats);
    ch->mana = (ch->mana - nomore);
    return;
  ...
  else if(ch->pcdata->tmpclass == 11 || twoheaded == 4)
  {
    ...
    act(AT_RED, "You launch a cone of choking smoke from your hand!", ch, NULL, victim, TO_CHAR);
  ...
  else if(ch->pcdata->tmpclass == 12 || twoheaded == 5)
  {
    ...
    act(AT_LBLUE, "$n launches a cone of water that engulfs you in its fury!", ch, NULL, victim, TO_VICT);
    act(AT_LBLUE, "$n launches a cone of water that engulfs $N in its fury!", ch, NULL, victim, TO_NOTVICT);
    global_retcode = damage(ch, victim, insane, gsn_cone);
    WAIT_STATE(ch, skill_table[gsn_cone]->beats);
    ch->mana = (ch->mana - nomore);
    return;
...
{ /* tohuman=TRUE if going to human, * =FALSE if going to dragon */
  ...
  if(tohuman)
  {
    ...
    act(AT_MAGIC, "You call upon ancient magic and assume human form.", ch, NULL, NULL, TO_CHAR);
    act(AT_MAGIC, "$n suddenly shimmers and assumes human form.", ch, NULL, NULL, TO_ROOM);
    act(AT_CYAN, "You fight down a sense of disorientation as your once massive body is human size!", ch, NULL, NULL, TO_CHAR);
  ...
  { /* going to dragon */
    ...
    act(AT_MAGIC, "$n suddenly shimmers and assumes the form of a mighty dragon!", ch, NULL, NULL, TO_ROOM);
    act(AT_CYAN, "You feel pure elation as your human body is restored to dragon size!", ch, NULL, NULL, TO_CHAR);
     ...
     */
    {
      ...
      {
        ch->max_hit *= 2; /* Since all we did was divide it might as well multiply it now
                           * but give bug message too. */
        bug("%s: %s had 0 tmpmax_hit and had to double their current hp.", __FUNCTION__, ch->name);
      }
      if(hitprcnt > 0.0)
        ch->hit = (int)(ch->max_hit * hitprcnt);
    }
...
/*        ch->pcdata->tmprace = 0;
          ch->pcdata->tmpclass = 0;  removed all instances of this to try to stop the bug.
*/
    /*
     * Ok this is a simple way of handling it, we toss through the sns and set
     * everything up as we go 
...
void do_human_form(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_AFFECTED(ch, AFF_DRAGONLORD) && ch->in_room->height <= 120 && ch->in_room->height >= 12)
  {
    send_to_char("Your too massive as a dragon to leave human form here.\r\n", ch);
    return;
  }
  if(IS_AFFECTED(ch, AFF_DRAGONLORD))
  {
...
void do_lick(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char("You can't do that skill.", ch);
    return;
  }
  ...
  if(can_use_skill(ch, number_percent(), gsn_lick))
  {
    ...
    if(IS_AFFECTED(ch, AFF_DRAGONLORD))
    {
      if(victim == ch)
      {
        act(AT_CYAN, "You lick your wounds.", ch, NULL, victim, TO_CHAR);
        act(AT_CYAN, "$n licks $s wounds!", ch, NULL, victim, TO_NOTVICT);
        act(AT_MAGIC, "Your wounds begin to heal up and stop bleeding!", ch, NULL, victim, TO_CHAR);
      }
    }
    if(!IS_AFFECTED(ch, AFF_DRAGONLORD))
    {
      if(victim == ch)
      {
        act(AT_CYAN, "You open your mighty jaws, and lick your wounds.", ch, NULL, victim, TO_CHAR);
        act(AT_CYAN, "$n opens $s mighty jaws and licks $s wounds!", ch, NULL, victim, TO_NOTVICT);
        act(AT_MAGIC, "Your wounds begin to heal up and stop bleeding!", ch, NULL, victim, TO_CHAR);
      }
      else
      {
        act(AT_CYAN, "You open your mighty jaws, and lick $N's wounds.", ch, NULL, victim, TO_CHAR);
        act(AT_CYAN, "$n opens $s mighty jaws, and licks your wounds!", ch, NULL, victim, TO_VICT);
        act(AT_CYAN, "$n opens $s mighty jaws and licks $N's wounds!", ch, NULL, victim, TO_NOTVICT);
        act(AT_MAGIC, "Your wounds begin to heal up and stop bleeding!", ch, NULL, victim, TO_VICT);
      }
    }
    ...
    act(AT_CYAN, "You try to focus your mana into healing energies, but get distracted.", ch, NULL, victim, TO_CHAR);
...
void SendDBHelp(CHAR_DATA *ch)
{
  ...
  switch (ch->Class)
  {
    default:
      ...
      break;
    case CLASS_BLACK:
      sprintf(buf, "shadow, %s%saoe\r\n", (ch->level < MidDBLevel) ? "" : "darkness, ", (ch->level < HighDBLevel) ? "" : "death, ");
      break;
    case CLASS_RED:
      sprintf(buf, "smoke, %s%saoe\r\n", (ch->level < MidDBLevel) ? "" : "fire, ", (ch->level < HighDBLevel) ? "" : "hellfire, ");
      break;
    case CLASS_SILVER:
      sprintf(buf, "iron, %s%saoe\r\n", (ch->level < MidDBLevel) ? "" : "steel, ", (ch->level < HighDBLevel) ? "" : "titanium, ");
      break;
    case CLASS_BLUE:
      sprintf(buf, "water, %s%saoe\r\n", (ch->level < MidDBLevel) ? "" : "ice, ", (ch->level < HighDBLevel) ? "" : "glacial, ");
      break;
    case CLASS_GOLD:
      sprintf(buf, "holy, %s%saoe\r\n", (ch->level < MidDBLevel) ? "" : "divine, ", (ch->level < HighDBLevel) ? "" : "heavenly, ");
      break;
    case CLASS_TWOHEADED:
      sprintf(buf, "shadow, smoke, iron, water, holy, %s%saoe\r\n",
              (ch->level < MidDBLevel) ? "" : "darkness, fire, steel, ice, divine,\r\n", (ch->level < HighDBLevel) ? "" : "heavenly, death, titanium, hellfire, glacial, ");
      break;
  }
  send_to_char(buf, ch);
  return;
...
void do_breathe(CHAR_DATA *ch, char *argument)
{
  ...
  if((!str_cmp(arg, "aoe") && !*arg2) || !str_cmp(arg2, "aoe"))
  {
    ...
    if(ch->Class == CLASS_GOLD || twoheaded == 1)
    {
      ...
      act(AT_YELLOW, "$n breathes out a cloud of holy energy, engulfing everything in its fury!", ch, NULL, NULL, TO_ROOM);
    }
    else if(ch->Class == CLASS_RED || twoheaded == 2)
    {
      act(AT_RED, "You breathe out a choking cloud of smoke!", ch, NULL, NULL, TO_CHAR);
      act(AT_RED, "$n breathes out a choking cloud of smoke, engulfing everything in its fury!", ch, NULL, NULL, TO_ROOM);
    }
    else if(ch->Class == CLASS_BLACK || twoheaded == 3)
    {
      act(AT_DGREY, "You breathe out a cloud of shadow energy!", ch, NULL, NULL, TO_CHAR);
      act(AT_DGREY, "$n breathes out a cloud of shadow energy, engulfing everything in its fury!", ch, NULL, NULL, TO_ROOM);
    }
    else if(ch->Class == CLASS_SILVER || twoheaded == 4)
    {
      act(AT_GREY, "You breathe out a huge mass of iron shards!", ch, NULL, NULL, TO_CHAR);
      act(AT_GREY, "$n breathes out a huge mass of iron shards, engulfing everything in its fury!", ch, NULL, NULL, TO_ROOM);
    }
    else if(ch->Class == CLASS_BLUE || twoheaded == 5)
    {
      act(AT_LBLUE, "You breathe out a heavy stream of water!", ch, NULL, NULL, TO_CHAR);
      act(AT_LBLUE, "$n breathes out a heavy stream of water, engulfing everything in its fury!", ch, NULL, NULL, TO_ROOM);
    }
    else
    {
      act(AT_RED, "You breathe out a choking cloud of smoke!", ch, NULL, NULL, TO_CHAR);
  ...
  if(!IS_NPC(victim))
  {
    ...
    if(victim != ch)
    {
      ...
      {
        act(AT_DGREEN, "You block $n's dragon breath with your shield.", ch, NULL, victim, TO_VICT);
        ch_printf(ch, "&G%s shield blocks your dragon breath!", victim->name);
        return;
      }
      else if(victim->pcdata->learned[skill_lookup("tumble")] > 0 && coin > 2)
      {
        act(AT_CYAN, "You tumble away from $n's dragon breath.", ch, NULL, victim, TO_VICT);
        ch_printf(ch, "&c%s tumbles away from your dragon breath.", victim->name);
        return;
      }
      else if(victim->pcdata->learned[skill_lookup("phase")] > 0 && coin > 2)
      {
        act(AT_DGREEN, "Your body phases, absorbing $n's dragon breath.", ch, NULL, victim, TO_VICT);
        ch_printf(ch, "&c%s body phases, absorbing your dragon breath.", victim->name);
        return;
      }
    }
  }
...
// increase - Aurin 11/21/2010
  ...
  {
    act(AT_SKILL, "Your other dragon head also breathes at the same time!", ch, NULL, NULL, TO_CHAR);
    act(AT_SKILL, "$n's other dragon head also breathes at the same time!", ch, NULL, NULL, TO_ROOM);
    if(ch->fighting)
      victim = who_fighting(ch);
    ...
    if(ch->Class == CLASS_BLACK || ch->Class == CLASS_TWOHEADED)
    {
      if(!str_prefix(arg2, "death") && ch->level >= HighDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_DGREY, "You breathe out a cloud of pure, malevolent EVIL!", ch, NULL, victim, TO_CHAR);
          act(AT_DGREY, "$n breathes out a cloud of pure, malevolent EVIL that engulfs you!", ch, NULL, victim, TO_VICT);
          act(AT_DGREY, "$n breathes out a cloud of pure, malevolent EVIL, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          if(chance < 11)
          {
// stop players being changed to human priests - Vladaar
            ...
            if(IS_AFFECTED(victim, AFF_POISON))
              return;
            act(AT_DGREY, "Your malevolent EVIL cloud has greatly dispelled and poisoned $N!", ch, NULL, victim, TO_CHAR);
            act(AT_DGREY, "$n's malevolent EVIL cloud has greatly dispelled and poisoned you!", ch, NULL, victim, TO_VICT);
            af.type = gsn_poison;
            af.duration = ch->level;
            af.level = ch->level;
            af.location = APPLY_STR;
          ...
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 3) + (ch->level * 2), gsn_breath);
        }
        else
        {
      ...
      if(!str_prefix(arg2, "darkness") && ch->level >= MidDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_DGREY, "You breathe out a cloud of pure darkness!", ch, NULL, victim, TO_CHAR);
          act(AT_DGREY, "$n breathes out a cloud of darkness that engulfs you!", ch, NULL, victim, TO_VICT);
          act(AT_DGREY, "$n breathes out a cloud of darkness, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 2), gsn_breath);
          if(chance < 11)
          {
            act(AT_DGREY, "Your cloud of darkness has greatly dispelled $N!", ch, NULL, victim, TO_CHAR);
            act(AT_DGREY, "$n's cloud of darkness has greatly dispelled you!", ch, NULL, victim, TO_VICT);
            while(victim->first_affect)
              affect_remove(victim, victim->first_affect);
            if(IS_AFFECTED(victim, AFF_SANCTUARY))
            {
      ...
      if(!str_prefix(arg2, "shadow"))
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_DGREY, "You breathe out a cloud of shadow energy!", ch, NULL, victim, TO_CHAR);
          act(AT_DGREY, "$n breathes out a cloud of shadow energy that engulfs you!", ch, NULL, victim, TO_VICT);
          act(AT_DGREY, "$n breathes out a cloud of shadow energy, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          if(chance < 11)
          {
            if(IS_AFFECTED(victim, AFF_POISON))
              return;
            act(AT_DGREY, "Your cloud of darkness has poisoned $N!", ch, NULL, victim, TO_CHAR);
            act(AT_DGREY, "$n's cloud of darkness has poisoned you!", ch, NULL, victim, TO_VICT);
            af.type = gsn_poison;
            af.duration = ch->level;
            af.level = ch->level;
            af.location = APPLY_STR;
    ...
    if(ch->Class == CLASS_RED || ch->Class == CLASS_TWOHEADED || IS_NPC(ch))
    {
      // RED dragons - Torin
      if(!str_prefix(arg2, "hellfire") && ch->level >= HighDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          act(AT_RED, "You breathe out the pure, white-hot flames of HELL!!!", ch, NULL, victim, TO_CHAR);
          act(AT_RED, "$n breathes out the pure, white-hot flames of HELL that engulf you!", ch, NULL, victim, TO_VICT);
          act(AT_RED, "$n breathes out the pure, white-hote flames of HELL, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          if(chance < 11)
          {
            act(AT_RED, "Your hellfire breath catches $N on fire!", ch, NULL, victim, TO_CHAR);
            act(AT_RED, "$n's hellfire breath catches you on fire!", ch, NULL, victim, TO_VICT);
            af.type = gsn_thaitin;
            af.duration = 50;
            af.location = APPLY_NONE;
            af.modifier = 0;
      ...
      if(!str_prefix(arg2, "fire") && ch->level >= MidDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_RED, "You breathe out a massive cloud of fire!", ch, NULL, victim, TO_CHAR);
          act(AT_RED, "$n breathes out massive cloud of fire that engulfs you!", ch, NULL, victim, TO_VICT);
          act(AT_RED, "$n breathes out a massive cloud of fire, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 2), gsn_breath);
          if(chance < 11)
          {
            act(AT_RED, "Your massive cloud of fire catches $N on fire!", ch, NULL, victim, TO_CHAR);
            act(AT_RED, "$n's massive cloud of fire catches you on fire!", ch, NULL, victim, TO_VICT);
            af.type = gsn_thaitin;
            af.duration = 50;
            af.location = APPLY_NONE;
            af.modifier = 0;
      ...
      if(!str_prefix(arg2, "smoke"))
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_RED, "You breathe out a choking cloud of smoke!", ch, NULL, victim, TO_CHAR);
          act(AT_RED, "$n breathes out a cloud of smoke that chokes you!", ch, NULL, victim, TO_VICT);
          act(AT_RED, "$n breathes out a cloud of smoke, choking $N!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
    ...
    if(ch->Class == CLASS_SILVER || ch->Class == CLASS_TWOHEADED)
    {
      ...
      if(!str_prefix(arg2, "titanium") && ch->level >= HighDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_GREY, "You breathe out a huge, solid mass of titanium!!!", ch, NULL, victim, TO_CHAR);
          act(AT_GREY, "$n breathes out a huge, solid mass of titanium that slams into you!", ch, NULL, victim, TO_VICT);
          act(AT_GREY, "$n breathes out a huge, solid mass of titanium that slams into $N!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          if(chance < 11)
          {
            act(AT_GREY, "Your titanium breath knocks down and weakens $N!", ch, NULL, victim, TO_CHAR);
            act(AT_GREY, "$n's titanium breath weakens and knocks you down!!!", ch, NULL, victim, TO_VICT);
            af.type = gsn_breath;
            af.location = APPLY_NONE;
            af.modifier = 0;
            af.duration = 1;
      ...
      if(!str_prefix(arg2, "steel") && ch->level >= MidDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_GREY, "You breathe out a stream of molten steel!", ch, NULL, victim, TO_CHAR);
          act(AT_GREY, "$n breathes out a stream of molten steel that engulfs you!", ch, NULL, victim, TO_VICT);
          act(AT_GREY, "$n breathes out a stream of molten steel, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          if(chance < 11)
          {
      ...
      if(!str_prefix(arg2, "iron"))
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_GREY, "You breathe out a huge mass of iron shards!", ch, NULL, victim, TO_CHAR);
          act(AT_GREY, "$n breathes out a huge mass of iron shards that tear into you!", ch, NULL, victim, TO_VICT);
          act(AT_GREY, "$n breathes out a huge mass of iron shards that tear into $N!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          if(chance < 11)
          {
    ...
    if(ch->Class == CLASS_BLUE || ch->Class == CLASS_TWOHEADED)
    {
      ...
      if(!str_prefix(arg2, "glacial") && ch->level >= HighDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_LBLUE, "You breathe out a huge, solid mass of ice!!!", ch, NULL, victim, TO_CHAR);
          act(AT_LBLUE, "$n breathes out a huge, solid mass of ice that slams into you!", ch, NULL, victim, TO_VICT);
          act(AT_LBLUE, "$n breathes out a huge, solid mass of ice that slams into $N!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 3), gsn_breath);
          if(chance < 11 && !char_died(victim))
          {
            act(AT_LBLUE, "Your glacial breath gives frostbite and starts to drown $N!", ch, NULL, victim, TO_CHAR);
            act(AT_LBLUE, "$n's glacial breath gives you frostbite and starts to drown you!", ch, NULL, victim, TO_VICT);
            if(IS_NPC(victim))
            {
              global_retcode = damage(ch, victim, number_range(300, 400), gsn_breath);
            }
      ...
      if(!str_prefix(arg2, "ice") && ch->level >= MidDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_LBLUE, "You breathe out a hail of ice shards!", ch, NULL, victim, TO_CHAR);
          act(AT_LBLUE, "$n breathes out a hail of ice shards that engulf you!", ch, NULL, victim, TO_VICT);
          act(AT_LBLUE, "$n breathes out a hail of ice shards, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 2), gsn_breath);
          if(chance < 11 && !char_died(victim))
          {
            act(AT_LBLUE, "Your breathe of ice gives frost bite to $N!", ch, NULL, victim, TO_CHAR);
            act(AT_LBLUE, "$n's breathe of ice gives frost bite to you!", ch, NULL, victim, TO_VICT);
            if(IS_NPC(victim))
            {
              global_retcode = damage(ch, victim, number_range(100, 300), gsn_breath);
            }
            else if(!IS_NPC(victim))
            {
              global_retcode = damage(ch, victim, number_range(100, 300), gsn_breath);
      ...
      if(!str_prefix(arg2, "water"))
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_LBLUE, "You breathe out a heavy stream of water!", ch, NULL, victim, TO_CHAR);
          act(AT_LBLUE, "$n breathes out a heavy stream of water that engulfs you in its fury!", ch, NULL, victim, TO_VICT);
          act(AT_LBLUE, "$n breathes out a heavy stream of water that engulfs $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 1), gsn_breath);
          if(chance < 11 && !char_died(victim))
          {
            act(AT_LBLUE, "Your breathe of water begins to drown $N!", ch, NULL, victim, TO_CHAR);
            act(AT_LBLUE, "$n's breathe of heavy water begins to drown you!", ch, NULL, victim, TO_VICT);
            if(IS_NPC(victim))
            {
              global_retcode = damage(ch, victim, number_range(50, 200), gsn_breath);
            }
            else if(!IS_NPC(victim))
            {
              global_retcode = damage(ch, victim, number_range(50, 200), gsn_breath);
    ...
    if(ch->Class == CLASS_GOLD || ch->Class == CLASS_TWOHEADED)
    {
      ...
      if(!str_prefix(arg2, "heavenly") && ch->level >= HighDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_YELLOW, "You breathe out a maelstrom of heavenly power!!!", ch, NULL, victim, TO_CHAR);
          act(AT_YELLOW, "$n breathes out a maelstrom of heavenly power that engulfs you!", ch, NULL, victim, TO_VICT);
          act(AT_YELLOW, "$n breathes out a maelstrom of heavely power that engulfs $N in its glorious power!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 3), gsn_breath);
          if(chance < 11)
          {
            act(AT_YELLOW, "Your maelstrom of heavenly power cures your afflictions, and absorbs some of $N's health!", ch, NULL, victim, TO_CHAR);
      ...
      if(!str_prefix(arg2, "divine") && ch->level >= MidDBLevel)
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_YELLOW, "You breathe out a stream of divine energy!", ch, NULL, victim, TO_CHAR);
          act(AT_YELLOW, "$n breathes out a stream of divine energy that engulfs you!", ch, NULL, victim, TO_VICT);
          act(AT_YELLOW, "$n breathes out a stream of divine energy, engulfing $N in its fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 2), gsn_breath);
          if(chance < 11)
          {
            act(AT_YELLOW, "Your stream of divine energy absorbs $N's health and returns some to you!", ch, NULL, victim, TO_CHAR);
            act(AT_YELLOW, "$n's stream of divine energy absorbs your health and heals them!", ch, NULL, victim, TO_VICT);
      ...
      if(!str_prefix(arg2, "holy"))
      {
        if(can_use_skill(ch, number_percent(), gsn_breath))
        {
          ...
          if(ch->fighting)
            victim = who_fighting(ch);
          act(AT_YELLOW, "You breathe out a cloud of holy energy!", ch, NULL, victim, TO_CHAR);
          act(AT_YELLOW, "$n breathes out a cloud of holy energy that engulfs you!", ch, NULL, victim, TO_VICT);
          act(AT_YELLOW, "$n breathes out a cloud of holy energy, engulfing $N inits fury!", ch, NULL, victim, TO_NOTVICT);
          learn_from_success(ch, gsn_breath);
          global_retcode = damage(ch, victim, GetBreathDam(ch->level, 1), gsn_breath);
          if(chance < 11)
          {
            act(AT_YELLOW, "Your cloud of holy energy cures your afflictions!", ch, NULL, victim, TO_CHAR);
            affect_strip(ch, gsn_maim);
            affect_strip(ch, gsn_brittle_bone);
            affect_strip(ch, gsn_festering_wound);
            affect_strip(ch, gsn_poison);
...
void do_stomp(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_stomp))
  {
    ...
    act(AT_RED, "A shadow appears overhead as $n's foot comes crashing down at you!", ch, NULL, victim, TO_VICT);
    act(AT_RED, "$n drives a massive foot down upon $N smashing them into the ground!", ch, NULL, victim, TO_NOTVICT);
    ...
    global_retcode = damage(ch, victim, extrahigh, gsn_stomp);
  }
  else
  {
...
void do_deathroll(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_deathroll))
  {
    ...
    act(AT_RED, "You dive toward $N snatching them up with your powerful jaws, putting them into a deathroll!", ch, NULL, victim, TO_CHAR);
    act(AT_RED, "A wavery shadow appears overhead in the water as $n comes crashing down at you, snatching you with their jaws, sending you into a deathroll!", ch, NULL, victim, TO_VICT);
    act(AT_RED, "$n dives down upon $N snatching them with their jaws, sending them into a death roll!", ch, NULL, victim, TO_NOTVICT);
    ...
    if(ch->in_room->sector_type == SECT_UNDERWATER
       ...
       || ch->in_room->sector_type == SECT_DOCK || ch->in_room->sector_type == SECT_WATER_SWIM || ch->in_room->sector_type == SECT_WATER_NOSWIM || ch->in_room->sector_type == SECT_SWAMP)
    {
      act(AT_RED, "You drag $N deeper into the water!", ch, NULL, victim, TO_CHAR);
      act(AT_RED, "$n drags you deeper into the water, before releasing.", ch, NULL, victim, TO_VICT);
    ...
    else
    {
      ...
      act(AT_RED, "$n throws you from their jaws to the ground!", ch, NULL, victim, TO_VICT);
...
void do_hellfire(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_hellfire))
  {
    ...
    act(AT_RED, "You open you massive jaws and release an intense inferno towards $N!", ch, NULL, victim, TO_CHAR);
    act(AT_RED, "$n opens their massive jaws and releases an intense inferno toward you.", ch, NULL, victim, TO_VICT);
    act(AT_RED, "$n opens their massive jaws releasing an intense inferno toward $N.", ch, NULL, victim, TO_NOTVICT);
    ...
    {
      act(AT_RED, "You burst through $N's shield of sanctuary, causing it to falter!", ch, NULL, victim, TO_CHAR);
      act(AT_RED, "$n burst through your sanctuary, causing it to falter.", ch, NULL, victim, TO_VICT);
      xREMOVE_BIT(victim->affected_by, AFF_SANCTUARY);
...
void do_devour(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char("You must be a player dragon to do that.\r\n", ch);
    return;
  }
  ...
  {
    send_to_char("You are so full you cannot devour right now.\r\n", ch);
    return;
  }
  ...
  if(can_use_skill(ch, number_percent(), gsn_devour))
  {
    ...
    act(AT_RED, "$n swoops down and devours $p completely in one bite!", ch, obj, NULL, TO_ROOM);

    if(ch->pcdata->condition[COND_THIRST] < STATED)
    {
      ch->pcdata->condition[COND_THIRST] += ch->level / 2;
    }
    if(ch->pcdata->condition[COND_FULL] < STATED)
    {
      ch->pcdata->condition[COND_FULL] += ch->level / 2;
    }
    if(ch->level < 8) // lessen new player whiners
    {
...
void do_whiplash(CHAR_DATA *ch, char *argument)
{
  AFFECT_DATA             af;
  CHAR_DATA              *victim;
  char                    arg[MIL];
   ...
   */
  if(IS_NPC(ch) && chance <= 7)
    return;

  if(arg[0] == '\0' && !ch->fighting)
  {
    send_to_char("Do a whiplash on who?\r\n", ch);
    return;
  }
  ...
  WAIT_STATE(ch, skill_table[gsn_whiplash]->beats);
  if(xIS_SET(ch->act, PLR_BATTLE))
    send_to_char("!!SOUND(sound/tailswipe.wav)\r\n", ch);
  if(can_use_skill(ch, (number_percent()), gsn_whiplash))
  {
    if(!IS_AFFECTED(victim, AFF_PARALYSIS))
    {
      WAIT_STATE(victim, PULSE_VIOLENCE);
      act(AT_WHITE, "$n whips $s snake-like tail into you!", ch, NULL, victim, TO_VICT);
      act(AT_WHITE, "You whiplash $N with your snake-like tail!", ch, NULL, victim, TO_CHAR);
      act(AT_WHITE, "$n whips $s snake-like tail at $N!", ch, NULL, victim, TO_NOTVICT);
      af.type = gsn_whiplash;
      ...
      affect_to_char(victim, &af);
      update_pos(victim);
      learn_from_success(ch, gsn_whiplash);
      global_retcode = damage(ch, victim, extrahigh, gsn_whiplash);
    }
  }
  else
  {
    ...
    if(!IS_NPC(ch))
      learn_from_failure(ch, gsn_whiplash);
    act(AT_WHITE, "$n whips $s snake-like tail at you, but you dodge it.", ch, NULL, victim, TO_VICT);
    act(AT_WHITE, "You try to whiplash $N, but $E dodges out of the way.", ch, NULL, victim, TO_CHAR);
    act(AT_WHITE, "$n whips their snake-like tail at $N, but misses them narrowly.", ch, NULL, victim, TO_NOTVICT);
  }
return;
}
...
void do_tail_swipe(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, (number_percent()), gsn_tail_swipe))
  {
    if(!IS_AFFECTED(victim, AFF_PARALYSIS))
    {
      ...
      global_retcode = damage(ch, victim, medium, gsn_tail_swipe);
    }
  }
  else
  {
    ...
    act(AT_WHITE, "$n swipes their tail at you, but you dodge it.", ch, NULL, victim, TO_VICT);
    act(AT_WHITE, "You try to tail swipe $N, but $E dodges out of the way.", ch, NULL, victim, TO_CHAR);
    act(AT_WHITE, "$n swipes their tail at $N, but misses them narrowly.", ch, NULL, victim, TO_NOTVICT);
  }
}
...
void do_rage(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_rage))
  {
    ...
    {
      act(AT_RED, "You harness the power of animal rage and feel its strength rush into you!", ch, NULL, NULL, TO_CHAR);
    }
    else
    {
      act(AT_RED, "You harness the power of dragon rage and feel its strength rush into you!", ch, NULL, NULL, TO_CHAR);
    }
...
void do_truesight(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_truesight))
  {
    ...
    if(ch->Class == CLASS_PSIONIC)
    {
      ...
      send_to_char("&cYou send kinetic energy to enhancing your vision.\r\n", ch);
    }
    else
  ...
  send_to_char("&cYour vision blurs, as you fail to use your enhanced sight.\r\n", ch);
  return;
}
...
void do_fly_home(CHAR_DATA *ch, char *argument)
{
  ...
  if(!(sroom = get_room_index(ch->pcdata->htown->recall)))
  {
    send_to_char("Can't find the room to send you to if you fly home.\r\n", ch);
    bug("%s: failed to find room vnum %d.", __FUNCTION__, ch->pcdata->htown->recall);
    return;
  }

  if(IS_AFFECTED(ch, AFF_IMPRISONED))
  {
    send_to_char("You cannot fly home from the magical cage.\r\n", ch);
    return;
  }
  ...
  if(ch->in_room->sector_type == SECT_INSIDE || ch->in_room->sector_type == SECT_UNDERGROUND || IS_SET(ch->in_room->room_flags, ROOM_INDOORS))
  {
    if(ch->mana < 50)
    {
...
void do_flyby(CHAR_DATA *ch, char *argument)
{
  CHAR_DATA              *victim;
  char                    arg[MIL];
  ...
  {
    send_to_char("You can't flyby someone from the ground, you should be flying first.\r\n", ch);
    return;
  }
  ...
  if(arg[0] == '\0')
  {
    ...
    {
      send_to_char("Do a flyby on who?\r\n", ch);
      return;
    }
  }
  else if((victim = get_char_room(ch, arg)) == NULL)
  ...
  if(can_use_skill(ch, number_percent(), gsn_flyby) || IS_NPC(ch))
  {
    if(xIS_SET(ch->act, PLR_BATTLE))
      send_to_char("!!SOUND(sound/flyby.wav)\r\n", ch);
    WAIT_STATE(ch, skill_table[gsn_flyby]->beats);
    act(AT_RED, "You fly downward and do a flyby slamming into them!", ch, NULL, victim, TO_CHAR);
    act(AT_RED, "$n dived from the sky and slammed into you!", ch, NULL, victim, TO_VICT);
    act(AT_RED, "$n dived from the sky and flyby $N slamming into them!", ch, NULL, victim, TO_NOTVICT);
    if(ch->race == RACE_DRAGON)
      global_retcode = damage(ch, victim, insane, gsn_flyby);
    else
      global_retcode = damage(ch, victim, high, gsn_flyby);
    learn_from_success(ch, gsn_flyby);
  }
  else
  {
    learn_from_failure(ch, gsn_flyby);
    global_retcode = damage(ch, victim, 0, gsn_flyby);
    act(AT_RED, "You try to dive down, and do a flyby on $N, but miss your target!", ch, NULL, victim, TO_CHAR);
  }
  return;
...
void do_rake(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_AFFECTED(ch, AFF_FLYING) && !IS_NPC(ch))
  {
    send_to_char("You can't rake someone from the ground, you should be flying first.\r\n", ch);
    return;
  }
  ...
  if(can_use_skill(ch, number_percent(), gsn_rake))
  {
    if(xIS_SET(ch->act, PLR_BATTLE))
      send_to_char("!!SOUND(sound/rake.wav)\r\n", ch);
    WAIT_STATE(ch, skill_table[gsn_rake]->beats);
    act(AT_RED, "You fly downward and rake $N with your claws!", ch, NULL, victim, TO_CHAR);
    act(AT_RED, "$n dived from the sky and raked you with mighty claws!", ch, NULL, victim, TO_VICT);
    act(AT_RED, "$n dived from the sky and raked $N with mighty claws!", ch, NULL, victim, TO_NOTVICT);
    if(ch->race == RACE_DRAGON)
      global_retcode = damage(ch, victim, ludicrous, gsn_rake);
    else
      global_retcode = damage(ch, victim, mediumhigh, gsn_rake);
    learn_from_success(ch, gsn_rake);
  }
  else
  {
    learn_from_failure(ch, gsn_rake);
    global_retcode = damage(ch, victim, 0, gsn_rake);
    act(AT_RED, "You try to dive down, and rake $N, but miss your target!", ch, NULL, victim, TO_CHAR);
  }
  return;
...
void do_pluck(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_pluck))
  {
    ...
    act(AT_RED, "$n dived from the sky and plucked you up in mighty claws!", ch, NULL, victim, TO_VICT);
    act(AT_RED, "$n dived from the sky and plucked up $N in mighty claws!", ch, NULL, victim, TO_NOTVICT);
    global_retcode = damage(ch, victim, low, gsn_pluck);
    ...
    if(victim->position != POS_DEAD)
    {
      ...
      act(AT_WHITE, "$n soars high into the air taking you way off the ground!", ch, NULL, victim, TO_VICT);
      act(AT_WHITE, "$n soars high into the air with $N!", ch, NULL, victim, TO_NOTVICT);
      act(AT_RED, "You release your grip on $N and let them fall to the ground!", ch, NULL, victim, TO_CHAR);
      act(AT_RED, "$n releasing grip causes you to fall to the ground!", ch, NULL, victim, TO_VICT);
      act(AT_RED, "$n drops $N and they fall from the sky to the ground!", ch, NULL, victim, TO_NOTVICT);
  ...
  else
  {
    ...
    act(AT_RED, "You try to dive down, and pluck up $N, but miss your target!", ch, NULL, victim, TO_CHAR);
  }
  return;
}
...
void do_snatch(CHAR_DATA *ch, char *argument)
{
  ...
  if(can_use_skill(ch, number_percent(), gsn_snatch))
  {
    ...
    act(AT_RED, "$n has snatched you up, mauling you in powerful jaws!", ch, NULL, victim, TO_VICT);
    act(AT_RED, "$n reaches out and snatches up $N in powerful jaws!", ch, NULL, victim, TO_NOTVICT);
    learn_from_success(ch, gsn_snatch);
    global_retcode = damage(ch, victim, medium, gsn_snatch);
  }
  else
  {
...
void do_impale(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_AWAKE(victim) || can_use_skill(ch, percent, gsn_impale))
  {
    ...
    if(xIS_SET(ch->act, PLR_BATTLE))
      send_to_char("!!SOUND(sound/impale.wav)\r\n", ch);
 ...
 * for things like if the mob can't see you, if you're sneaking etc */
  ...
  else
  {
    ...
    if ( can_see(victim, ch ))
     global_retcode = damage(victim, ch, 0, gsn_impale );
    check_illegal_pk(ch, victim);
  }
  return;
}
...
ch_ret spell_acidic_touch(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  if(!IS_NPC(victim))
    ch_printf(ch, "Acidic materials collect at the tips of your talons as you touch %s.\r\n", victim->name);
  else
    ch_printf(ch, "Acidic materials collect at the tips of your talons as you touch %s.\r\n", capitalize(victim->short_descr));
...
ch_ret spell_searing_light(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  if(!IS_NPC(victim))
    ch_printf(ch, "You summon a searing light which blinds %s.\r\n", victim->name);
  else
    ch_printf(ch, "You summon a searing light which blinds %s.\r\n", victim->short_descr);

  ch_printf(victim, "%s summons a searing light which blinds you.\r\n", ch->name);
...
ch_ret spell_firestorm(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  act(AT_MAGIC, "You release a burst of flames into the air, damaging everyone around you!", ch, NULL, NULL, TO_CHAR);
    ...
    if(vch->in_room == ch->in_room)
    {
      if(ch->level - vch->level >= 10 && ch->alignment <= -500)
      {
        ...
        else if(chance < 10)
        {
          ...
          ch_printf(ch, "You fail to properly summon your dark powers, leaving you drained.&R%s.&d\r\n", ch->name);
          ch->hit = 1;
          ch->move = 1;
          ch->mana = 1;
          ch->alignment += 50;
        ...
        {
          ch_printf(vch, "&z%s invades your mental barriers, causing you massive pain.&d\r\n", ch->name);
      ...
      {
        ch_printf(vch, "&z%s makes a feeble attempt to bypass your mental barriers, causing some pain.&d\r\n", ch->name);
...
ch_ret spell_clavus(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  act(AT_MAGIC, "You scream in fury as you release dozens of metallic spikes into the air.", ch, NULL, NULL, TO_CHAR);
  act(AT_MAGIC, "$n screams in fury as they release dozens of metallic spikes into the air.", ch, NULL, NULL, TO_ROOM);
  if(xIS_SET(ch->act, PLR_BATTLE))
...
void do_breech(CHAR_DATA *ch, char *argument)
{
  ...
  if(victim == ch)
  {
    send_to_char("Why are you trying to breech to yourself?\r\n", ch);
    return;
  }

  if(IS_SET(victim->in_room->room_flags, ROOM_CLANSTOREROOM))
  {
    send_to_char("You cannot breech a clan storeroom location.\r\n", ch);
    return;
  }

  if(IS_AFFECTED(ch, AFF_IMPRISONED))
  {
    send_to_char("You cannot breech from the magical cage.\r\n", ch);
    return;
  }
  ...
  if(can_use_skill(ch, number_percent(), gsn_breech))
  {
    ...
    {
      send_to_char("&GYou commit all your energy to breech where forces have shielded your power!\r\n", ch);
      if(ch->move < (ch->max_move - 50))
      {
        ch->move = 0;
        send_to_char("&GYou did not have enough energy to breech the shielded location.\r\n", ch);
        return;
      }
      ch->move = 0;
    }
    ...
    if(!IS_SET(victim->in_room->room_flags, ROOM_NO_ASTRAL) && !IS_SET(victim->in_room->area->flags, AFLAG_NOASTRAL))
    {
      ...
      {
        send_to_char("You failed to summon enough energy to breech the void.\r\n", ch);
        return;
      }
    }
    ...
    {
      send_to_char("&GYou cannot breech to a Staff member.\r\n", ch);
      return;
    }
    ...
    if(ch->in_room && (ch->in_room->sector_type == SECT_WATER_SWIM
                       ...
                       || ch->in_room->sector_type == SECT_RIVER
                       || ch->in_room->sector_type == SECT_WATERFALL
                       || ch->in_room->sector_type == SECT_LAKE || ch->in_room->sector_type == SECT_OCEANFLOOR || ch->in_room->sector_type == SECT_UNDERWATER))
      msg = (char *)"in the water making a small splash";

    act(AT_WHITE, "You leave the void and land $T.\r\n", ch, NULL, msg, TO_CHAR);
    act(AT_YELLOW, "$n lands $T!", ch, NULL, msg, TO_ROOM);
