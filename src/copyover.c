...
void do_copyover(CHAR_DATA *ch, char *argument)
{
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    ...
    if(d->connected == CON_EDITING || d->connected == CON_NOTE_TO || d->connected == CON_NOTE_SUBJECT || d->connected == CON_NOTE_EXPIRE || d->connected == CON_EDITING)
      editing_count++;
    ...
    if((d->connected == CON_PLAYING) && (victim = d->character) != NULL && !IS_NPC(victim) && victim->fighting)
      fighting_count++;
  }
  ...
  for(d = first_descriptor; d; d = de_next)
  {
    ...
    {
      fprintf(fp, "%d %d %d %d %d %d %s %s\n", d->descriptor, 0, och->in_room->vnum, och->was_in_room ? och->was_in_room->vnum : och->in_room->vnum, d->port, d->idle, och->name, d->host);
      /*
  ...
  for(puppet = first_char; puppet; puppet = next_pup)
  {
    ...
    fprintf(fp, "%d %d %d %d %d %d %s %s\n ", 0, 1, puppet->in_room->vnum, puppet->was_in_room ? puppet->was_in_room->vnum : puppet->in_room->vnum, 0, 0, puppet->name, puppet->redirect->name);
...
#ifdef I3
      snprintf( buf3, sizeof( buf3 ), "%d", I3_socket );
#else
   strncpy( buf3, "-1", sizeof( buf3 ) );
#endif

  execl(EXE_FILE, "6dragons", buf, "copyover", buf2, buf3, (char *)NULL);
  /*
...
void copyover_recover()
{
  ...
  int                     desc, dcompress, room, lastroom, dport, fscanned, idle, maxp = 0;
  bool                    fOld;
  ...
  {
    d = NULL;
    fscanned = fscanf(fp, "%d %d %d %d %d %d %s %s\n", &desc, &dcompress, &room, &lastroom, &dport, &idle, name, host);
    if(desc == -1 || feof(fp))
      break;
    ...
    if(dcompress == 1)
    { /* puppet! all chars loaded at this stage */
...
//    char_from_room( d->character );
      ...
      else
      {
        ...
        if(!d->character->in_room && !d->character->was_in_room)
        {
          ...
          d->character->was_in_room = get_room_index(d->character->pcdata->htown->recall);
        }
