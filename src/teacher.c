...
CHAR_DATA              *get_teacher(int vnum)
{
  ...
  for(wch = first_char; wch; wch = wch->next)
    if(IS_NPC(wch) && vnum == wch->pIndexData->vnum)
      if(IS_NPC(wch) && (xIS_SET(wch->act, ACT_PRACTICE) || xIS_SET(wch->act, ACT_NEWBPRACTICE)))
        return wch;
  return NULL;
}
...
{
  char                    buf[MSL];
  bool                    newpract = FALSE, isteacher = FALSE;
...
bool can_learn_elsewhere(CHAR_DATA *ch, CHAR_DATA *teacher, int sn)
{
  char                    buf[MSL];
  bool                    newpract = FALSE;
...
bool can_show_teach(CHAR_DATA *ch, CHAR_DATA *teacher, int sn)
{
  ...
  if(VLD_STR(skill_table[sn]->teachers))
  {
    ...
    if(!is_name(buf, skill_table[sn]->teachers))
      ...
      return TRUE;  /* If they are a teacher of it, show it */
  }
...
void do_teacher(CHAR_DATA *ch, char *argument)
{
  ...
  {
    act(AT_TELL, "\r\n&W$n tells you 'I can teach any spell/skill under level 20.", mob, NULL, ch, TO_VICT);
  ...
  for(sn = 0; sn < top_sn; sn++)
  {
    ...
    if(!can_show_teach(ch, mob, sn))  /* If they can't teach it no point in showing it */
      continue;
    ...
    if(skill_table[sn]->type != lasttype)
    {
      ...
      set_pager_color(AT_CYAN, ch);
      pager_printf_color(ch, " &c%ss&C ", !str_cmp(skill_tname[skill_table[sn]->type], "skill") ? "Abilitie" : skill_tname[skill_table[sn]->type]);
      set_pager_color(AT_CYAN, ch);
...
void do_listteachers(CHAR_DATA *ch, char *argument)
{
   ...
   * check the world for an exact match 
   */
  send_to_char("&CThe current list of teachers and locations in 6Dragons&D\r\n\r\n", ch);
  send_to_char("&rTeacher name                  Teacher Vnum        Teacher Room Location\r\n", ch);
  send_to_char("&R-----------------------------------------------------------------------\r\n", ch);
  for(wch = first_char; wch; wch = wch->next)
  {
    if(IS_NPC(wch) && (xIS_SET(wch->act, ACT_PRACTICE) || xIS_SET(wch->act, ACT_NEWBPRACTICE)))
      ch_printf(ch, "%-35s          %-8d        %-8d.\r\n", VLD_STR(wch->pIndexData->short_descr) ? wch->pIndexData->short_descr : "No short_descr set", wch->pIndexData->vnum, wch->in_room->vnum);
  }
}
