...
bool                    packetdebug = FALSE;           /* Packet debugging toggle, can be 
                                                        * turned on to check outgoing
                                                        * packets */
...
int                     I3_socket;
int                     i3wait;                        /* Number of game loops to wait
                                                        * before attempting to reconnect
                                                        * when a socket dies */
int                     i3timeout;                     /* Number of loops to wait before
                                                        * giving up on an initial router
                                                        * connection */
...
#ifdef I3_WILEY
time_t                  i3_time;                       /* Current clock time for the
                                                        * client */
#else
    ...
    for ( src_p = txt, dest_p = tbuf; *src_p && strlen( tbuf ) <= LGST; ) {
        if ( strpbrk( src_p, first_chars ) == src_p ) {
            ...
            if ( strpbrk( src_p + 1, first_chars ) == src_p + 1 ) {
                /*
                 * Special case: We found our token escaped, so that means just send the
                 * single version straight out and skip ahead, thus making it have no
                 * other meaning. 
                 */
                match_found = TRUE;
                *dest_p++ = *src_p++;
    ...
    *tbuf = '\0';
    ...
    for ( src_p = txt, dest_p = tbuf; *src_p && strlen( tbuf ) <= LGST; ) {
        if ( src_p[0] == '%' && src_p[1] == '^' ) {    /* Start of a Pinkfish sequence */
            if ( src_p[2] == '%' && src_p[3] == '^' ) { /* Special case of a Pinkfish
                                                         * escape sequence */
                match_found = TRUE;
                *dest_p++ = *src_p++;
                *dest_p++ = *src_p++;
    ...
    while ( 1 ) {
        ...
        if ( ps[0] != '0' ) {
            if ( !( channel = find_I3_channel_by_name( chan ) ) ) {
                ...
                i3_log( "New channel %s has been added from router %s", channel->I3_name,
                        I3_ROUTER_NAME );
            }
            else {
                // i3_log( "Channel %s has been updated from router %s",
                // channel->I3_name, I3_ROUTER_NAME );
            }
            ...
            if ( ( channel = find_I3_channel_by_name( chan ) ) != NULL ) {
                if ( channel->local_name && channel->local_name[0] != '\0' )
                    i3_log( "Locally configured channel %s has been purged from router %s",
...
void i3_send_channel_t( I3_CHANNEL * channel, const char *name, const char *tmud, const char *tuser,
...
void I3_process_channel_m( I3_HEADER * header, char *s )
{
    ...
    snprintf( buf, LGST, format, local->tm_hour, local->tm_min, channel->local_name, visname,
              header->originator_mudname, ps );
#else
    snprintf( buf, LGST, channel->layout_m, channel->local_name, visname,
              header->originator_mudname, ps );
#endif
    for ( d = first_descriptor; d; d = d->next ) {
        vch = d->original ? d->original : d->character;
...
void i3_process_chan_who_req( I3_HEADER * header, char *s )
{
    ...
    if ( !channel->local_name ) {
        snprintf( buf, LGST, "The channel you specified (%s) is not registered at %s", ps,
                  I3_THISMUD );
        I3_send_error( header->originator_mudname, header->originator_username, "unk-channel",
                       buf );
        return;
...
char                   *i3_center_line( const char *string, int length )
{
    ...
    amount = length - strlen( stripped );              /* Determine amount to put in
                                                        * front of line */
...
void i3_process_who_req( I3_HEADER * header, char *s )
{
    ...
    i3_write_buffer( ",\" \",}),({\"" );
    snprintf( buf, SMST, "&Y-=[ &Wtelnet://%s:%d &Y]=-", this_i3mud->telnet,
    ...
    for ( d = first_descriptor; d; d = d->next ) {
        ...
        if ( person && d->connected >= CON_PLAYING ) {
            ...
            snprintf( personbuf, LGST, "%s %s%s", stats, CH_I3NAME( person ),
                      CH_I3TITLE( person ) );
    ...
    for ( d = first_descriptor; d; d = d->next ) {
        ...
        if ( person && d->connected >= CON_PLAYING ) {
            if ( I3PERM( person ) < I3PERM_IMM || I3ISINVIS( person )
                 || i3_ignoring( person, ibuf ) )
                continue;
    ...
    i3_write_buffer( ",\"" );
#ifdef I3_WILEY
    snprintf( tailbuf, LGST, "&Y[&WHomepage: %s&Y] [&W%d Max Since Reboot&Y]", HTTP_PLACEHOLDER,
              MAXPLAYERS_PLACEHOLDER );
    send_to_i3( i3_escape( tailbuf ) );
    i3_write_buffer( "\",})," );

    i3_write_buffer( "({\"" );
    snprintf( tailbuf, LGST, "&Y[&W%d logins since last reboot on %s&Y]", NUMLOGINS_PLACEHOLDER,
              boottimebuf );
    send_to_i3( i3_escape( tailbuf ) );
    i3_write_buffer( "\"," );
    snprintf( smallbuf, SMST, "%ld", bogusidle );
...
void I3_process_channel_adminlist_reply( I3_HEADER * header, char *s )
{
    ...
    if ( ( ch = i3_find_user( header->target_username ) ) == NULL ) {
        i3_bug( "I3_process_channel_adminlist_reply(): user %s not found.",
    ...
    I3_remove_quotes( &ps );
    if ( !( channel = find_I3_channel_by_name( ps ) ) ) {
        i3_bug( "I3_process_channel_adminlist_reply(): Invalid local channel %s reply received.",
                ps );
        return;
    }
...
void I3_parse_packet( void )
{
    ...
    if ( !strcasecmp( ptype, "router-shutdown" ) ) {
        ...
        if ( delay == 0 ) {
            ...
            i3_log( "Router %s is rebooting and will be back in %d second%s.", I3_ROUTER_NAME,
                    delay, delay == 1 ? "" : "s" );
            i3_close_connection( TRUE );
        }
    }
...
void i3_init_char( CHAR_DATA *ch )
{
    ...
    I3REPLY( ch ) = NULL;
    I3FLAG( ch ) = 0;
    I3SET_BIT( I3FLAG( ch ), I3_COLORFLAG );           /* Default color to on. People can 
                                                        * turn this off if they hate it. */
        ...
        switch ( word[0] ) {
            ...
            case 'e':
                ...
                if ( !strcasecmp( word, "end" ) ) {
                     ...
                     */
                    snprintf( lib_buf, LGST, "%s %s.%s", CODEBASE_VERSION_TITLE,
...
bool i3_read_config( int mudport )
{
    ...
    for ( ;; ) {
        ...
        if ( !strcasecmp( word, "I3CONFIG" ) && this_i3mud == NULL ) {
            ...
            this_i3mud->player_port = mudport;         /* Passed in from the mud's
                                                        * startup script */
            this_i3mud->password = 0;
            this_i3mud->mudlist_id = 0;
            this_i3mud->chanlist_id = 0;
            this_i3mud->minlevel = 10;                 /* Minimum default level before I3 
...
void i3_write_channel_config( void )
{
    ...
    for ( channel = first_I3chan; channel; channel = channel->next ) {
        /*
         * Save ALL channels... we don't want to have to wait to rebuild the list from I3 
         * every frigging reboot 
         */
        // if( channel->local_name )
        // {
...
void i3_save_mudlist( void )
{
    ...
    for ( mud = first_mud; mud; mud = mud->next ) {
        ...
        fprintf( fp, "Services %d %d %d %d %d %d %d %d %d %d %d %d\n",
                 mud->tell, mud->beep, mud->emoteto, mud->who, mud->finger, mud->locate,
                 mud->channel, mud->news, mud->mail, mud->file, mud->auth, mud->ucache );
        fprintf( fp, "OOBports %d %d %d %d %d %d %d\n", mud->smtp, mud->ftp, mud->nntp, mud->http,
                 mud->pop3, mud->rcp, mud->amrcp );
        fprintf( fp, "%s", "End\n\n" );
    }
    fprintf( fp, "%s", "#END\n" );
    I3FCLOSE( fp );
...
void i3_close_connection( bool reconnect )
{
    ...
    if ( reconnect ) {
        if ( router->reconattempts <= 3 ) {
            ...
            bytes_received = 0;
            i3wait = 100;
            i3_log( "Will attempt new connection to %s in approximately 15 seconds.",
                    router->next->name );
        }
        else {
            bytes_sent = 0;
...
void i3_shutdown( int delay )
{
    ...
    I3_output_pointer = 4;
    i3_save_id(  );
    sleep( 2 );                                        /* Short delay to allow the socket 
                                                        * to close */
}
...
void router_connect( const char *router_name, bool forced, int mudport, bool isconnected )
{
    ...
    if ( ( !this_i3mud->autoconnect && !forced && !isconnected )
         || ( isconnected && I3_socket < 1 ) ) {
        i3_log( "%s",
...
I3_CMD( i3_show_ucache_contents )
{
    ...
    i3_send_to_pager
        ( "&wUser                          | Gender ( 0 = Male, 1 = Female, 2 = Neuter )\r\n", ch );
    i3_send_to_pager
        ( "&w---------------------------------------------------------------------------\r\n", ch );
    for ( user = first_ucache; user; user = user->next ) {
...
I3_CMD( i3_setup_channel )
{
    ...
    if ( localname[0] == '\0' ) {
        ...
        i3_log( "setup_channel: removing %s as %s@%s", channel->local_name, channel->I3_name,
                channel->host_mud );
        i3_send_channel_listen( channel, FALSE );
        I3STRFREE( channel->local_name );
        i3_write_channel_config(  );
        ...
        i3_printf( ch, "%s@%s is now locally known as %s\r\n", channel->I3_name, channel->host_mud,
                   channel->local_name );
        i3_log( "setup_channel: setting up %s@%s as %s", channel->I3_name, channel->host_mud,
                channel->local_name );
        i3_send_channel_listen( channel, TRUE );
        i3_write_channel_config(  );
    }
...
I3_CMD( i3_listen_channel )
{
    ...
    if ( !strcasecmp( argument, "all" ) ) {
        for ( channel = first_I3chan; channel; channel = channel->next ) {
            ...
            if ( I3PERM( ch ) >= channel->i3perm
                 && !i3_has_name( I3LISTEN( ch ), channel->local_name ) )
...
I3_CMD( i3_chanlayout )
{
    ...
    if ( !argument || argument[0] == '\0' ) {
        ...
        i3_to_char
            ( "&wFormat can be any way you want it to look, provided you have the proper number of %s tags in it.\r\n",
              ch );
        return;
    }
...
I3_CMD( i3_setconfig )
{
    ...
    if ( arg[0] == '\0' ) {
        ...
        i3_to_char( "&wMudtype    : &GWhat type of mud you have. Diku, LP, CoffeeMUD, etc.\r\n",
                    ch );
        i3_to_char
            ( "&wBasemudlib : &GThe base version of the codebase you have. DikuMUD Alfa, Merc, Smaug, etc.\r\n",
              ch );
        i3_to_char
            ( "&wMudlib     : &GWhat you call the current version of your codebase.  Such as SmaugFUSS 1.23\r\n",
              ch );
...
char                   *i3_find_social( CHAR_DATA *ch, char *sname, char *person, char *mud,
                                        bool victim )
{
    ...
    for ( cmd = 0; social_table[cmd].name[0] != '\0'; cmd++ ) {
        if ( sname[0] == social_table[cmd].name[0]
             && !i3_str_prefix( sname, social_table[cmd].name ) ) {
            found = TRUE;
            break;
        }
    }
    ...
    while ( *format != '\0' ) {
        ...
        if ( ( !vic )
            ...
            switch ( *format ) {
                ...
                case 'e':
                    i = should_upper ?
                        i3_capitalize( he_she[URANGE( 0, CH_I3SEX( ch ), 2 )] ) :
                        he_she[URANGE( 0, CH_I3SEX( ch ), 2 )];
                    break;

                case 'E':
                    i = should_upper ?
                        i3_capitalize( he_she[URANGE( 0, CH_I3SEX( vic ), 2 )] ) :
                        he_she[URANGE( 0, CH_I3SEX( vic ), 2 )];
                    break;

                case 'm':
                    i = should_upper ?
                        i3_capitalize( him_her[URANGE( 0, CH_I3SEX( ch ), 2 )] ) :
                        him_her[URANGE( 0, CH_I3SEX( ch ), 2 )];
                    break;

                case 'M':
                    i = should_upper ?
                        i3_capitalize( him_her[URANGE( 0, CH_I3SEX( vic ), 2 )] ) :
                        him_her[URANGE( 0, CH_I3SEX( vic ), 2 )];
                    break;

                case 's':
                    i = should_upper ?
                        i3_capitalize( his_her[URANGE( 0, CH_I3SEX( ch ), 2 )] ) :
                        his_her[URANGE( 0, CH_I3SEX( ch ), 2 )];
                    break;

                case 'S':
                    i = should_upper ?
                        i3_capitalize( his_her[URANGE( 0, CH_I3SEX( vic ), 2 )] ) :
