...
struct tzone_type
{
  ...
  int                     dst_offset; /* Day Light Savings Time offset, Not used but left 
                                       * it in anyway */
};
...
void do_timezone(CHAR_DATA *ch, char *argument)
{
  ...
  if(!argument || argument[0] == '\0')
  {
    ...
    send_to_char("-------------------------------------------------------------------------\r\n", ch);
    send_to_char("\r\nSyntax: Timezone < &CGMT(&Y-&W/&Y+&C)#&W >\r\n", ch);
    return;
  }
...
char                   *c_time(time_t curtime, int tz)
{
  ...
  const static char      *month[] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov",
    "Dec"
  };
  ...
  if(tz > -1 && tz < MAX_TZONE)
  {
    ...
    curtime += _timezone;
#else
    curtime += timezone;  /* timezone external variable in time.h holds the difference in
                           * seconds to GMT. */
#endif
  ...
  snprintf(strtime, 128, "%3s %3s %d, %d %d:%02d:%02d %cM %s",
           ...
           ptime->tm_year + 1900, ptime->tm_hour == 0 ? 12 : ptime->tm_hour > 12 ? ptime->tm_hour - 12 : ptime->tm_hour, ptime->tm_min, ptime->tm_sec, ptime->tm_hour < 12 ? 'A' : 'P', tzonename);
  return strtime;
}
...
char                   *mini_c_time(time_t curtime, int tz)
{
  ...
  snprintf(strtime, 128, "%02d/%02d/%02d %02d:%02d%c", ptime->tm_mon + 1,
           ptime->tm_mday, ptime->tm_year - 100, ptime->tm_hour == 0 ? 12 : ptime->tm_hour > 12 ? ptime->tm_hour - 12 : ptime->tm_hour, ptime->tm_min, ptime->tm_hour < 12 ? 'A' : 'P');
  return strtime;
}
...
const char             *const month_name[] = {
  "the Grand Struggle", "Renewal", "Nature", "Hope",  /* spring */
  "the Dragon", "the Sun", "the Heat", "the Battle",  /* summer */
  "West Winds", "the Long Shadows", "Futility", "the Ancient Darkness", /* fall */
  "the Great Evil", "the Winter Wolf", "the Frost Giant", "the Old Forces"  /* winter */
};

const char             *const season_name[] = {
  "&gspring", "&Ysummer", "&Ofall", "&Cwinter"
};
...
void do_time(CHAR_DATA *ch, char *argument)
{
  ...
  ch_printf(ch,
            ...
            day_name[(time_info.day) % sysdata.daysperweek], day, suf, month_name[time_info.month], season_name[time_info.season], time_info.year, str_boot_time, c_time(current_time, -1));
  ...
  for(;;)
  {
    ...
    switch (UPPER(word[0]))
    {
      ...
      case 'E':
        ...
        {
          if(!day->announce)
            day->announce = STRALLOC("Today is a holiday, but who the hell knows which one.");
          return;
        }
        break;
...
/* Create a new holiday by name arg1 */
  if(!str_cmp(arg2, "create"))
  {
    ...
    newday->announce = STRALLOC("Today is the holiday of when some moron forgot to set the announcement for this one!");
...
/* List all the months with a counter next to them*/
      count = 1;
      while(month_name[x] != NULL && str_cmp(month_name[x], " ") && x < sysdata.monthsperyear)
      {
...
/* Anything match? */
  ...
  ch_printf(ch, "&g--------------------\r\n");
  ch_printf(ch, "&GMonth &W%s (%d)\t&GDay &W%2d\r\n", month_name[day->month], day->month, day->day);
