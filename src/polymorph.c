...
void do_morphstat(CHAR_DATA *ch, char *argument)
{
  ...
  if(argument[0] == '\0')
  {
    ...
    pager_printf(ch,
                 ...
                 SEX_FEMALE ? "female" : "neutral", morph->pkill == ONLY_PKILL ? "YES" : morph->pkill == ONLY_PEACEFULL ? "NO" : "n/a", morph->timefrom, morph->timeto);
    ...
    pager_printf(ch,
                 ...
                 morph->saving_poison_death, morph->saving_wand, morph->saving_para_petri, morph->saving_breath, morph->saving_spell_staff, morph->dodge, morph->parry, morph->tumble);
    pager_printf(ch, "  &cHps     : &w%s    &cBlood  : &w%s    &cMana   : &w%s    &cMove      : &w%s\r\n", morph->hit, morph->blood, morph->mana, morph->move);
    ...
    send_to_pager("&B[----------------------------------------------------------------------------]\r\n", ch);
    pager_printf(ch, "  &cAffected by: &C%s\r\n", !xIS_EMPTY(morph->affected_by) ? ext_flag_string(&morph->affected_by, a_flags) : "Nothing");
/*
...
*/
    ...
    send_to_pager("&B[----------------------------------------------------------------------------]\r\n", ch);
    pager_printf(ch, "  &cNot affected by: &C%s\r\n", !xIS_EMPTY(morph->no_affected_by) ? ext_flag_string(&morph->no_affected_by, a_flags) : "Nothing");
...
ext_flag_string(&morph->no_affected_by2, a2_flags) : "Nothing"); */
  ...
  else if(!str_cmp(argument, "help") || !str_cmp(argument, "desc"))
  {
    ...
    pager_printf(ch, "  &cLongdesc    : &w%s", (morph->long_desc[0] == '\0') ? "(none set)\r\n" : morph->long_desc);
...
int do_morph_char(CHAR_DATA *ch, MORPH_DATA * morph)
{
  ...
  if(!IS_AFFECTED(ch, AFF_ANIMALFORM))
  {
    if(morph->hpused)
    {
...
bool can_morph(CHAR_DATA *ch, MORPH_DATA * morph, bool is_cast)
{
  if(morph == NULL)
    return FALSE;
  if(IS_IMMORTAL(ch) || IS_NPC(ch)) /* Let immortals morph to anything Also NPC can do *
                                     * any morph */
    return TRUE;
...
void do_morph(CHAR_DATA *ch, MORPH_DATA * morph)
{
  ...
  if(!IS_AFFECTED(ch, AFF_ANIMALFORM))
  {
    ...
    if(VLD_STR(morph->hitroll))
      ch_morph->hitroll = dice_parse(ch, ch, morph->level, morph->hitroll);
    else
      ch_morph->hitroll = 0;
    ch->hitroll += ch_morph->hitroll;
    if(VLD_STR(morph->damroll))
      ch_morph->damroll = dice_parse(ch, ch, morph->level, morph->damroll);
    else
      ch_morph->damroll = 0;
    ch->damroll += ch_morph->damroll;
    if(VLD_STR(morph->hit))
      ch_morph->hit = dice_parse(ch, ch, morph->level, morph->hit);
    else
      ch_morph->hit = 0;
    ...
    ch->hit += ch_morph->hit;
    if(VLD_STR(morph->move))
      ch_morph->move = dice_parse(ch, ch, morph->level, morph->move);
    else
      ch_morph->move = 0;
    ...
    {
      if(VLD_STR(morph->mana))
        ch_morph->mana = dice_parse(ch, ch, morph->level, morph->mana);
      ...
      case 'C':
        ...
        if(!str_cmp(word, "Class"))
        {
          ...
          while(VLD_STR(arg))
          {
            ...
            for(i = 0; i < MAX_PC_CLASS; i++)
              if(class_table[i] && VLD_STR(class_table[i]->who_name) && !str_cmp(temp, class_table[i]->who_name))
              {
        ...
        if(!str_cmp(word, "Race"))
        {
          ...
          while(VLD_STR(arg))
          {
            ...
            for(i = 0; i < MAX_PC_RACE; i++)
              if(VLD_STR(race_table[i]->race_name) && !str_cmp(temp, race_table[i]->race_name))
              {
...
void sort_morphs()
{
  ...
  do
  {
    ...
    if(morph_start == NULL)
      return;
    for(morph = morph_start, temp = morph->next; morph && temp; morph = morph->next, temp = temp->next)
    {
