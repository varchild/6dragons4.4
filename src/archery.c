...
OBJ_DATA               *find_magic_missile(CHAR_DATA *ch)
{
  OBJ_DATA               *arrow;

  for(arrow = ch->last_carrying; arrow; arrow = arrow->prev_content)
  {
    if(can_see_obj(ch, arrow))
    {
      if(!str_cmp(arrow->name, "magic missile"))
        return arrow;
    }
  }
  return NULL;
...
void do_dislodge(CHAR_DATA *ch, char *argument)
{
  ...
  if(get_eq_char(ch, WEAR_LODGE_RIB) != NULL)
  {
    ...
    act(AT_CARNAGE, "With a wrenching pull, you dislodge $p from your ribs.", ch, arrow, NULL, TO_CHAR);
    act(AT_CARNAGE, "$n winces in pain as $e dislodges $p from $s ribs.", ch, arrow, NULL, TO_ROOM);
...
ch_ret ranged_got_target(CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *weapon, OBJ_DATA *projectile, short dist, short dt, char *stxt, short color)
{
  ...
  if(number_percent() > 25 || (projectile && weapon && can_use_skill(ch, number_percent(), wtype)))
  {
...
ch_ret ranged_attack(CHAR_DATA *ch, char *argument, OBJ_DATA *weapon, OBJ_DATA *projectile, short dt, short range)
{
  ...
  if(pexit)
  {
    if((IS_SET(pexit->exit_flags, EX_FORTIFIED)
        ...
        || IS_SET(pexit->exit_flags, EX_LIGHT) || IS_SET(pexit->exit_flags, EX_CRUMBLING)) && !IS_SET(pexit->exit_flags, EX_WINDOW) && !IS_SET(pexit->exit_flags, EX_ASLIT))
    {
      send_to_char("Are you expecting to fire through a wall!?\r\n", ch);
      return rNONE;
    }
  ...
  while(dist <= range)
  {
    ...
    if(IS_SET(pexit->exit_info, EX_CLOSED))
    {
      ...
      if(projectile)
      {
        act_printf(color, ch, NULL, NULL, TO_CHAR, "You see your %s pierce a door in the distance to the %s.", myobj(projectile), dir_name[dir]);
        act_printf(color, ch, projectile, NULL, TO_ROOM, "$p flies in from %s and implants itself solidly in the %sern door.", dtxt, dir_name[dir]);
        if(projectile->in_obj)
          obj_from_obj(projectile);
        if(projectile->carried_by)
          obj_from_char(projectile);
      ...
      {
        act_printf(color, ch, NULL, NULL, TO_CHAR, "You see your %s hit a door in the distance to the %s.", stxt, dir_name[dir]);
        snprintf(buf, MSL, "%s flies in from %s and implants itself solidly in the %sern door.", aoran(stxt), dtxt, dir_name[dir]);
        buf[0] = UPPER(buf[0]);
    ...
    if((pexit = get_exit(ch->in_room, dir)) == NULL)
    {
      if(projectile)
      {
        ...
        act(color, "$p strikes the $Tern wall and falls harmlessly to the ground.", ch, projectile, (char *)dir_name[dir], TO_ROOM);
        if(projectile->in_obj)
          obj_from_obj(projectile);
        if(projectile->carried_by)
          obj_from_char(projectile);
      ...
      else
      {
        ...
        act(color, "$t strikes the $Tern wall and falls harmlessly to the ground.", ch, aoran((char *)stxt), (char *)dir_name[dir], TO_ROOM);
      }
      break;
    }
    if(projectile)
...
void do_fire(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_NPC(ch) && IS_AFFECTED(ch, AFF_MAGIC_MISSILE))
  {

    if((arrow = find_magic_missile(ch)) == NULL)
    {
      send_to_char("You have no more magic missiles!\r\n", ch);

      for(paf = ch->first_affect; paf; paf = paf_next)
      {
        paf_next = paf->next;

        if(xIS_SET(paf->bitvector, AFF_MAGIC_MISSILE))
        {
          affect_remove(ch, paf);
        }
      }
      xREMOVE_BIT(ch->affected_by, AFF_MAGIC_MISSILE);
      return;
    }
    ...
    act_printf(AT_ACTION, ch, NULL, victim, TO_ROOM, "$n fires %s at $N.", arrow->short_descr);
    act_printf(AT_ACTION, ch, NULL, victim, TO_CHAR, "You fire %s at $N.", arrow->short_descr);

    damage(ch, victim, number_range(40 + ch->level, 120 + ch->level), TYPE_HIT);
  ...
  {
    send_to_char("You are not holding a projectile in your hand!\r\n", ch);
    return;
  }
  ...
  if(xIS_SET(ch->act, PLR_BATTLE))
    send_to_char("!!SOUND(sound/fire.wav)\r\n", ch);
