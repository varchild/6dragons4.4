...
void                    CheckRoom(CHAR_DATA *ch, ROOM_INDEX_DATA *location, int PosX, int PosY, unsigned short int map[50][50]);

void                    CheckExitDir(CHAR_DATA *ch, ROOM_INDEX_DATA *location, int PosX, int PosY, int Exd, unsigned short int map[50][50]);

int                     centerx, centery, MapWidth, MapLength;
...
char                   *map_type(int map_position)
{
  ...
  switch (map_position)
  {
    ...
    case (SECT_MAX + 3): /* -3 to +3 levels of player */
      sect = "&G@";
      break;
    case (SECT_MAX + 4): /* 4 or 5 levels higher then * * * * player by soft range */
      sect = "&Y@";
      break;
    case (SECT_MAX + 5): /* Can't enter because of * * * * * * * * hard_range */
      sect = "&R@";
      break;
    case (SECT_MAX + 6):
      sect = "&Y*";
