 ...
 ***************************************************************************
 * - MOR programs module based on mob program code by N'Atas-ha            *
 ***************************************************************************/
...
char                   *get_secondclass(CHAR_DATA *ch);
char                   *get_thirdclass(CHAR_DATA *ch);
char                   *get_race(CHAR_DATA *ch);

char                   *get_class(CHAR_DATA *ch)
{
  if(IS_NPC(ch) && ch->Class < MAX_NPC_CLASS && ch->Class >= 0)
    return ((char *)npc_class[ch->Class]);
...
char                   *get_secondclass( CHAR_DATA *ch )
{
    if ( !IS_NPC( ch ) && ch->secondclass < MAX_PC_CLASS && ch->secondclass >= 0 )
        return class_table[ch->secondclass]->who_name;
    return ( ( char * ) "Unknown" );
}

char                   *get_thirdclass( CHAR_DATA *ch )
{
    if ( !IS_NPC( ch ) && ch->thirdclass < MAX_PC_CLASS && ch->thirdclass >= 0 )
        return class_table[ch->thirdclass]->who_name;
    return ( ( char * ) "Unknown" );
}
*/
/*
 ...
 * in mprog_driver. */
/* Moved these to mud.h as I needed two of them for mppause -rkb */
...
/* Global variables to handle pauseing mud progs. */
MPPAUSE_DATA           *first_mppause = NULL;
MPPAUSE_DATA           *last_mppause = NULL;
MPPAUSE_DATA           *current_mppause = NULL;
...
char                   *mprog_one_command(char *argument, char *arg_first)
{
  ...
  while(*argument != '\0' || ++count >= 255)
  {
    if(*argument == '\n' || *argument == '\r')
    {
      ...
      if(lch == '+')
      { /* If the last char was a + we want to continue */
        if(newappend)
        {
...
bool mprog_beval(bool lhs, char *opr, char *rhs, CHAR_DATA *mob)
{
  ...
  progbug(mob, "Improper MOBprog operator '%s' for Boolean Evaluation", opr);
  return lhs;
}
...
bool mprog_seval(char *lhs, char *opr, char *rhs, CHAR_DATA *mob)
{
  ...
  if(!str_cmp(opr, "!/"))
    return (bool)(str_infix(rhs, lhs));
  progbug(mob, "Improper MOBprog operator '%s' [%s]", opr);
  return 0;
}
...
char                   *parse_var(const char *cmnd, struct char_data *mob, struct char_data *actor, OBJ_DATA *obj, const void *vo, struct char_data *rndm)
{
  static char             results[MSL * 2];
  int                     x;
  struct char_data       *chkchar = NULL;
...
/*
                    if ( !str_cmp( word, "secondclass" ) )
                        mudstrlcat( results, get_secondclass( chkchar ), MSL );
                    if ( !str_cmp( word, "thirdclass" ) )
                        mudstrlcat( results, get_thirdclass( chkchar ), MSL );
*/
        ...
        if(chkobj != NULL)
        {
          ...
          if(!str_cmp(word, "name"))
            mudstrlcat(results, format("%s", chkobj->name), MSL);
        }
        if(cmnd[x] == '\0')
          return results;
        else
...
int mprog_do_ifcheck(char *ifcheck, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, CHAR_DATA *rndm)
{
  ...
  char                   *chck, *cvar, *argv[MAX_IF_ARGS], *rval = (char *)"";
  char                   *q, *p = buf;
  int                     argc = 0;
  ...
  if(*p != '(')
  {
    progbug(mob, "%s", "Ifcheck Syntax error (missing left bracket)");
    return BERR;
  }
  *p++ = '\0';
  /*
    ...
    switch (*p)
    {
      ...
      case ')':
        *p++ = '\0';
        ...
        break;
      default:
        progbug(mob, "Ifcheck Syntax warning (missing right bracket) [%s]", mob->name);
        goto doneargs;
        break;
    }
  }
doneargs:
  ...
  *p = '\0';
  ...
  if(cvar[0] == '$')
  {
    switch (cvar[1])
    {
      ...
      case 'p':
        ...
        break;
      default:
        progbug(mob, "Bad argument '%c' to '%s' [%s]", cvar[0], chck, mob->name);
        return BERR;
    }
    if(!chkchar && !chkobj)
      return BERR;
  ...
  if(!str_cmp(chck, "quest"))
  {
    ...
    quest = get_quest_from_name(cvar);

    if(!quest)
    {
      progbug(mob, "Bad quest name passed to 'if quest' [%s]", mob->name);
      return BERR;
    }

    if(!rval)
    {
      progbug(mob, "Bad chapter passed to 'if quest' [%s]", mob->name);
      return BERR;
    }

    return mprog_veval(GET_CHAPTER(actor, quest), opr, atoi(rval), mob);
  }
  if(!str_cmp(chck, "qkamount"))
  {
    QUEST_DATA             *quest;
    ...
    if(!rval)
    {
      progbug(mob, "Bad kamount passed to 'if qkamount' [%s]", mob->name);
      return BERR;
    }

    return mprog_veval(GET_CHKAMOUNT(actor, quest), opr, atoi(rval), mob);
  }
  ...
  if(!str_cmp(chck, "economy"))
  {
    ...
    if(!idx)
    {
      if(!mob->in_room)
      {
        progbug(mob, "'economy' ifcheck: mob in NULL room with no room vnum argument [%s]", mob->name);
        return BERR;
      }
      room = mob->in_room;
    }
    ...
    if(!room)
    {
      progbug(mob, "Bad room vnum passed to 'economy' [%s]", mob->name);
      return BERR;
    }
    return mprog_veval(((room->area->high_economy[DEFAULT_CURR] > 0) ? 1000000000 : 0) + room->area->low_economy[DEFAULT_CURR], opr, atoi(rval), mob);
  }
  ...
  if(!str_cmp(chck, "mobinarea"))
  {
    ...
    {
      progbug(mob, "Bad vnum to 'mobinarea' [%s]", mob->name);
      return BERR;
    }
  ...
  if(!str_cmp(chck, "mobinroom"))
  {
    ...
    if(vnum < 1 || vnum > MAX_VNUM)
    {
      progbug(mob, "Bad vnum to 'mobinroom' [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
  ...
  if(!str_cmp(chck, "mobinworld"))
  {
    ...
    if(vnum < 1 || vnum > MAX_VNUM)
    {
      progbug(mob, "Bad vnum to 'mobinworld' [%s]", mob->name);
      return BERR;
    }
  ...
  if(!str_cmp(chck, "timeskilled"))
  {
    ...
    {
      progbug(mob, "TimesKilled ifcheck: bad vnum [%s]", mob->name);
      return BERR;
    }
  ...
  if(!str_cmp(chck, "ovnumhere"))
  {
    ...
    if(vnum < 1 || vnum > MAX_VNUM)
    {
      progbug(mob, "OvnumHere: bad vnum [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
  ...
  if(!str_cmp(chck, "objinworld"))
  {
    ...
    if(vnum < 1 || vnum > MAX_VNUM)
    {
      progbug(mob, "Bad vnum to 'objinworld' [%s]", obj->name);
      return BERR;
    }
  ...
  if(!str_cmp(chck, "otypehere"))
  {
    ...
    {
      progbug(mob, "OtypeHere: bad type [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
  ...
  if(!str_cmp(chck, "ovnumroom"))
  {
    ...
    if(vnum < 1 || vnum > MAX_VNUM)
    {
      progbug(mob, "OvnumRoom: bad vnum [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
    for(pObj = mob->in_room->first_content; pObj; pObj = pObj->next_content)
  ...
  if(!str_cmp(chck, "otyperoom"))
  {
    ...
    if(type < 0 || type > MAX_ITEM_TYPE)
    {
      progbug(mob, "OtypeRoom: bad type [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
  ...
  if(!str_cmp(chck, "otypecarry"))
  {
    ...
    if(type < 0 || type > MAX_ITEM_TYPE)
    {
      progbug(mob, "OtypeCarry: bad type [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
    for(pObj = mob->first_carrying; pObj; pObj = pObj->next_content)
  ...
  if(!str_cmp(chck, "ovnumwear"))
  {
    ...
    if(vnum < 1 || vnum > MAX_VNUM)
    {
      progbug(mob, "OvnumWear: bad vnum [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
    for(pObj = mob->first_carrying; pObj; pObj = pObj->next_content)
  ...
  if(!str_cmp(chck, "otypewear"))
  {
    ...
    if(type < 0 || type > MAX_ITEM_TYPE)
    {
      progbug(mob, "OtypeWear: bad type [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
    for(pObj = mob->first_carrying; pObj; pObj = pObj->next_content)
  ...
  if(!str_cmp(chck, "ovnuminv"))
  {
    ...
    if(vnum < 1 || vnum > MAX_VNUM)
    {
      progbug(mob, "OvnumInv: bad vnum [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
  ...
  if(!str_cmp(chck, "otypeinv"))
  {
    ...
    if(type < 0 || type > MAX_ITEM_TYPE)
    {
      progbug(mob, "OtypeInv: bad type [%s]", mob->name);
      return BERR;
    }
    lhsvl = 0;
    for(pObj = mob->first_carrying; pObj; pObj = pObj->next_content)
  ...
  if(chkchar)
  {
    ...
    {
      return mprog_beval((IS_NPC(chkchar) != 0), opr, rval, mob);
      // return IS_NPC( chkchar ) ? TRUE : FALSE;
    }
    if(!str_cmp(chck, "cansee"))
    {
      return mprog_beval((can_see(mob, mob) != 0), opr, rval, mob);
      // return can_see( mob, mob );
    }
    ...
    {
      return mprog_beval((IS_PKILL(chkchar) != 0), opr, rval, mob);
      // return IS_PKILL( chkchar ) ? TRUE : FALSE;
    }
    if(!str_cmp(chck, "canpkill"))
    {
      return mprog_beval((CAN_PKILL(chkchar) != 0), opr, rval, mob);
      // return CAN_PKILL( chkchar ) ? TRUE : FALSE;
    }

    if(!str_cmp(chck, "chieftain"))
    {
      CLAN_DATA              *clan;
      ...
      clan = chkchar->pcdata->clan;

      if(!str_cmp(chkchar->name, clan->chieftain))
        return TRUE;
      if(str_cmp(chkchar->name, clan->chieftain))
        return FALSE;
    }
    if(!str_cmp(chck, "warmaster"))
    {
      CLAN_DATA              *clan;
      ...
      if(!str_cmp(chkchar->name, clan->warmaster))
        return TRUE;
      if(str_cmp(chkchar->name, clan->warmaster))
        return FALSE;
    }
    ...
    {
      return (chkchar->position == POS_MOUNTED) ? TRUE : FALSE;
    }
    if(!str_cmp(chck, "ismorphed"))
    {
      // return mprog_beval( ( ( chkchar->morph ) != 0 ), opr, rval, mob );
      return (chkchar->morph != NULL) ? TRUE : FALSE;
    }
    if(!str_cmp(chck, "isnuisance"))
    {
      // return mprog_beval( ( !IS_NPC( chkchar ) ? chkchar->pcdata->nuisance != 0
      // ),
      // opr, rval, mob );
    ...
    {
      return mprog_beval((IS_GOOD(chkchar) != 0), opr, rval, mob);
      // return IS_GOOD( chkchar ) ? TRUE : FALSE;
    }
    if(!str_cmp(chck, "isneutral"))
    {
      return mprog_beval((IS_NEUTRAL(chkchar) != 0), opr, rval, mob);
      // return IS_NEUTRAL( chkchar ) ? TRUE : FALSE;
    }
    if(!str_cmp(chck, "isevil"))
    {
      return mprog_beval((IS_EVIL(chkchar) != 0), opr, rval, mob);
      // return IS_EVIL(chkchar) ? TRUE : FALSE;
    }
    if(!str_cmp(chck, "isfight"))
    {
      return mprog_beval((who_fighting(chkchar) != 0), opr, rval, mob);
    }
    ...
    {
      return mprog_beval((IS_AFFECTED(chkchar, AFF_CHARM) != 0), opr, rval, mob);
      // return IS_AFFECTED( chkchar, AFF_CHARM ) ? TRUE : FALSE;
    }
    if(!str_cmp(chck, "isflying"))
    {
      return mprog_beval((IS_AFFECTED(chkchar, AFF_FLYING) != 0), opr, rval, mob);
    ...
    if(!str_cmp(chck, "isaffected"))
    {
      ...
      {
        progbug(mob, "Unknown affect being checked [%s]", mob->name);
        return BERR;
      }
      return mprog_beval((IS_AFFECTED(chkchar, value) != 0), opr, rval, mob);
    ...
    if(!str_cmp(chck, "martial")) // focus support to progs. -Taon
    {
      if(IS_NPC(chkchar))
        return FALSE;

      return mprog_veval(chkchar->martialarts, opr, atoi(rval), mob);
    }
    ...
    if(!str_cmp(chck, "goldamt"))
      return mprog_veval(GET_MONEY(chkchar, CURR_GOLD), opr, atoi(rval), mob);
    if(!str_cmp(chck, "silveramt"))
      return mprog_veval(GET_MONEY(chkchar, CURR_SILVER), opr, atoi(rval), mob);
    if(!str_cmp(chck, "copperamt"))
      return mprog_veval(GET_MONEY(chkchar, CURR_COPPER), opr, atoi(rval), mob);
    if(!str_cmp(chck, "bronzeamt"))
      return mprog_veval(GET_MONEY(chkchar, CURR_BRONZE), opr, atoi(rval), mob);
    if(!str_cmp(chck, "class"))
    {
      if(IS_NPC(chkchar))
        return mprog_seval((char *)npc_class[chkchar->Class], opr, rval, mob);
      return mprog_seval((char *)class_table[chkchar->Class]->who_name, opr, rval, mob);
    }
    // Check current quest points via mobprog. -Taon
    if(!str_cmp(chck, "quest_curr"))
    {
      if(IS_NPC(chkchar))
        return FALSE;
      return mprog_veval(chkchar->quest_curr, opr, atoi(rval), mob);
    }
    // Added in second and third class support here. -Taon
/*
        if ( !str_cmp( chck, "secondclass" ) ) {
            if ( IS_NPC( chkchar ) )
                return mprog_seval( ( char * ) npc_class[chkchar->secondclass], opr, rval, mob );
            return mprog_seval( ( char * ) class_table[chkchar->secondclass]->who_name, opr, rval,
                                mob );
        }
        if ( !str_cmp( chck, "ismulti" ) ) {
            if ( chkchar->secondclass > -1 )
                return TRUE;

            return FALSE;
        }

        if ( !str_cmp( chck, "thirdclass" ) ) {
            if ( IS_NPC( chkchar ) )
                return mprog_seval( ( char * ) npc_class[chkchar->thirdclass], opr, rval, mob );
            return mprog_seval( ( char * ) class_table[chkchar->thirdclass]->who_name, opr, rval,
                                mob );
        }
*/
    ...
    if(!str_cmp(chck, "multi"))
    {
      ...
      for(ch = first_char; ch; ch = ch->next)
        if(!IS_NPC(chkchar) && !IS_NPC(ch) && ch->desc && chkchar->desc && VLD_STR(ch->desc->host) && VLD_STR(chkchar->desc->host) && !str_cmp(ch->desc->host, chkchar->desc->host))
          lhsvl++;
      rhsvl = atoi(rval);
      if(rhsvl < 0)
        rhsvl = 0;
    ...
    {
      /*
       * Lets see if this does what we want it to 
       */
    ...
    if(!str_cmp(chck, "htown"))
    {
      if(IS_NPC(chkchar) || !chkchar->pcdata->htown)
        return FALSE;
      if(!str_cmp(chkchar->pcdata->htown_name, "Dakar City"))
        return mprog_seval(chkchar->pcdata->htown_name, opr, rval, mob);
      if(!str_cmp(chkchar->pcdata->htown_name, "Forbidden City"))
        return mprog_seval(chkchar->pcdata->htown_name, opr, rval, mob);
      if(!str_cmp(chkchar->pcdata->htown_name, "Paleon City"))
        return mprog_seval(chkchar->pcdata->htown_name, opr, rval, mob);
    }

    if(!str_cmp(chck, "city"))
    {
      if(IS_NPC(chkchar) || !chkchar->pcdata->city)
        return FALSE;
      if(!str_cmp(chkchar->pcdata->city_name, "Dakar City"))
        return mprog_seval(chkchar->pcdata->city_name, opr, rval, mob);
      if(!str_cmp(chkchar->pcdata->city_name, "Forbidden City"))
        return mprog_seval(chkchar->pcdata->city_name, opr, rval, mob);
      if(!str_cmp(chkchar->pcdata->city_name, "Paleon City"))
        return mprog_seval(chkchar->pcdata->city_name, opr, rval, mob);
    ...
    if(!str_cmp(chck, "iscarrying"))
    {
      ...
      if(vnum < 1 || vnum > MAX_VNUM)
      {
        progbug(mob, "iscarrying: bad vnum [%s]", mob->name);
        return BERR;
      }

      if(str_cmp(opr, "=="))
      {
        progbug(mob, "iscarrying: bad check [%s]", mob->name);
        return BERR;
      }
    ...
    if(!str_cmp(chck, "iswearing"))
    {
      ...
      if(vnum < 1 || vnum > MAX_VNUM)
      {
        progbug(mob, "iswearing: bad vnum [%s]", mob->name);
        return BERR;
      }

      if(str_cmp(opr, "=="))
      {
        progbug(mob, "iswearing: bad check [%s]", mob->name);
        return BERR;
      }
...
/*
  if(!str_cmp(chck, "number"))
  {
    if(chkchar)
    {
      if(!IS_NPC(chkchar))
        return FALSE;
      lhsvl = (chkchar == mob) ? GET_MONEY(chkchar, DEFAULT_CURR) : chkchar->pIndexData->vnum;
...
*/
  ...
  { /* -- Gorog */
    ...
    for(d = first_descriptor; d; d = d->next)
      if(d->connected == CON_PLAYING && d->character && get_trust(d->character) < LEVEL_IMMORTAL && nifty_is_name(d->character->name, cvar))
        return TRUE;
    return FALSE;
  }
  ...
  { /* -- Gorog */
    ...
    for(ch = first_char; ch; ch = ch->next)
      if((!IS_NPC(ch)) && ch->in_room->area == mob->in_room->area && get_trust(ch) < LEVEL_IMMORTAL && nifty_is_name(ch->name, cvar))
        return TRUE;
    return FALSE;
  }
   ...
   */
  progbug(mob, "Unknown ifcheck [%s] [name: %s]", chck, mob->name);
  return BERR;
}
...
/* Fix crash bug :)  SHADDAI */
  ...
  *t = '\0';
  switch (ch)
  {
    ...
    default:
      progbug(mob, "Bad $var [%s]", mob->name);
      break;
  }
...
void mprog_driver(char *com_list, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, bool single_step)
{
  char                    tmpcmndlst[MSL], tmpcmnd[MSL];
  char                   *command_list;
  char                   *cmnd;
  CHAR_DATA              *rndm = NULL;
  CHAR_DATA              *vch = NULL;
  int                     count = 0;
  int                     count2 = 0, rand_pick = 0;
  int                     ignorelevel = 0;
  int                     iflevel, result;
  bool                    ifstate[MAX_IFS][DO_ELSE + 1];
  static int              prog_nest;
  MPPAUSE_DATA           *mppause = NULL;
  char                    arg[MIL];
  ...
  if(mob == actor)
  {
    progbug(mob, "triggering oneself. [%s]", mob->name);
    return;
  }
  if(++prog_nest > MAX_PROG_NEST)
  {
    progbug(mob, "max_prog_nest exceeded. [%s] [%d]: prog_nest: %d", mob->name, mob->pIndexData->vnum, prog_nest);
    --prog_nest;
    return;
  }
  /*
  ...
  /*
   * mppause - Restore the environment -rkb 
   */
  if(current_mppause)
  {
    ignorelevel = current_mppause->ignorelevel;
    iflevel = current_mppause->iflevel;
    if(single_step)
      mob->mpscriptpos = 0;
    for(count = 0; count < MAX_IFS; count++)
    {
      for(count2 = 0; count2 <= DO_ELSE; count2++)
        ifstate[count][count2] = current_mppause->ifstate[count][count2];
    }
    current_mppause = NULL;
  }
  if(single_step)
  {
    if((unsigned int)(mob->mpscriptpos) > strlen(tmpcmndlst))
      mob->mpscriptpos = 0;
    else
...
//    command_list = mprog_next_command(command_list);
    ...
    command_list = mprog_one_command(command_list, tmpcmnd);
    cmnd = tmpcmnd;
    ...
    if(cmnd[0] == '\0')
    {
      if(ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE])
        progbug(mob, "Missing endif [%s]", mob->name);
      --prog_nest;
      return;
    }
    /*
     * mppause - Check if we should pause -rkb 
     */
    if(!str_prefix("mppause", cmnd))
    {
      /*
       * If there is no command_list why even add it? 
       */
      if(!VLD_STR(command_list))
      {
        --prog_nest;
        return;
      }
      if((ifstate[iflevel][IN_IF] == TRUE && ifstate[iflevel][DO_IF] == FALSE) || (ifstate[iflevel][IN_ELSE] == TRUE && ifstate[iflevel][DO_ELSE] == FALSE))
      {
        continue;
      }
      else
      {
        ...
        CREATE(mppause, MPPAUSE_DATA, 1);
        /*
         * State variables 
         */
        mppause->ignorelevel = ignorelevel;
        mppause->iflevel = iflevel;
        for(count = 0; count < MAX_IFS; count++)
        {
          for(count2 = 0; count2 <= DO_ELSE; count2++)
            mppause->ifstate[count][count2] = ifstate[count][count2];
        }
        /*
         * Driver arguments 
         */
        mppause->com_list = STRALLOC(command_list);
        mppause->mob = mob;
        mppause->actor = actor;
        mppause->obj = obj;
        mppause->vo = vo;
        mppause->single_step = single_step;
        /*
         * Time to pause 
         */
        ...
        if(arg[0] == '\0')
          ...
          mppause->timer = atoi(arg);
        if(mppause->timer < 1)
        {
          progbug(mob, "mppause - bad arg, using default [%s]", mob->name);
          mppause->timer = 4;
        }
//VOLK: Added support for faster/slower mppauses!
        if(actor && !IS_NPC(actor) && actor->pcdata && actor->pcdata->textspeed)
        {
          int                     speed = actor->pcdata->textspeed;
          float                   timer = mppause->timer;

// Now, we need a formula to make sure timers aren't TOO fast or TOO slow.
          ...
          else
          {
            ...
            // long double ceill(long double timer);
          }

          if(speed < 5 || speed > 5)
            mppause->timer = (int)timer;
        }

        /*
         * Save type of prog, room, object or mob 
         */
        if(mppause->mob && mppause->mob->pIndexData && mppause->mob->pIndexData->vnum == 3)
        {
          if(!str_prefix("Room", mppause->mob->description))
          {
            mppause->type = MP_ROOM;
            mppause->room = mppause->mob->in_room;
          }
          else if(!str_prefix("Object", mppause->mob->description))
            mppause->type = MP_OBJ;
        }
        else
          mppause->type = MP_MOB;
        LINK(mppause, first_mppause, last_mppause, next, prev);
        --prog_nest;
        return;
      }
    }
     ...
     */
    result = mprog_do_command(cmnd, mob, actor, obj, vo, rndm, (ifstate[iflevel][IN_IF] && !ifstate[iflevel][DO_IF]) || (ifstate[iflevel][IN_ELSE] && !ifstate[iflevel][DO_ELSE]), (ignorelevel > 0));
    /*
     ...
     */
    // Changed some of the below progbug msg to give us a better tracking 
    // information. -Taon
    switch (result)
    {
      case COMMANDOK:
      ...
      case IFTRUE:
        ...
        if(iflevel == MAX_IFS)
        {
          progbug(mob, "Maximum nested ifs exceeded [name: %s] [vnum: %d]", mob->name, mob->pIndexData->vnum);
          --prog_nest;
          return;
        }
      ...
      case ORTRUE:
        ...
        {
          progbug(mob, "Unmatched or [name: %s] [vnum: %d]", mob->name, mob->pIndexData->vnum);
          --prog_nest;
          return;
        }
        ifstate[iflevel][DO_IF] = TRUE;
      ...
      case FOUNDELSE:
        ...
        {
          progbug(mob, "Found else in an else section [name: %s] [vnum: %d]", mob->name, mob->pIndexData->vnum);
          --prog_nest;
          return;
        }
        if(!ifstate[iflevel][IN_IF])
        {
          progbug(mob, "Unmatched else [name: %s] [vnum: %d]", mob->name, mob->pIndexData->vnum);
          --prog_nest;
          return;
        }
      ...
      case FOUNDENDIF:
        ...
        if(!(ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE]))
        {
          progbug(mob, "Unmatched endif [name:%s] [vnum: %d]", mob->name, mob->pIndexData->vnum);
          --prog_nest;
          return;
        }
        if(ignorelevel > 0)
      ...
      case IFIGNORED:
        ...
        if(!(ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE]))
        {
          progbug(mob, "Parse error, ignoring if while not in if or else [name: %s] [vnum: %d]", mob->name, mob->pIndexData->vnum);
          --prog_nest;
          return;
        }
        ignorelevel++;
      ...
      case ORIGNORED:
        ...
        if(ignorelevel == 0)
        {
          progbug(mob, "Parse error, mistakenly ignoring or [name: %s] [vnum: %d]", mob->name, mob->pIndexData->vnum);
          --prog_nest;
          return;
        }
        break;
...
int mprog_do_command(char *cmnd, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo, CHAR_DATA *rndm, bool ignore, bool ignore_ors)
{
  ...
  /*
   * Do nothing but allow it to ignore these 
   */
  ...
  /*
   * If the command is 'abort', we not only bail out, we make sure it knows to cancel
   * anything else. 
   */
...
/* Just a fast way to see if the mobile is paused or not currently */
bool is_mppaused(CHAR_DATA *mob)
{
  MPPAUSE_DATA           *mppause;

  for(mppause = first_mppause; mppause; mppause = mppause->next)
  {
    if(mppause->mob && mppause->mob == mob)
      return TRUE;
  }
  return FALSE;
}
...
{
  MPPAUSE_DATA           *mppause;
  MPPAUSE_DATA           *tmpMppause;
  bool                    delete_it;

  mppause = first_mppause;
  while(mppause)
  {
    delete_it = FALSE;

    if(mppause->mob)
      delete_it = char_died(mppause->mob);

    if(mppause->actor && !delete_it)
      delete_it = char_died(mppause->actor);

    if(mppause->obj && !delete_it)
      delete_it = obj_extracted(mppause->obj);

    if(delete_it)
    {
      /*
       * log_string("mppause_update - Deleting expired prog."); 
       */

      tmpMppause = mppause;
      mppause = mppause->next;
      STRFREE(tmpMppause->com_list);
      UNLINK(tmpMppause, first_mppause, last_mppause, next, prev);
      DISPOSE(tmpMppause);
  ...
  mppause = first_mppause;
  while(mppause)
  { /* Find progs to continue */
    if(--mppause->timer <= 0)
    {
      current_mppause = mppause;

      if(mppause->type == MP_ROOM)
        rset_supermob(mppause->room);
      else if(mppause->type == MP_OBJ)
        set_supermob(mppause->obj);

      mprog_driver(mppause->com_list, mppause->mob, mppause->actor, mppause->obj, mppause->vo, mppause->single_step);
...
/*
rprog_command_trigger( CHAR_DATA * ch, char *txt )
{
...
mprog_command_trigger( CHAR_DATA * actor, char *txt )
{
   CHAR_DATA *vmob;
...
oprog_command_trigger( CHAR_DATA * ch, char *txt )
{
   OBJ_DATA *vobj;
...
void mob_act_add(CHAR_DATA *mob)
{
  ...
  for(runner = mob_act_list; runner != NULL; runner = runner->next)
  {
    if(runner->vo == mob)
      return;
  }
...
void mprog_bribe_trigger(CHAR_DATA *mob, CHAR_DATA *ch, int amount)
{
  ...
  if(IS_NPC(mob) && can_see(mob, ch) && HAS_PROG(mob->pIndexData, BRIBE_PROG))
  {
    ...
    obj = create_money(amount, DEFAULT_CURR);
...
bool rprog_pre_enter_trigger(CHAR_DATA *ch, ROOM_INDEX_DATA *room)
{
  ...
  return GlobalAbort; // If it was told to abort, this
  // will now equal true
}
...
void mprog_give_trigger(CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj)
{
  ...
  {
    /*
     * Don't let a mob trigger itself, nor one instance of a mob trigger another
     * instance. 
     */
    if(IS_NPC(ch) && ch->pIndexData == mob->pIndexData)
      return;
    ...
    for(mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next)
    {
      ...
      if(mprg->type == GIVE_PROG && (!str_cmp(obj->name, mprg->arglist) || !str_cmp(obj->name, buf) || !str_cmp("all", buf) || (vnum != -1 && obj->pIndexData->vnum == vnum)))
      {
        mprog_driver(mprg->comlist, mob, ch, obj, NULL, FALSE);
        break;
      }
...
//Reminder: Taon, fix me I don't check for player inventoried objects. -Taon
//Volk: Done mate, no worries at all. :)
void oprog_speech_trigger(char *txt, CHAR_DATA *ch)
{
  OBJ_DATA               *vobj, *obj_next;
  CHAR_DATA              *victim;
  ...
  for(victim = ch->in_room->first_person; victim; victim = victim->next)
    for(vobj = victim->first_carrying; vobj; vobj = obj_next)
    {
...
{
  if(HAS_PROG(weapon->pIndexData, FIGHT_PROG))
  {
    set_supermob(weapon);
    oprog_percent_check(supermob, ch, weapon, vch, FIGHT_PROG);
    release_supermob();
  }
  return;
}
...
bool oprog_use_trigger(CHAR_DATA *ch, OBJ_DATA *obj, CHAR_DATA *vict, OBJ_DATA *targ, void *vo)
{
  ...
  {
    set_supermob(obj);
    if(obj->item_type == ITEM_STAFF || obj->item_type == ITEM_SCROLL)
    {
      if(vict)
...
/*
 * call in do_trash, right before extract_obj
 */
void oprog_trash_trigger(CHAR_DATA *ch, OBJ_DATA *obj)
{
  if(HAS_PROG(obj->pIndexData, TRASH_PROG))
  {
    set_supermob(obj);
    oprog_percent_check(supermob, ch, obj, NULL, TRASH_PROG);
    release_supermob();
  }
}
...
/*
    if(tmp_act->buf == NULL)
    {
     log_printf("%s trigged a null act prog.\r\n", ch->name);
     return;
    }
*/
