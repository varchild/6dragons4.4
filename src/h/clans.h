...
struct clan_data
{
  ...
  char                   *intro;  /* Clan introduction (seen when logging on) */
  int                     pkills[11]; /* Number of pkills on behalf of clan */
  int                     pdeaths[11];  /* Number of pkills against clan */
  ...
  short                   arena_victory;  /* Total arena victories by all members. */
...
#define IS_COUNCILED(ch) ( !IS_NPC( (ch) ) && (ch)->pcdata->council )
