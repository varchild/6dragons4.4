...
void get_obj(CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container)
{
  ...
  if(xIS_SET(obj->extra_flags, ITEM_PKDISARMED) && !IS_NPC(ch))
  {
    if(CAN_PKILL(ch) && !get_timer(ch, TIMER_PKILLED))
    {
      ...
      {
        send_to_char_color("\r\n&bThe Staff of 6D freeze your outstretched hand.\r\n", ch);
        return;
      }
      else
      {
  ...
  {
    send_to_char("The Staff of 6D prevent you from getting close to it.\r\n", ch);
    return;
  }
  ...
  if(IS_OBJ_STAT(obj, ITEM_COVERING))
    ...
    weight = get_obj_weight(obj, FALSE);  /* Allow them to pick up portals * with alot of 
                                           * weight */
  ...
  else
  {
    ...
    {
      act(AT_RED, "$p refuses to be utilized by a different owner, and burns your hand.", ch, obj, NULL, TO_CHAR);
      separate_obj(obj);
      obj_from_char(obj);
...
void do_get(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char("You think to yourself this stuff is too much and drop it to the ground to reorganize your gear.\r\n", ch);
  ...
  if(xIS_SET(ch->act, PLR_ENHANCED) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_ENHANCED)))
    ch_printf(ch, "!!SOUND(sound/get.wav)\r\n");
  ...
  else
  {
    ...
    switch (container->item_type)
    {
      ...
      case ITEM_CORPSE_PC:
      {
         ...
         * * corpses --Shaddai 
         */
        if(IS_OBJ_STAT(container, ITEM_CLANCORPSE) && !IS_NPC(ch) && !IS_IMMORTAL(ch) && VLD_STR(container->action_desc) && str_cmp(name, ch->name) && str_cmp(container->action_desc, ch->name))
        {
        ...
        {
          send_to_char("Frequent looting has left this corpse protected from further loot.\r\n", ch);
          return;
        }
        if(IS_OBJ_STAT(container, ITEM_CLANCORPSE) && !IS_NPC(ch) && IS_SET(ch->pcdata->flags, PCFLAG_DEADLY) && container->value[4] - ch->level < 6 && container->value[4] - ch->level > -6)
          break;
    ...
    else
    {
      ...
      {
        send_to_char("The staff of 6D frown upon such an act of greed!\r\n", ch);
        return;
      }
      if(IS_OBJ_STAT(container, ITEM_CLANCORPSE) && !IS_IMMORTAL(ch) && !IS_NPC(ch) && str_cmp(ch->name, container->name + 7))
      {
        send_to_char("The staff of 6D frown upon such wanton greed!\r\n", ch);
        return;
      }
      if(!str_cmp(arg1, "all"))
        fAll = TRUE;
...
void do_put(CHAR_DATA *ch, char *argument)
{
  ...
  if((!str_cmp(arg2, "into") || !str_cmp(arg2, "inside") || !str_cmp(arg2, "in") || !str_cmp(arg2, "under") || !str_cmp(arg2, "onto") || !str_cmp(arg2, "on")) && argument[0] != '\0')
    argument = one_argument(argument, arg2);
  ...
  {
    if(container->item_type != ITEM_CONTAINER && container->item_type != ITEM_KEYRING && container->item_type != ITEM_QUIVER && container->item_type != ITEM_CORPSE_NPC)
    {
      send_to_char("That's not a container.\r\n", ch);
      return;
    }
  ...
  if(number <= 1 && str_cmp(arg1, "all") && str_prefix("all.", arg1))
  {
    ...
    if((IS_OBJ_STAT(container, ITEM_COVERING) && (get_obj_weight(obj, FALSE) / obj->count) > ((get_obj_weight(container, FALSE) / container->count) - container->weight)))
    {
    ...
    {
      act(AT_CYAN, "You cannot put trash into a clan storage container.", ch, NULL, NULL, TO_CHAR);
      return;
    }
     ...
     */
    if(((get_real_obj_weight(obj) / obj->count) + (get_real_obj_weight(container) / container->count) > container->value[0]) && container->item_type != ITEM_CORPSE_NPC)
    {
      send_to_char("It won't fit.\r\n", ch);
      return;
    }

    if(container->item_type == ITEM_CORPSE_NPC && container->value[5] == 1)
    { /* Skeleton */
      send_to_char("That's not a container.\r\n", ch);
      return;
    }

    if((container->item_type == ITEM_CORPSE_NPC) && (obj->weight > container->value[2]) && !IS_IMMORTAL(ch))
    {
      send_to_char("It won't fit.\r\n", ch);
      return;
    }
  ...
  else
  {
    ...
    for(obj = ch->first_carrying; obj; obj = obj_next)
    {
      ...
      if((fAll || nifty_is_name(chk, obj->name))
         ...
         && obj != container
         && can_drop_obj(ch, obj) && (container->item_type != ITEM_KEYRING || obj->item_type == ITEM_KEY) && (container->item_type != ITEM_QUIVER || obj->item_type == ITEM_PROJECTILE))
      {
        if((get_obj_weight(obj, TRUE) + get_obj_weight(container, TRUE)) > container->value[0])
        {
          full = TRUE;
          continue;
        }
    ...
    if(!found)
    {
      ...
      {
        if(fAll)
          act(AT_PLAIN, "$p is too full to hold what you're carrying.", ch, container, NULL, TO_CHAR);
        else
...
void do_drop(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_NPC(ch) && xIS_SET(ch->act, PLR_LITTERBUG))
  {
    ...
    send_to_char("The enviromental police prevent you from dropping anything here...\r\n", ch);
    return;
  }
  ...
  if(number > 0)
  {
    ...
    if((type = get_currency_type(arg)))
    {
      ...
      if(number < 100)
      {
        ...
        act_printf(AT_ACTION, ch, NULL, NULL, TO_CHAR, "You let a few %s coins slip from your hand.", curr_types[type]);
      }
      else if(number < 500)
      {
        act(AT_ACTION, "$n drops a lot of $T coins.", ch, NULL, curr_types[type], TO_ROOM);
        act_printf(AT_ACTION, ch, NULL, NULL, TO_CHAR, "You drop a lot of %s coins.", curr_types[type]);
      }
      else if(number < 2000)
      {
        act(AT_ACTION, "$n drops a pile of $T coins.", ch, NULL, curr_types[type], TO_ROOM);
        act_printf(AT_ACTION, ch, NULL, NULL, TO_CHAR, "You drop a pile of %s coins.", curr_types[type]);
      }
      else if(number < 10000)
      {
        act(AT_ACTION, "$n drops a heap of $T coins.", ch, NULL, curr_types[type], TO_ROOM);
        act_printf(AT_ACTION, ch, NULL, NULL, TO_CHAR, "You drop a heap of %s coins.", curr_types[type]);
      }
      else if(number < 100000)
      {
        act(AT_ACTION, "$n drops a massive heap of $T coins!", ch, NULL, curr_types[type], TO_ROOM);
        act_printf(AT_ACTION, ch, NULL, NULL, TO_CHAR, "You drop a massive heap of %s coins!", curr_types[type]);
      }
      else if(number < 1000000)
      {
        act(AT_ACTION, "$n drops a HUGELY MASSIVE pile of $T coins!", ch, NULL, curr_types[type], TO_ROOM);
        act_printf(AT_ACTION, ch, NULL, NULL, TO_CHAR, "You drop a HUGELY MASSIVE pile of %s coins!", curr_types[type]);
      }
      else
      {
        act(AT_ACTION, "$n pulls out an extradimensional pocketbank(tm), and pours out over a million $T coins!!", ch, NULL, curr_types[type], TO_ROOM);
        act_printf(AT_ACTION, ch, NULL, NULL, TO_CHAR, "You pull out an extradimensional pocketbank(tm), and pour out over a million %s coins!!", curr_types[type]);
      }
  ...
  if(number <= 1 && str_cmp(arg, "all") && str_prefix("all.", arg))
  {
    ...
    if(!IS_NPC(ch))
    {
      if(obj->pIndexData->vnum == 41002 && obj->item_type == ITEM_SABOTAGE)
      {
        ...
        if(foundsiege == TRUE)
        {
          send_to_char("You cannot harden city defenses during a active siege.\r\n", ch);
          return;
        }
        ...
        {
          send_to_char("You need to be in your homeland city to harden the defenses.\r\n", ch);
          return;
        }
        ...
        if(city)
        {
          if(city->hardened < 10)
          {
            city->hardened += 1;
            save_city(city);
            send_to_char("Your crafted defense adds to the city defenses!\r\n", ch);
            separate_obj(obj);
            obj_from_char(obj);
            extract_obj(obj);
            return;
          ...
          if(city->hardened > 9)
          {
            send_to_char("This city defenses are already hardened as far as they can be.\r\n", ch);
            return;
          }
    ...
    {
      snprintf(buf, MSL, "%s dropping %s clan object! make sure he doesn't allow another to pick up.", ch->name, obj->short_descr);
      log_string(buf);
    }

    if(obj->owner && obj->pIndexData->vnum == 41032)
    {
      CHAR_DATA              *prisoner;

      prisoner = get_char_world(ch, obj->owner);
      prisoner->regoto = ch->in_room->vnum;
    }
    ...
    if(xIS_SET(ch->act, PLR_ENHANCED) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_ENHANCED)))
      send_to_char("!!SOUND(sound/drop.wav)\r\n", ch);
...
void do_give(CHAR_DATA *ch, char *argument)
{
  ...
  if((type = get_currency_type(arg1)) && (type == CURR_GOLD || type == CURR_SILVER || type == CURR_BRONZE || type == CURR_COPPER))
  {
    if(number <= 0)
    {
      send_to_char("You can't do that.\r\n", ch);
    ...
    act_printf(AT_ACTION, ch, NULL, victim, TO_VICT, "$n gives you %d %s coin%s.", number, curr_types[type], number == 1 ? "" : "s");
    act_printf(AT_ACTION, ch, NULL, victim, TO_NOTVICT, "$n gives $N some %s coins.", curr_types[type]);
    act_printf(AT_ACTION, ch, NULL, victim, TO_CHAR, "You give $N %d %s coin%s.", number, curr_types[type], number == 1 ? "" : "s");
  ...
  if(!in_arena(victim))
  {
    if(victim->carry_number + (get_obj_number(obj) / obj->count) > can_carry_n(victim))
    {
      ...
      if(IS_NPC(ch))
      {
        act(AT_SAY, "$n says, 'I see you have your hands full, I'll just set this on the ground.'", ch, NULL, victim, TO_ROOM);
        obj_from_char(obj);
        obj_to_room(obj, victim->in_room);
      }
      return;
    }
    if(victim->carry_weight + (get_obj_weight(obj, FALSE) / obj->count) > can_carry_w(victim))
    {
  ...
  if(!IS_NPC(ch) && IS_OBJ_STAT(obj, ITEM_CLANOBJECT))
  {
    snprintf(buf, MSL, "%s attempting to give %s %s clan object!", ch->name, victim->name, obj->short_descr);
    log_string(buf);
  ...
  if(xIS_SET(victim->act, ACT_PET) && obj->item_type == ITEM_FOOD && victim->master)
  {
    if(victim->master->pcdata->pethungry >= 1000)
    {
      act(AT_ACTION, "$n is full.", victim, NULL, NULL, TO_ROOM);
      obj_from_char(obj);
      obj_to_char(obj, ch);
      return;
    }

    if(victim->master->pcdata->petaffection < 49)
    {
      victim->master->pcdata->petaffection += 1;
    }
    act(AT_ACTION, "$n eats $p.", victim, obj, NULL, TO_ROOM);
    obj_from_char(obj);
    victim->master->pcdata->pethungry += 100;
  }
  if(xIS_SET(victim->act, ACT_PET) && obj->item_type == ITEM_DRINK_CON && victim->master)
  {
    if(victim->master->pcdata->petthirsty >= 1000)
    {
      act(AT_ACTION, "$n is full.", victim, NULL, NULL, TO_ROOM);
      obj_from_char(obj);
      obj_to_char(obj, ch);
    ...
    act(AT_ACTION, "$n drinks up $p.", victim, obj, NULL, TO_ROOM);
    obj_from_char(obj);
    victim->master->pcdata->petthirsty += 100;
  }

  if(obj->owner && str_cmp(victim->name, obj->owner) && obj->item_type == ITEM_TRASH)
  {
    act(AT_RED, "$p refuses to be utilized by a different owner, and burns your hand.", victim, obj, NULL, TO_CHAR);
    obj_from_char(obj);
    obj_to_room(obj, victim->in_room);
...
bool remove_obj(CHAR_DATA *ch, int iWear, bool fReplace)
{
  ...
  if(IS_AFFECTED(ch, AFF_GRAFT) && obj == get_eq_char(ch, WEAR_DUAL_WIELD))
  {
    send_to_char("You can't remove that it is grafted to your hand!\r\n", ch);
    return FALSE;
  }
...
void wear_obj(CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, short wear_bit)
{
  ...
  if(obj->glory > 1)
  { /* Wearing an item with glory, * let's check how many items they * * * * * * * * * *
     * can have.. */
    ...
    if((ch->level < LEVEL_AVATAR && count > 0) || count > 1)
    { /* avs can wear * TWO glory * * pieces */
      send_to_char("&RYou are already wearing glory modified equipment! Remove some first.\r\n", ch);
      WAIT_STATE(ch, 2);
      return;
    }
  }
...
/* Volk - moved size here */
    ...
    bool                    toobig = FALSE, toosmall = FALSE, big = FALSE, small = FALSE;
...
(ie $n tries to wear whatever but it is too big/too small/just squeezes into it/drowning in */
      ...
      if(big)
      {
        switch (obj->item_type)
        {
          ...
          case ITEM_WEAPON:
            send_to_char("It's a little big to wield, but you manage to hold it correctly.\r\n", ch);
            break;
      ...
      if(small)
      {
        switch (obj->item_type)
        {
          ...
          case ITEM_WEAPON:
            send_to_char("It's a little small to wield, but you manage to hold it correctly.\r\n", ch);
            break;
  ...
  if(xIS_SET(ch->act, PLR_ENHANCED) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_ENHANCED)))
    ch_printf(ch, "!!SOUND(sound/equip.wav)\r\n");
...
/*
  ...
  switch (1 << bit)
  {
    ...
    case ITEM_WEAR_FINGER:
      if(get_eq_char(ch, WEAR_FINGER_L) && get_eq_char(ch, WEAR_FINGER_R) && !remove_obj(ch, WEAR_FINGER_L, fReplace) && !remove_obj(ch, WEAR_FINGER_R, fReplace))
        return;
      ...
      if(!get_eq_char(ch, WEAR_FINGER_L))
      {
        if(!oprog_use_trigger(ch, obj, NULL, NULL, NULL))
        {
          if((ch)->race == RACE_DRAGON && !IS_NPC(ch) && !IS_AFFECTED(ch, AFF_DRAGONLORD))
          {
          ...
          {
            send_to_char("Animals cannot wear that.\r\n", ch);
            return;
          }
    ...
    case ITEM_WEAR_FACE:
      ...
      {
        send_to_char("It won't fit over what your wearing.\r\n", ch);
        return;
      }
      if(obj->pIndexData->layers > 0)
      {
        ...
        {
          send_to_char("Only Massive Dragons can wear that.\r\n", ch);
          return;
        }
      }
      if(ch->race != RACE_DRAGON)
    ...
    case ITEM_WEAR_L_WING:
      ...
      {
        send_to_char("Only Dragons may wear wing gear.\r\n", ch);
        return;
      }
      ...
      if(!oprog_use_trigger(ch, obj, NULL, NULL, NULL))
      {
        act(AT_ACTION, "$n attaches $p on $s left wing.", ch, obj, NULL, TO_ROOM);
        act(AT_ACTION, "You attach $p on your left wing.", ch, obj, NULL, TO_CHAR);
      }
    ...
    case ITEM_WEAR_R_WING:
      ...
      if(!oprog_use_trigger(ch, obj, NULL, NULL, NULL))
      {
        act(AT_ACTION, "$n attaches $p on $s right wing.", ch, obj, NULL, TO_ROOM);
        act(AT_ACTION, "You attach $p on your right wing.", ch, obj, NULL, TO_CHAR);
      }
    ...
    case ITEM_WEAR_WRIST:
      if(get_eq_char(ch, WEAR_WRIST_L) && get_eq_char(ch, WEAR_WRIST_R) && !remove_obj(ch, WEAR_WRIST_L, fReplace) && !remove_obj(ch, WEAR_WRIST_R, fReplace))
        return;
    ...
    case ITEM_WEAR_ANKLE:
      if(get_eq_char(ch, WEAR_ANKLE_L) && get_eq_char(ch, WEAR_ANKLE_R) && !remove_obj(ch, WEAR_ANKLE_L, fReplace) && !remove_obj(ch, WEAR_ANKLE_R, fReplace))
        return;
    ...
    case ITEM_WEAR_SHIELD:
      if(get_eq_char(ch, WEAR_DUAL_WIELD)
         || (get_eq_char(ch, WEAR_WIELD) && get_eq_char(ch, WEAR_MISSILE_WIELD)) || ((get_eq_char(ch, WEAR_WIELD) || get_eq_char(ch, WEAR_MISSILE_WIELD)) && get_eq_char(ch, WEAR_HOLD)))
      {
    ...
    case ITEM_WIELD:
      ...
      else
      {
        ...
        if(tmpobj)
        {
          ...
          if(get_obj_weight(obj, FALSE) + get_obj_weight(tmpobj, FALSE) > str_app[get_curr_str(ch)].wield)
          {
          ...
          if(IS_TWOHANDS(obj))
          {
            if(tmpobj)
            {
              send_to_char("You can't use a two handed weapon AND another weapon!\r\n", ch);
              return;
            }
            if(mw)
            {
              send_to_char("You can't use a two handed weapon AND a missile weapon!\r\n", ch);
              return;
            }
            if(sd)
            {
        ...
        if(mw)
        {
          ...
          if(get_obj_weight(obj, FALSE) + get_obj_weight(mw, FALSE) > str_app[get_curr_str(ch)].wield)
          {
            send_to_char("It is too heavy for you to wield.\r\n", ch);
            return;
          }
    ...
    case ITEM_HOLD:
      ...
      if((get_eq_char(ch, WEAR_WIELD) || get_eq_char(ch, WEAR_MISSILE_WIELD)) && get_eq_char(ch, WEAR_SHIELD))
      {
      ...
      if(obj->item_type == ITEM_WAND
         ...
         || obj->item_type == ITEM_BLOOD || obj->item_type == ITEM_PIPE || obj->item_type == ITEM_HERB || obj->item_type == ITEM_KEY || !oprog_use_trigger(ch, obj, NULL, NULL, NULL))
      {
...
void do_dye(CHAR_DATA *ch, char *argument)
{
  ...
  if(number_percent() < chance)
  {
    ...
    oldshort = STRALLOC(myobj(obj));
    if(obj->short_descr)
      STRFREE(obj->short_descr);
    sprintf(newshort, "a %s %s", adj, oldshort);
    obj->short_descr = STRALLOC(newshort);
    if(xIS_SET(ch->act, PLR_BATTLE) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_BATTLE)))
      send_to_char("!!SOUND(sound/dye.wav)\r\n", ch);
...
void do_resize(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg1[0] == '\0')
  {
    act(AT_TELL, "$n tells you 'Resizing is expensive, the more size difference the higher the cost.'", mob, NULL, ch, TO_VICT);
  ...
  if(arg1[0] != '\0' && arg2[0] == '\0')
  {
    act(AT_TELL, "$n tells you 'Did you need it smaller, bigger, or attempt to enchant it to size magical?'", mob, NULL, ch, TO_VICT);
    send_to_char("\r\n&cType Resize <object> <choice>.\r\n", ch);
    return;
  }
  ...
  if(obj->size == 0)
  {
    act(AT_TELL, "$n tells you 'That object is magical, it adjusts its size for anyone to wear.'", mob, NULL, ch, TO_VICT);
    return;
  }
  ...
  if(abs(obj->size - obj->pIndexData->size) > 3 && str_cmp(arg2, "magical"))
  {
    ...
    act(AT_TELL, "\r\n$n tells you 'You cannot resize $p further it would be worthless.'", mob, obj, ch, TO_VICT);
  ...
  if(!str_cmp(arg2, "smaller"))
  {
    if(obj->size == 1)
    {
      act(AT_TELL, "$n tells you 'That object is as small as it gets.'", mob, NULL, ch, TO_VICT);
      return;
    }
    act(AT_TELL, "$n tells you 'Very well.'", mob, NULL, ch, TO_VICT);
    act(AT_GREEN, "\r\n$n takes $N's $p to the table, and begins working on it.", mob, obj, ch, TO_ROOM);
    obj->size = obj->size - 1;
    if(abs(obj->size - obj->pIndexData->size) == 3)
    {
      if(obj->value[0] > 1)
        obj->value[0] = obj->value[0] / 2;
      act(AT_TELL, "$n tells you '$p isn't as great as it was because of the third resize.'", mob, obj, ch, TO_VICT);
    }
    act(AT_CYAN, "\r\n$n smiles as $e accepts the gold and hands $N's the newly resized $p back.", mob, obj, ch, TO_ROOM);
  ...
  if(!str_cmp(arg2, "bigger"))
  {
    if(obj->size == 7)
    {
      act(AT_TELL, "$n tells you 'That object is as big as it gets.'", mob, NULL, ch, TO_VICT);
      return;
    }
    act(AT_TELL, "$n tells you 'Very well.'", mob, NULL, ch, TO_VICT);
    act(AT_GREEN, "\r\n$n takes $N's $p to the table gets some extra material, and begins working on it.", mob, obj, ch, TO_ROOM);
    obj->size = obj->size + 1;
    if(abs(obj->size - obj->pIndexData->size) == 3)
    {
      if(obj->value[0] > 1)
  ...
  {
    act(AT_TELL, "$n tells you 'Very well.'", mob, NULL, ch, TO_VICT);
    act(AT_GREEN, "\r\n$n takes $N's $p to the table, and begins saying arcane words.", mob, obj, ch, TO_ROOM);
    obj->size = 0;
    act(AT_CYAN, "\r\n$n smiles as $e accepts the gold and hands $N's the newly resized $p back.", mob, obj, ch, TO_ROOM);
...
{
  char                    arg1[MIL];
  OBJ_DATA               *obj, *sobj, *wield, *dwield, *mwield;
  int                     count = 0;
  ...
  if(ch->race == RACE_DRAGON)
  {
    send_to_char("What use has a dragon for a sheath?\r\n", ch);
    return;
  }
  ...
  while(sobj = obj->first_content)
  {
    ...
    {
      act(AT_ACTION, "You start to unsheath your weapon but realize you don't have a free hand.", ch, NULL, NULL, TO_CHAR);
      return;
    }
...
{
  char                    arg1[MIL];
  OBJ_DATA               *obj = NULL, *obj2 = NULL, *sobj;
  ...
  if(obj)
  {
    ...
    if(obj == get_eq_char(ch, WEAR_WIELD) || obj == get_eq_char(ch, WEAR_DUAL_WIELD))
    {
      unequip_char(ch, obj);
    }
    obj_from_char(obj);
  ...
  if(obj2 != obj)
  {
    ...
    if(obj2 == get_eq_char(ch, WEAR_WIELD) || obj2 == get_eq_char(ch, WEAR_DUAL_WIELD))
    {
    ...
    for(int i = 1; i < 129; i++)  // i = layer number
    {
      for(obj = ch->first_carrying; obj != NULL; obj = obj_next)
      {
        ...
        if(obj->pIndexData->layers < 1)
          continue;
        if(obj->wear_loc == WEAR_NONE && can_see_obj(ch, obj) && obj->pIndexData->layers == i)
        {
...
void do_trash(CHAR_DATA *ch, char *argument)
{
  ...
  if(xIS_SET(obj->extra_flags, ITEM_PKDISARMED) && (!IS_NPC(ch) && !xIS_SET(ch->act, ACT_BEASTMELD)))
  {
    if(CAN_PKILL(ch) && !get_timer(ch, TIMER_PKILLED))
    {
      if(ch->level - obj->value[5] > 5 || obj->value[5] - ch->level > 5)
  ...
  if(xIS_SET(ch->act, PLR_ENHANCED) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_ENHANCED)))
    ch_printf(ch, "!!SOUND(sound/trash.wav)\r\n");

  if(xIS_SET(ch->act, ACT_BEASTMELD))
    act(AT_ACTION, "You toss aside $p as trash.", ch, obj, NULL, TO_CHAR);

  if(!IS_NPC(ch) && !xIS_SET(ch->act, ACT_BEASTMELD))
  {
    if(obj->currtype && obj->currtype < MAX_CURR_TYPE && obj->cost > 0 && obj->item_type != ITEM_CONTAINER)
    {
      if(ch->level < 20)
      {
        GET_MONEY(ch, CURR_COPPER) += 1;
        ch_printf(ch, "The Staff of 6D give you one copper coin for your enviromental contribution.\r\n");
      }
      if(ch->level > 19)
      {
    ...
    { /* Corpses should at least give 1 copper */
      if(ch->level < 20)
      {
        GET_MONEY(ch, CURR_COPPER) += 1;
        ch_printf(ch, "The Staff of 6D give you one copper coin for your enviromental contribution.\r\n");
...
void do_brandish(CHAR_DATA *ch, char *argument)
{
  ...
  WAIT_STATE(ch, 2 * (int) PULSE_VIOLENCE);
...
void do_zap(CHAR_DATA *ch, char *argument)
{
  ...
  WAIT_STATE(ch, 1 * (int) PULSE_VIOLENCE);
...
void do_rolldie(CHAR_DATA *ch, char *argument)
{
  ...
  while(roll_count++ < numrolls)
  {
    ...
    if(die->value[1] == 1)
    {
      ...
      if(face_name)
      {
        char                   *face_name_copy = strdup(face_name); /* Since I want * * * 
                                                                     * * * * * to *
                                                                     * tekenize * * * *
                                                                     * * * * * without *
                                                                     * * * * modifying * 
...
void make_scraps(OBJ_DATA *obj)
{
  ...
  if((obj->item_type == ITEM_CONTAINER || obj->item_type == ITEM_KEYRING || obj->item_type == ITEM_QUIVER || obj->item_type == ITEM_CORPSE_PC || obj->item_type == ITEM_SHEATH) && obj->first_content)
  {
    if(ch && ch->in_room)
    {
...
// Value1 numdice  Value2 sizedice

  damdice = obj->value[2];

// Safety check to make sure it set appropriately before we add or take away from it.

  obj->value[1] = set_min_chart(level);
  obj->value[2] = set_max_chart(level);
  obj->pIndexData->value[1] = set_min_chart(level);
  obj->pIndexData->value[2] = set_max_chart(level);

// If weapon is less than 30 sharp it takes 1 off damdie
  if(obj->value[6] < 30)
  {
  ...
  if(obj->value[6] >= 30 && obj->value[6] <= 50)
  { /* Weapon is neither sharp nor * blunt * - make it normal and * * * * * * * * return. 
     */
    obj->value[2] = damdice;
    return;
  }
...
void identify_object(CHAR_DATA *ch, OBJ_DATA *obj)
{
  ...
  ch_printf(ch, "%s appears to be %s.\r\n", capitalize(obj->short_descr), aoran(item_type_name(obj)));
  ...
  ch_printf(ch, "Special properties:  %s\r\nIts weight is %d and level is %d.\r\n", !xIS_EMPTY(obj->extra_flags) ? ext_flag_string(&obj->extra_flags, o_flags) : "Nothing", obj->weight, obj->level);

  /*
   * Only show size needed if can wear it 
   */
  wflags = obj->wear_flags;
  if(IS_SET(wflags, ITEM_TAKE)) /* Strip take first so it just * * checks other wear *
                                 * locations */
  ...
  switch (obj->item_type)
  {
    ...
    case ITEM_TREASURE:
      ch_printf(ch, "%s appears to %s.\r\n", capitalize(obj->short_descr),
                ...
                3 ? "offer maximum help to a dragon's healing rate." : obj->value[2] ==
                4 ? "The massive amount of treasure is all you need for healing rate." : "offer no help to a dragon's healing rate.");
      break;
    case ITEM_CONTAINER:
      ch_printf(ch, "%s appears to be %s.\r\n", capitalize(obj->short_descr),
                obj->value[0] < 76 ? "of a small capacity" :
                obj->value[0] < 150 ? "of a small to medium capacity" :
                obj->value[0] < 300 ? "of a medium capacity" : obj->value[0] < 550 ? "of a medium to large capacity" : obj->value[0] < 751 ? "of a large capacity" : "of a giant capacity");
    ...
    case ITEM_PROJECTILE:
      ...
      break;
    case ITEM_ARMOR:
      if(obj->size != 77 && IS_DEXTERITY(ch) && !IS_NPC(ch))
      {
        ch_printf(ch, "&cYou realize wearing this would give you an armor penalty.\r\n");
      }
      else if(obj->size == 77 && IS_DEXTERITY(ch) && !IS_NPC(ch))
      {
        ch_printf(ch, "&cYou think this piece appears to be lightweight and would be good for moving deftly.\r\n\r\n");
      }
      set_char_color(AT_MAGIC, ch);
