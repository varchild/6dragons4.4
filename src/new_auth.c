...
void do_authorize(CHAR_DATA *ch, char *argument)
{
  ...
  if(!arg1 || arg1[0] == '\0')
  {
    ...
    send_to_char("To authorize all names: auth all\r\n", ch );
  ...
  if(!str_cmp(arg1, "all"))
  {
    for(auth = first_auth_name; auth; auth = auth->next)
    {
         CHAR_DATA *wch;
          ...
          {
            if ( NOT_AUTHED(wch) || NEW_AUTH(wch) || IS_WAITING_FOR_AUTH(wch))
              {
                  auth = get_auth_name(wch->name);
        ...
        save_auth_list();
        send_to_pager("All names authed.\r\n", ch );
        return;
  }
...
void do_mpapplyb(CHAR_DATA *ch, char *argument)
{
  ...
  if(victim->secondclass == -1 && victim->level > 100)
  {
    return;
  }

  if(victim->secondclass != -1 && victim->level > 100 && victim->secondlevel > 100)
  {
    return;
  }
  ...
  snprintf(buf, MSL, "Please welcome new player %s to the realms of Six Dragons!", victim->name);
  announce(buf);
...
void auth_update(void)
{
  ...
  for(auth = first_auth_name; auth; auth = auth->next)
  {
    ...
    {
      snprintf(buf, MIL, "Name: %s      Status: %s\r\n", auth->name, (auth->state == AUTH_ONLINE) ? "Online" : "Offline");
