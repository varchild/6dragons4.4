...
void do_whereis(CHAR_DATA *ch, char *argument)
{
  ...
  if(xIS_SET(ch->act, ACT_BEASTMELD))
    htown = get_htown(ch->master->pcdata->htown_name);
  else
  ...
  if(ch->in_room && str_cmp(ch->in_room->area->filename, "paleon.are")
     ...
     && str_cmp(ch->in_room->area->filename, "tutorial.are") && str_cmp(ch->in_room->area->filename, "etutorial.are") && str_cmp(ch->in_room->area->filename, "dtutorial.are"))
  {
  ...
  {
    if(ch->in_room && !str_cmp(ch->in_room->area->filename, "paleon.are") || !str_cmp(ch->in_room->area->filename, "tutorial.are"))
    {
      ...
      send_to_char("Type 'whereis [&Cmob&c]' to get a suggested route (eg. 'whereis Vaekon').\r\n", ch);
      return;
    }
  }
  else if(arg[0] == '\0' && !str_cmp(htown->name, "Dakar City"))
  {
    if(ch->in_room && !str_cmp(ch->in_room->area->filename, "dakar.are") || !str_cmp(ch->in_room->area->filename, "etutorial.are"))
    {
      send_to_char("&c    Destination : Mob name\r\n", ch);
      send_to_char("-----------------------------\r\n", ch);
      ...
      send_to_char("Type 'whereis [&Cmob&c]' to get a suggested route (eg. 'whereis Rautal').\r\n", ch);
      return;
    }
  }
  else if(arg[0] == '\0' && !str_cmp(htown->name, "Forbidden City"))
  {
    if(ch->in_room && !str_cmp(ch->in_room->area->filename, "forbidden.are") || !str_cmp(ch->in_room->area->filename, "dtutorial.are"))
    {
      ...
      send_to_char("&cType 'whereis [&Cmob&c]' to get a suggested route (eg. 'whereis Ulave').\r\n", ch);
      return;
    }
  }

  if(!str_cmp(ch->in_room->area->filename, "forbidden.are") && (!str_cmp(htown->name, "Paleon City") || !str_cmp(htown->name, "Dakar City")))
  {
    send_to_char("This is not your homeland you don't know your way around.\r\n", ch);
    return;
  }
  else if(!str_cmp(ch->in_room->area->filename, "paleon.are") && (!str_cmp(htown->name, "Forbidden City") || !str_cmp(htown->name, "Dakar City")))
  {
    send_to_char("This is not your homeland you don't know your way around.\r\n", ch);
    return;
  }
  else if(!str_cmp(ch->in_room->area->filename, "dakar.are") && (!str_cmp(htown->name, "Paleon City") || !str_cmp(htown->name, "Forbidden City")))
  {
    send_to_char("This is not your homeland you don't know your way around.\r\n", ch);
    return;
  }
  ...
  if(str_cmp(arg, "central"))
  {
    ...
    {
      send_to_char("&cType 'whereis [&Cmob&c]' to get a suggested route (eg. 'whereis Vaekon'). \r\n", ch);
      return;
    }
  }
   ...
   */
  {
    ...
    else if(!str_cmp(arg, "central") && !str_cmp(ch->in_room->area->filename, "paleon.are"))
    {
      location = get_room_index(16007);
    }
    else if(!str_cmp(arg, "central") && !str_cmp(ch->in_room->area->filename, "forbidden.are"))
    {
      location = get_room_index(35515);
    }
    else if(!str_cmp(arg, "central") && !str_cmp(ch->in_room->area->filename, "tutorial.are"))
    {
      location = get_room_index(5100);
    }
    else if(!str_cmp(arg, "central") && !str_cmp(ch->in_room->area->filename, "etutorial.are"))
    {
      location = get_room_index(33210);
    }
    else if(!str_cmp(arg, "central") && !str_cmp(ch->in_room->area->filename, "dtutorial.are"))
    {
