...
bool spec_tiamat(CHAR_DATA *ch)
{
  ...
  if(ch->fighting)
  {
    ...
    act(AT_ACTION, "$n's five dragon heads roar deafeningly in concert!", ch, NULL, NULL, TO_ROOM);
    interpret(ch, (char *)"mppause 10");
    ...
    case 1:
      if(victim->hit < victim->max_hit)
      {
        ...
        else if(victim->level > 19 && chance == 4)
        {
          ...
          {
            send_to_char("&WA white hot light fills you as your body receives life energies!\r\n", victim);
...
bool spec_guard(CHAR_DATA *ch)
{
  ...
  for(victim = ch->in_room->first_person; victim; victim = v_next)
  {
    ...
    if(victim->fighting && who_fighting(victim) != ch && victim->alignment < 300 && IS_NPC(victim))
      ech = victim;
  ...
  if(victim)
  {
    snprintf(buf, MSL, "yell %s is a %s!  PROTECT THE INNOCENT!  MORE BLOOOOD!!!", victim->name, crime);
    interpret(ch, buf);
    multi_hit(ch, victim, TYPE_UNDEFINED);
    return TRUE;
  }
...
bool spec_janitor(CHAR_DATA *ch)
{
  ...
  for(trash = ch->in_room->first_content; trash; trash = trash_next)
  {
    ...
    if(!IS_SET(trash->wear_flags, ITEM_TAKE) || IS_OBJ_STAT(trash, ITEM_BURIED))
      continue;
    if(trash->item_type == ITEM_DRINK_CON || trash->item_type == ITEM_TRASH || trash->cost < 500 || (trash->pIndexData->vnum == OBJ_VNUM_SHOPPING_BAG && !trash->first_content))
    {
...
bool spec_thief(CHAR_DATA *ch)
{
  ...
  for(victim = ch->in_room->first_person; victim; victim = v_next)
  {
    ...
    if(IS_NPC(victim) || victim->level >= LEVEL_IMMORTAL || number_bits(2) != 0 || !can_see(ch, victim))
      continue;

    if(IS_AWAKE(victim) && number_range(0, ch->level) == 0)
    {
      act(AT_ACTION, "You discover $n's hands in your sack of money!", ch, NULL, victim, TO_VICT);
      act(AT_ACTION, "$N discovers $n's hands in $S sack of money!", ch, NULL, victim, TO_NOTVICT);
      return TRUE;
    }
    else
    {
      ...
      money = GET_MONEY(victim, type) * number_range(1, URANGE(2, ch->level / 4, 10)) / 100;
