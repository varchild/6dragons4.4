...
/*
    if ( IS_NPC( ch ) || class_table[ch->Class]->attr_prime == APPLY_INT || IS_IMMORTAL( ch ) )
        max = 25;
    else if ( ch->race == RACE_PIXIE || ch->race == RACE_DROW || ch->race == RACE_ELF ) {
        max = 25;
...
/*
    if ( IS_NPC( ch ) || class_table[ch->Class]->attr_prime == APPLY_CON || IS_IMMORTAL( ch ) )
        max = 25;
    else if ( ch->race == RACE_TROLL || ch->race == RACE_DWARF ) {
        max = 25;
        return URANGE( 3, ch->perm_con + ch->mod_con, max );
    }
...
void modify_skill(CHAR_DATA *ch, int sn, int mod, bool fAdd)
{
  if(!IS_NPC(ch))
  {
    if(fAdd)
      ...
      ch->pcdata->learned[sn] = URANGE(0, ch->pcdata->learned[sn] + mod, get_maxadept(ch, sn, TRUE));
  }
  return;
}
...
void affect_modify(CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd)
{
  ...
  switch (paf->location % REVERSE_APPLY)
  {
    ...
    case APPLY_FULL:
      if(!IS_NPC(ch))
        ch->pcdata->condition[COND_FULL] = URANGE(0, ch->pcdata->condition[COND_FULL] + mod, STATED);
      break;

    case APPLY_THIRST:
      if(!IS_NPC(ch))
        ch->pcdata->condition[COND_THIRST] = URANGE(0, ch->pcdata->condition[COND_THIRST] + mod, STATED);
      break;
   ...
   */
  if(!IS_NPC(ch) && saving_char != ch && (wield = get_eq_char(ch, WEAR_WIELD)) != NULL && get_obj_weight(wield, FALSE) > str_app[get_curr_str(ch)].wield)
  {
    static int              depth;
...
void obj_from_char(OBJ_DATA *obj)
{
  ...
  {
    bug("%s null ch for object %s.", __FUNCTION__, obj->short_descr);
    return;
  }
...
void extract_char(CHAR_DATA *ch, bool fPull)
{
  ...
  if(!fPull)
  {
    ...
    if((wch = get_char_room(ch, (char *)"healer")) != NULL)
    {
      ...
      snprintf(buf, MSL, "say %s, if you need it there is an undertaker that can get your corpse.\r\n", capitalize(ch->name));
      interpret(wch, buf);
    }
    else
      act(AT_MAGIC, "$n appears from some strange swirling mists!", ch, NULL, NULL, TO_ROOM);
...
/*    for ( rch = ch->in_room->first_person; rch; rch = rch->next_in_room )
        if ( rch->race == RACE_ANIMAL && rch != ch ) {
            return rch;
        }
*/
...
CHAR_DATA              *get_char_area(CHAR_DATA *ch, char *argument)
{
  ...
  for(wch = ch->in_room->first_person; wch; wch = wch->next_in_room)
    if(wch->in_room->area == ch->in_room->area && ((nifty_is_name(arg, wch->name)) || (IS_NPC(wch) && vnum == wch->pIndexData->vnum)))
    {
...
OBJ_DATA               *get_obj_list(CHAR_DATA *ch, char *argument, OBJ_DATA *list)
{
  ...
  for(obj = list; obj; obj = obj->next_content)
    if(can_see_obj(ch, obj) && nifty_is_name(arg, obj->name))
      if((count += obj->count) >= number || obj->guid == number)
        return obj;
...
{
  OBJ_HERE               *next, *prev;
  OBJ_DATA               *obj;  /* Just a pointer to the object that is here */
};
  ...
  if(ms > 15) /* range 1 to 20 -- feel free to * add more */
    switch (number_range(UMAX(1, (ms / 5 - 15)), (ms + 4) / 5))
    {
      ...
      case 19:
        ...
        break;
      case 20:
        t = "A 6Dragons General has no need for that.\r\n";
        break;
    }
  else
  {
...
bool room_is_dark(ROOM_INDEX_DATA *pRoomIndex)
{
  ...
  if(IS_SET(pRoomIndex->room_flags, ROOM_LIGHTED) || IS_SET(pRoomIndex->area->flags, AFLAG_LIGHTED))
    return FALSE;

  /*
   * Volk - we can test for aflag darkness now because we HAVE NO LIGHT! 
   */
  if(IS_SET(pRoomIndex->room_flags, ROOM_DARK) || IS_SET(pRoomIndex->area->flags, AFLAG_DARKNESS))
    return TRUE;
...
CHAR_DATA              *room_is_dnd(CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex)
{
  ...
  {
    if(!IS_NPC(rch) && rch->pcdata && IS_IMMORTAL(rch) && IS_SET(rch->pcdata->flags, PCFLAG_DND) && get_trust(ch) < get_trust(rch) && can_see(ch, rch))
      return rch;
  }
  return NULL;
}
...
bool can_see(CHAR_DATA *ch, CHAR_DATA *victim)
{
  ...
  if(victim == supermob && ch->level < 108)
    return FALSE;

  if(xIS_SET(ch->act, ACT_BEASTMELD))
  {
    if(xIS_SET(victim->act, ACT_MOBINVIS))
      return FALSE;
    else
      return TRUE;
  }
  ...
  if(!ch)
  {
    if(IS_AFFECTED(victim, AFF_INVISIBLE) || IS_AFFECTED(victim, AFF_HIDE) || IS_AFFECTED(victim, AFF_BURROW) || xIS_SET(victim->act, PLR_WIZINVIS))
      return FALSE;
    else
      return TRUE;
  }
...
/* Deadlies link-dead over 2 ticks aren't seen by mortals -- Blodkai */
  ...
  {
    if(victim->level > ch->level && IS_AFFECTED(victim, AFF_INVISIBLE) && !IS_NPC(victim))
      return FALSE;
  ...
  {
    if((IS_SET(ch->pcdata->tag_flags, TAG_PLAYING) || IS_SET(ch->pcdata->tag_flags, TAG_WAITING)) && (IS_SET(victim->pcdata->tag_flags, TAG_PLAYING) || IS_SET(victim->pcdata->tag_flags, TAG_WAITING)))
      return TRUE;
  }

  if(IS_AFFECTED(victim, AFF_INVISIBLE) && (!IS_AFFECTED(ch, AFF_DETECT_INVIS) || victim->level > ch->level))
    return FALSE;
...
/* While not blind, truesight/demonic on players sees all (except invis)*/
  if((IS_AFFECTED(ch, AFF_TRUESIGHT) || IS_AFFECTED(ch, AFF_DEMONIC_SIGHT)) && (!IS_AFFECTED(ch, AFF_BLINDNESS) || IS_AFFECTED(ch, AFF_NOSIGHT)))
    return TRUE;
...
OBJ_DATA               *clone_object(OBJ_DATA *obj)
{
  ...
  if(obj->whichcity != NULL)
    clone->whichcity = QUICKLINK(obj->whichcity);
...
OBJ_DATA               *group_object(OBJ_DATA *obj1, OBJ_DATA *obj2)
{
  ...
  if(obj1->pIndexData == obj2->pIndexData
     &&
     ((VLD_STR(obj1->name) && VLD_STR(obj2->name) && !str_cmp(obj1->name, obj2->name))
      || (!obj1->name && !obj2->name)) && ((VLD_STR(obj1->short_descr)
                                            && VLD_STR(obj2->short_descr)
                                            && !str_cmp(obj1->short_descr,
                                                        obj2->short_descr))
                                           || (!obj1->short_descr
                                               && !obj2->short_descr))
     &&
     ((VLD_STR(obj1->description) && VLD_STR(obj2->description)
       && !str_cmp(obj1->description, obj2->description)) || (!obj1->description
                                                              && !obj2->description))
     && obj1->item_type == obj2->item_type && xSAME_BITS(obj1->extra_flags, obj2->extra_flags)
     && obj1->wear_flags == obj2->wear_flags && obj1->wear_loc == obj2->wear_loc
     && obj1->weight == obj2->weight && obj1->cost == obj2->cost
     && !IS_OBJ_STAT(obj1, ITEM_NOGROUP) && !IS_OBJ_STAT(obj2, ITEM_NOGROUP)
     && obj1->level == obj2->level && obj1->timer == obj2->timer
     && obj1->value[0] == obj2->value[0] && obj1->value[1] == obj2->value[1]
     && obj1->value[2] == obj2->value[2] && obj1->value[3] == obj2->value[3]
     && obj1->value[4] == obj2->value[4] && obj1->value[5] == obj2->value[5]
     && obj1->value[6] == obj2->value[6] && !obj1->first_content && !obj2->first_content
     && obj1->count + obj2->count > 0 && obj1->currtype == obj2->currtype && obj1->color == obj2->color && has_same_affects(obj1, obj2) && has_same_extradescs(obj1, obj2))
  {
...
void regroup_obj(OBJ_DATA *obj)
{
  ...
  if(obj->reset)  /* Don't do this with objects that are resets, causes alot of issues */
    return;
  ...
  if(destobj || (!destroom && !ch && (destobj = obj->in_obj) != NULL))
  {
    for(otmp = obj->first_content; otmp; otmp = otmp_next)
    {
      ...
      if((destobj->item_type == ITEM_CONTAINER || destobj->item_type == ITEM_KEYRING
          || destobj->item_type == ITEM_QUIVER) && get_obj_weight(otmp, FALSE) + get_obj_weight(destobj, FALSE) > destobj->value[0])
        continue;
      obj_from_obj(otmp);
...
void worsen_mental_state(CHAR_DATA *ch, int mod)
{
  ...
  if(!IS_NPC(ch) && ch->pcdata->nuisance && ch->pcdata->nuisance->flags > 2)
    c = (int)(c + .4 * ((ch->pcdata->nuisance->flags - 2) * ch->pcdata->nuisance->power));
...
{
  int                     hasgold = ((tarea->high_economy[type] > 0) ? 1 : 0) * 1000000000 + tarea->low_economy[type];
...
void add_kill(CHAR_DATA *ch, CHAR_DATA *mob)
{
  ...
  memmove((char *)ch->pcdata->killed + sizeof(KILLED_DATA), ch->pcdata->killed, (MAX_KILLTRACK - 1) * sizeof(KILLED_DATA));
...
char                   *affect_bit_name(EXT_BV * vector)
{
  ...
  if(xIS_SET(*vector, AFF_BLINDING_SPEED))
    mudstrlcat(buf, " blinding_speed", 512);
  if(xIS_SET(*vector, AFF_DETECT_HIDDEN))
    mudstrlcat(buf, " detect_hidden", 512);
  if(xIS_SET(*vector, AFF_ENTHRALLED))
    mudstrlcat(buf, " enthralled", 512);
  ...
  if(xIS_SET(*vector, AFF_DARK_BLESSINGS))
    mudstrlcat(buf, " dark_blessings", 512);
  ...
  if(xIS_SET(*vector, AFF_DISGUISE))
    mudstrlcat(buf, " disguise", 512);
...
void set_position(CHAR_DATA *ch, int position)
{
  if(ch->position == POS_CRAWL)
  {
    ...
    if(position == POS_FIGHTING || position == POS_BERSERK || position == POS_EVASIVE || position == POS_DEFENSIVE || position == POS_AGGRESSIVE)
    {
...
{
  AFFECT_DATA            *paf, *tmpaf, *tmpaf_next;
  ...
  for(paf = obj->first_affect; paf; paf = paf->next)
  {
    ...
    for(tmpaf = paf->next; tmpaf; tmpaf = tmpaf_next)
    {
      ...
      if(tmpaf->type == paf->type && tmpaf->location == paf->location && xSAME_BITS(tmpaf->bitvector, paf->bitvector))
      {
...
*/
void sound_to_char(const char *fname, int vol, int repeats, int priority, const char *type, const char *url, CHAR_DATA *ch)
{

  if(!xIS_SET(ch->act, PLR_COMMUNICATION) && !xIS_SET(ch->act, PLR_MUSIC) && !xIS_SET(ch->act, PLR_BATTLE) && !xIS_SET(ch->act, PLR_CRAFTS))
    return;
 ...
 * more detailed information at: http://www.zuggsoft.com/
*/
void music_to_char(const char *fname, int vol, int repeats, int continu, const char *type, const char *url, CHAR_DATA *ch)
{
...
/* stop playing sound */
void reset_communication(CHAR_DATA *ch)
{
...
/* stop playing sound */
void reset_crafts(CHAR_DATA *ch)
{
...
/* stop playing music */
void reset_enhanced(CHAR_DATA *ch)
{
