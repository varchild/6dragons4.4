...
#include <ctype.h>  /* fixes tolower implicit declarations */
#include <string.h>
#include "h/mud.h"
#include "h/files.h"
#include "h/clans.h"
...
short DRAGON_AC(CHAR_DATA *ch)
{
  ...
  {
    hide = (short)(ch->armor + ((ch->level + 20) * -2.0));
    return hide;
  }
  else
    return 0;
...
void violence_update(void)
{
  ...
  for(ch = last_char; ch; lst_ch = ch, ch = gch_prev)
  {
    ...
    {
      bug("FATAL: %s: %s->prev->next doesn't point to ch. Short-cutting here", __FUNCTION__, ch->name);
      ch->prev = NULL;
      gch_prev = NULL;
    }
    ...
    if(!ch->in_room || !ch->name)
    {
      ...
      log_printf("ch: %ld  ch->in_room: %ld  ch->prev: %ld  ch->next: %ld", (long)ch, (long)ch->in_room, (long)ch->prev, (long)ch->next);
      log_string(lastplayercmd);
      if(lst_ch)
        log_printf("lst_ch: %ld  lst_ch->prev: %ld  lst_ch->next: %ld", (long)lst_ch, (long)lst_ch->prev, (long)lst_ch->next);
      else
    ...
    if(IS_NPC(ch) && IS_AFFECTED(ch, AFF_CHARM) && who_fighting(ch->master) != NULL && !xIS_SET(ch->act, ACT_MOUNTABLE))
    {
      if(!ch->fighting)
      {
    ...
    for(paf = ch->first_affect; paf; paf = paf_next)
    {
      ...
      else
      {
        ...
        if(IS_AFFECTED(ch, AFF_SILENCE) && xIS_SET(ch->act, PLR_SILENCE))
        {
        ...
        if(IS_AFFECTED(ch, AFF_IMPRISONED))
        {
          ...
          char_from_room(ch);
          pRoomIndex = get_room_index(ch->regoto);
          ...
          {
            if(obj->pIndexData->vnum == 41032 && obj->in_room != NULL)
              break;
          }
          if(obj)
            make_scraps(obj);
    ...
    {
      log_printf("violence_update: %s is attempting to fight %s in a SAFE room.", ch->name, victim->name);
      stop_fighting(ch, TRUE);
    }
    else if(IS_AWAKE(ch) && ch->in_room == victim->in_room)
    {
      retcode = multi_hit(ch, victim, TYPE_UNDEFINED);
      if(!IS_NPC(ch) && xIS_SET(ch->act, PLR_AUTOGLANCE) && ch != NULL && victim != NULL && who_fighting(ch) == victim && !char_died(victim) && !char_died(ch))
      {
        ...
        if(30 + (ch->level / 4) >= number_percent())
        {
          ...
          switch (attacktype)
          {
            ...
            case ATCK_DRAIN:
              ...
              break;
            case ATCK_MENTALASSAULT:
              do_mental_assault(ch, (char *)"");
              retcode = global_retcode;
              break;
            case ATCK_GSMITE:
              retcode = spell_greater_smite(skill_lookup("greater smite"), ch->level, ch, victim);
              break;
            case ATCK_BREATHE:
              do_breathe(ch, (char *)"");
              retcode = global_retcode;
            ...
            case ATCK_BLIGHTNING:
              retcode = spell_black_lightning(skill_lookup("black lightning"), ch->level, ch, victim);
              break;
            case ATCK_LTOUCH:
              retcode = spell_smaug(skill_lookup("lich touch"), ch->level, ch, victim);
              break;
            case ATCK_INFERNO:
              act(AT_MAGIC, "$n utters a few incantations.", ch, NULL, NULL, TO_ROOM);
              do_inferno(ch, (char *)"");
            ...
            case ATCK_POISON:
              ...
              break;
            case ATCK_SLOW:
              retcode = spell_slow(skill_lookup("slow"), ch->level, ch, victim);
              break;
            case ATCK_THROATPUNCH:
              do_throat_punch(ch, (char *)"");
              retcode = global_retcode;
              break;
            case ATCK_RAKE:
              do_rake(ch, (char *)"");
            ...
            case ATCK_JUDOCHOP:
              ...
              retcode = global_retcode;
              break;
            case ATCK_SHRIEKINGNOTE:
              do_shrieking_note(ch, (char *)"");
              retcode = global_retcode;
              break;
            case ATCK_MURAZORWHIP:
              do_whip_of_murazor(ch, (char *)"");
              retcode = global_retcode;
              break;
            case ATCK_BUTTERFLYKICK:
              do_butterfly_kick(ch, (char *)"");
              retcode = global_retcode;
              break;
            case ATCK_SNATCH:
              do_snatch(ch, (char *)"");
            ...
            case ATCK_FIREBALL:
              ...
              break;
            case ATCK_LIGHTNING:
              retcode = spell_lightning(skill_lookup("lightning"), ch->level, ch, victim);
              break;
            case ATCK_WEAKEN:
              act(AT_MAGIC, "$n utters a few incantations.", ch, NULL, NULL, TO_ROOM);
      ...
      if(!xIS_EMPTY(ch->defenses))
      {
        ...
        if(50 + (ch->level / 4) > number_percent())
        {
          ...
          switch (attacktype)
          {
            ...
            case DFND_NETTLESKIN:
              act(AT_MAGIC, "$n mutters a few incantations...and looks a little better.", ch, NULL, NULL, TO_ROOM);
              retcode = spell_nettle_skin(skill_lookup("nettle skin"), ch->level, ch, ch);
              break;

            case DFND_SPECTRALWARD:
              do_spectral_ward(ch, (char *)"");
              break;

            case DFND_MINORRESTORE:
              if(ch->hit < ch->max_hit)
              {
                do_minor_restore(ch, (char *)"self");
              }
              break;

            case DFND_TORPOR:
              act(AT_MAGIC, "$n mutters a few incantations...and looks a little better.", ch, NULL, NULL, TO_ROOM);
              retcode = spell_torpor(skill_lookup("torpor"), ch->level, ch, ch);

              break;

            case DFND_KINETICBARRIER:
              do_kinetic_barrier(ch, (char *)"");
              break;

            case DFND_HAWKSPIRIT:
              do_hawk_spirit(ch, (char *)"");
              break;
            ...
            case DFND_COMBATHEAL:
              do_combat_heal(ch, ch->name);
              break;

            case DFND_WILLOWSKIN:
              do_willow_skin(ch, ch->name);
              break;
            case DFND_GRENDALSTANCE:
              if(ch->hit < ch->max_hit)
              {
                do_grendals_stance(ch, (char *)"");
              }
              break;
            case DFND_DISPELMAGIC:
              ...
              break;
            case DFND_WOLFSPIRIT:
              act(AT_MAGIC, "$n utters an incantation...", ch, NULL, NULL, TO_ROOM);
              retcode = spell_spirit_wolf(skill_lookup("spirit wolf"), ch->level, ch, victim);
              break;
            case DFND_REGENERATE:
              break;
            case DFND_BIOFEEDBACK:
              break;

            case DFND_MAINTAINPOSITION:
              do_maintain_position(ch, (char *)"");
              break;

            case DFND_HUNKERDOWN:
              do_hunker_down(ch, (char *)"");
              break;
            ...
            case DFND_ACIDMIST:
              if(!IS_AFFECTED(ch, AFF_ACIDMIST))
              {
                ...
                retcode = rNONE;
              break;
            case DFND_FINDWEAKNESS:
              break;

            case DFND_BEARSPIRIT:
              do_bear_spirit(ch, (char *)"");
              break;
    ...
    for(rch = ch->in_room->first_person; rch; rch = rch_next)
    {
      ...
      if((!IS_NPC(ch) && !IS_NPC(rch)) && (rch != ch) && (rch->fighting) && (who_fighting(rch->fighting->who) == ch) && (!xIS_SET(rch->fighting->who->act, ACT_AUTONOMOUS)) && (rch->style < ch->style))
      {
...
/* Start of compressed attacks */
  ...
  if(IS_AFFECTED(ch, AFF_SURREAL_SPEED) || IS_AFFECTED(ch, AFF_FURY))
    chance += 10;
  if((IS_AFFECTED(ch, AFF_BLINDNESS) && !IS_AFFECTED(ch, AFF_NOSIGHT)) || IS_AFFECTED(ch, AFF_SLOW))
    chance -= 10;
  if(IS_AFFECTED(ch, AFF_SURREAL_SPEED) || IS_AFFECTED(ch, AFF_FURY))
    chance += 10;
  if((IS_AFFECTED(ch, AFF_BLINDNESS) && !IS_AFFECTED(ch, AFF_NOSIGHT)) || IS_AFFECTED(ch, AFF_SLOW))
   ...
   */
  chance = IS_NPC(ch) ? ch->level : (int)(LEARNED(ch, gsn_sixth_attack) + (get_curr_dex(ch) * 2));
   ...
   */
  chance = IS_NPC(ch) ? ch->level : (int)(LEARNED(ch, gsn_seventh_attack) + (get_curr_dex(ch) * 2));
   ...
   */
  chance = IS_NPC(ch) ? ch->level : (int)(LEARNED(ch, gsn_eighth_attack) + (get_curr_dex(ch) * 2));
  ...
  // speed assault
  if((IS_AFFECTED(ch, AFF_BLINDING_SPEED) && ch->race == RACE_VAMPIRE) || IS_AFFECTED(ch, AFF_QUICK_BLADE))
    ch->success_attack *= 2;
  ...
  {
    if(obj->value[4] == WEP_2H_LONG_BLADE || obj->value[4] == WEP_2H_AXE || obj->value[4] == WEP_2H_BLUDGEON)
    {
      ch->success_attack /= 2;
      if(ch->success_attack < 2)
      {
...
/* End of compressed attacks */
  ...
  if(IS_AFFECTED(ch, AFF_WOLF_SPIRIT) && ch->success_attack > 0)
  {
    short                   wolf = number_range(1, 10);

    if(wolf > 8)
    {
      short                   attacks = get_curr_dex(ch) / 5 - 2;

      if(attacks < 1)
        attacks = 1;
      ch_printf(ch, "&cThe spirit of the wolf speeds up your attacks by %d\r\n", attacks);
      ch->success_attack += attacks;
    }
  }
  ...
  if(retcode == rNONE)
  {
    ...
    if(!IS_AFFECTED(ch, AFF_FLYING) && !IS_AFFECTED(ch, AFF_FLOATING))
      if(ch->in_room && ch->in_room->sector_type)
...
//  chance = number_chance( 1, 10 );
  ...
  if(number_chance(1, 10) == 10)
  {
    if(IS_AFFECTED(ch, AFF_SLOW))
    {
      act(AT_GREEN, "You try to attack, but move in a slow motion state.", ch, NULL, NULL, TO_CHAR);
      act(AT_GREEN, "$n tries to attack, but moves in a slow motion state.", ch, NULL, NULL, TO_ROOM);
      return rNONE;
    }
  }
     ...
     && dt == TYPE_UNDEFINED && IS_NPC(ch) && !xIS_EMPTY(ch->attacks))
  {
    ...
    switch (attacktype)
    {
      ...
      case ATCK_STATICPULSE:
        retcode = spell_static_pulse(skill_lookup("static pulse"), ch->level, ch, victim);
        break;
      ...
      case ATCK_BLURATTACK:
        do_blur_attack(ch, (char *)"");
        retcode = global_retcode;
        break;

      case ATCK_ACIDSPRAY:
        do_acid_spray(ch, (char *)"");
        retcode = global_retcode;
        break;
      ...
      case ATCK_ASTRALATTACK:
        do_astral_attack(ch, (char *)"");
        retcode = global_retcode;
        break;

      case ATCK_BODYDROP:
        do_body_drop(ch, (char *)"");
        retcode = global_retcode;
        break;

      case ATCK_MISTACID:
        do_mist_acid(ch, (char *)"");
        retcode = global_retcode;
        break;

      case ATCK_CREEPINGDOOM:
        do_creeping_doom(ch, (char *)"");
        retcode = global_retcode;
        break;
      ...
      case ATCK_QUIVERINGPALM:
        do_quivering_palm(ch, (char *)"");
        retcode = global_retcode;
        break;

      case ATCK_THUNDEROUSHYMN:
        do_thunderous_hymn(ch, (char *)"");
        retcode = global_retcode;
        break;

      case ATCK_PARALYZE:
        retcode = spell_paralyze(skill_lookup("paralyze"), ch->level, ch, victim);
        break;
      ...
      case ATCK_PSIONICBLAST:
        do_psionic_blast(ch, (char *)"");
        retcode = global_retcode;
        break;

      case ATCK_FEROCIOUSSTRIKE:
        do_ferocious_strike(ch, (char *)"");
        retcode = global_retcode;
        break;
  ...
  {
    thac0 = interpolate(ch->level, thac0_00, thac0_32) - (GET_HITROLL(ch) + ch->level / 5);
  }
  else
  {
  ...
  if(diceroll == 0 || (diceroll != 19 && diceroll < thac0 - victim_ac))
  {
    ...
    if(diceroll < thac0 - victim_ac && diceroll >= thac0 - (10 + (dex_app[get_curr_dex(ch)].defensive / 10) + (bonus / 10) && diceroll != 19 && diceroll != 0))
    {
      ...
      if(victim->Class == CLASS_MAGE)
      {
        if(drama > 50)
        {
          act(AT_LBLUE, "Your attack bounces off $N's shimmering wall of energy.", ch, NULL, victim, TO_CHAR);
          act(AT_LBLUE, "Your wall of energy shimmers as it deflects $n's attack.", ch, NULL, victim, TO_VICT);
          act(AT_LBLUE, "$n's attack bounces off $N's shimmering wall of energy.", ch, NULL, victim, TO_NOTVICT);
        }
        else
        {
          act(AT_ORANGE, "Your attack penetrates $N's shimmering wall of energy, but glances off $m.", ch, NULL, victim, TO_CHAR);
          act(AT_ORANGE, "Your wall of energy fails to deflects $n's attack, but it glances off you.", ch, NULL, victim, TO_VICT);
          act(AT_ORANGE, "$n's attack penetrates $N's wall of energy, but it glances off $m.", ch, NULL, victim, TO_NOTVICT);
        }
      }
      if(victim->race == RACE_DRAGON)
      {
        if(drama > 50)
        {
          act(AT_YELLOW, "Sparks fly as your attack is deflected by $N's thick dragon scales.", ch, NULL, victim, TO_CHAR);
          act(AT_YELLOW, "Sparks fly as your dragon hide deflects $n's attack.", ch, NULL, victim, TO_VICT);
          act(AT_YELLOW, "Sparks fly as $n's attack is deflected by $N's thick dragon scales.", ch, NULL, victim, TO_NOTVICT);
        }
        else
        {
          act(AT_ORANGE, "Your attack barely penetrates $N's smooth dragon scales.", ch, NULL, victim, TO_CHAR);
          act(AT_ORANGE, "$n's attack barely penetrates your smooth dragon scales.", ch, NULL, victim, TO_VICT);
          act(AT_ORANGE, "$n's attack barely penetrates $N's smooth dragon scales.", ch, NULL, victim, TO_NOTVICT);
        }
      }
      if(victim->Class != CLASS_MAGE && victim->race != RACE_DRAGON)
      {
        if(drama > 50)
        {
          act(AT_YELLOW, "Sparks fly as your attack is deflected by $N's armor.", ch, NULL, victim, TO_CHAR);
          act(AT_YELLOW, "Sparks fly as your armor deflects $n's attack.", ch, NULL, victim, TO_VICT);
          act(AT_YELLOW, "Sparks fly as $n's attack is deflected by $N's armor.", ch, NULL, victim, TO_NOTVICT);
        }
        else
        {
          act(AT_ORANGE, "Your attack gets through $N's armor, but is only superficial in damage.", ch, NULL, victim, TO_CHAR);
          act(AT_ORANGE, "$n's attack penetrates your armor, but only causes superficial damage.", ch, NULL, victim, TO_VICT);
          act(AT_ORANGE, "$n's attack penetrates $N's armor, but only causes supericial damage.", ch, NULL, victim, TO_NOTVICT);
        }
      }
  ...
  hull = number_range(1, 10);

  if(!IS_NPC(ch) && ch->pcdata->ship > 0 && hull > 8)
  {
    if(ch->pcdata->hull < 11)
    {
      ch->pcdata->hull = 0;
      ch_printf(ch, "&CThe first mate hollars, all hands for themself, the ship is sinking!\r\n");
      // add do_depart abandon for all players on ship.
      if(ch->pcdata->ship == 1)
      {
        int                     lroom, hroom;
        ...
        {
          if(!IS_NPC(ch) && (ch->in_room->vnum <= hroom || ch->in_room->vnum >= lroom))
          {
            interpret(ch, (char *)"depart abandon");
          }
        }
      }
    ...
    if(ch && ch->pcdata->hull > 10)
    {
      ch->pcdata->hull -= 1;
      ch_printf(ch, "&CThe first mate hollars, Captain the ship hull integrity is at %d\r\n", ch->pcdata->hull);
      if(xIS_SET(ch->act, PLR_COMMUNICATION))
        send_to_char("!!SOUND(sound/downwithhisship.wav)\r\n", ch);
  ...
  if(!IS_NPC(ch) && get_eq_char(ch, WEAR_WIELD) == NULL)
  {
    if(ch->Class != CLASS_MONK)
    {
      if(ch->pcdata->learned[gsn_pugilism] > 0 && ch->pcdata->learned[gsn_pugilism] < 40)
        dam += get_curr_str(ch) / 4;
      else if(ch->pcdata->learned[gsn_pugilism] > 39 && ch->pcdata->learned[gsn_pugilism] < 80)
        dam += get_curr_str(ch) / 2;
      else if(ch->pcdata->learned[gsn_pugilism] > 79)
        dam += get_curr_str(ch);
    }
    if(ch->Class == CLASS_MONK)
    {
  ...
  if(IS_NPC(ch) && IS_ATTACK(ch, ATCK_PUGILISM))
  {
    dam += get_curr_str(ch) / 4;
  }
  ...
  if(wield)
  { /* bare hand dice formula fixed by Thoric */
    ...
    if(CAN_SHARPEN(wield) && wield->value[6] > 10 && wield->pIndexData->vnum != 41002)
    {
...
//        update_weapon(ch, wield);
  ...
  if(!IS_NPC(ch))
  {
    ...
    {
      dam += (ch->pcdata->condition[COND_DRUNK] / 8);
    }
  }
 ...
 */
  ...
  chance = number_range(1, 100);
  chance += lck_app[get_curr_lck(ch)].luck; // make luck relevant
  if(chance > 99)
  {
  ...
  {
    if ( ch->level < 50 )
    dam += number_range(5, get_curr_str(ch));
    if ( ch->level > 49 && ch->level < 75)
        dam += number_range(get_curr_str(ch), (get_curr_str(ch)*2));
    if ( ch->level > 74 )   
        dam += number_range(get_curr_str(ch), (get_curr_str(ch)*3));
    ch->success_attack += 1;
    ch_printf(ch, "&OYour hooves trample %s for %d damage.\r\n", victim->name, dam );
    ch_printf(victim, "&O%s's hooves trample you for %d damage.\r\n", ch->name, dam );
    act(AT_ORANGE, "$n's hooves trample $N.", ch, NULL, victim, TO_NOTVICT);
  }

  if(ch->Class == CLASS_BLACK && chance < 26)
  {
    if ( ch->level < 50 )
    dam += number_range(5, get_curr_str(ch));
    if ( ch->level > 49 && ch->level < 75)
    ...
    if ( ch->level > 74 )
        dam += number_range(get_curr_str(ch), (get_curr_str(ch)*3));
    ch->success_attack += 1;
    ch_printf(ch, "&cYour tail whips out smacking %s for %d damage.\r\n", victim->name, dam );
    ch_printf(victim, "&c%s's tail whips out smacking you for %d damage.\r\n", ch->name, dam);
    act(AT_CYAN, "$n's tail whips out smacking $N!", ch, NULL, victim, TO_NOTVICT);
  }
  ...
  if(!IS_NPC(ch) && ch->pcdata->learned[gsn_martial_arts] > 0 && get_eq_char(ch, WEAR_WIELD) == NULL && get_eq_char(ch, WEAR_DUAL_WIELD) == NULL)
  {
    short                   martial = 0;
  ...
  if(!IS_NPC(victim) && IS_NPC(ch) && victim->race == RACE_DRAGON)
  {
    if(ch->level >= victim->level)
    {
      ...
      else if(victim->level > 9 && victim->level < 20)
      {
        dam += number_range(5, 20);
      }
      else if(victim->level > 19 && victim->level < 30)
      {
        dam += number_range(10, 30);

      }
      else if(victim->level > 29 && victim->level < 40)
      {
        dam += number_range(20, 40);
      }
      else if(victim->level > 39 && victim->level < 50)
      {
        dam += number_range(30, 60);
      }
      else if(victim->level > 49 && victim->level < 60)
      {
        dam += number_range(40, 80);
      }
      else if(victim->level > 59 && victim->level < 70)
      {
        dam += number_range(50, 100);
      }
      else if(victim->level > 69 && victim->level < 80)
      {
        dam += number_range(60, 120);
      }
      else if(victim->level > 79)
      {
        dam += number_range(60, 150);
      }
    }
  }
  ...
  if(xIS_SET(ch->defenses, DFND_FINDWEAKNESS))
  {
    dam += (int)(get_curr_str(ch) * 2);
    ch->move -= 2;
  }
  ...
  if(IS_AFFECTED(victim, AFF_KINETIC))
  {
    ...
    {
      act(AT_YELLOW, "Your attack strikes $N's kinetic barrier.", ch, NULL, victim, TO_CHAR);
      if(ch != victim)
        act(AT_YELLOW, "$n's attack strikes your kinetic barrier.", ch, NULL, victim, TO_ROOM);
      else
        act(AT_YELLOW, "$n's attack strikes $N's kinetic barrier.", ch, NULL, victim, TO_ROOM);
      victim->kinetic_dam -= dam;
      dam = 0;
    }
    else
    {
      act(AT_YELLOW, "Your attack has shattered $N's kinetic barrier!", ch, NULL, victim, TO_CHAR);
      if(ch != victim)
        act(AT_YELLOW, "$n's attack has shattered your kinetic barrier!", ch, NULL, victim, TO_ROOM);
      else
        act(AT_YELLOW, "$n's attack has shattered $N's kinetic barrier!", ch, NULL, victim, TO_ROOM);
      affect_strip(victim, gsn_kinetic_barrier);
      xREMOVE_BIT(victim->affected_by, AFF_KINETIC);
    }
  }

  if(IS_AFFECTED(victim, AFF_WARD) || IS_AFFECTED(victim, AFF_ETHEREAL))
  {
    if(victim->ward_dam > dam)
    {
      act(AT_YELLOW, "Your attack strikes $N's protective ward.", ch, NULL, victim, TO_CHAR);
      act(AT_YELLOW, "$n's attack strikes $N's protective ward.", ch, NULL, victim, TO_ROOM);
      victim->ward_dam -= dam;
      dam = 0;
    }
    else
    {
      act(AT_YELLOW, "Your attack has shattered $N's protective ward!", ch, NULL, victim, TO_CHAR);
      act(AT_YELLOW, "$n's attack has shattered $N's protective ward!", ch, NULL, victim, TO_ROOM);
      affect_strip(victim, gsn_chilled_ward);
      affect_strip(victim, gsn_frost_ward);
      affect_strip(victim, gsn_ice_ward);
      affect_strip(victim, gsn_spirits_ward);
      affect_strip(victim, gsn_spectral_ward);
      affect_strip(victim, gsn_shocking_ward);
      affect_strip(victim, gsn_lightning_ward);
      affect_strip(victim, gsn_energy_ward);
      affect_strip(victim, gsn_static_ward);
      affect_strip(victim, gsn_ward_of_ethereal);
      xREMOVE_BIT(victim->affected_by, AFF_WARD);
      xREMOVE_BIT(victim->affected_by, AFF_ETHEREAL);
    }
  }
  ...
  // Monk's possibly have damage mitigated to move points
  if(victim->Class == CLASS_MONK && !IS_NPC(victim))
  {
    if(victim->move / 4 > victim->pcdata->monkblood && (victim->pcdata->mb == 0 || victim->pcdata->mb == 1))
    {
      victim->pcdata->monkblood += dam;
      victim->move -= dam;
      dam = dam - get_curr_con(victim);
    }
    if(victim->move / 4 <= victim->pcdata->monkblood)
    {
      dam = victim->pcdata->monkblood;
      ch->success_attack = 1;
      victim->pcdata->monkblood = 0;
      victim->pcdata->mb = 2;
    }
  }
  ...
  if(IS_AFFECTED(victim, AFF_SPROUT_SPIKES) && !IS_AFFECTED(ch, AFF_SPROUT_SPIKES))
    retcode = spell_stick(skill_lookup("stick"), off_shld_lvl(victim, ch), victim, ch);
  if(retcode != rNONE || char_died(ch) || char_died(victim))
    return retcode;
  ...
  if(IS_AFFECTED(victim, AFF_ICESHIELD) && !IS_AFFECTED(ch, AFF_ICESHIELD))
    retcode = spell_iceshard(skill_lookup("iceshard"), off_shld_lvl(victim, ch), victim, ch);
  if(retcode != rNONE || char_died(ch) || char_died(victim))
    return retcode;
...
ch_ret damage(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt)
{
  ...
  short                   anopc = 0, bnopc = 0;
  bool                    npcvict, loot;
  ch_ret                  retcode = rNONE;

  if(!ch)
  {
    bug("%s Damage: null ch!", __FUNCTION__);
    return rERROR;
  }
  if(!victim)
  {
  ...
  if(IS_AFFECTED(ch, AFF_ENTHRALLED))
    return rNONE;

  /*
   * Lets set up the soldier to fight better 
   */
  if(IS_NPC(ch) && !ch->soldierset && ch->city && ch->siege)
  {
    CITY_DATA              *city = ch->city;

    if(city && (city->soldier_mob == ch->pIndexData->vnum || city->archer_mob == ch->pIndexData->vnum || city->guard_mob == ch->pIndexData->vnum || city->warrior_mob == ch->pIndexData->vnum))
    {
      if(city->soldier_mob == ch->pIndexData->vnum || city->archer_mob == ch->pIndexData->vnum || city->guard_mob == ch->pIndexData->vnum || city->warrior_mob == ch->pIndexData->vnum)
        set_soldier(ch, victim, FALSE);
      else
        set_soldier(ch, victim, TRUE);
    }
  }
  else if(IS_NPC(victim) && !victim->soldierset && victim->city && victim->siege)
  {
    CITY_DATA              *city = victim->city;

    if(city
       && (city->soldier_mob == victim->pIndexData->vnum
           || city->archer_mob == victim->pIndexData->vnum || city->guard_mob == victim->pIndexData->vnum || city->warrior_mob == victim->pIndexData->vnum))
    {
      if(city->soldier_mob == victim->pIndexData->vnum || city->archer_mob == victim->pIndexData->vnum || city->guard_mob == victim->pIndexData->vnum || city->warrior_mob == victim->pIndexData->vnum)
        set_soldier(victim, ch, FALSE);
      else
        set_soldier(victim, ch, TRUE);
    }
  }
    ...
    if(!IS_AFFECTED(ch, AFF_HATE))
    {
      if(dam > 0)
      {
        if(dam <= 10)
          ...
          ch->hate_level = 42;
        else  /* Shrug might as well have a * * * * * * * * * * default for * * anything
               * * * * else */
          ch->hate_level = 43;
      }
    }
...
/* On second thought, HTF to check if they HAVEN'T used murder command? need a bool-in-function.
   ...
   first place! */
    ...
    if(victim && victim->level > 20)
    {
      if(xIS_SET(victim->act, ACT_BEASTPET))
        dam /= 2;
    }

    bool                    defense = FALSE;  // need to set all these damage reductions

    // to TRUE and check if FALSE to work.

    if(!IS_NPC(victim))
    {
      if(defense == FALSE && (victim->Class == CLASS_FISH || victim->race == RACE_DRAGON))
      {
        if(dam > 0 && victim->pcdata->learned[gsn_hardened_scales] > 0)
        {
          short                   skin;

          skin = number_range(1, 10);
          defense = TRUE;
          if(victim->Class == CLASS_FISH)
          {
            dam = (int)(dam / 2);
          }
          else
          {
            if(victim->pcdata->learned[gsn_hardened_scales] < 25 && skin > 4)
              dam = (int)(dam / 2);
            if((victim->pcdata->learned[gsn_hardened_scales] < 50 && victim->pcdata->learned[gsn_hardened_scales] > 24) && skin > 3)
              dam = (int)(dam / 4);
            if((victim->pcdata->learned[gsn_hardened_scales] < 100 && victim->pcdata->learned[gsn_hardened_scales] > 49))
              dam = (int)(dam / 5);
          }
          if(skin == 8)
            learn_from_success(victim, gsn_hardened_scales);
        }
      }
      if(defense == FALSE && (victim->Class == CLASS_BEAST))
      {
        if(dam > 0 && victim->pcdata->learned[gsn_shroud_spirit] > 0)
        {
          short                   skin;
          ...
          if(victim->pcdata->learned[gsn_shroud_spirit] < 25 && skin > 4)
            dam = (int)(dam / 10);
          if((victim->pcdata->learned[gsn_shroud_spirit] < 50 && victim->pcdata->learned[gsn_shroud_spirit] > 24) && skin > 3)
            dam = (int)(dam / 5);
          if((victim->pcdata->learned[gsn_shroud_spirit] < 100 && victim->pcdata->learned[gsn_shroud_spirit] > 49) && skin > 1)
            dam = (int)(dam / 2);
          if(skin <= 2)
            learn_from_success(victim, gsn_shroud_spirit);
        }
      }
      if(defense == FALSE && (victim->Class == CLASS_TREANT || victim->Class == CLASS_DRUID))
      {
        if(dam > 0 && victim->pcdata->learned[gsn_bark_skin] > 0)
        {
          short                   skin;
          ...
          if(victim->pcdata->learned[gsn_bark_skin] < 25 && skin > 4)
            dam = (int)(dam / 10);
          if((victim->pcdata->learned[gsn_bark_skin] < 50 && victim->pcdata->learned[gsn_bark_skin] > 24) && skin > 3)
            dam = (int)(dam / 5);
          if((victim->pcdata->learned[gsn_bark_skin] < 100 && victim->pcdata->learned[gsn_bark_skin] > 49) && skin > 1)
            dam = (int)(dam / 2);
          if(skin == 8)
            learn_from_success(victim, gsn_bark_skin);
        }
      }

     if (IS_AFFECTED(victim, AFF_SEARING_HAZE) && !IS_AFFECTED(ch, AFF_BLISTERING))
      {
      AFFECT_DATA             af;
      ...
      af.bitvector = meb(AFF_BLISTERING);
      affect_to_char(ch, &af);
      }


      if(IS_AFFECTED(victim, AFF_SANCTUARY)|| IS_AFFECTED(victim, AFF_BOOST ))
        dam /= 2;
    ...
    if(!IS_NPC(victim))
    {
      if(dam > 0 && defense == FALSE && (victim->pcdata->learned[gsn_iron_skin] > 0))
      {
        short                   iron = 0;
        ...
        if(iron == 10)
          learn_from_success(victim, gsn_iron_skin);
      }
    }
    bool                    shield = FALSE;
    short                   deflect;
    ...
    if((obj = get_eq_char(victim, WEAR_SHIELD)) != NULL && deflect >= 2)
    {
      ...
      {
        obj->value[0] = 1;
        act(AT_ACTION, "Your shield crumples around your arm and is now useless.", victim, NULL, NULL, TO_CHAR);
        act(AT_ACTION, "$n's shield crumples around $s arm and is now useless.", victim, NULL, NULL, TO_ROOM);
      }
    }
    ...
    if(dam > 0 && defense == FALSE && !IS_NPC(victim) && (victim->pcdata->learned[gsn_ethereal_body] > 0))
    {
      short                   meep = number_range(1, 10);

      defense = TRUE;
      dam = (int)(dam / 2);

      if(meep == 10)
        learn_from_success(victim, gsn_ethereal_body);

    }

    if(!IS_NPC(victim) && get_eq_char(victim, WEAR_WIELD) == NULL)
    {
      if(victim->pcdata->learned[gsn_pugilism] > 0 && (number_range(1, 20) > 15))
        learn_from_success(victim, gsn_pugilism);
    }

    if(dam > 0 && defense == FALSE && !IS_NPC(victim) && (victim->pcdata->learned[gsn_mitigate] > 0))
    {
      short                   mitty;
      ...
      // need checks so multiclass don't stack damage reductions
      if(victim->pcdata->learned[gsn_tumble] == 0)
      {
        dam = (int)(dam / 2);

        if(IS_AFFECTED(victim, AFF_HUNKER_DOWN))
          dam = (int)(dam / 2);
    ...
    if(dam > 0 && defense == FALSE && !IS_NPC(victim) && (victim->pcdata->learned[gsn_density] > 0))
    {
      short                   den;

      den = number_range(1, 10);
      defense = TRUE;
      if(victim->secondclass == -1)
      {
        ...
        if(den == 7)
          learn_from_success(victim, gsn_density);
      }
    }

    if(dam > 0 && defense == FALSE && !IS_NPC(victim) && (victim->pcdata->learned[gsn_tumble] > 0))
    {
      short                   tum;

      defense = TRUE;
      tum = number_range(1, 10);
      // don't let it stack for multiclass
      if(victim->pcdata->learned[gsn_mitigate] == 0)
      {
        dam = (int)(dam / 2);
    ...
    if(dam > 0 && defense == FALSE && !IS_NPC(victim) && (victim->pcdata->learned[gsn_marble_skin] > 0))
    {
      short                   skin;
    ...
    if(dam > 0 && defense == FALSE && !IS_NPC(victim) && (victim->pcdata->learned[gsn_glacial_armor] > 0))
    {
      short                   glacial;
    ...
    if(IS_VALID_SN(mod) && (skill = skill_table[mod]) != NULL
       ...
       && ch->Class != CLASS_ANGEL && ch->Class != CLASS_HELLSPAWN && ch->race != RACE_DRAGON && ch->Class != CLASS_DRAGONLORD && ch->Class != CLASS_BARD)
    {
      dam *= 2;
    }

    if(IS_VALID_SN(mod) && (skill = skill_table[mod]) != NULL && skill->type == SKILL_SPELL && NOT_FWARRIOR(ch) && NOT_SWARRIOR(ch) && NOT_TWARRIOR(ch) && ch->secondclass != -1)
    {
      dam = (int)(dam * 1.5);
    }
    ...
    if(mod == gsn_kick || mod == gsn_heavy_hands || mod == gsn_punch || mod == gsn_bite
       ...
       || mod == ATCK_STATICPULSE || mod == ATCK_BACKHAND || mod == ATCK_LOWBLOW || mod == ATCK_EBOLT
       || mod == ATCK_STUN || mod == ATCK_GOUGE || mod == ATCK_BACKSTAB || mod == ATCK_FEED
       || mod == ATCK_DRAIN || mod == ATCK_MENTALASSAULT || mod == ATCK_GSMITE
       || mod == ATCK_BREATHE || mod == ATCK_GUT || mod == ATCK_IMPALE
       || mod == ATCK_FROST || mod == ATCK_POISON || mod == ATCK_SLOW
       || mod == ATCK_THROATPUNCH || mod == ATCK_RAKE || mod == ATCK_THAITIN
       || mod == ATCK_JUDOCHOP || mod == ATCK_SHRIEKINGNOTE || mod == ATCK_MURAZORWHIP
       || mod == ATCK_BUTTERFLYKICK || mod == ATCK_SNATCH || mod == ATCK_FIREBALL
       || mod == ATCK_LIGHTNING || mod == ATCK_WEAKEN || mod == ATCK_PUGILISM || mod == ATCK_INFERNO || mod == ATCK_BLIZZARD || mod == ATCK_BLIGHTNING || mod == ATCK_BRAINBOIL || mod == ATCK_LTOUCH)
    {
      ch->success_attack = 1;
    }

    if(IS_VALID_SN(mod) && (skill = skill_table[mod]) != NULL && skill->type == SKILL_SPELL && ch->success_attack != 1)
    {
      ch->success_attack = 1;
    }
    else if(IS_VALID_SN(mod) && (skill = skill_table[mod]) != NULL && skill->type == SKILL_SPELL && victim->success_attack != 1)
    {
      victim->success_attack = 1;
    }
    else if(IS_VALID_SN(mod) && (skill = skill_table[mod]) != NULL && skill->type == SKILL_SKILL && ch->success_attack != 1)
    {
      ch->success_attack = 1;
    }
    else if(IS_VALID_SN(mod) && (skill = skill_table[mod]) != NULL && skill->type == SKILL_SKILL && dt > TYPE_HIT && victim->success_attack != 1)
    {
      victim->success_attack = 1;
    }

    if(IS_VALID_SN(mod) && (skill = skill_table[mod]) != NULL && skill->type == SKILL_SKILL && IS_NPC(victim))
    {
      dam = (int)(dam * 1.8);
    }
    ...
    if(dt >= TYPE_HIT && ch->in_room == victim->in_room)
    {
      ...
      {
        if(IS_NPC(ch) && xIS_SET(ch->attacks, ATCK_BACKHAND) && ch->level > 5 && number_percent() < ch->level / 2)
          trip(ch, victim);
      }
...
// Volk Jan-16: Bug fix to prevent 0 damage when above settings are below 100%
    if(dampmod > 0)
    {
      dam = (dam * dampmod);
      if(dam > 0 && dam < 100)
  ...
  if(dam > 50 && dt != TYPE_UNDEFINED && dt != gsn_poison)
  {
    ...
    if(!in_arena(victim))
    {
      dameq = number_range(WEAR_LIGHT, WEAR_R_WING);
      damobj = get_eq_char(victim, dameq);
      if(damobj)
      {
        if(dam > get_obj_resistance(damobj) && number_bits(1) == 0 && number_chance(1, 10) >= 8)
        {
          set_cur_obj(damobj);
          damage_obj(damobj);
        }
  ...
  short                   arrow = number_range(1, 4);

  if(!IS_NPC(ch))
  {
    if(ch->pcdata->learned[gsn_archery] > 80)
      arrow += 1;
  ...
  if(IS_AFFECTED(victim, AFF_REACTIVE) && dam > 0 && victim->hit > 0 && number_percent() > 40 && chance > 1)
  {
    act(AT_MAGIC, "$n reactively heals some damage.", victim, NULL, NULL, TO_ROOM);
    act(AT_MAGIC, "An energy fills you and you reactively heal some of the damage.", victim, NULL, NULL, TO_CHAR);
    if(victim->level < 20)
    ...
    if(victim->level > 59 && victim->level < 80)
      victim->hit += number_range(18, 72);
    if(victim->level > 79)
      victim->hit += number_range(40, 120);
  ...
  // may
  // adjust later.
  if((IS_AFFECTED(victim, AFF_CRAFTED_FOOD1) || IS_AFFECTED(victim, AFF_CRAFTED_FOOD2) || IS_AFFECTED(victim, AFF_CRAFTED_FOOD3)) && number_percent() > 40 && chance > 2)
  {
    act(AT_MAGIC, "Your belly feels warm as the crafted food regeneration begins.", victim, NULL, NULL, TO_CHAR);

    if(IS_AFFECTED(victim, AFF_CRAFTED_FOOD1) && (!IS_NPC(victim)))
      victim->hit += (int)(double)number_range(1, 12);
    if(IS_AFFECTED(victim, AFF_CRAFTED_FOOD2) && (!IS_NPC(victim)))
      victim->hit += (int)(double)number_range(3, 18);
    if(IS_AFFECTED(victim, AFF_CRAFTED_FOOD3) && (!IS_NPC(victim)))
      victim->hit += (int)(double)number_range(6, 36);
    if(victim->hit > victim->max_hit)
      victim->hit = victim->max_hit;
  }
   ...
   */
  if(dam && ch != victim && !IS_NPC(ch) && ch->fighting && ch->fighting->xp && (!IS_NPC(victim) || !xIS_SET(victim->act, ACT_IMMORTAL)))
  {
    int                     xp_gain = 0;
   ...
   */
  if(!IS_NPC(victim) && (victim->level >= LEVEL_IMMORTAL || NEW_AUTH(victim)) && victim->hit < 1)
    victim->hit = 1;
   ...
   */
  if(IS_BLOODCLASS(victim) && victim->level < 50)
  {
  ...
  if(IS_BLOODCLASS(victim) && victim->level > 49)
  {
    if(dam >= (victim->hit / 4))
      victim->blood = (victim->blood - 1 - (victim->level / 20));

    if(victim->hit <= (victim->max_hit / 4) && victim->blood > 5)
    {
      if(victim->blood >= victim->blood / 2)
      {
        victim->hit += URANGE(12, (victim->max_hit / 30), victim->level * 2);
        victim->blood = (victim->blood - URANGE(6, victim->level / 10, 12));
      }
      else
      {
        victim->blood = (victim->blood - URANGE(3, victim->level / 10, 8));
        victim->hit += URANGE(4, (victim->max_hit / 30), victim->level * 2);
      }
      set_char_color(AT_BLOOD, victim);
      send_to_char("You howl with rage as the beast within stirs!\r\n", victim);
    }
  ...
  {
    case POS_MORTAL:
      act(AT_RED, "$n is mortally wounded, and will die soon, if not aided.", victim, NULL, NULL, TO_ROOM);
      act(AT_RED, "You are mortally wounded, and will die soon, if not aided.", victim, NULL, NULL, TO_CHAR);
      break;

    case POS_INCAP:
      act(AT_RED, "$n is incapacitated and will slowly die, if not aided.", victim, NULL, NULL, TO_ROOM);
      act(AT_RED, "You are incapacitated and will slowly die, if not aided.", victim, NULL, NULL, TO_CHAR);
      break;
    ...
    case POS_STUNNED:
      if(!IS_AFFECTED(victim, AFF_PARALYSIS))
      {
        ...
        act(AT_RED, "You are stunned, but will probably recover.", victim, NULL, NULL, TO_CHAR);
      }
      break;
    ...
    default:
      ...
      {
        act(AT_RED, "You wish that your wounds would stop BLEEDING so much!", victim, 0, 0, TO_CHAR);
        if(number_bits(2) == 0)
          worsen_mental_state(victim, 1);
      }
      break;
  ...
  if(victim->position == POS_DEAD)
  {
    ...
    if ( !IS_NPC(ch) ){
      ...
      if(victim != ch && IS_CLANNED(ch) && IS_CLANNED(victim) && victim->pcdata->clan == clan)
      {
      ...
      clan = ch->pcdata->clan;
        ...
        if(ch->pcdata->clan)
        {
          ...
          clan->totalpoints += 3;
          ch_printf(ch, "&G%s clan has gained 3 points from your kill, now totaling %d clan status points!\r\n", clan->name, clan->totalpoints);
    ...
    if(!npcvict)
    {
      if(IS_THIRDCLASS(victim))
      {
        ...
        {
          victim->firstexp -= (int)(exp_class_level(victim, victim->firstlevel + 1, victim->Class) * .8);
        }
        else if(!xIS_SET(victim->act, PLR_EXTREME))
        {
          victim->firstexp -= (int)(exp_class_level(victim, victim->firstlevel + 1, victim->Class) * .5);
        }
        ...
        if(xIS_SET(victim->act, PLR_EXTREME))
        {
          victim->secondexp -= (int)(exp_class_level(victim, victim->secondlevel + 1, victim->secondclass) * .8);
        }
        else if(!xIS_SET(victim->act, PLR_EXTREME))
        {
          victim->secondexp -= (int)(exp_class_level(victim, victim->secondlevel + 1, victim->secondclass) * .5);
        }
        ...
        if(xIS_SET(victim->act, PLR_EXTREME))
        {
          victim->thirdexp -= (int)(exp_class_level(victim, victim->thirdlevel + 1, victim->thirdclass) * .8);
        }
        else if(!xIS_SET(victim->act, PLR_EXTREME))
        {
          victim->thirdexp -= (int)(exp_class_level(victim, victim->thirdlevel + 1, victim->thirdclass) * .5);
        }
      ...
      else
      {
        ...
        {
          victim->exp -= (int)(exp_level(victim, victim->level + 1) * .5);
        }
        if(victim->exp < 0)
          victim->exp = 0;
      }
    ...
    { /* keep track of mob vnum killed */
      if(!happyhouron || !IS_GROUPED(ch) || sysdata.happy)
      {
        add_kill(ch, victim);
        /*
      ...
      if(xIS_SET(ch->act, PLR_AUTOMONEY))
      {
        ...
        if((corpse = get_obj_here(ch, (char *)"corpse")))
        {
          ...
          for(mobj = corpse->first_content; mobj; mobj = mobj->next_content)
          {
            ...
            do_get(ch, mbuf); /* Get the gold/silver from the corpse */
            ...
            if(diff > 1)
            {
              for(gch = ch->in_room->first_person; gch; gch = gch->next_in_room)
              {
                ...
                {
                  members = TRUE;
                  snprintf(buf1, MSL, "%d %s", diff, curr_types[mobj->value[2]]);
                  do_split(ch, buf1);
                }
              }
              /*
...
bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim, bool show_messg)
{
  ...
  if(!victim)
  { /* Gonna find this is_safe crash bug -Blod */
  ...
  if(IS_PACIFIST(victim) && !IS_AFFECTED(victim, AFF_DISABLED))
  { /* Gorog */
    if(show_messg)
    {
      set_char_color(AT_MAGIC, ch);
  ...
  if(!IS_NPC(ch) && !IS_NPC(victim) && ch != victim && (IS_SET(victim->in_room->area->flags, AFLAG_ANTIPKILL) || IS_SET(victim->in_room->area->flags, AFLAG_NOPKILL)))
  {
    if(show_messg)
    {
      set_char_color(AT_IMMORT, ch);
      send_to_char("The staff have forbidden player killing in this area.\r\n", ch);
    }
    return TRUE;
  }
  ...
  if(IS_NPC(ch) && IS_AFFECTED(ch, AFF_CHARM) && (ch->level - victim->level > 10) && !in_arena(ch))
    return FALSE;
  ...
  { /* 10 and lower */
    if((ch->level - victim->level >= 3) || (victim->level - ch->level >= 3))
    {
      ...
      {
        set_char_color(AT_IMMORT, ch);
        send_to_char("The STAFF of 6 Dragons do not allow murder when there is such a difference in level.\r\n", ch);
      }
      return TRUE;
    }
  }

  if(ch->level > 29 && !in_arena(ch))
  { /* 30 and upwards */
  ...
  else if(ch->level < 30 && !in_arena(ch))
  { /* less than 30 */
...
void set_distance(CHAR_DATA *ch, int distance)
{
  ...
  if(ch->fighting->who && ch->fighting->who->fighting && ch->fighting->who->fighting->who == ch)
  {
...
//    ch->fighting->who->fighting->distance = ch->fighting->distance;
    ...
    act(AT_GREEN, "Yelling a battle cry you rush in towards $N.", ch, NULL, ch->fighting->who, TO_CHAR);
    act(AT_GREEN, "$n yelling a battle cry rushes in towards you.", ch, NULL, ch->fighting->who, TO_VICT);
    act(AT_GREEN, "$n yelling a battle cry rushes in towards $N.", ch, NULL, ch->fighting->who, TO_NOTVICT);
    WAIT_STATE(ch, 12);
  }
}
...
// stop arena fleeing from saying multiple characers

  if(!in_arena(ch) && !IS_NPC(victim))
  {
    if(victim->fightingip != NULL && !IS_NPC(ch))
    {
      if(ch->pcdata->lastip)
      {
        if(!str_cmp(victim->fightingip, ch->pcdata->lastip))
        {
          send_to_char("You are already fighting them with one of your characters.\r\n", ch);
          return;
        }
      }
    }
  ...
  if(!IS_NPC(ch) && !IS_NPC(victim))
  {
    if(ch->pcdata->lastip)
    {
      if(victim->fightingip)
      {
        STRFREE(victim->fightingip);
      }
      victim->fightingip = STRALLOC(ch->pcdata->lastip);
    }
  }
...
void stop_fighting(CHAR_DATA *ch, bool fBoth)
{
  ...
  for(fch = first_char; fch; fch = fch->next)
  {
    ...
    {
      free_fight(fch);
// make sure feign death and gust of wind work properly - Vladaar
      if(fch->fightingip != NULL)
      {
        STRFREE(fch->fightingip);
      }
...
void death_cry(CHAR_DATA *ch)
{
  ...
  if(!ch)
  {
    bug("%s DEATH_CRY: null ch!", __FUNCTION__);
    return;
  }
...
void raw_kill(CHAR_DATA *ch, CHAR_DATA *victim)
{
  ...
  {
    bug("%s: killing someone still in tutorial.", __FUNCTION__);
    return;
  }
  ...
  if(!IS_NPC(ch))
  {
    ...
    if(victim->level > ch->level)
      ch->pcdata->failsafe = victim->level - ch->level;
  }

// This check will be for Pet fighting alongside beast master killed mob 
  if(xIS_SET(ch->act, ACT_BEASTPET) && !xIS_SET(ch->act, ACT_BEASTMELD))
  {
    ...
    if(master)
    {
      if(ch->level - master->level > 15)
      {
        send_to_char("Your pet cannot gain any more experience until you level the beastmaster.\r\n", master);
      }
      else
      {
        if(master->pcdata->petlevel < 100)
        {
          master->pcdata->petexp += 1;
          gain_exp(master, number_range(5000, 12000));
          ch_printf(master, "&WYour pet gains experience from this battle!\r\n");
        }

        if(master->pcdata->petexp > 17)
        {
          master->pcdata->petexp = 0;
          if(master->pcdata->petlevel < 100)
          {
            master->pcdata->petlevel += 1;
            ch_printf(master, "&WYour pet has advanced to level %d!\r\n", master->pcdata->petlevel);
            if(xIS_SET(ch->act, PLR_COMMUNICATION))
              send_to_char("!!SOUND(sound/level.wav)\r\n", ch);

            ch->level += 1;
            ch->max_hit += number_range(60, 80);
            ch->hit = ch->max_hit;
            master->pcdata->pethp = ch->max_hit;
            set_hitroll(ch->level);
            set_damroll(ch->level);
          }
        }
      }
    }
...
// This check will be for Pet fighting alongside beast master killed mob 
  if(victim->level >= ch->level)
  {
    if(BEASTMELD(ch))
    {
      ...
      if(ch->level - temp->level > 15)
      {
        send_to_char("You cannot gain any more experience until you level your beastmaster.\r\n", ch);
      }
      else
      {
        if(temp->pcdata->petlevel < 100)
        {
          short                   points = 0;

          points = number_range(1, 4);
          temp->pcdata->petexp += points;
          ch_printf(ch, "&WYour pet gains %d experience points from this battle!\r\n", points);
        }

        if(temp->pcdata->petexp > 17)
        {
          temp->pcdata->petexp = 0;
          if(temp->pcdata->petlevel < 100)
          {
            temp->pcdata->petlevel += 1;
            ch_printf(ch, "&WYour pet has advanced to level %d!\r\n", temp->pcdata->petlevel);
            if(xIS_SET(ch->act, PLR_COMMUNICATION) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_COMMUNICATION)))
              send_to_char("!!SOUND(sound/level.wav)\r\n", ch);
            ch->level += 1;
            ch->max_hit += number_range(60, 80);
            ch->hit = ch->max_hit;
            ch->master->pcdata->pethp = ch->max_hit;
            set_hitroll(ch->level);
            set_damroll(ch->level);
          }
        }
...
// if Beast Master killed the mob Give pet gain 
  if(ch->Class == CLASS_BEAST && !xIS_SET(ch->act, ACT_BEASTMELD) && (victim->level >= ch->level))
  {
// Only perform if the pet is present in the same room 
    CHAR_DATA              *beastpet;

    for(beastpet = first_char; beastpet; beastpet = beastpet->next)
    {
      if(xIS_SET(beastpet->act, ACT_BEASTPET) && beastpet->master)
        if(!str_cmp(ch->name, beastpet->master->name))
          if(ch->in_room->vnum == beastpet->in_room->vnum)
          {
            if(ch->level - beastpet->level > 15)
            {
              send_to_char("Your pet cannot gain anymore levels, until you level the Beastmaster.\r\n", ch);
            }
            else
            {
              if(ch->pcdata->petlevel < 100)
              {
                ch->pcdata->petexp += 1;
                ch_printf(ch, "&WYour pet gains experience from this battle!\r\n");
              }
              if(ch->pcdata->petexp > 17)
              {
                ch->pcdata->petexp = 0;
                if(ch->pcdata->petlevel < 100)
                {
                  ch->pcdata->petlevel += 1;
                  if(xIS_SET(ch->act, PLR_COMMUNICATION))
                    send_to_char("!!SOUND(sound/level.wav)\r\n", ch);

                  ch_printf(ch, "&WYour pet has advanced to level %d!\r\n", ch->pcdata->petlevel);
                  beastpet->level += 1;
                  beastpet->max_hit += number_range(60, 80);
                  beastpet->hit = beastpet->max_hit;
                  ch->pcdata->pethp = beastpet->max_hit;
                  set_hitroll(beastpet->level);
                  set_damroll(beastpet->level);
                }
              }
            }
  ...
  if(victim->race == RACE_ANIMAL)
  {
    affect_strip(victim, gsn_animal_form);
    xREMOVE_BIT(victim->affected_by, AFF_ANIMALFORM);
    animal_change(victim, FALSE);
  ...
  if(VLD_STR(victim->fightingip))
  {
    STRFREE(victim->fightingip);
    victim->fightingip = NULL;
  }
  ...
  if(IS_AFFECTED(victim, AFF_SHRINK))
  {
    ...
    if(!IS_NPC(victim))
    {
        if(victim->height != race_table[victim->race]->height)
          victim->height = race_table[victim->race]->height;
        if(victim->weight != race_table[victim->race]->weight)
          victim->weight = race_table[victim->race]->weight;
  ...
  {
    xREMOVE_BIT(victim->affected_by, AFF_ENLARGE);
    if(!IS_NPC(victim))
    {
        if(victim->height != race_table[victim->race]->height)
          victim->height = race_table[victim->race]->height;
  ...
  {
    xREMOVE_BIT(victim->affected_by, AFF_INSANE);
  }
  ...
  if(IS_NPC(victim))
  {
    if(victim->pIndexData->vnum == MOB_VNUM_SOLDIERS || victim->pIndexData->vnum == MOB_VNUM_ARCHERS || victim->pIndexData->vnum == MOB_VNUM_GUARDS || victim->pIndexData->vnum == MOB_VNUM_WARRIORS)
    {
      short                   rand;
      ...
      if(rand > 6)
      {
        send_to_char("&WYou gain one glory for killing the enemy in this siege!\r\n", ch);
        ch->quest_curr += 1;
      }
      else
      {
      ...
      if(!IS_NPC(ch))
      {
        if(ch->pcdata->city)
        {
          ...
          if(ch->in_room->vnum == PALEON_DEFEND || ch->in_room->vnum == PALEON_DEFEND2 || ch->in_room->vnum == PALEON_DEFEND3)
          {
            city = get_city("Paleon City");
            if(city->dguards > 0 && city->defense > 0)
            {
              city->dguards -= 1;
              city->defense -= 1;
            }
          ...
          else if(ch->in_room->vnum == FORBIDDEN_DEFEND || ch->in_room->vnum == FORBIDDEN_DEFEND2 || ch->in_room->vnum == FORBIDDEN_DEFEND3)
          {
          ...
          else if(ch->in_room->vnum == DAKAR_DEFEND || ch->in_room->vnum == DAKAR_DEFEND2 || ch->in_room->vnum == DAKAR_DEFEND3)
          {
          ...
          else if(ch->in_room->vnum == DAKAR_SIEGE || ch->in_room->vnum == DAKAR_SIEGE2 || ch->in_room->vnum == DAKAR_SIEGE3)
          {
            city = get_city(ch->pcdata->city_name);
            if(city->osoldiers > 0 && city->offense > 0)
            {
              city->osoldiers -= 1;
              city->offense -= 1;
            }
          ...
          else if(ch->in_room->vnum == FORBIDDEN_SIEGE || ch->in_room->vnum == FORBIDDEN_SIEGE2 || ch->in_room->vnum == FORBIDDEN_SIEGE3)
          {
            city = get_city(ch->pcdata->city_name);
            if(city->osoldiers > 0 && city->offense > 0)
            {
          ...
          else if(ch->in_room->vnum == PALEON_SIEGE || ch->in_room->vnum == PALEON_SIEGE2 || ch->in_room->vnum == PALEON_SIEGE3)
          {
            city = get_city(ch->pcdata->city_name);
            if(city->osoldiers > 0 && city->offense > 0)
            {
...
/* Hack way to try to make players think they get some exp gains from mounts when crusader or shadowknight */
  if(ch->Class == CLASS_BEAST)
  {
    ...
    if(beast == TRUE && ch->in_room->vnum == mob->in_room->vnum && victim->level >= ch->level)
    {
      int                     extra = 0;

      extra = number_range(ch->level * 100, ch->level * 1000) + (get_curr_lck(ch) * 2);
      if(extra > 32000)
        extra = 32000 + number_range(1, 1000);
      if(ch->pcdata->petaffection < 50)
        ch->pcdata->petaffection += 1;

      ch_printf(ch, "&WYour affinity with animals in battle gains you %d experience.\r\n", extra);
      gain_exp(ch, extra);
    }
  }
...
/* Hack way to try to make players think they get some exp gains from mounts when crusader or shadowknight */
  ...
  {
    if(ch->mount && ch->in_room == ch->mount->in_room && IS_STEED(ch) && victim->level >= ch->level)
    {
      int                     extra = 0;
      ...
      if(extra > 32000)
        extra = 32000 + number_range(1, 1000);
      ch_printf(ch, "&WYour affinity with your warhorse in battle gains you %d experience.\r\n", extra);
      gain_exp(ch, extra);
    }
  }
  ...
  if(!IS_NPC(ch))
  {
    ...
    if(IS_NPC(victim))
    {
      if(ch->pcdata->clan && victim->level >= ch->level)
      {
        ...
        clan = ch->pcdata->clan;
        ch_printf(ch, "&G%s clan has gained a point from your kill, now totaling %d clan status points!\r\n", clan->name, clan->totalpoints);
...
/*  Not sure where else to put the announcement - Volk  */
  ...
  {
    if(victim != ch)
      snprintf(buf, MSL, "%s has been killed by %s!", victim->name, (IS_NPC(ch) ? ch->short_descr : ch->name));
    else
      snprintf(buf, MSL, "%s has been killed!", victim->name);
    announce(buf);

    if(victim != ch)
      snprintf(buf, MSL, "%s (%d) killed by %s at %d", victim->name, victim->level, (IS_NPC(ch) ? ch->short_descr : ch->name), victim->in_room->vnum);
    else
      snprintf(buf, MSL, "%s (%d) bled to death at %d", victim->name, victim->level, victim->in_room->vnum);
    log_string(buf);
    to_channel(buf, "Monitor", LEVEL_IMMORTAL);

  }

  if(xIS_SET(victim->act, PLR_BATTLE) || (xIS_SET(victim->act, ACT_BEASTMELD) && xIS_SET(victim->master->act, PLR_BATTLE)))
  {
...
/*
   if( IS_AFFECTED( mob, AFF_BLADEBARRIER ) )
      flags += 2;
*/
  if(IS_RESIS(mob, RIS_FIRE) || IS_RESIS(mob, RIS_COLD) || IS_RESIS(mob, RIS_ELECTRICITY) || IS_RESIS(mob, RIS_ENERGY) || IS_RESIS(mob, RIS_ACID))
    flags++;

  if(IS_RESIS(mob, RIS_BLUNT) || IS_RESIS(mob, RIS_SLASH) || IS_RESIS(mob, RIS_PIERCE) || IS_RESIS(mob, RIS_HACK) || IS_RESIS(mob, RIS_LASH))
    flags++;

  if(IS_RESIS(mob, RIS_SLEEP) || IS_RESIS(mob, RIS_CHARM) || IS_RESIS(mob, RIS_HOLD) || IS_RESIS(mob, RIS_POISON) || IS_RESIS(mob, RIS_PARALYSIS))
    flags++;

  if(IS_RESIS(mob, RIS_PLUS1) || IS_RESIS(mob, RIS_PLUS2) || IS_RESIS(mob, RIS_PLUS3) || IS_RESIS(mob, RIS_PLUS4) || IS_RESIS(mob, RIS_PLUS5) || IS_RESIS(mob, RIS_PLUS6))
    flags++;
  ...
  if(IS_IMMUNE(mob, RIS_FIRE) || IS_IMMUNE(mob, RIS_COLD) || IS_IMMUNE(mob, RIS_ELECTRICITY) || IS_IMMUNE(mob, RIS_ENERGY) || IS_IMMUNE(mob, RIS_ACID))
    flags += 2;

  if(IS_IMMUNE(mob, RIS_BLUNT) || IS_IMMUNE(mob, RIS_SLASH) || IS_IMMUNE(mob, RIS_PIERCE) || IS_IMMUNE(mob, RIS_HACK) || IS_IMMUNE(mob, RIS_LASH))
    flags += 2;

  if(IS_IMMUNE(mob, RIS_SLEEP) || IS_IMMUNE(mob, RIS_CHARM) || IS_IMMUNE(mob, RIS_HOLD) || IS_IMMUNE(mob, RIS_POISON) || IS_IMMUNE(mob, RIS_PARALYSIS))
    flags += 2;

  if(IS_IMMUNE(mob, RIS_PLUS1) || IS_IMMUNE(mob, RIS_PLUS2) || IS_IMMUNE(mob, RIS_PLUS3) || IS_IMMUNE(mob, RIS_PLUS4) || IS_IMMUNE(mob, RIS_PLUS5) || IS_IMMUNE(mob, RIS_PLUS6))
    flags += 2;
  ...
  if(IS_SUSCEP(mob, RIS_FIRE) || IS_SUSCEP(mob, RIS_COLD) || IS_SUSCEP(mob, RIS_ELECTRICITY) || IS_SUSCEP(mob, RIS_ENERGY) || IS_SUSCEP(mob, RIS_ACID))
    flags -= 2;

  if(IS_SUSCEP(mob, RIS_BLUNT) || IS_SUSCEP(mob, RIS_SLASH) || IS_SUSCEP(mob, RIS_PIERCE) || IS_SUSCEP(mob, RIS_HACK) || IS_SUSCEP(mob, RIS_LASH))
    flags -= 3;

  if(IS_SUSCEP(mob, RIS_SLEEP) || IS_SUSCEP(mob, RIS_CHARM) || IS_SUSCEP(mob, RIS_HOLD) || IS_SUSCEP(mob, RIS_POISON) || IS_SUSCEP(mob, RIS_PARALYSIS))
    flags -= 3;

  if(IS_SUSCEP(mob, RIS_PLUS1) || IS_SUSCEP(mob, RIS_PLUS2) || IS_SUSCEP(mob, RIS_PLUS3) || IS_SUSCEP(mob, RIS_PLUS4) || IS_SUSCEP(mob, RIS_PLUS5) || IS_SUSCEP(mob, RIS_PLUS6))
    flags -= 2;
...
*/
  ...
  if(IS_ATTACK(mob, ATCK_MENTALASSAULT) || IS_ATTACK(mob, ATCK_BREATHE) || IS_ATTACK(mob, ATCK_GUT) || IS_ATTACK(mob, ATCK_IMPALE) || IS_ATTACK(mob, ATCK_FROST) || IS_ATTACK(mob, ATCK_GSMITE))
    flags += 2;

  if(IS_ATTACK(mob, ATCK_JUDOCHOP) || IS_ATTACK(mob, ATCK_FIREBALL) || IS_ATTACK(mob, ATCK_LIGHTNING))
    flags += 2;

  if(IS_ATTACK(mob, ATCK_PUGILISM) || IS_ATTACK(mob, ATCK_ACIDSPRAY) || IS_ATTACK(mob, ATCK_SWARM)
     || IS_ATTACK(mob, ATCK_ASTRALATTACK) || IS_ATTACK(mob, ATCK_PARALYZE) || IS_ATTACK(mob, ATCK_FEAR) || IS_ATTACK(mob, ATCK_STOMP))
    flags += 2;

  if(IS_ATTACK(mob, ATCK_BITE) || IS_ATTACK(mob, ATCK_CLAWS)
     || IS_ATTACK(mob, ATCK_TAIL_SWIPE) || IS_ATTACK(mob, ATCK_STATICPULSE) || IS_ATTACK(mob, ATCK_PLUCK)
     || IS_ATTACK(mob, ATCK_PUNCH) || IS_ATTACK(mob, ATCK_KICK) || IS_ATTACK(mob, ATCK_FEROCIOUSSTRIKE)
     || IS_ATTACK(mob, ATCK_BACKHAND) || IS_ATTACK(mob, ATCK_GOUGE) || IS_ATTACK(mob, ATCK_PSIONICBLAST)
     || IS_ATTACK(mob, ATCK_DRAIN) || IS_ATTACK(mob, ATCK_POISON) || IS_ATTACK(mob, ATCK_CRITIC)
     || IS_ATTACK(mob, ATCK_SLOW) || IS_ATTACK(mob, ATCK_THROATPUNCH) || IS_ATTACK(mob, ATCK_CRANEKICK)
     || IS_ATTACK(mob, ATCK_RAKE) || IS_ATTACK(mob, ATCK_THAITIN) || IS_ATTACK(mob, ATCK_BLURATTACK)
     || IS_ATTACK(mob, ATCK_SHRIEKINGNOTE) || IS_ATTACK(mob, ATCK_MURAZORWHIP) || IS_ATTACK(mob, ATCK_FLYBY)
     || IS_ATTACK(mob, ATCK_BUTTERFLYKICK) || IS_ATTACK(mob, ATCK_SNATCH) || IS_ATTACK(mob, ATCK_HEADBUTT)
     || IS_ATTACK(mob, ATCK_WEAKEN) || IS_ATTACK(mob, ATCK_HHANDS) || IS_ATTACK(mob, ATCK_CORROSIVE))
    flags++;
  ...
  if(IS_DEFENSE(mob, DFND_NETTLESKIN))
    flags++;
  ...
  if(IS_DEFENSE(mob, DFND_SPECTRALWARD) || IS_DEFENSE(mob, DFND_SHIELD)
     || IS_DEFENSE(mob, DFND_TORPOR) || IS_DEFENSE(mob, DFND_KINETICBARRIER)
     || IS_DEFENSE(mob, DFND_HAWKSPIRIT)
     || IS_DEFENSE(mob, DFND_LICK) || IS_DEFENSE(mob, DFND_COMBATHEAL)
     || IS_DEFENSE(mob, DFND_WILLOWSKIN)
     || IS_DEFENSE(mob, DFND_GRENDALSTANCE) || IS_DEFENSE(mob, DFND_WOLFSPIRIT)
     || IS_DEFENSE(mob, DFND_REGENERATE) || IS_DEFENSE(mob, DFND_MAINTAINPOSITION)
     || IS_DEFENSE(mob, DFND_HUNKERDOWN) || IS_DEFENSE(mob, DFND_FINDWEAKNESS) || IS_DEFENSE(mob, DFND_BEARSPIRIT) || IS_DEFENSE(mob, DFND_MINORRESTORE))
    flags++;
...
void group_gain(CHAR_DATA *ch, CHAR_DATA *victim)
{
  ...
  int                     xp = 0, max_level = 0, members = 0;
  ...
  if(IS_NPC(ch))
  {
    if(ch->leader)
    {
      if(!IS_NPC(ch->leader) && ch->leader == ch->master && IS_AFFECTED(ch, AFF_CHARM) && ch->in_room == ch->leader->in_room)
        ch = ch->master;
    }
  }
 ...
 *  divide by 10 too (so potential to gain more experience than normal for lower/higher mobs) */
  ...
  if(!IS_NPC(gch))
  {
    if(xp < 1000)
    {
      if(gch->pcdata->failsafe == 1)
        xp += number_range(300, 600);
      else if(gch->pcdata->failsafe == 2)
        xp += number_range(700, 1000);
      else if(gch->pcdata->failsafe == 3)
        xp += number_range(1000, 2000);
      else if(gch->pcdata->failsafe == 4)
        xp += number_range(2000, 3000);
      else if(gch->pcdata->failsafe > 4)
        xp += number_range(3000, 8000);
    }
  }
...
void new_dam_message(CHAR_DATA *ch, CHAR_DATA *victim, int dam, unsigned int dt, OBJ_DATA *obj)
{
  ...
  bool                    gcflag = FALSE;
  bool                    gvflag = FALSE;
  int                     d_index = 1, w_index;
  ROOM_INDEX_DATA        *was_in_room;
  short                   chance;


  if ( IS_STAFF(victim) && victim->max_hit == 0 )
    victim->max_hit = victim->hit;  // crash fix for staff that use life command with zero max_hp in battle?
...
//      bug ( "Dam_message: bad dt %d from %s in %d.", dt, ch->name, ch->in_room->vnum );
  ...
  if(ch->success_attack > 0 && (w_index != 0 || dt == TYPE_HIT + DAM_HIT) && !IS_AFFECTED(victim, AFF_DISABLED))
  {
    if(xIS_SET(ch->act, PLR_BATTLE) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_BATTLE)))
      send_to_char("!!SOUND(sound/playerhit.wav)\r\n", ch);

  }
  if(ch->success_attack < 1 && (w_index != 0 || dt == TYPE_HIT + DAM_HIT) && !IS_AFFECTED(victim, AFF_DISABLED))
  {
    if(xIS_SET(ch->act, PLR_BATTLE) || (xIS_SET(ch->act, ACT_BEASTMELD) && xIS_SET(ch->master->act, PLR_BATTLE)))
      send_to_char("!!SOUND(sound/miss.wav)\r\n", ch);
...
// Add failsafe here for w_index == 0 even though wielded this alos makes sn == -1 which I haven't solved yet
  OBJ_DATA               *weapon;
  ...
  if(dt == TYPE_HIT && (!IS_NPC(ch) && ch->martialarts == 0 && (ch->pcdata->learned[gsn_martial_arts] > 0)))
  {

    if(ch->level > 49 && chance <= 5) // small proc at level 50 to compete with small
      // proc grapple has
      interpret(ch, (char *)"judo");
    if(chance <= 10)
    {
      snprintf(buf1, 256, "$n's elbow %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your elbow %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's elbow %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 30)
    {
      snprintf(buf1, 256, "$n's palmheel attack %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your palmheel attack %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's palmheel attack %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 40)
    {
      snprintf(buf1, 256, "$n's sidekick %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your sidekick %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's sidekick %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 50)
    {
      snprintf(buf1, 256, "$n's knifehand strike %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your knifehand strike %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's knifehand strike %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 60)
    {
      snprintf(buf1, 256, "$n's axekick %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your axekick %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's axekick %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 70)
    {
      snprintf(buf1, 256, "$n's hammer fist %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your hammer fist %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's hammer fist %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    // Following three added in by Taon.
    else if(chance <= 80)
    {
      snprintf(buf1, 256, "$n's right hook %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your right hook %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's right hook %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);

    }
    else if(chance <= 90)
    {
      snprintf(buf1, 256, "$n's left hook %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your left hook %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's left hook %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else
    {
      snprintf(buf1, 256, "$n's jab %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your jab %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's jab %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
  }
  else if(ch->martialarts == 1 && (ch->pcdata->learned[gsn_grapple] > 0) && get_eq_char(ch, WEAR_WIELD) == NULL)
  {
    if(ch->level > 49 && chance <= 5) // small proc at level 50 to compete with small
      // proc grapple has
      interpret(ch, (char *)"disarm");
    ...
    if(chance <= 10)
    {
      snprintf(buf1, 256, "$n's guillotine choke %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your guillotine choke %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's guillotine choke %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 30)
    {
      snprintf(buf1, 256, "$n's arm bar %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your arm bar %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's arm bar %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 40)
    {
      snprintf(buf1, 256, "$n's rear naked choke %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your rear naked choke %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's rear naked choke %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 50)
    {
      snprintf(buf1, 256, "$n's ankle lock %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your ankle lock %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's ankle lock %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 60)
    {
      snprintf(buf1, 256, "$n's arm triangle %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your arm triangle %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's arm triangle %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 70)
    {
      snprintf(buf1, 256, "$n's hip throw %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your hip throw %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's hip throw %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else if(chance <= 80)
    {
      snprintf(buf1, 256, "$n's half nelson %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your half nelson %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's half nelson %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);

    }
    else if(chance <= 90)
    {
      snprintf(buf1, 256, "$n's head lock %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your head lock %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's head lock %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else
    {
      snprintf(buf1, 256, "$n's elbow crank %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your elbow crank %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's elbow crank %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
  }
  else if(ch->martialarts == 3 && !IS_NPC(ch))
  {
    short                   bustamove = number_range(1, 100);

    dam = dam + get_curr_dex(ch);

    if(bustamove < 30)
    {
      snprintf(buf1, 256, "Moving with the morris dance $n's left knee lauches upward and %s $N %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Moving with the morris dance your left knee launches upward and %s $N %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "Moving with the morris dance $n's left knee launches upward and %s you %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }

    if(bustamove < 60 && bustamove > 29)
    {
      snprintf(buf1, 256, "$n's held out arms from the morris dance smack %s $N %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your held out arms from the morris dance smack %s $N %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's held out arms from the morris dance smack %s you %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }

    if(bustamove > 59)
    {
      snprintf(buf1, 256, "Moving with the morris dance $n leaps into the air which %s $N %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Moving with the morris dance you leap into the air which %s $N %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "Moving with the morris dance $n leaps into the air which %s you %d %s for %d total damage%c", vp,
               ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
  }

  else if(!IS_NPC(ch) && ch->pcdata->learned[gsn_pugilism] > 0 && get_eq_char(ch, WEAR_WIELD) == NULL && get_eq_char(ch, WEAR_MISSILE_WIELD) == NULL && ch->Class != CLASS_MONK && ch->martialarts == 0
          && ch->race != RACE_ANIMAL && ch->race != RACE_DRAGON && dt < (unsigned int)TYPE_HIT + sizeof(attack_table) / sizeof(attack_table[0]) && !IS_VALID_SN(dt))
  {
    chance = number_range(1, 100);
  ...
  else if(IS_NPC(ch) && IS_ATTACK(ch, ATCK_PUGILISM) && get_eq_char(ch, WEAR_WIELD) == NULL && ch->success_attack > 0)
  {
    chance = number_range(1, 100);
...
// start of animal form damage
  else if(((ch)->Class == CLASS_OWL || (ch)->Class == CLASS_GRYPHON))
  {
    snprintf(buf1, 256, "$n's talons %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf2, 256, "Your talons %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf3, 256, "$n's talons %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
  }
  else if(((ch)->Class == CLASS_BEAR || (ch)->Class == CLASS_WOLF) || xIS_SET(ch->act, ACT_BEASTMELD))
  {

    if(dt == gsn_crushing_jaw)
    {
      snprintf(buf1, 256, "$n's crushing jaw attack %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your crushing jaw attack %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's crushing jaw attack %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else
    {
      snprintf(buf1, 256, "$n's claw %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your claws %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's claws %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
  }
  else if(((ch)->Class == CLASS_FISH))
  {
    snprintf(buf1, 256, "$n's bite %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf2, 256, "Your bite %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf3, 256, "$n's bite %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
  }
  else if(((ch)->Class == CLASS_TREANT))
  {
    snprintf(buf1, 256, "$n's branches %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf2, 256, "Your branches %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf3, 256, "$n's branches %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
  }
...
// end of animal
  ...
  else if(dt == TYPE_HIT && ((ch)->race == RACE_DRAGON))
  {
    ...
    {
      snprintf(buf1, 256, "$n's massive maw %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your massive maw %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's massive maw %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
    else
    {
      snprintf(buf1, 256, "$n's tail %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf2, 256, "Your tail %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
      snprintf(buf3, 256, "$n's tail %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    }
  }
  else if(dt == TYPE_HIT && ch->martialarts != 3)
  {
    snprintf(buf1, 256, "$n %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf2, 256, "You %s $N %d %s for %d total damage%c", vs, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf3, 256, "$n %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
  }
  else if(dt > (unsigned int)TYPE_HIT && is_wielding_poisoned(ch))
  {
    ...
    set_char_color(AT_GREEN, ch);
    snprintf(buf1, 256, "$n's poisoned %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf2, 256, "Your poisoned %s $N %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf3, 256, "$n's poisoned %s you %d %s for %d total damage%c", vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
  }
  else
  {
    if(skill)
    ...
    snprintf(buf1, 256, "$n's %s %s $N %d %s for %d total damage%c", attack, vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf2, 256, "Your %s %s $N %d %s for %d total damage%c", attack, vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
    snprintf(buf3, 256, "$n's %s %s you %d %s for %d total damage%c", attack, vp, ch->success_attack, (ch->success_attack == 1) ? "time" : "times", ch->damage_amount, punct);
  }

  act(AT_GREEN, buf1, ch, NULL, victim, TO_NOTVICT);
  if(!gcflag && !IS_AFFECTED(victim, AFF_DISABLED))
...
void do_kill(CHAR_DATA *ch, char *argument)
{
  ...
  {
    xREMOVE_BIT(victim->affected_by, AFF_ENTHRALLED);
  }
...
/* Volk: Support for arena. */
  ...
  if(xIS_SET(ch->act, PLR_BATTLE) && ch->Class == CLASS_MONK)
    send_to_char("!!SOUND(sound/brucelee.wav)\r\n", ch);
...
bool check_illegal_pk(CHAR_DATA *ch, CHAR_DATA *victim)
{
  ...
  if(!IS_NPC(victim) && !IS_NPC(ch))
  {
    if((!IS_SET(victim->pcdata->flags, PCFLAG_DEADLY)
        || ch->level - victim->level > 10 || !IS_SET(ch->pcdata->flags, PCFLAG_DEADLY)) && !in_arena(ch) && ch != victim && !(IS_IMMORTAL(ch) && IS_IMMORTAL(victim)))
    {
      if(IS_NPC(ch))
...
void do_flee(CHAR_DATA *ch, char *argument)
{
  ...
  for(attempt = 0; attempt < 8; attempt++)
  {
    ...
    if(xIS_SET(ch->act, PLR_AUTODOOR) && pexit && crash == 2 )
    {
      if(pexit && get_exit(ch->in_room, DIR_EAST) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
       {
        interpret(ch, (char *)"open east");
        crash = 1;
        break;
       }
      if(pexit && get_exit(ch->in_room, DIR_WEST) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
      {
        interpret(ch, (char *)"open west");
        crash = 1;
        break;
       }
      if(pexit && get_exit(ch->in_room, DIR_NORTH) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
      {
        interpret(ch, (char *)"open north");
        crash = 1;
        break;
      }
      if(pexit && get_exit(ch->in_room, DIR_SOUTH) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
       {
        interpret(ch, (char *)"open south");
        crash = 1;
        break;
       }
      if(pexit && get_exit(ch->in_room, DIR_NORTHEAST) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
       {
        interpret(ch, (char *)"open ne");
        crash = 1;
        break;
       }
      if(pexit && get_exit(ch->in_room, DIR_NORTHWEST) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
       {
        interpret(ch, (char *)"open nw");
        crash = 1;
        break;
       }
      if(pexit && get_exit(ch->in_room, DIR_SOUTHEAST) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
       {
        interpret(ch, (char *)"open se");
        crash = 1;
        break;
       }
      if(pexit && get_exit(ch->in_room, DIR_SOUTHWEST) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
       {
        interpret(ch, (char *)"open sw");
        crash = 1;
        break;
       }
      if(pexit && get_exit(ch->in_room, DIR_DOWN) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
       {
        interpret(ch, (char *)"open down");
        crash = 1;
        break;
       }
      if(pexit && get_exit(ch->in_room, DIR_UP) != NULL && pexit->to_room && IS_SET(pexit->exit_info, EX_CLOSED))
       {
        interpret(ch, (char *)"open UP");
        crash = 1;
        break;
       }
    }
    ...
    if((pexit = get_exit(was_in, door)) == NULL
       || !pexit->to_room
       || IS_SET(pexit->exit_info, EX_NOFLEE) || (IS_SET(pexit->exit_info, EX_CLOSED) && !IS_AFFECTED(ch, AFF_PASS_DOOR)) || (IS_NPC(ch) && IS_SET(pexit->to_room->room_flags, ROOM_NO_MOB)))
      continue;
    ...
    if(!IS_NPC(ch))
    {
      ...
      if(ch->Class == CLASS_BEAST)
      {
        ...
        for(mob = first_char; mob; mob = mob->next)
        {
          if(mob)
          {
            if(IS_NPC(mob) && mob->in_room && ch->in_room)
            {
              if(mob->master)
              {
                if(!str_cmp(ch->name, mob->master->name) && (mob->in_room != ch->in_room) && mob->fighting)
                {
                  found = TRUE;
                  break;
                }
        ...
        if(mob && found == TRUE)
        {
          act(AT_FLEE, "$n flees head over heels!", mob, NULL, NULL, TO_ROOM);
          char_from_room(mob);
          char_to_room(mob, (ch->in_room));
          act(AT_FLEE, "$n glances around for signs of pursuit.", mob, NULL, NULL, TO_ROOM);
        }
      }
      if(ch->level < 20)
  ...
  act(AT_FLEE, "You attempt to flee from combat but can't break away!", ch, NULL, NULL, TO_CHAR);

  if(!IS_AVATAR(ch) && !IS_DUALAVATAR(ch) && !IS_TRIAVATAR(ch)
     && number_bits(1) == 0 && ch->fighting && ch->fighting->who && (!IS_NPC(ch->fighting->who) || !xIS_SET(ch->fighting->who->act, ACT_IMMORTAL)))
    gain_exp(ch, 0 - los);
  return;
}
...
void make_corpse(CHAR_DATA *ch, CHAR_DATA *killer)
{
  ...
  if(IS_NPC(ch))
  {
    if(ch->pIndexData->vnum == MOB_VNUM_ANIMATED_CORPSE || ch->pIndexData->vnum == MOB_VNUM_ANIMATED_SKEL)
    {
      act(AT_MAGIC, "$n disintegrates to the dust from whence it came.", ch, NULL, ch->short_descr, TO_ROOM);
      return;
    }
    name = ch->short_descr;
    ...
    if(ch->description)
    {
      ...
      LINK(ed, corpse->first_extradesc, corpse->last_extradesc, next, prev);
      ed->keyword = STRALLOC("cor corpse corp");
    ...
    /*
     * used. corpse->value[0] = (int)ch->pIndexData->vnum; corpse->value[1] = * * * *
     * * (int)ch->max_hit; 
     *//*
    ...
    if(!IN_WILDERNESS(ch) && !IN_RIFT(ch))
    {
      if(ch->level > 1 && ch->level < 10)
        obj_to_obj(create_money(number_range(1, 50), CURR_COPPER), corpse);
      else if(ch->level > 9 && ch->level < 20)
        obj_to_obj(create_money(number_range(1, 40), CURR_BRONZE), corpse);
      else if(ch->level > 19 && ch->level < 60)
        obj_to_obj(create_money(number_range(1, 25), CURR_SILVER), corpse);
      else if(ch->level > 59 && ch->level < 100)
        obj_to_obj(create_money(number_range(1, 20), CURR_GOLD), corpse);
  ...
  else
  {
    ...
    if(ch->level > 19)
    {
      short                   gold = 0, silver = 0, copper = 0, bronze = 0;

      if(GET_MONEY(ch, CURR_GOLD) > 0)
      {
        gold = GET_MONEY(ch, CURR_GOLD);
        GET_MONEY(ch, CURR_GOLD) = 0;
      }
      if(GET_MONEY(ch, CURR_SILVER) > 0)
      {
        silver = GET_MONEY(ch, CURR_SILVER);

        GET_MONEY(ch, CURR_SILVER) = 0;
      }
      if(GET_MONEY(ch, CURR_BRONZE) > 0)
      {
        bronze = GET_MONEY(ch, CURR_BRONZE);
        GET_MONEY(ch, CURR_BRONZE) = 0;
      }
      if(GET_MONEY(ch, CURR_COPPER) > 0)
      {
        copper = GET_MONEY(ch, CURR_COPPER);
        GET_MONEY(ch, CURR_COPPER) = 0;
      }
      if(gold > 0)
        obj_to_obj(create_money(gold, CURR_GOLD), corpse);
      if(silver > 0)
        obj_to_obj(create_money(silver, CURR_SILVER), corpse);
      if(bronze > 0)
        obj_to_obj(create_money(bronze, CURR_BRONZE), corpse);
      if(copper > 0)
        obj_to_obj(create_money(copper, CURR_COPPER), corpse);
    }
