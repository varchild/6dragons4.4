...
void do_solve(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(ch->pcdata->authexp, arg))
  {
    ...
    send_to_char("You have entered the correct solve code and your triple exp timer has been updated.\r\n", ch);
...
void show_balance(CHAR_DATA *ch, BANK_DATA * bank)
{
  ...
  ch_printf(ch, "\r\n&CFor bank account '%s' the following funds are available:\r\n", bank->name);
...
void do_balance(CHAR_DATA *ch, char *argument)
{
  if(!IS_NPC(ch) && ch->pcdata)
  {
    BANK_DATA              *bank;
      ...
      send_to_char("You have no active account! Enter a bank and use the ACCOUNT command to create or set an account.\n\r", ch);
      else
      bank = find_bank(ch->pcdata->account);
      show_balance(ch, bank);
  }
  return;
}
...
void do_money(CHAR_DATA *ch, char *argument)
{
  ...
  if(ch->pcdata && VLD_STR(ch->pcdata->account) && (bank = find_bank(ch->pcdata->account)))
  {
    show_balance(ch, bank);
  }
  return;
...
{
  send_to_char("&C===========================================================================\r\n", ch);
  send_to_char("&C=====                   Current Mob Hate Level                        =====\r\n", ch);
  send_to_char("&C=====                                                                 =====\r\n", ch);
  set_pager_color(AT_LBLUE, ch);
  if(ch->hate_level == 10)
  {
    pager_printf(ch, "=====         Currently you rank: &R%d&C in Hate from Mobs                =====\r\n", ch->hate_level);
  }
  else
  {
    pager_printf(ch, "=====         Currently you rank: &R%d&C in Hate from Mobs                 =====\r\n", ch->hate_level);
...
char                   *percent_bar_display(CHAR_DATA *ch, int percent)
{
  ...
  if(percent > 10 || percent <= 100)
  {
    sprintf(pbuf, "&Y[&R|%s&Y%s%s&B%s%s&G%s%s%s%s&Y]", percent > 20 ? "|" : " ",
            ...
            percent > 50 ? "|" : " ", percent > 60 ? "|" : " ", percent > 70 ? "|" : " ", percent > 80 ? "|" : " ", percent > 90 ? "|" : " ", percent >= 100 ? "|" : " ");
  }
  else  /* Under 10% just show the percent * in a number */
    sprintf(pbuf, "&W[%s%d%%&W]", percent >= 60 ? "&G" : percent >= 40 ? "&B" : percent >= 20 ? "&Y" : "&R", percent);
  return pbuf;
}
...
void do_say(CHAR_DATA *ch, char *argument)
{
  ...
  snprintf(buf, MSL, "say %s", argument);
  interpret(ch, buf);
...
skill_table[sn]->name, ch->pcdata->learned[sn], get_maxadept( ch, sn, FALSE )
*/
...
void do_skill(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char("Syntax: Skill name\r\n", ch);
    send_to_char("or using skills command will show you all of them at once.\r\n", ch);
    return;
  }
...
void do_blood(CHAR_DATA *ch, char *argument)
{
  if(!IS_BLOODCLASS(ch))
  {
    send_to_char("Huh?  Your not a vampire or a demon.\r\n", ch);
    return;
  }
  ch_printf(ch, "%d of %d", ch->blood, ch->max_blood);
  return;
}

void do_hp(CHAR_DATA *ch, char *argument)
{
  ch_printf(ch, "%d of %d", ch->hit, ch->max_hit);
  return;
}

void do_armor(CHAR_DATA *ch, char *argument)
{
  ch_printf(ch, "%d", GET_AC(ch));
  return;
}

void do_mana(CHAR_DATA *ch, char *argument)
{
  ch_printf(ch, "%d of %d", ch->mana, ch->max_mana);
}

void do_move(CHAR_DATA *ch, char *argument)
{
  ch_printf(ch, "%d of %d", ch->move, ch->max_move);
}
...
void do_petscore(CHAR_DATA *ch, char *argument)
{
  ...
  set_pager_color(AT_LBLUE, ch);
  pager_printf(ch, "\r\n&RScore Sheet for %s's pet %s&C.\r\n", ch->master->name, ch->short_descr);
  pager_printf(ch,
               "Your current stats are the following:\r\nStr %d Int %d Wis %d Con %d Dex %d Cha %d Lck %d\r\n",
               get_curr_str(ch), get_curr_int(ch), get_curr_wis(ch), get_curr_con(ch), get_curr_dex(ch), get_curr_cha(ch), get_curr_lck(ch));
...
void do_bscore(CHAR_DATA *ch, char *argument)
{
  ...
  if(ch->secondclass == -1)
  {
    pager_printf(ch, "You're a %s race with a level %d %s class.\r\n",
                 !VLD_STR(race_table[ch->race]->race_name) ? "Unknown" : race_table[ch->race]->race_name,
                 ch->firstlevel > 0 ? ch->firstlevel : ch->level, capitalize(class_table[ch->Class]->who_name));
  }
  else if(ch->secondclass != -1 && ch->thirdclass == -1)
  {
    pager_printf(ch, "You're a %s race with a level %d %s/%s class.\r\n",
                 !VLD_STR(race_table[ch->race]->race_name) ? "Unknown" : race_table[ch->race]->race_name,
                 ch->level, capitalize(class_table[ch->Class]->who_name), class_table[ch->secondclass]->who_name);
  }
  else if(ch->secondclass != -1 && ch->thirdclass != -1)
  {
    pager_printf(ch, "You're a %s race with a level %d %s/%s/%s class\r\n",
                 !VLD_STR(race_table[ch->race]->race_name) ? "Unknown" : race_table[ch->race]->race_name,
                 ch->level, capitalize(class_table[ch->Class]->who_name), class_table[ch->secondclass]->who_name, class_table[ch->thirdclass]->who_name);
  }
  pager_printf(ch, "You are %d years old and %s is your hometown.\r\n", calculate_age(ch), !ch->pcdata->htown ? "None" : VLD_STR(ch->pcdata->htown->name) ? ch->pcdata->htown->name : "Unknown");
  pager_printf(ch,
               "Your current stats are the following:\r\nStr %d Int %d Wis %d Con %d Dex %d Cha %d Lck %d\r\n",
               get_curr_str(ch), get_curr_int(ch), get_curr_wis(ch), get_curr_con(ch), get_curr_dex(ch), get_curr_cha(ch), get_curr_lck(ch));
  pager_printf(ch, "You have a HitRoll of %d a Damroll of %d and %d Armor Class.\r\n", GET_HITROLL(ch), GET_DAMROLL(ch), GET_AC(ch));
...
void do_score(CHAR_DATA *ch, char *argument)
{
  int                     iLang, x, count = 0;
  const char             *divider = "================================================================================\r\n";
  char                    buf[MSL], hr_buf[MSL], dr_buf[MSL], print_buf[MSL];
  int                     feet, inches;
  ...
  pager_printf(ch, "Hometown: %s\r\n", !ch->pcdata->htown ? "None" : VLD_STR(ch->pcdata->htown->name) ? ch->pcdata->htown->name : "Unknown");
  pager_printf(ch, "Level : %-3d         Race : %-10.10s          Played: %ld hours\r\n",
               ch->level, !VLD_STR(race_table[ch->race]->race_name) ? "Unknown" : race_table[ch->race]->race_name, GET_TIME_PLAYED(ch));

  pager_printf(ch, "Class : (%3d)%-12s\r\n", ch->firstlevel > 0 ? ch->firstlevel : ch->level, capitalize(class_table[ch->Class]->who_name));
  if(IS_SECONDCLASS(ch))
    pager_printf(ch, "Class : (%3d)%-12s\r\n", ch->secondlevel, capitalize(class_table[ch->secondclass]->who_name));
  if(IS_THIRDCLASS(ch))
    pager_printf(ch, "Class : (%3d)%-12s\r\n", ch->thirdlevel, capitalize(class_table[ch->thirdclass]->who_name));
  ...
  if(ch->race == RACE_DRAGON)
    pager_printf(ch, "Dragon Size Bonus: &R%-3d&C", (bonus / 2) + 10 + get_curr_str(ch));
  send_to_pager("\r\n", ch);
  pager_printf(ch, "STR   : &Y%2.2d&C(&W%2.2d&C)      %13s              Saved : %s", get_curr_str(ch), ch->perm_str, hr_buf, ch->save_time ? ctime(&(ch->save_time)) : "N/A\r\n");
  pager_printf(ch, "INT   : &Y%2.2d&C(&W%2.2d&C)      %13s              Time  : %s", get_curr_int(ch), ch->perm_int, dr_buf, ctime(&current_time));
  if(GET_AC(ch) >= 200)
  ...
  pager_printf(ch, "DEX   : &Y%2.2d&C(&W%2.2d&C)      Align: %-17.17s   Items : %5.5d (Max %5.5d)\r\n", get_curr_dex(ch), ch->perm_dex, print_buf, ch->carry_number, can_carry_n(ch));
  switch (ch->position)
  {
    case POS_DEAD:
  ...
  pager_printf(ch, "CON   : &Y%2.2d&C(&W%2.2d&C)      Pos'n: %-17.17s   Equipment Weight: %5.5d\r\n", get_curr_con(ch), ch->perm_con, buf, ch->carry_weight);
  pager_printf(ch, "CHA   : &Y%2.2d&C(&W%2.2d&C)      Wimpy: %-5d&C               (Max Can Carry: %7.7d)\r\n", get_curr_cha(ch), ch->perm_cha, ch->wimpy, can_carry_w(ch));
  pager_printf(ch, "LCK   : &Y%2.2d&C(&W%2.2d&C) \r\n", get_curr_lck(ch), ch->perm_lck);
  pager_printf(ch,
               "PRACT : %4.4d        &YHitpoints: %-6d of %6d&C   Pager  : [%c]%2d    AutoExit [%c]",
               ch->practice, ch->hit, ch->max_hit, IS_SET(ch->pcdata->flags, PCFLAG_PAGERON) ? '*' : ' ', ch->pcdata->pagerlen, xIS_SET(ch->act, PLR_AUTOEXIT) ? '*' : ' ');
  if(IS_MINDFLAYER(ch))
  ...
  pager_printf(ch, "&CEXP   : %-9.9s   %-29.29s   &CMKills : %-5.5d    AutoLoot [%c]\r\n", buf, print_buf, ch->pcdata->mkills, xIS_SET(ch->act, PLR_AUTOLOOT) ? '*' : ' ');
  pager_printf(ch, "                    &GMove     : %-6d of %6d&C   Mdeaths: %-5.5d    AutoTrash[%c]\r\n", ch->move, ch->max_move, ch->pcdata->mdeaths, xIS_SET(ch->act, PLR_AUTOTRASH) ? 'X' : ' ');
...
/* Volk - fun stuff, display percentage of areas discovered */
  ...
  ch_printf(ch, "&CGlory Points:&W %d&C      Areas discovered:&W %.2f&D\r\n", ch->quest_curr, areaperc);
  ...
  if(!IS_IMMORTAL(ch) && ch->pcdata)
  {
    if(ch->pcdata->double_exp_timer > 0)
    {
      ...
      if(ch->pcdata->double_exp_timer < 10)
        ch_printf(ch, "&C[&R|             &C] &W%d Percent&D", ch->pcdata->double_exp_timer);
      ...
      if(ch->pcdata->double_exp_timer < 40 && ch->pcdata->double_exp_timer >= 30)
        ch_printf(ch, "&C[&R|||&O|        &C] &W%d Percent&D", ch->pcdata->double_exp_timer);
      ...
      if(ch->pcdata->double_exp_timer < 70 && ch->pcdata->double_exp_timer >= 60)
        ch_printf(ch, "&C[&R|||&O|||&Y|    &C] &W%d Percent&D", ch->pcdata->double_exp_timer);
      ...
      if(ch->pcdata->double_exp_timer < 100 && ch->pcdata->double_exp_timer >= 90)
        ch_printf(ch, "&C[&R|||&O|||&Y|||| &C] &W%d Percent&D", ch->pcdata->double_exp_timer);
      if(ch->pcdata->double_exp_timer >= 100)
        ch_printf(ch, "&C[&R|||&O|||&Y||||&G|&C] &W%d Percent&D", ch->pcdata->double_exp_timer);
  ...
  if(IS_IMMORTAL(ch))
  {
    ...
    pager_printf(ch, "&CBamfin       : %s\r\n",
                 VLD_STR(ch->pcdata->bamfin) ? ch->pcdata->bamfin :
                 "&C[OoC]: $n please &Rvote&C for us at &Yhttp://6dragons.tk/bin/vote.html &Cthis message suddenly &REXPLODES!&c and $n is standing here!");
    pager_printf(ch, "&CBamfout      : %s&C\r\n",
                 VLD_STR(ch->pcdata->bamfout) ? ch->pcdata->bamfout : "&C[OoC]: $n please &Rvote&C for us at &Yhttp://6dragons.tk/bin/vote.html &Cthis message suddenly &REXPLODES!&c and $n is GONE!");
    /*
...
void blind_level(CHAR_DATA *ch)
{
  if(xIS_SET(ch->act, ACT_BEASTMELD))
  {
    ch_printf(ch, "&cBeastmaster's pet %s is Level &C%d&D\r\n", ch->master->pcdata->pet->name, ch->master->pcdata->petlevel);
    ch_printf(ch, "&cThe %s has &C%d&c experience points &C%d&c more needed to level.\r\n", ch->master->pcdata->pet->name, ch->master->pcdata->petexp, 18 - ch->master->pcdata->petexp);
    return;
  }
  ...
  if(IS_SECONDCLASS(ch))
  {
    ...
    ch_printf(ch, "Class level: %d\r\n", ch->secondlevel);
    ch_printf(ch, "You currently have %d experience points.\r\n", ch->secondexp);
    ch_printf(ch, "You need %d to achieve next level.\r\n\r\n", (exp_class_level(ch, ch->secondlevel + 1, ch->secondclass) - ch->secondexp));
  }
  ...
  if(ch->pcdata->tradeclass == 20)
  {
    ...
    if(ch->pcdata->tradelevel < 20)
      ch_printf(ch, "You need %d to achieve next trade level.\r\n\r\n", exp_craft_level(ch, ch->pcdata->tradelevel + 1, ch->pcdata->tradeclass) - ch->pcdata->craftpoints);
  }
  ...
  if(IS_GROUPED(ch) && !happyhouron)
    ...
    send_to_char("&WYou will receive Triple Experience for being grouped during HAPPY HOUR!\r\n", ch);
  else if(sysdata.happy && (!ch->pcdata || !ch->pcdata->getsdoubleexp))
    send_to_char("&WYou will receive Happy Hour Double Experience.&D\r\n", ch);
...
void do_level(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if(!xIS_SET(ch->act, PLR_BOUGHT_PET) && ch->level > 4)
      ch_printf(ch, "&cPet Level - No pet tamed yet.\r\n");
    else
    {
      if(ch->pcdata->petlevel > 0)
        ch_printf(ch, "&cBeastmaster's pet %s is Level &C%d&D\r\n", ch->pcdata->pet->name, ch->pcdata->petlevel);
      if(ch->pcdata->petlevel > 0 && ch->pcdata->petlevel < 100)
        ch_printf(ch, "&cThe %s has &C%d&c experience points &C%d&c more needed to level.\r\n", ch->pcdata->pet->name, ch->pcdata->petexp, 18 - ch->pcdata->petexp);
    }
  }
  ...
  if(!IS_SECONDCLASS(ch) && !IS_THIRDCLASS(ch))
  {
    ...
    snprintf(buf2, MSL, " exp  (Needed:  %12s)", num_punct(exp_level(ch, ch->level + 1) - ch->exp));
  ...
  else
  {
    ...
    snprintf(buf2, MSL, " exp  (Needed:  %12s)", num_punct(exp_class_level(ch, ch->firstlevel + 1, ch->Class) - ch->firstexp));

    lowlvl = UMAX(1, ch->firstlevel);
    hilvl = URANGE(ch->firstlevel, ch->firstlevel + 1, MAX_LEVEL);

    for(x = lowlvl; x <= hilvl; x++)
      ch_printf(ch, "(%2d) %12s%s\r\n", x, num_punct(exp_class_level(ch, x, ch->Class)), (x == ch->firstlevel) ? buf : (x == ch->firstlevel + 1) ? buf2 : " exp");
  }
  ...
  if(IS_SECONDCLASS(ch))
  {
    ...
    snprintf(buf2, MSL, " exp  (Needed:  %12s)", num_punct(exp_class_level(ch, ch->secondlevel + 1, ch->secondclass) - ch->secondexp));

    lowlvl = UMAX(1, ch->secondlevel);
    hilvl = URANGE(ch->secondlevel, ch->secondlevel + 1, MAX_LEVEL);

    for(x = lowlvl; x <= hilvl; x++)
      ch_printf(ch, "(%2d) %12s%s\r\n", x, num_punct(exp_class_level(ch, x, ch->secondclass)), (x == ch->secondlevel) ? buf : (x == ch->secondlevel + 1) ? buf2 : " exp");
  }
  ...
  if(IS_THIRDCLASS(ch))
  {
    ...
    snprintf(buf2, MSL, " exp  (Needed:  %12s)", num_punct(exp_class_level(ch, ch->thirdlevel + 1, ch->thirdclass) - ch->thirdexp));

    lowlvl = UMAX(1, ch->thirdlevel);
    hilvl = URANGE(ch->thirdlevel, ch->thirdlevel + 1, MAX_LEVEL);

    for(x = lowlvl; x <= hilvl; x++)
      ch_printf(ch, "(%2d) %12s%s\r\n", x, num_punct(exp_class_level(ch, x, ch->thirdclass)), (x == ch->thirdlevel) ? buf : (x == ch->thirdlevel + 1) ? buf2 : " exp");
  }

  if(ch->pcdata->tradeclass == 20 || ch->pcdata->tradeclass == 21 || ch->pcdata->tradeclass == 22 || ch->pcdata->tradeclass == 24 || ch->pcdata->tradeclass == 25 || ch->pcdata->tradeclass == 26)
  {
    ...
    if(ch->pcdata->tradeclass == 20)
      ...
      ch_printf(ch, "\r\n&cClass:&C Carpenter &c \r\n");
    else if(ch->pcdata->tradeclass == 26)
      ch_printf(ch, "\r\n&cClass:&C Dragonic &c \r\n");

    snprintf(buf, MSL, " craftpoints  (Current: %12s)", num_punct(ch->pcdata->craftpoints));
    if(ch->pcdata->tradelevel < 20)
      snprintf(buf2, MSL, " craftpoints  (Needed:  %12s)", num_punct(exp_craft_level(ch, ch->pcdata->tradelevel + 1, ch->pcdata->tradeclass) - ch->pcdata->craftpoints));

    for(x = lowlvl; x <= hilvl; x++)
      ch_printf(ch, "(%2d) %12s%s\r\n", x, num_punct(exp_craft_level(ch, x, ch->pcdata->tradeclass)), (x == ch->pcdata->tradelevel) ? buf : (x == ch->pcdata->tradelevel + 1) ? buf2 : " craftpoints");
  }
  ...
  if(ch->pcdata)
  {
    ...
    {
      int                     dexptime = 3600 - (current_time - ch->pcdata->last_dexpupdate);
  ...
  if(IS_GROUPED(ch) && !happyhouron)
    send_to_char("&WYou will receive Triple Experience for being grouped.\r\n", ch);
  else if(IS_GROUPED(ch) && happyhouron)
    send_to_char("&WYou will receive Triple Experience for being grouped during HAPPY HOUR!\r\n", ch);
...
{ /* type ie * SKILL_SONG etc */
  ...
  if(paf->location == APPLY_NONE)
    ch_printf(ch, "%-58s &Bat lvl&R %-3d&B for&R %-4d&B seconds.&D\r\n", skill->name, paf->level, paf->duration);
  else
  {
    ch_printf(ch, "%-14s &W- &PModifies&w %-14s&B", skill->name, a_types[paf->location % REVERSE_APPLY]);
...
void do_remort(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(argument, "yes") && (IS_AVATAR(ch) || IS_DUALAVATAR(ch) || IS_TRIAVATAR(ch)))
  {
...
// set all spells/skills to zero
    for(sn = 0; sn < top_sn; sn++)
    {
      if(skill_table[sn]->name
         && (sn != gsn_mine && sn != gsn_forge && sn != gsn_tan && sn != gsn_bake && sn != gsn_gather && sn != gsn_mix && sn != gsn_hunt && sn != gsn_jewelry && sn != gsn_fell && sn != gsn_mill))
      {
...
// remove all effects
    ...
    snprintf(buf, MSL, "&RAnnounce: &Y%s a new %s is reborn among us!", ch->name, capitalize(class_table[ch->Class]->who_name));
...
void do_talents(CHAR_DATA *ch, char *argument)
{
  ...
  set_pager_color(AT_CYAN, ch);
  pager_printf_color(ch, " &R                                     Talents&D \r\n");
  set_pager_color(AT_CYAN, ch);
  ...
  for(sn = 0; sn < top_sn; sn++)
  {
    ...
    if(sn == gsn_mine || sn == gsn_forge || sn == gsn_tan || sn == gsn_bake || sn == gsn_gather || sn == gsn_mix || sn == gsn_hunt || sn == gsn_fell || sn == gsn_mill || sn == gsn_jewelry)
      continue;
    ...
    if(skill_table[sn]->type == SKILL_SKILL || skill_table[sn]->type == SKILL_SPELL)
    {
      int                     count = 0, type_length;

      count++;
      set_pager_color(AT_CYAN, ch);
      pager_printf(ch, "&c%20.20s&D  %3d&D", skill_table[sn]->name, ch->pcdata->learned[sn]);
      if(count == 2)
      {
        count = 0;
        send_to_char("\r\n", ch);
...
void do_skills(CHAR_DATA *ch, char *argument)
{
  ...
  set_pager_color(AT_CYAN, ch);
    if(xIS_SET(ch->act, ACT_BEASTMELD))
  {  
     send_to_char("You can't use that command in beast meld mode.\r\n", ch );
     return;
  }  
  ...
  for(sn = 0; sn < top_sn; sn++)
  {
    ...
    if(sn == gsn_mine || sn == gsn_forge || sn == gsn_tan || sn == gsn_bake || sn == gsn_gather || sn == gsn_mix || sn == gsn_hunt || sn == gsn_jewelry || sn == gsn_fell || sn == gsn_mill)
      continue;
...
void do_spells(CHAR_DATA *ch, char *argument)
{
  ...
  for(sn = 0; sn < top_sn; sn++)
  {
    ...
    if(sn == gsn_mine || sn == gsn_forge || sn == gsn_tan || sn == gsn_bake || sn == gsn_gather || sn == gsn_mix || sn == gsn_hunt || sn == gsn_jewelry || sn == gsn_mill || sn == gsn_fell)
      continue;
...
void do_rprate(CHAR_DATA *ch, char *argument)
{
  ...
  if(!victim->pcdata->lastrated || ((current_time - 1800) > victim->pcdata->lastrated))
  {
    ...
    send_to_char("Sorry, that person has been rated in the last hour. Please try again later.\n\r", ch);
  return;
}
...
short checkobj(OBJ_DATA *obj)
{

  if(obj->wear_loc != WEAR_WIELD && obj->wear_loc != WEAR_SHEATH
     && obj->wear_loc != WEAR_DUAL_WIELD && obj->wear_loc != WEAR_MISSILE_WIELD && obj->wear_loc != WEAR_LODGE_RIB && obj->wear_loc != WEAR_LODGE_ARM && obj->wear_loc != WEAR_LODGE_LEG)
    return TRUE;
  else
    return FALSE;
}

void do_items(CHAR_DATA *ch, char *argument)
{
  OBJ_DATA               *obj;
  int                     iWear;
  bool                    empty = FALSE, dragon = FALSE;

  for(iWear = 0; iWear < MAX_WEAR; iWear++)
  {

    // loop to show all item slots if configed showslots
    for(obj = ch->first_carrying; obj; obj = obj->next_content)
    {
      ...
      if(obj->wear_loc == iWear && checkobj(obj) == TRUE)
      {
        ch_printf(ch, "%s %s\r\n", race_table[ch->race]->where_name[iWear], obj->short_descr);
        break;
      }
      if(empty == FALSE)
      {
        ch_printf(ch, "%s Nothing\r\n", race_table[ch->race]->where_name[iWear]);
        break;
      }
    }
...
/* Volk Jan-16: Support for arguments (screen readers) */
  if(argument[0] != '\0' && !IS_NPC(ch))
  {
    one_argument(argument, arg);

    bool                    foundwloc = FALSE, founditem = FALSE;
    int                     num;

    for(num = 0; num < gear; num++)
    {
      if(!str_cmp(arg, item_w_flags[num]))
      {
        foundwloc = TRUE;
        send_to_char(race_table[ch->race]->where_name[num], ch);
        for(obj = ch->first_carrying; obj; obj = obj->next_content)
        {
          if(obj->wear_loc == num)
          {
    ...
    if(!foundwloc)
      send_to_char("Not a valid wear location.\r\n", ch);

    return;
  }

  set_char_color(AT_RED, ch);
  send_to_char("You are using:       Condition    Equipment&C\r\n", ch);
  found = FALSE;
  if(ch->race == RACE_DRAGON)
    gear = 33;
  else
...
void do_statreport(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_BLOODCLASS(ch))
  {
    ...
    snprintf(buf, sizeof(buf), "$n reports: %d/%d hp %d/%d blood %d/%d mv %d xp.", ch->hit, ch->max_hit, ch->blood, ch->max_blood, ch->move, ch->max_move, ch->exp);
  ...
  else
  {
    ...
    snprintf(buf, sizeof(buf), "$n reports: %d/%d hp %d/%d mana %d/%d mv %d xp.", ch->hit, ch->max_hit, ch->mana, ch->max_mana, ch->move, ch->max_move, ch->exp);
    act(AT_REPORT, buf, ch, NULL, NULL, TO_ROOM);
  }
  ...
  snprintf(buf, sizeof(buf),
           "$n's base stats:    %-2d str %-2d wis %-2d int %-2d dex %-2d con %-2d cha %-2d lck.", ch->perm_str, ch->perm_wis, ch->perm_int, ch->perm_dex, ch->perm_con, ch->perm_cha, ch->perm_lck);
  act(AT_REPORT, buf, ch, NULL, NULL, TO_ROOM);
...
void do_petreport(CHAR_DATA *ch, char *argument)
{
  ...
  if(!IS_NPC(ch) && xIS_SET(ch->act, PLR_BOUGHT_PET))
  {
    ...
    if(found == FALSE)
    {
      send_to_char("You need to be in the same room first, maybe beckon your pet.\r\n", ch);
      return;
    }
    if(found == TRUE)
    {
      if(ch->Class == CLASS_BEAST)
      {
        snprintf(buf, MIL, "$n reports: %d/%d hp %d/%d mv %d affection %d xp.\r\n", mob->hit, mob->max_hit, mob->move, mob->max_move, ch->pcdata->petaffection, ch->pcdata->petexp);
      }
      else
      {
        snprintf(buf, MIL, "$n reports: %d/%d hp %d/%d mv.\r\n", mob->hit, mob->max_hit, mob->move, mob->max_move);
      }
      act(AT_REPORT, buf, mob, NULL, NULL, TO_CHAR);
      act(AT_REPORT, buf, mob, NULL, NULL, TO_ROOM);
      return;
    }
  }
  ...
  if(xIS_SET(ch->act, ACT_PET) && IS_NPC(ch))
  {
    ...
    {
      snprintf(buf, MIL, "$n reports: %d/%d hp %d/%d mv %d affection %d xp.\r\n", ch->hit, ch->max_hit, ch->move, ch->max_move, ch->master->pcdata->petaffection, ch->master->pcdata->petexp);
    }
    else
    {
...
void do_fprompt(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg, "Monk"))
  {
    ...
    ch->pcdata->fprompt = STRALLOC("&W[&Y%h&W/&Y%H&Chp &G%v&W/&G%V&Cmv &P%q&Cmb&W] &PLP %d &R%c");
    return;
  }
...
void do_deadly(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char("You cannot be a pkiller in the Halcyon Clan, first you must leave the clan.\r\n", ch);
    return;
  }
  if(!IS_SET(ch->pcdata->flags, PCFLAG_DEADLY))
    SET_BIT(ch->pcdata->flags, PCFLAG_DEADLY);
  set_char_color(AT_YELLOW, ch);
  send_to_char("You have joined the ranks of the deadly. The staff cease to protect you!\r\n\r\n", ch);
  if(ch->secondclass == -1)
  {
    if(ch->level < 10)
    {
...
void do_roll(CHAR_DATA *ch, char *argument)
{
  ...
  if(winamount > 0) /* Only change it when they * * * * * * actually gain, they already * 
                     * put * * * * * * 10
                     * * in the * * pot to * play */
  ...
  if(winamount < 0)
  {
    ...
    act_printf(AT_ACTION, ch, NULL, mob, TO_CHAR, "$N reaches into the pot and takes %d bronze coins.", winamount);
  }
  else if(winamount > 0)
    act_printf(AT_ACTION, ch, NULL, mob, TO_CHAR, "$N reaches into the pot and hands you %d bronze coins.", winamount);
}
...
void glory_add(CHAR_DATA *ch, OBJ_DATA *obj, char *argument)
{
  if(!argument || argument[0] == '\0')
  {
    ...
    send_to_char("&cValid stats   - &Pmana hp strength intelligence wisdom dexterity constitution charisma\r\n", ch);
    send_to_char("                &Phitroll damroll armor rename rekey\r\n", ch);
    send_to_char("&cValid affs    - &Pdetect_invis detect_hidden detect_magic sneak hide invisible infrared\r\n", ch);
    send_to_char("                &Pflying floating aqua_breath scrying pass_door\r\n", ch);
    send_to_char("&cValid toggles - &Pglow hum loyal bless metal\r\n\r\n", ch);
    send_to_char("&RPlease type 'glory costs' for prices or see HELP GLORYAFFECTS for more info.\r\n", ch);
    return;
  }
  ...
  if(cost <= 0)
  {
    send_to_char("That is not a valid glory affect. Please type 'glory add (object)' without an argument to see a list of affects.\r\n", ch);
    return;
  }
  ...
  {
    ch_printf(ch, "This equals %d glory points. There is currently %d points left on the object.\r\n", cost, (maxglory - obj->glory));
...
//deal with rename/rekey FIRST then lump the rest
  ...
  if(!str_cmp(arg, "rename"))
  {
    if(argument && argument[0] != '\0')
    {
      ...
      send_to_char("You must specify the new keywords, ie 'rename whatever' or 'rename big pink sword'.\r\n", ch);
    return;
  }
  ...
  if(!str_cmp(arg, "rekey"))
  {
    if(argument && argument[0] != '\0')
    {
      ...
      send_to_char("You must specify the new keywords, ie 'rekey whatever' or 'rekey big pink sword'.\r\n", ch);
    return;
  }
  ...
  if(loc < 1 || loc >= APPLY_AFFECT)
  { /* If didn't find str, int, con etc.. must be -1, * should be APPLY_AFFECT */
    loc = APPLY_AFFECT;
    value = get_aflag(arg); /* Now looking for invisible, flying etc.. */

    if(value < 0 || value >= MAX_AFFECTED_BY)
    { /* Didn't find it either! Must be object * flag. */
      value = get_oflag(arg);
      if(value < 0 || value > MAX_BITS)
      { /* I'm stumped, we should NEVER get here! bug out * heh */
...
void do_glory(CHAR_DATA *ch, char *argument)
{
  ...
  {
    ch->practice += 5;
    send_to_char("You have gained 5 practice sessions.\r\n", ch);
    save_char_obj(ch);
    return;
  }
  ...
  if(!str_cmp(arg1, "str"))
  {
    ...
    if(ch->perm_str >= max)
    {
      send_to_char("You are already at maximum strength that a player may gain through glory.\r\n", ch);
      return;
    }

    if(125 > ch->quest_curr)
    {
      send_to_char("You don't have enough glory points.\r\n", ch);
      return;
    }
  ...
  if(!str_cmp(arg1, "lck"))
  {
    ...
    if(ch->perm_lck >= max)
    {
      send_to_char("You are already at maximum luck that a player may gain through glory.\r\n", ch);
      return;
    }
    if(125 > ch->quest_curr)
    {
  ...
  if(!str_cmp(arg1, "int"))
  {
    ...
    if(ch->perm_int >= max)
    {
      send_to_char("You are already at maximum intelligence that a player may gain through glory.\r\n", ch);
      return;
    }
    if(125 > ch->quest_curr)
    {
  ...
  if(!str_cmp(arg1, "wis"))
  {
    ...
    if(ch->perm_wis >= max)
    {
      send_to_char("You are already at maximum wisdom that a player may gain through glory.\r\n", ch);
      return;
    }
    if(125 > ch->quest_curr)
    {
  ...
  if(!str_cmp(arg1, "con"))
  {
    ...
    if(ch->perm_con >= max)
    {
      send_to_char("You are already at maximum constitution that a player may gain through glory.\r\n", ch);
      return;
    }
    if(125 > ch->quest_curr)
    {
  ...
  if(!str_cmp(arg1, "dex"))
  {
    ...
    if(ch->perm_dex >= max)
    {
      send_to_char("You are already at maximum dexterity that a player may gain through glory.\r\n", ch);
      return;
    }
    if(125 > ch->quest_curr)
    {
  ...
  if(!str_cmp(arg1, "cha"))
  {
    ...
    if(ch->perm_cha >= max)
    {
      send_to_char("You are already at maximum charisma that a player may gain through glory.\r\n", ch);
      return;
    }
    if(125 > ch->quest_curr)
    {
  ...
  if(argument && argument[0] != '\0')
  {
    ...
    if(!str_cmp(arg1, "delete"))
    {
      ...
      ch_printf(ch, "&RAll glory affects stripped from &C%s&R. &cThis is &C%d &cpoints worth!\r\n", obj->short_descr, obj->glory);
      separate_obj(obj);
      extract_obj(obj);
    ...
    else
    {
      ...
      ch_printf(ch, "This is NON REVERSIBLE, no exceptions. If you wish to continue, please write 'glory delete (obj)'.\r\n");
      return;
    }
  ...
  if(!str_cmp(arg1, "costs"))
  {
    ...
    {
      ch_printf(ch, "&c%-15s &P%5d     ", gl_names[x], gl_costs[x]);  /* found it */
      if((x + 1) % 3 == 0)
        send_to_char("\r\n", ch);
    }
    send_to_char("\r\n                  &CGLORY COSTS TO PLAYER PERMANENT AFFECTS\r\n", ch);
    send_to_char
      ("\r\n        &cmana          &P25  &chp          &P25    &cmove          &P25\r\n        &cstr           &P125  &cint         &P125    &cwis           &P125\r\n        &cdex           &P125  &ccon         &P125    &ccha           &P125\r\n        &clck           &P125\r\n        &cpractice        &P5\r\n",
       ch);
    send_to_char("\r\n", ch);
    send_to_char("Note player affects increase hp/mana/move by 25, abilities only by 1.\r\n", ch);
    send_to_char("You will only be able to increase hp/mana/move by a maximum of 2000 over your current max.\r\r\n\n", ch);
...
void do_train(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg, "yes"))
  {
    ...
    if(sn == gsn_mine || sn == gsn_forge || sn == gsn_bake || sn == gsn_gather || sn == gsn_fell || sn == gsn_mill || sn == gsn_jewelry || sn == gsn_tan || sn == gsn_hunt)
    {
      send_to_char("\r\nYou cannot train in a tradeskill.\r\n", ch);
      return;
    }
    ...
    switch (skill->type)
    {
      default:
      case SKILL_SKILL:
        act(AT_GREEN, "\r\nYou perform the skill on them until they appear to grasp it.", victim, NULL, NULL, TO_CHAR);
      ...
      case SKILL_SPELL:
        act(AT_GREEN, "\r\nYou teach the required componets, and gestures of the magic spell.", victim, NULL, NULL, TO_CHAR);
      ...
      case SKILL_TONGUE:
        act(AT_GREEN, "\r\nYou teach the basic words of the language.", victim, NULL, NULL, TO_CHAR);
      ...
      case SKILL_SONG:
        act(AT_GREEN, "\r\nYou teach the basic words of the song.", victim, NULL, NULL, TO_CHAR);
...
void do_addaffects(CHAR_DATA *ch, char *argument)
{
  ...
  if(!arg1 || arg1[0] == '\0' || !arg2 || arg2[0] == '\0' || !is_number(arg2) || (rating = atoi(arg2)) < 1 || rating > 15)
  {
    send_to_char("Syntax: 'addaffects (obj) (rating) [number]'\r\n  where (rating) is a number from 1 to 15 (avg 10) and optional (number)\r\n  the number of affects (no number = random)\r\n", ch);
    return;
  }
...
/* For now, we'll work this out the same as glory.. */
  ...
  ch_printf(ch, "&cObject has been set at &C%d&c. Taking into account the object's level, this equals &C%d&c points (obj rating).\r\n", rating, glory);
  ...
  {
    ch_printf(ch, "&cInvalid number of affects - &C%d&c. Please choose a number between 1 and 5.\r\n", numaffects);
    return;
  }
...
/* What is the best way to randomise glory picks but also get the maximum into the item? I'd say 
  ...
  {
    if(x == 1)
    { /* Panic mode - no more affects, so let's FILL the item from here. */
      if(glory > 75)
      { /* too much for flag, let's hp/mana it, anything to glory_costs[10] */
...
#define MAX_SCORES 20 /* variable for easy adjustment of how many to display */
...
/*
void do_petratings(CHAR_DATA *ch, char *argument)
{
  SCORE_DATA             *score;
  short                   counter = 1;

  ch_printf(ch, "     &cThe Top &C20&c Players with 6 Dragons!\r\r\n\n");
  ch_printf(ch, "\t      Owner         Pet        Score\r\n");

  for(score = first_score; score; score = score->next)
  {
    if(!IS_BLIND(ch))
    {
      ch_printf(ch, "\t&c[%2d]&C %-12s %-32d\r\n", counter, score->master->name, score->name, score->score);
      ++counter;
    }
    else if(IS_BLIND(ch))
    {
  ...
  ch_printf(ch, "\t&cSee &WHELP PETRATINGS&c for more information.\r\n");
