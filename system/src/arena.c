...
void do_arena(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_IMMORTAL(ch))
  {
    ...
    if(!*maxlvl || !*area || !*type)
    {
      ...
      send_to_char("Types available: 1, 2, 3, 4, 5\r\n", ch);
      send_to_char("1 = Player vs. Player, 2 = Freeze Tag, 3 = Dragon Tease, 4 = ZOT Challenge,  5= Random Arena\r\n", ch);
      return;
    }
    ...
    if(!strcmp(area, "jungle") || !strcmp(area, "desert") || !strcmp(area, "arctic") || !strcmp(area, "water"))
      establish_area(area);
    else
    {
    ...
    if(arena_type == 3)
    {
      snprintf(buf, MIL, "%s has opened a level %d or below arena in the desert area!", ch->name, max_level);
    }
    else
    {
      snprintf(buf, MIL, "%s has opened a level %d or below arena in the %s area!", ch->name, max_level, area);
    }
    announce(buf);
  }
  else
...
void arena_enter(CHAR_DATA *ch)
{
  ...
  if(arena_type == 3)
  {
    snprintf(buf, MIL, "%s has joined the Dragon Tease Event! -[Won: %d / Lost: %d]-", ch->name, ch->arena_wins, ch->arena_loss);
    arena_chan(buf);
  }
  if(arena_type != 3)
  {
    snprintf(buf, MIL, "%s has joined the bloodbath! -[Won: %d / Lost: %d]-", ch->name, ch->arena_wins, ch->arena_loss);
    arena_chan(buf);
  }
  if(arena_type == 2)
...
void run_game()
{
  CHAR_DATA              *player = NULL, *uplayer = NULL, *mob;
...
// stop players being changed to human priests - Vladaar
    ...
    if(xIS_SET(player->act, PLR_BATTLE))
      send_to_char("!!SOUND(sound/arena.wav)\r\n", player);
    if(arena_type == 4)
    {
      ch_printf(player, "Zot Arena Challenge invokes a surge of lightning to zot the life out of you.\r\n");
      player->hit = 1;
      player->mana = 1;
      player->move = (get_curr_con(player) * 4);
    }
...
void end_game(bool silent)
{
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    ...
    player = d->character;
    if(!player->in_room || player->in_room->vnum < (arena_low_vnum - 1) || player->in_room->vnum > (arena_high_vnum + 1))
      continue;
...
void setup_type()
{
  ...
  else if(arena_type == 4)
  {
    snprintf(buf, MIL, "                  &R<-[&WZOT Challenge&R]->&d");
    arena_chan(buf);
    game_length = 25;
  }
  else
...
void do_game(void)
{
  ...
  else
  {
    ...
    {
      if(get_arena_count() > arena_population)
        log_printf("Error: Arena has %d unidentified player(s) within the game.", get_arena_count() - arena_population);
      else
        log_printf("Error: Arena is missing %d player(s) from the game.", arena_population - get_arena_count());
...
void do_challenge(CHAR_DATA *ch, char *argument)
{
  ...
  if(victim->level < 6)
  {
    send_to_char("You should be ashamed of yourself for attempting to fight such a weak foe.\r\n", ch);
    return;
  }
...
void do_accept(CHAR_DATA *ch, char *argument)
{
  ...
  {
    bug("%s: Couldn't find room %d for the dual to take place.", __FUNCTION__, arena_prep_vnum);
    send_to_char("There was a problem in creating the rooms so not going to continue with the dual.\r\n", ch);
    send_to_char("There was a problem in creating the rooms so not going to continue with the dual.\r\n", challenger);
    arena_population = 0;
    ch->challenged = NULL;
    challenger->challenge = NULL;
...
void find_arena_winner(CHAR_DATA *ch)
{
  ...
  for(descr = first_descriptor; descr; descr = descr->next)
  {
    if(!descr->connected)
    {
      ...
      if(player->in_room->vnum >= arena_low_vnum - 1 && (player->in_room->vnum <= arena_high_vnum + 1))
      {
        restore_char(player);
        ...
        if(!ch)
        {
          ...
          if(!challenge)
          {
            ...
            ch_printf(player, "Your glory has been increased by 25 for winning the arena event!\r\n");
          }
          if(xIS_SET(player->act, PLR_TEASE))
            xREMOVE_BIT(player->act, PLR_TEASE);
        }
        else
        {
          ...
          if(!challenge)
          {
            ch->quest_curr += 25;
            ch->quest_accum += 25;
            ch_printf(ch, "Your glory has been increased by 25 for winning the arena event!\r\n");
          }
          if(xIS_SET(ch->act, PLR_TEASE))
            xREMOVE_BIT(ch->act, PLR_TEASE);
        }
...
void parse_entry(short time)
{
  ...
  if(time >= 1 && !challenge)
  {
    snprintf(buf, MIL, "Use the &WARENA&C command to enter the arena. Only %d hour(s) until chaos begins.", time);
    arena_chan(buf);
  }
}
...
void do_aglance(CHAR_DATA *ch, char *argument)
{
  ...
  if(ch->in_room != NULL)
  {
    if(IS_SET(ch->in_room->room_flags, ROOM_ARENA) && str_cmp(ch->in_room->area->filename, "throngfort.are") && str_cmp(ch->in_room->area->filename, "allifort.are"))
    {
  ...
  for(d = first_descriptor; d != NULL; d = d->next)
  {
    if(d->character != NULL)
    {
      ...
      {
        if(d->character->in_room && !str_cmp(d->character->in_room->area->filename, "throngfort.are") || !str_cmp(d->character->in_room->area->filename, "allifort.are"))
          continue;
