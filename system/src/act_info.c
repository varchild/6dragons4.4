...
void show_list_to_char(OBJ_DATA *list, CHAR_DATA *ch, bool fShort, bool fShowNothing)
{
  ...
  if(!ch->desc && !xIS_SET(ch->act, ACT_BEASTMELD))
    return;
  ...
  if(!list)
  {
    if(fShowNothing)
    {
      if((IS_NPC(ch) && !xIS_SET(ch->act, ACT_BEASTMELD)) || xIS_SET(ch->act, PLR_COMBINE))
        send_to_char("     ", ch);
  ...
  {
    ms = (ch->mental_state ? ch->mental_state : 1) * (IS_NPC(ch) ? 1 : (ch->pcdata->condition[COND_DRUNK] ? (ch->pcdata->condition[COND_DRUNK] / 12) : 1));
  }
  if(abs(ms) > 40)
  {
  ...
  for(obj = list; obj; obj = obj->next_content)
  {
    ...
    if(obj->wear_loc == WEAR_NONE && (can_see_obj(ch, obj)
                                      || (IS_OBJ_STAT(obj, ITEM_INVIS)
                                          && IS_OBJ_STAT(obj, ITEM_GLOW) && !IS_OBJ_STAT(obj, ITEM_BURIED) && !IS_OBJ_STAT(obj, ITEM_HIDDEN))) && (obj->item_type != ITEM_TRAP || obj->value[3] == 0))
    {
...
/* Volk - Money function. Looks good to me. Updated in mud.h to check all names and can_see */
  if(victim->morph != NULL)
    mudstrlcpy(name, victim->morph->morph->short_desc, MSL);
  else
...
// Only Sanctuary and Shield
  ...
  {
    ch_printf(ch, "&Z%s is surrounded in a magical energy force.\r\n", name);
  }
...
void show_char_to_char_0(CHAR_DATA *victim, CHAR_DATA *ch, int num)
{
  ...
  if(!IS_NPC(victim) && !victim->desc)
  {
    ...
    else if(!IS_AFFECTED(victim, AFF_POSSESS) && !IS_SET(victim->pcdata->flags, PCFLAG_PUPPET) && !IS_AFFECTED(victim, AFF_BEASTMELD))
  ...
  if(!IS_NPC(victim))
  {
    if((xIS_SET(victim->act, PLR_RED) && xIS_SET(victim->act, PLR_PLAYING)) || xIS_SET(victim->act, PLR_WAITING) && xIS_SET(victim->act, PLR_RED))
    {
      mudstrlcat(buf, "&W[&RRED&W]&w ", MSL);
    }
    if((xIS_SET(victim->act, PLR_BLUE) && xIS_SET(victim->act, PLR_PLAYING)) || xIS_SET(victim->act, PLR_WAITING) && xIS_SET(victim->act, PLR_BLUE))
    {
  ...
  if(IS_NPC(victim) && IS_IMMORTAL(ch) && xIS_SET(victim->act, ACT_PROTOTYPE))
    mudstrlcat(buf, "(PROTO) ", MSL);
  if(IS_NPC(victim) && ch->mount && ch->mount == victim && xIS_SET(victim->act, ACT_MOUNTED) && ch->in_room == ch->mount->in_room)
    mudstrlcat(buf, "(Mounted) ", MSL);
  ...
  if((victim->position == victim->defposition && victim->long_descr && victim->long_descr[0] != '\0') || (victim->morph && victim->morph->morph && victim->morph->morph->defpos == victim->position))
  {
    if(victim->morph != NULL)
    {
      if(!IS_IMMORTAL(ch))
  ...
  switch (victim->position)
  {
    ...
    case POS_RESTING:
      if(IS_AFFECTED(victim, AFF_DISABLED))
      {
        mudstrlcat(buf, " is writhing in agony on the ground.&G", MSL);
        break;
      }
      if(victim->on != NULL)
      {
    ...
    case POS_STANDING:
      ...
      else if((victim->in_room->sector_type == SECT_UNDERWATER || victim->in_room->sector_type == SECT_OCEANFLOOR) && !IS_AFFECTED(victim, AFF_AQUA_BREATH) && !IS_NPC(victim))
...
void show_char_to_char_1(CHAR_DATA *victim, CHAR_DATA *ch)
{
  ...
  if(!IS_NPC(victim) && !IS_NPC(ch) && !IS_IMMORTAL(ch) && ch != victim)
  {
    ch_printf(ch, "\r\n%s appears to be a %s.\r\n", capitalize(victim->name), race_table[victim->race]->race_name);
  }

  if(VLD_STR(victim->description))
  {
    if(victim->morph != NULL && victim->morph->morph != NULL && VLD_STR(victim->morph->morph->description))
  ...
  else
  {
    if(victim->morph != NULL && victim->morph->morph != NULL && VLD_STR(victim->morph->morph->description))
    {
      ...
      act(AT_PLAIN, "$E has not made a description yet.", ch, NULL, victim, TO_CHAR);
    else
      act(AT_PLAIN, "You have not set your description yet.  See help description.", ch, NULL, NULL, TO_CHAR);
  }
  ...
  for(iWear = 0; iWear < MAX_WEAR; iWear++)
  {
    if((obj = get_eq_char(victim, iWear)) != NULL && can_see_obj(ch, obj))
    {
      ...
      if((!IS_NPC(victim)) && (victim->race > 0) && (victim->race < MAX_PC_RACE) && VLD_STR(race_table[victim->race]->where_name[iWear]))
  ...
  {
    if(IS_NPC(victim))
      ch_printf(ch, "\r\nMobile #%d '%s' ", victim->pIndexData->vnum, VLD_STR(victim->name) ? victim->name : "");
    else
...
bool is_same_mob(CHAR_DATA *i, CHAR_DATA *j)
{
  ...
  if(i->pIndexData == j->pIndexData && GET_POS(i) == GET_POS(j)
     ...
     && ((!VLD_STR(i->description) && !VLD_STR(j->description)) || (VLD_STR(i->description) && VLD_STR(j->description) && !str_cmp(i->description, j->description))))
    return TRUE;
  return FALSE;
}
...
void show_char_to_char(CHAR_DATA *list, CHAR_DATA *ch)
{
  ...
  for(rch = list; rch; rch = rch->next_in_room)
  {
     ...
     */
    if(can_see(ch, rch) && (!room_is_dark(ch->in_room) || xIS_SET(ch->act, PLR_HOLYLIGHT)))
    {
      if(IS_NPC(rch)
         && (rch->pIndexData->vnum == MOB_VNUM_SOLDIERS || rch->pIndexData->vnum == MOB_VNUM_ARCHERS || rch->pIndexData->vnum == MOB_VNUM_GUARDS || rch->pIndexData->vnum == MOB_VNUM_WARRIORS))
      {
        soldier_count = 1;
        ...
        while(rch->next_in_room)
        {
          if(!IS_NPC(rch->next_in_room)
             || (rch->next_in_room->pIndexData->vnum != MOB_VNUM_SOLDIERS
                 && rch->next_in_room->pIndexData->vnum != MOB_VNUM_ARCHERS && rch->next_in_room->pIndexData->vnum != MOB_VNUM_GUARDS && rch->next_in_room->pIndexData->vnum != MOB_VNUM_WARRIORS))
            break;
          soldier_count++;
      ...
      // see ANYTHING?
    {
      if(room_is_dark(ch->in_room) && (IS_AFFECTED(ch, AFF_INFRARED) || IS_AFFECTED(ch, AFF_DEMONIC_SIGHT) || IS_AFFECTED(ch, AFF_TRUESIGHT)))
      {
        set_char_color(AT_BLOOD, ch);
  ...
  if(WILD_DESC_ON(ch))
  {
    ...
    switch (ch->in_room->sector_type)
    {
      ...
      case SECT_MOUNTAIN:
        ...
        break;
      case SECT_AIR:
        send_to_char("High above the ground, there are small clouds around here.\r\nEverything appears small on the ground below.  The air is\r\nthinner up in the high altitudes here.", ch);
        break;
      case SECT_CITY:
        send_to_char
      ...
      case SECT_CAMPSITE:
        ...
        break;
      case SECT_AREA_ENT:
        send_to_char("The wilderness clears up here.  You may enter or leave the\r\nwilderness here.  The exit to the wilderness appears to enter\r\ninto a new place.\r\n", ch);
        break;
      case SECT_LAVA:
        send_to_char("Lava gushes over the entire terrain, burning and scalding\r\nanything it touches. Very few creatures can survive within\r\nits immense heat.\r\n", ch);
        break;
      ...
      case SECT_BEACH:
        ...
        break;
      case SECT_FOG:
        send_to_char("A dense fog obscures everything from view.  The ground\r\nis not even visible, nor are your own hands\r\nwhen held at arm length.\r\n", ch);
        break;
      case SECT_SKY:
      ...
      case SECT_ORE:
        ...
        break;
      default:
        log_printf("Wilderness Error: Sector out of bounds in %d couldn't establish description.\r\n", ch->in_room->vnum);
        break;
    }
  }
  send_to_char("\r\n", ch);
...
void gen_wilderness_name(CHAR_DATA *ch)
{
  ...
  switch (ch->in_room->sector_type)
  {
    ...
    case SECT_ORE:
      ...
      break;
    default:
      log_printf("Wilderness Error: Sector out of bounds in %d, couldn't establish name. ", ch->in_room->vnum);
...
void do_look(CHAR_DATA *ch, char *argument)
{
  ...
  if(xIS_SET(ch->act, ACT_BEASTMELD))
  {
    ...
    {
      if(xIS_SET(vch->act, ACT_AGGRESSIVE) || xIS_SET(vch->act, ACT_WILD_AGGR))
      {
        set_fighting(vch, ch);
        start_hating(vch, ch);
      }
  ...
  if(arg1[0] == '\0' || !str_cmp(arg1, "auto"))
  {
    ...
    {
      ch_printf(ch, "&w[&cArea Flags:&C %s &w]", flag_string(ch->in_room->area->flags, area_flags));
...
/* Volk - moved dark code here so infra can still see room name! */
    ...
    if(!IS_NPC(ch)
       ...
       && !IS_AFFECTED(ch, AFF_DEMONIC_SIGHT) && !IS_AFFECTED(ch, AFF_TRUESIGHT) && ((IS_SET(ch->in_room->area->flags, AFLAG_DARKNESS) && ch->in_room->light <= 0) || room_is_dark(ch->in_room)))
    {
    ...
    switch (obj->item_type)
    {
      ...
      case ITEM_DRINK_CON:
        ...
        ch_printf(ch, "It's %s full of a %s liquid.\r\n",
                  obj->value[1] < obj->value[0] / 4 ? "less than" : obj->value[1] < 3 * obj->value[0] / 4 ? "about" : "more than", liq_table[obj->value[2]].liq_color);
      ...
      case ITEM_RESOURCE_BAG:
        if(IS_BLIND(ch) && (obj->item_type == ITEM_RESOURCE_BAG) && obj->value[0] == 500)
        {
          ch_printf(ch,
                    "%s contains:\r\n%d fir\r\n%d cedar\r\n%d pine\r\n%d walnut\r\n%d oak\r\n%d hickory",
                    obj->short_descr, obj->craft1, obj->craft2, obj->craft3, obj->craft4, obj->craft5, obj->craft6);
        ...
        if(IS_BLIND(ch) && (obj->item_type == ITEM_RESOURCE_BAG) && obj->value[0] == 125)
        {
          ch_printf(ch,
                    "%s contains:\r\n%d titanium\r\n%d vibranium\r\n%d adamantium\r\n%d carbonadium\r\n%d meteor", obj->short_descr, obj->craft1, obj->craft2, obj->craft3, obj->craft4, obj->craft5);
        ...
        if(IS_BLIND(ch) && (obj->item_type == ITEM_RESOURCE_BAG) && obj->value[0] != 500 && obj->value[0] != 125)
        {
          if(ch->pcdata->tradeclass == 24)
          {
            ch_printf(ch,
                      "%s contains:\r\n%d bronze\r\n%d silver\r\n%d gold\r\n%d iron\r\n%d steel\r\n%d titanium\r\n%d amethyst\r\n%d pearl\r\n%d emerald\r\n%d ruby\r\n%d sapphire\r\n%d diamond",
                      obj->short_descr, obj->craft1, obj->craft2, obj->craft3, obj->craft4, obj->craft5, obj->craft6, obj->craft7, obj->craft8, obj->craft9, obj->craft10, obj->craft11, obj->craft12);
          }
          else
          {
            ch_printf(ch,
        ...
        if(!IS_BLIND(ch) && (obj->item_type == ITEM_RESOURCE_BAG) && obj->value[0] != 500 && obj->value[0] != 125)
        {
          ...
          {
            ch_printf(ch,
                      "%s contains:\r\n%d bronze %d silver %d gold %d iron %d steel %d titanium\r\n%d amethyst %d pearl %d emerald %d ruby %d sapphire %d diamond\r\n",
                      obj->short_descr, obj->craft1, obj->craft2, obj->craft3, obj->craft4, obj->craft5, obj->craft6, obj->craft7, obj->craft8, obj->craft9, obj->craft10, obj->craft11, obj->craft12);
          }
          else
          {
            ch_printf(ch, "%s contains:\r\n%d bronze %d silver %d gold %d iron %d steel %d titanium", obj->short_descr, obj->craft1, obj->craft2, obj->craft3, obj->craft4, obj->craft5, obj->craft6);
          }
        }
        break;
      case ITEM_HUNTERS_BAG:
        if(IS_BLIND(ch) && (obj->item_type == ITEM_HUNTERS_BAG))
        {
          ch_printf(ch,
                    ...
                    obj->craft10, obj->craft11, obj->craft12, obj->craft13, obj->craft14, obj->craft15, obj->craft16, obj->craft17, obj->craft18, obj->craft19, obj->craft20);
        }
        else
        {
          ch_printf(ch,
                    "%s contains:\r\n%d rabbit %d goose %d badger %d turkey %d coyote %d leopard\r\n%d lion %d pheasant %d duck %d raccoon %d boar %d bobcat %d bear\r\n%d dove %d squirrel %d deer %d antelope %d wolf %d moose %d bugbear",
                    obj->short_descr, obj->craft1, obj->craft2, obj->craft3, obj->craft4,
                    obj->craft5, obj->craft6, obj->craft7, obj->craft8, obj->craft9,
                    obj->craft10, obj->craft11, obj->craft12, obj->craft13, obj->craft14, obj->craft15, obj->craft16, obj->craft17, obj->craft18, obj->craft19, obj->craft20);
        }
      ...
      case ITEM_GATHER_BAG:
        ...
        {
          ch_printf(ch,
                    "%s contains:\r\n%d strawberries %d watermelons %d lettuce %d pickles %d wheat\r\n%d oranges %d peaches %d carrots %d cinnamon %d fig\r\n%d blueberries %d pears %d onions %d sugar-cane %d ginger\r\n%d raspberries %d corn %d radishes %d garlic %d pepper",
                    obj->short_descr, obj->craft1, obj->craft2, obj->craft3, obj->craft4,
                    obj->craft5, obj->craft6, obj->craft7, obj->craft8, obj->craft9,
                    obj->craft10, obj->craft11, obj->craft12, obj->craft13, obj->craft14, obj->craft15, obj->craft16, obj->craft17, obj->craft18, obj->craft19, obj->craft20);
        }
  ...
  else
  {
    ...
    if(IS_IMMORTAL(ch) && victim != ch)
    {
      ...
      ch_printf(ch, "is a level %d %s %s.\r\n",
                ...
                MAX_PC_CLASS && class_table[victim->Class]->who_name && class_table[victim->Class]->who_name[0] != '\0' ? class_table[victim->Class]->who_name : "unknown");
...
void do_exits(CHAR_DATA *ch, char *argument)
{
  ...
  for(pexit = ch->in_room->first_exit; pexit; pexit = pexit->next)
  {
    if(pexit->to_room && !IS_SET(pexit->exit_info, EX_SECRET)
       && (!IS_SET(pexit->exit_info, EX_WINDOW) || IS_SET(pexit->exit_info, EX_ISDOOR)) && !IS_SET(pexit->exit_info, EX_HIDDEN) && !IS_SET(pexit->exit_info, EX_DIG))
    {
      found = TRUE;
      closed = IS_SET(pexit->exit_info, EX_CLOSED);
      locked = (IS_SET(pexit->exit_info, EX_LOCKED) || IS_SET(pexit->exit_info, EX_BOLTED));
      DT = ((IS_IMMORTAL(ch)) && IS_SET(pexit->to_room->room_flags, ROOM_DEATH));
      if(fAuto)
      {
        mudstrlcat(buf, " ", MSL);
        ...
        if(IS_SET(pexit->exit_info, EX_AUTOFULL))
          ...
          mudstrlcat(buf, DT ? strupper(dir_name[pexit->vdir]) : dir_name[pexit->vdir], MSL);
      ...
      else
      {
        ...
        if(!IS_SET(pexit->exit_info, EX_AUTOFULL))
          snprintf(buf + strlen(buf), MSL - strlen(buf), "%s" "%*s - %s \r\n", capitalize(dir_name[pexit->vdir]), spaces, /* number 
                   ...
                   locked ? "&R[&WClosed and Locked/Barred&R]&W" : closed ? "[Closed]" : room_is_dark(pexit->to_room) ? "Too dark to tell" : pexit->to_room->name);
        else
...
void do_helpcheck(CHAR_DATA *ch, char *argument)
{
  ...
  if(fSkills)
  { /* run through skill table */
    send_to_pager("\r\n&CMissing Skill/Spell Helps\r\n\r\n", ch);
    for(sn = 0; sn < top_sn; sn++)
    {
      if(!(help = get_help(ch, skill_table[sn]->name)))
      { /* no help entry */
...
void do_hset(CHAR_DATA *ch, char *argument)
{
  ...
  if(!str_cmp(arg1, "save"))
  {
    ...
    fprintf(fpout, "#HELPS\n\n");
    for(pHelp = first_help; pHelp; pHelp = pHelp->next)
      fprintf(fpout, "%d %ld %s~\n%s~\n\n", pHelp->level, pHelp->modified_time, VLD_STR(pHelp->keyword) ? pHelp->keyword : "", VLD_STR(pHelp->text) ? help_fix(pHelp->text) : "");
    gettimeofday(&now_time, NULL);
...
{
  int                     min, max, value, cnt, level, count, col;
  char                    arg[MIL];
  HELP_DATA              *help;
  ...
  for(level = -1; level <= MAX_LEVEL; level++)
  {
    ...
    {
      if(help->level >= min && help->level <= max && help->level == level && (!idx || nifty_is_name_prefix(idx, help->keyword)))
      {
        count++;
        if(count == 1)
          pager_printf(ch, "\r\n[Level %3d]\r\n", level);
...
int new_who(CHAR_DATA *ch, char *argument)
{
  ...
  char                    constate[16];
  char                    rank[200], clan_name[MIL], city_name[MIL], invis_str[50], council_name[MIL];
  char                    col[MSL];
  int                     pcount = 0, color = 0;
  bool                    skipconstate;
  bool                    first;
  bool                    sdeadly = FALSE;
  ...
  for(person = first_char; person; person = person->next)
  {
    ...
    if(IS_PLR_FLAG(person, PLR_AFK))
      ...
      snprintf(constate, sizeof(constate), " %s[RP]", s5);
    else
      constate[0] = '\0';
    ...
    {
      if(IS_BLIND(ch))
        snprintf(rank, 200, "%d tri class %s %s %s.", person->level, class_table[person->Class]->who_name, class_table[person->secondclass]->who_name, class_table[person->thirdclass]->who_name);
      else
        snprintf(rank, 200, "%s[%s%-3d%s][%s%-3.3s%s/%s%-3.3s%s/%s%-3.3s%s]",
                 s5, col, person->level, s5, s6, class_table[person->Class]->who_name, s5, s6, class_table[person->secondclass]->who_name, s5, s6, class_table[person->thirdclass]->who_name, s5);
    }
    else if(IS_SECONDCLASS(person))
    {
      if(IS_BLIND(ch))
        snprintf(rank, 200, "%d dual class %s %s.", person->level, class_table[person->Class]->who_name, class_table[person->secondclass]->who_name);
      else
        snprintf(rank, 200, "%s[%s%-3d%s][%s%-5.5s%s/%s%-5.5s%s ]", s5, col, person->level, s5, s6, class_table[person->Class]->who_name, s5, s6, class_table[person->secondclass]->who_name, s5);
    }
    else
    {
      if(IS_BLIND(ch))
        snprintf(rank, 200, "%d class %s.", person->level, class_table[person->Class]->who_name);
      else
        snprintf(rank, 200, "%s[%s%-3d%s][%s%-12.12s%s]", s5, col, person->level, s5, s6, class_table[person->Class]->who_name, s5);
    }
    ...
    if(IS_CITY(person))
    {
      ...
      if(VLD_STR(person->pcdata->city->duke) && !str_cmp(person->name, person->pcdata->city->duke))
      {
      ...
      else if(VLD_STR(person->pcdata->city->baron) && !str_cmp(person->name, person->pcdata->city->baron))
      {
        if(person->sex == SEX_FEMALE)
      ...
      else if(VLD_STR(person->pcdata->city->captain) && !str_cmp(person->name, person->pcdata->city->captain))
        mudstrlcat(city_name, "Captain of ", MIL);
      else if(VLD_STR(person->pcdata->city->sheriff) && !str_cmp(person->name, person->pcdata->city->sheriff))
        mudstrlcat(city_name, "Sheriff of ", MIL);
      else if(VLD_STR(person->pcdata->city->knight) && !str_cmp(person->name, person->pcdata->city->knight))
    ...
    if(IS_CLANNED(person))
    {
      ...
      if(VLD_STR(person->pcdata->clan->chieftain) && !str_cmp(person->name, person->pcdata->clan->chieftain))
        mudstrlcat(clan_name, "Chieftain of ", MIL);
      else if(VLD_STR(person->pcdata->clan->warmaster) && !str_cmp(person->name, person->pcdata->clan->warmaster))
      {
    ...
    if(!IS_BLIND(ch))
      pager_printf(ch, " %s%s&z%s%s%s%s\r\n", IS_PKILL(person) ? "&R" : "&R", person->name, person->pcdata->title, clan_name, city_name, constate);
    else
      pager_printf(ch, " %s%s&z%s%s%s%s\r\n", IS_PKILL(person) ? " " : " ", person->name, person->pcdata->title, clan_name, city_name, constate);
  ...
  for(d = first_descriptor; d; d = d->next)
  {
    ...
    if(person)
    {
      ...
      if(VLD_STR(sysdata.clan_overseer) && !str_cmp(person->name, sysdata.clan_overseer))
        extra_title = " &W[&POverseer of Clans&W]&D";
      else if(VLD_STR(sysdata.clan_advisor) && !str_cmp(person->name, sysdata.clan_advisor))
      ...
      if(person->pcdata->council)
      {
        ...
        {
          if(VLD_STR(person->pcdata->council->head) && !str_cmp(person->name, person->pcdata->council->head))
            mudstrlcat(council_name, "Head of ", MIL);
        }
        else
        {
          if((VLD_STR(person->pcdata->council->head)
              && !str_cmp(person->name, person->pcdata->council->head)) || (VLD_STR(person->pcdata->council->head2) && !str_cmp(person->name, person->pcdata->council->head2)))
      ...
      pager_printf(ch, "%s &W%s&z%s%s%s%s\r\n", invis_str, person->name, title, extra_title, council_name, constate);
    }
  }
...
void do_who(CHAR_DATA *ch, char *argument)
{
  ...
  amount = 60;  /* Determine amount to put in front of line */
...
void do_where(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg[0] == '\0')
  {
    ...
    for(d = first_descriptor; d; d = d->next)
    {
      if((d->connected == CON_PLAYING || d->connected == CON_EDITING)
         ...
         && victim->in_room && victim->in_room->area == ch->in_room->area && can_see(ch, victim) && (get_trust(ch) >= get_trust(victim) || !IS_SET(victim->pcdata->flags, PCFLAG_DND)))
      {
        found = TRUE;
  ...
  else
  {
    ...
    {
      if(victim->in_room && victim->in_room->area == ch->in_room->area && !IS_AFFECTED(victim, AFF_HIDE) && !IS_AFFECTED(victim, AFF_SNEAK) && can_see(ch, victim) && is_name(arg, victim->name))
      {
        found = TRUE;
...
int first_learn_skill_level(CHAR_DATA *ch, int sn)
{
  ...
  if(sn == gsn_gather || sn == gsn_bake || sn == gsn_mix || sn == gsn_unearth)
    return 0;

  if(sn == gsn_hunt || sn == gsn_tan || sn == gsn_jewelry || sn == gsn_fell || sn == gsn_mill || sn == gsn_produce)
    return 0;
  ...
  if(tshow != SKILL_UNKNOWN || !VLD_STR(argument) || showdshow)
  {
    ...
    for(sn = 0; sn < top_sn; sn++)
    {
      ...
      if(sn == gsn_mine || sn == gsn_forge || sn == gsn_tan || sn == gsn_bake
         || sn == gsn_gather || sn == gsn_mix || sn == gsn_jewelry || sn == gsn_hunt || sn == gsn_fell || sn == gsn_mill || sn == gsn_produce || sn == gsn_unearth)
        continue;
...
// TRUE, or from the prac dshow list if dshowloearned == FALSE - Aurin
      ...
      if(ch->pcdata->tradeclass == 26 && ch->Class == CLASS_DRAGONLORD)
      {
        sn = gsn_produce;
        if((showdshow && ch->pcdata->dshowlearned[sn] == TRUE) || (!showdshow && ch->pcdata->dshowlearned[sn] == FALSE))
          pager_printf(ch, "&c%20s &W%3d%% \r\n", skill_table[sn]->name, ch->pcdata->learned[sn]);
  ...
  else
  {
    ...
    { /* Now we're practicing! */
      short                   percent = URANGE(3, number_range(3, get_curr_int(ch)), 50); /* This 
                                                                                           * is 
                                                                                           * the 
                                                                                           * maximum 
                                                                                           * it 
      ...
      switch (number_range(1, 10))
      {
        ...
        case 1:  /* good prac! */
        case 2:
          act(AT_ACTION, "You practice $T as hard as you can, and get the most out of this practice session!", ch, NULL, skill_table[sn]->name, TO_CHAR);
          act(AT_ACTION, "$n practices $T.", ch, NULL, skill_table[sn]->name, TO_ROOM);
          ch->pcdata->learned[sn] += percent + 5;
          break;

        case 3:  /* bad prac.. */
          act(AT_ACTION, "You practice $T, but your session is worse than usual.", ch, NULL, skill_table[sn]->name, TO_CHAR);
          act(AT_ACTION, "$n practices $T.", ch, NULL, skill_table[sn]->name, TO_ROOM);
      ...
      { /* Pity on low skills.. */
        send_to_char("You now have the minimum amount of knowledge required to use this skill.\n\r", ch);
      ...
      if(ch->pcdata->learned[sn] > (skill_table[sn]->skill_adept[ch->Class] - 6) || ch->pcdata->learned[sn] >= 50)
        act(AT_TELL, "$n tells you, 'You'll have to practice it on your own now...'", mob, NULL, ch, TO_VICT);
...
void do_wimpy(CHAR_DATA *ch, char *argument)
{
  ...
  if(IS_NPC(ch))
  {
    if(xIS_SET(ch->act, ACT_WIMPY))
    {
      ...
      act(AT_GREEN, "Wimpy setting is removed.", ch, NULL, NULL, TO_ROOM);
      return;
    }
    xSET_BIT(ch->act, ACT_WIMPY);
    act(AT_GREEN, "Wimpy setting is turned on.", ch, NULL, NULL, TO_ROOM);
    return;
  }
    ...
    for(command = command_hash[hash]; command; command = command->next)
    {
      if(command->level <= MAX_LEVEL)
      {
        ...
        if(ch->level < MAX_LEVEL && command->cshow == FALSE)
          continue;
        if(VLD_STR(ch->pcdata->bestowments) && is_name(command->name, ch->pcdata->bestowments))
        {
...
void do_klive( CHAR_DATA *ch, char *argument )
{
   char arg[MIL];
   ...
   if( arg == NULL || arg[0] == '\0' )
   {
      xTOGGLE_BIT( ch->act, PLR_KEEPALIVE );
      if( xIS_SET( ch->act, PLR_KEEPALIVE ) )
         ch_printf( ch, "KeepAlive time is now set to %d min%s.\r\n", ch->pcdata->kltime,
                    ch->pcdata->kltime == 1 ? "" : "s" );
      else
...
{
  char                    arg[MIL], s1[16], s2[16], s3[16], s4[16];
  ...
  if(!VLD_STR(arg))
  {
    ch_printf(ch, "\r\nConfigurations\r\nUse config name to toggle it on or off, see help config\r\n\r\n");
    ch_printf(ch, "Display:%s", s2);
    if(IS_BLIND(ch))
    {
      ch_printf(ch, "\r\n%-14s%s\r\n", IS_SET(ch->pcdata->flags, PCFLAG_PAGERON) ? "pager on" : "pager off", s2);
      ch_printf(ch, "%-14s%s\r\n", IS_SET(ch->pcdata->flags, PCFLAG_GAG) ? "gag on" : "gag off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_BRIEF) ? "brief on" : "brief off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_PROMPT) ? "prompt on" : "prompt off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_ANSI) ? "ansi on" : "ansi off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_RIP) ? "rip on" : "rip off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_COMBINE) ? "combine on" : "combine off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_BLANK) ? "blank on" : "blank off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_RP) ? "rp on" : "rp off", s2);
      ch_printf(ch, "%-14s%s\r\n", IS_SET(ch->pcdata->flags, PCFLAG_SHOWSLOTS) ? "slots on" : "slots off", s2);
      ch_printf(ch, "%-14s%s", xIS_SET(ch->act, PLR_OBJID) ? "id on" : "id off", s2);
    }
    else
    {
      ch_printf(ch, "\r\n%-14s%s", IS_SET(ch->pcdata->flags, PCFLAG_PAGERON) ? "&Gpager on" : "&Rpager off", s2);
      ch_printf(ch, "%-14s%s", IS_SET(ch->pcdata->flags, PCFLAG_GAG) ? "&Ggag on" : "&Rgag off", s2);
      ch_printf(ch, "%-14s%s", xIS_SET(ch->act, PLR_BRIEF) ? "&Gbrief on" : "&Rbrief off", s2);
      ch_printf(ch, "%-14s%s", xIS_SET(ch->act, PLR_PROMPT) ? "&Gprompt on" : "&Rprompt off", s2);
      ch_printf(ch, "%-14s%s", xIS_SET(ch->act, PLR_ANSI) ? "&Gansi on" : "&Ransi off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_RIP) ? "&Grip on" : "&Rrip off", s2);
      ch_printf(ch, "%-14s%s", xIS_SET(ch->act, PLR_COMBINE) ? "&Gcombine on" : "&Rcombine off", s2);
      ch_printf(ch, "%-14s%s", xIS_SET(ch->act, PLR_BLANK) ? "&Gblank on" : "&Rblank off", s2);
      ch_printf(ch, "%-14s%s", xIS_SET(ch->act, PLR_RP) ? "&Grp on" : "&Rrp off", s2);
      ch_printf(ch, "%-14s%s", IS_SET(ch->pcdata->flags, PCFLAG_SHOWSLOTS) ? "&Gslots on" : "&Rslots off", s2);
    ...
    if(IS_BLIND(ch))
    {
      ch_printf(ch, "\r\n%s%-16s%s\r\n", s2, xIS_SET(ch->act, PLR_AUTOWAKE) ? "autowake on" : "autowake off", s2);
      ch_printf(ch, "%s%-16s%s\r\n", s2, xIS_SET(ch->act, PLR_AUTOTRASH) ? "autotrash on" : "autotrash off", s2);
      ch_printf(ch, "%s%-16s%s\r\n", s2, xIS_SET(ch->act, PLR_AUTODOOR) ? "autodoor on" : "autodoor off", s2);
      ...
      ch_printf(ch, "%-16s%s\r\n", xIS_SET(ch->act, PLR_AUTOGLANCE) ? "autoglance on" : "autoglance off", s2);
      ch_printf(ch, "%-16s%s\r\n", IS_SET(ch->pcdata->flags, PCFLAG_HINTS) ? "hints on" : "hints off", s2);
    }
    else
    {
      ch_printf(ch, "\r\n%s%-18s%s", s2, xIS_SET(ch->act, PLR_AUTOWAKE) ? "&Gautowake on" : "&Rautowake off", s2);
      ch_printf(ch, "%s%-18s%s", s2, xIS_SET(ch->act, PLR_AUTOTRASH) ? "&Gautotrash on" : "&Rautotrash off", s2);
      ch_printf(ch, "%s%-18s%s", s2, xIS_SET(ch->act, PLR_AUTODOOR) ? "&Gautodoor on" : "&Rautodoor off", s2);
      ch_printf(ch, "%-18s%s\r\n", xIS_SET(ch->act, PLR_AUTOMONEY) ? "&Gautomoney on" : "&Rautomoney off", s2);
      ch_printf(ch, "%-18s%s", xIS_SET(ch->act, PLR_AUTOLOOT) ? "&Gautoloot on" : "&Rautoloot off", s2);
      ch_printf(ch, "%-18s%s", xIS_SET(ch->act, PLR_AUTOEXIT) ? "&Gautoexit on" : "&Rautoexit off", s2);
      ch_printf(ch, "%-18s%s", xIS_SET(ch->act, PLR_AUTOGLANCE) ? "&Gautoglance on" : "&Rautoglance off", s2);
      ch_printf(ch, "%-18s%s", IS_SET(ch->pcdata->flags, PCFLAG_HINTS) ? "&Ghints on" : "&Rhints off", s2);
    }
    ...
    if(IS_BLIND(ch))
    {
      ch_printf(ch, "\r\n%s%-16s%s\r\n", s2, IS_SET(ch->pcdata->flags, PCFLAG_NORECALL) ? "norecall on" : "norecall off", s2);
      ch_printf(ch, "%-16s%s\r\n", IS_SET(ch->pcdata->flags, PCFLAG_NOSUMMON) ? "nosummon on" : "nosummon off", s2);
      ch_printf(ch, "%-16s%s\r\n", IS_SET(ch->pcdata->flags, PCFLAG_NOBEEP) ? "nobeep on" : "nobeep off", s2);
    }
    else
    {
      ch_printf(ch, "\r\n%s%-16s%s", s2, IS_SET(ch->pcdata->flags, PCFLAG_NORECALL) ? "&Gnorecall on" : "&Rnorecall off", s2);
      ch_printf(ch, "%-16s%s", IS_SET(ch->pcdata->flags, PCFLAG_NOSUMMON) ? "&Gnosummon on" : "&Rnosummon off", s2);
      ch_printf(ch, "%-16s%s", IS_SET(ch->pcdata->flags, PCFLAG_NOBEEP) ? "&Gnobeep on" : "&Rnobeep off", s2);
    ...
    if(IS_BLIND(ch))
    {
      ch_printf(ch, "\r\n%s%-16s%s\r\n", s2, xIS_SET(ch->act, PLR_KEEPALIVE ) ? "keepalive on" : "keepalive off", s2 );
      ch_printf(ch, "%-16s%s\r\n", xIS_SET(ch->act, PLR_TELNET_GA) ? "telnetga on" : "telnetga off", s2);
      ch_printf(ch, "%-16s%s\r\n", IS_SET(ch->pcdata->flags, PCFLAG_GROUPWHO) ? "groupwho on" : "groupwho off", s2);
      ch_printf(ch, "%-16s%s\r\n", IS_SET(ch->pcdata->flags, PCFLAG_NOINTRO) ? "nointro on" : "nointro off", s2);
      ch_printf(ch, "%-16s%s\r\n", IS_BLIND(ch) ? "screenreader on" : "screenreader off", s2);
    }
    else
    {
      ch_printf(ch, "\r\n%s%-16s%s\r\n", s2, xIS_SET(ch->act, PLR_KEEPALIVE ) ? "keepalive on" : "keepalive off", s2 );
      ch_printf(ch, "%-16s%s", xIS_SET(ch->act, PLR_TELNET_GA) ? "&Gtelnetga on" : "&Rtelnetga off", s2);
      ch_printf(ch, "%-16s%s", IS_SET(ch->pcdata->flags, PCFLAG_GROUPWHO) ? "&Ggroupwho on" : "&Rgroupwho off", s2);
      ch_printf(ch, "%-16s%s", IS_SET(ch->pcdata->flags, PCFLAG_NOINTRO) ? "&Gnointro on" : "&Rnointro off", s2);
      ch_printf(ch, "%-16s%s", IS_BLIND(ch) ? "&Gscreenreader on" : "&Rscreenreader off", s2);
    }
    ...
    if(IS_BLIND(ch))
    {
      ...
      ch_printf(ch, "%s[%s%d%s] Pager Length\r\n", s2, ch->pcdata->pagerlen > 0 ? s4 : s3, ch->pcdata->pagerlen, s2);
      ch_printf(ch, "%sTo use triple experience points type 'config texp'\r\n", s2);
    }
    else
    {
      ch_printf(ch, "\r\n%s[%s%d%s] Wimpy   ", s2, ch->wimpy > 0 ? s4 : s3, ch->wimpy, s2);
      ch_printf(ch, "%s[%s%d%s] Pager Length   ", s2, ch->pcdata->pagerlen > 0 ? s4 : s3, ch->pcdata->pagerlen, s2);
      ch_printf(ch, "%sTo use triple experience points type 'config texp'\r\n", s2);
    }
    // Option to hear battle, communications, music, crafts, enhanced
    set_char_color(AT_CYAN, ch);
    send_to_char("\r\n\r\nSounds:", ch);
    if(IS_BLIND(ch))
    {
      ch_printf(ch, "\r\n%s%-16s%s\r\n", s2, xIS_SET(ch->act, PLR_MUSIC) ? "music on" : "music off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_BATTLE) ? "battle on" : "battle off", s2);
      ch_printf(ch, "%-16s%s\r\n", xIS_SET(ch->act, PLR_COMMUNICATION) ? "communication on" : "communication off", s2);
      ch_printf(ch, "%-14s%s\r\n", xIS_SET(ch->act, PLR_CRAFTS) ? "tradeskills on" : "tradeskills off", s2);
      ch_printf(ch, "%-14s%s", xIS_SET(ch->act, PLR_ENHANCED) ? "enhanced on" : "enhanced off", s2);

    }
    else
    {
      ch_printf(ch, "\r\n%s%-12s%s", s2, xIS_SET(ch->act, PLR_MUSIC) ? "&Gmusic on" : "&Rmusic off", s2);
      ch_printf(ch, "%-13s%s", xIS_SET(ch->act, PLR_BATTLE) ? "&Gbattle on" : "&Rbattle off", s2);
      ch_printf(ch, "%-21s%s", xIS_SET(ch->act, PLR_COMMUNICATION) ? "&Gcommunication on" : "&Rcommunication off", s2);
      ch_printf(ch, "%-19s%s", xIS_SET(ch->act, PLR_CRAFTS) ? "&Gtradeskills on" : "&Rtradeskills off", s2);
      ch_printf(ch, "%-19s%s", xIS_SET(ch->act, PLR_ENHANCED) ? "&Genhanced on" : "&Renhanced off", s2);
    ...
    if(IS_IMMORTAL(ch))
    {
      ...
      ch_printf(ch, "\r\n%s%-12s%s", s2, xIS_SET(ch->act, PLR_STAFF) ? "&Gstaff on" : "&Rstaff off", s2);
    }
  }
  else
  {
    ...
    if(!str_prefix(arg, "texp"))
    {
      ...
      if(ch->pcdata->double_exp_timer >= 100)
      {
        ...
        send_to_char("You don't have enough points to get an hour of triple experience.\r\n", ch);
      return;
    }
    ...
    else
    {
      ...
      ch_printf(ch, "%s has been turned %s.\r\n", capitalize(arg), IS_SET(ch->pcdata->flags, bit) ? "&Gon&D" : "&Roff&D");
      return;
    }
  }
  return;
...
{
  AREA_DATA              *pArea;
  const char             *header_string1 = "&c|&C    Authors&c    |&C              Areas             &c |&C Soft Ranges&c |&C Hard Ranges &c|\r\n";
  const char             *header_string2 = "&c+---------------+---------------------------------+-------------+-------------+\r\n";
  const char             *print_string = "&c|&C %-13s&c |&C %-31s&c |&C   %-3.2d-%3.2d&c   |&C   %-3.2d-%3.2d&c   |\r\n";
  const char             *print_string2 = "&c|&C %-13s&c |&C %-31s&c |&C   %-3.2d-%3.2d&c   |&C   %-3.2d-%3.2d&c   |&R Group&c\r\n";
  ...
  if(arg && arg[0] != '\0')
  {
    if(!is_number(arg))
    {
        ...
        if(temp != NULL)
        {
          pager_printf_color(ch, info_string, temp->name, temp->author,
                             ...
                             (!temp->htown
                              || temp->htown[0] ==
                              '\0') ? "None" : temp->htown,
                             temp->low_hard_range, temp->hi_hard_range, temp->low_soft_range, temp->hi_soft_range, (!temp->desc || temp->desc[0] == '\0') ? "None" : temp->desc);
        }
        else
        {
          send_to_char("\r\nSyntax: areas info <area name>\r\n", ch);
      ...
      else if(!str_cmp(arg, "old"))
      {
        ...
        send_to_pager(header_string2, ch);
        send_to_pager("For detailed information about a specific area, type : &Careas info <area name>\r\n", ch);
      ...
      else if(!str_cmp(arg, "group"))
      {
        ...
        for(pArea = first_area_name; pArea; pArea = pArea->next_sort_name)
        {
          ...
          if(!IS_SET(pArea->flags, AFLAG_GROUP))
            continue;
          pager_printf(ch, print_string2, pArea->author, pArea->name, pArea->low_soft_range, pArea->hi_soft_range, pArea->low_hard_range, pArea->hi_hard_range);
        }
  ...
  send_to_pager(header_string2, ch);
  send_to_pager("&RGroup&c next to the area level ranges means the area is intended for a group of players.\r\n", ch);
  send_to_pager("&cFor those nostalgic players, stock areas are available through the &Wtime machine&c\r\nquest in Gnome Tower area.\r\n", ch);
}
...
void do_slist(CHAR_DATA *ch, char *argument)
{
  ...
  int                     lowlev = 1, hilev = MAX_LEVEL, cl = 0, findclass = -2;
...
/* Need to allow players to either write - 'slist 1 10', 'slist necromancer', 'slist druid 1 20' */
  ...
  { /* no third argument - ie slist 1 * * * * 10 or slist druid 1 */
    ...
    { /* no 2nd argument - ie slist * * * * necromancer, slist 3 */
  ...
  for(i = lowlev; i <= hilev; i++)
  {
    ...
    for(sn = 0; sn < top_sn; sn++)
    {
      ...
      if(!Class && (i == skill_table[sn]->skill_level[ch->Class]
                    || (IS_SECONDCLASS(ch) && (i == skill_table[sn]->skill_level[ch->secondclass] || (IS_THIRDCLASS(ch) && i == skill_table[sn]->skill_level[ch->thirdclass])))))
      {
        if(!lFound)
        {
          lFound = 1;
        ...
        if(!IS_SECONDCLASS(ch))
          pager_printf(ch, "%7s: %30.30s  Current/Max: %3d/%-3d\r\n", skn, skill_table[sn]->name, ch->pcdata->learned[sn], get_maxadept(ch, sn, FALSE));
        else
        {
          findclass = (i == skill_table[sn]->skill_level[ch->Class]) ? ch->Class : (i == skill_table[sn]->skill_level[ch->secondclass]) ? ch->secondclass : ch->thirdclass;
          pager_printf(ch, "&D%4.2s (%-5.5s): %15.15s \t &DCurrent/Max: %3d/%-3d&D\r\n",
                       skn, findclass < -1 ? "Unknown" : findclass == -1 ? "Racial" : class_table[findclass]->who_name, skill_table[sn]->name, ch->pcdata->learned[sn], get_maxadept(ch, sn, FALSE));
        }
      }

      if(Class && i == skill_table[sn]->skill_level[cl])
      { /* CLASS exists so we're * * using * an argument, * ie * slist necromancer */
        if(!lFound)
        {
          lFound = 1;
          pager_printf(ch, "Level %d\r\n", i);
...
void do_land(CHAR_DATA *ch, char *argument)
{
  ...
  if(ch->in_room && (ch->in_room->sector_type == SECT_WATER_SWIM
                     ...
                     || ch->in_room->sector_type == SECT_LAKE || ch->in_room->sector_type == SECT_UNDERWATER || ch->in_room->sector_type == SECT_OCEANFLOOR))
  ...
  if(found)
  {
    if(ch->race == RACE_DRAGON)
    {
      ...
      {
        act(AT_GREY, "You fold your wings, but come down too fast!", ch, NULL, NULL, TO_CHAR);
        if(!str_cmp(msg, "in a swirl of dust"))
        {
          send_to_char("&RYou hit the ground hard!\r\n", ch);
          act(AT_GREY, "$n folds $s wings, but comes down too fast striking the ground!", ch, NULL, NULL, TO_ROOM);
        }
        else
        {
          send_to_char("&RYou make a huge splash!\r\n", ch);
          act(AT_GREY, "$n folds $s wings, but comes down too fast making a huge splash!", ch, NULL, NULL, TO_ROOM);
        }
    ...
    if(!IS_NPC(ch))
    {
      ...
      {
        act(AT_CYAN, "Your wings shrivel up to small appendages and fall off.", ch, NULL, NULL, TO_CHAR);
        act(AT_CYAN, "$n's wings shrivel up to small appendages and fall off.", ch, NULL, NULL, TO_ROOM);
...
void do_eye(CHAR_DATA *ch, char *argument)
{
  ...
  if(str_cmp(arg, "north") && str_cmp(arg, "n") && str_cmp(arg, "south")
     ...
     && str_cmp(arg, "east") && str_cmp(arg, "e") && str_cmp(arg, "west") && str_cmp(arg, "w") && str_cmp(arg, "look") && str_cmp(arg, "l") && str_cmp(arg, "release") && str_cmp(arg, "r"))
  {
  ...
  else if(!str_cmp(arg, "look") || !str_cmp(arg, "l"))
  {
    ...
    char_to_room(ch, original);
    ch->mana -= 2;
    return; /* Return here so it doesnt take moves */
  }
  else if(!str_cmp(arg, "release") || !str_cmp(arg, "r"))
  {
    send_to_char("You shutdown your flow of mana and concentration to the wizard eye.\r\n", ch);
    act(AT_MAGIC, "$n blinks out of existance as its power source wanes.", victim, NULL, NULL, TO_ROOM);
...
void do_expratio(CHAR_DATA *ch, char *argument)
{
  ...
  char                    arg2[MIL];
  char                    arg3[MIL];
  short                   firstexpratio, secondexpratio, thirdexpratio;
...
void do_ouch(CHAR_DATA *ch, char *argument)
{
  ...
  else if((victim = get_char_world(ch, arg)) != NULL)
  {
    if(chance == 1)
    {
      snprintf(buf, MSL, "[Ooc]: %s announces The STAFF of 6D send their condolences to %s.", ch->name, victim->name);
      echo_to_all(AT_LBLUE, buf, ECHOTAR_ALL);
    }
    else if(chance == 2)
    {
      snprintf(buf, MSL, "[Ooc]: %s sends flowers to %s next of kin.", ch->name, victim->name);
      echo_to_all(AT_LBLUE, buf, ECHOTAR_ALL);
    }
    else if(chance == 3)
    {
      snprintf(buf, MSL, "&W[&COoc&W]&C: %s comforts %s after their misfortune.", ch->name, victim->name);
      echo_to_all(AT_LBLUE, buf, ECHOTAR_ALL);
    }
  }
  return;
...
void do_gratz(CHAR_DATA *ch, char *argument)
{
  ...
  if(chance == 1)
    interpret(ch, (char *)"ooc ,wonders if this could be the mythical &ZLEVELING MACHINE!");
  else if(chance == 2)
    ...
    interpret(ch, (char *)"ooc ,takes his hat off to you and your simply superb performance!");
  else if(chance == 16)
    interpret(ch, (char *)"ooc &ZCongratulations, or as they say G'JOB!");
  else if(chance == 17)
    interpret(ch, (char *)"ooc ,can see by the fanfare and all the commotion...that somebody special just got a level!");
  else if(chance == 18)
    interpret(ch, (char *)"ooc ,recognizes the Stunning performance!");
  else if(chance == 19)
    interpret(ch, (char *)"ooc &ZYou met the challenge with determination, strength, and total confidence! Congratulations!");
}
...
void do_textspeed(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char("Please use a number between 1 and 10 as your speed. Setting you to default (5).\r\n", ch);
