...
void do_wizinfo(CHAR_DATA *ch, char *argument)
{
  ...
  send_to_pager("Name         Email Address\r\n", ch);
  send_to_pager("------------+-------------------\r\n", ch);

  for(wiz = first_wizinfo; wiz; wiz = wiz->next)
  {
    snprintf(buf, MAX_STRING_LENGTH, "%-12s %-33s", wiz->name, wiz->email);
    strncat(buf, "\r\n", MAX_STRING_LENGTH);
...
void do_finger(CHAR_DATA *ch, char *argument)
{
  ...
  else
  {
    ...
    loaded = load_char_obj(d, argument, FALSE, FALSE, TRUE);  /* Volk - TRUE is quiet
                                                               * mode!! hahah */
  ...
  if(!skip)
  {
    ...
    ch_printf(ch, "&wLevel     : &G%-12d &w    Sex: &G%s\r\n", victim->level, victim->sex == SEX_MALE ? "Male" : victim->sex == SEX_FEMALE ? "Female" : "Other");
    ch_printf(ch, "&wRace      : &G%-12s &w  Class: &G", capitalize(race_table[victim->race]->race_name));
    if(IS_THIRDCLASS(victim))
      ch_printf(ch, "%-3.3s/%-3.3s/%-3.3s\r\n", capitalize(class_table[victim->Class]->who_name), class_table[victim->secondclass]->who_name, class_table[victim->thirdclass]->who_name);
    else if(IS_SECONDCLASS(victim))
      ch_printf(ch, "%-5.5s/%-5.5s\r\n", capitalize(class_table[victim->Class]->who_name), class_table[victim->secondclass]->who_name);
    else
    ...
    if(victim->pcdata->tradeclass == 26)
      ch_printf(ch, "&wTrade     : &GDragonic       &wCrafting Level:&G %d\r\n", victim->pcdata->tradelevel);
    ...
    {
      send_to_char("&wLast on   : Mon Aug  8 06:11:33 2016\r\n", ch);
    }
    else
    {
      if(!loaded)
...
/* Volk - fun stuff, display percentage of areas discovered */
    ...
    if(IS_IMMORTAL(ch))
    {
      ...
      send_to_char("--------------------\r\n", ch);
      if(!str_cmp(victim->name, "Vladaar")) // Allows me to have my alts at
      {
        ch_printf(ch, "&wIP Info       : &G%s\r\n", !victim->desc ? "Unknown" : "127.0.0.1");
        ch_printf(ch, "&wLast Known IP : &G127.0.0.1\r\n");
      }
      else
      {
        ch_printf(ch, "&wIP Info       : &G%s\r\n", !victim->desc ? "Unknown" : victim->desc->host);
        ch_printf(ch, "&wLast Known IP : &G%s\r\n", !victim->pcdata->lastip ? "Unknown" : victim->pcdata->lastip);
      }
